...
function bundledDebugToolsPath(): string | null {
  if (!process.resourcesPath) return null;
  const candidate = path.join(process.resourcesPath, 'debug-tools');
 ...
 * @param codenHome   Writable user profile containing progress and the
 *                    versioned LeetCode user-solution overlay. Required for the bundled
...
export async function startServer(
  ...
  if (bundled) {
    ...
    // The root README is the product overview; challenge reference docs live
    // in the canonical DSA packages below.
    const overviewDoc = path.join(process.resourcesPath, 'README.md');
    // Canonical challenge packages live in extraResources/dsa/leetcode.
    const dsaDir = path.join(process.resourcesPath, 'dsa');
    const debugPython = bundledDebugPythonPath();
    const debugTools = bundledDebugToolsPath();
    if (!debugPython) {
      console.warn(
    ...
    child = spawn(bundled, [], {
      env: {
        ...
        CODEN_OVERVIEW_DOC: overviewDoc,
        CODEN_DSA_DIR: dsaDir,
        CODEN_PACKAGED_SERVER: '1',
        CODEN_DESKTOP_BRIDGE_TOKEN: desktopBridgeToken,
        ...(debugPython ? { CODEN_DEBUG_PYTHON: debugPython } : {}),
        ...(debugTools ? { CODEN_DEBUG_TOOLS_DIR: debugTools } : {}),
      },
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          CODEN_DESKTOP_BRIDGE_TOKEN: desktopBridgeToken,
          ...(process.env.CODEN_DEV_API_PORT
            ? { CODEN_PORT: process.env.CODEN_DEV_API_PORT }
            : {}),
        },
        stdio: ['ignore', 'pipe', 'pipe'],
        windowsHide: true,
