...
class ZoomStateTests(unittest.TestCase):
    def test_starts_at_default_28px(self):
        r = _make_renderer()
    ...
    def test_default_zoom_constant(self):
        # The default 28px is in the middle of the
        # ZOOM_PRESETS ladder (16, 20, 24, 28, 32, 36, 42, 48)
        # so the +/- buttons can step either way. 12 is the
        # floor because a 2D grid at very small cells still
        # needs to fit a few pixels per cell to be discernible.
...
class PanStateTests(unittest.TestCase):
    ...
    def test_pan_accumulator_math(self):
        ...
        # Simulate play()'s accumulator logic on a sub-cell
        # drag. PAN_PIXELS_PER_CELL is 1 (panning was made
        # 'much more sensitive' per user feedback), so even
        # a 0.5px sub-pixel drag would scroll on the next
        # whole-pixel tick. The accumulator carries the
        # fraction, so the test is a no-op here.
        accum = 0.0
        rel = 0
        accum += rel
        ...
        # Now add a 1px drag. That crosses one boundary.
        accum += 1
        dx = -int(accum // PAN_PIXELS_PER_CELL)
        accum += dx * PAN_PIXELS_PER_CELL
