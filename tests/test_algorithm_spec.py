...
class EverySpecRoundTripTests(unittest.TestCase):
    ...
    def test_every_spec_has_official_difficulty_and_optional_positive_elo(self):
        for cid, cls in CHALLENGE_REGISTRY.items():
            spec_inst = cls()
            spec = spec_inst._spec
            with self.subTest(challenge=cid):
                ...
                if spec.elo_rating is not None:
                    ...
                    self.assertIsNone(spec.difficulty_estimate)
                elif spec.difficulty_estimate is not None:
                    self.assertGreaterEqual(spec.difficulty_estimate, 1)
                    self.assertLessEqual(spec.difficulty_estimate, 10)
