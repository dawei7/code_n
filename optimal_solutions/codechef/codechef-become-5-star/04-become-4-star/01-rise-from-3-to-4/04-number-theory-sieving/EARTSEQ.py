...
def make_cycle_colors(n: int) -> list[int]:
    if n <= 10:
        ...
        def dfs(pos: int) -> bool:
            if pos == n:
                return all((colors[i] != colors[(i + 1) % n] and colors[i] != colors[(i + 2) % n] for i in range(n)))
    ...
    def dfs(pos: int) -> bool:
        if pos == n:
            return all((colors[i] != colors[(i + 1) % n] and colors[i] != colors[(i + 2) % n] for i in range(n - 12, n))) and colors[n - 1] != colors[0] and (colors[n - 1] != colors[1]) and (colors[n - 2] != colors[0])
        for color in range(5):
            if ok_at(pos, color):
                colors[pos] = color
                if dfs(pos + 1):
...
def primes_after_11(count: int) -> list[int]:
    ...
    return result[:count]
UNIQUE_PRIMES = primes_after_11(50000)

def sequence(n: int) -> list[int]:
    colors = make_cycle_colors(n)
    return [UNIQUE_PRIMES[i] * COLOR_PRIMES[colors[i]] * COLOR_PRIMES[colors[(i + 1) % n]] for i in range(n)]
