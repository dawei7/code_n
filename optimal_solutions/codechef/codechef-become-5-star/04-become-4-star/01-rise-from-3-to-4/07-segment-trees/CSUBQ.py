...
def solve() -> None:
    ...
    def query(prefix: list[int], suffix: list[int], count: list[int], left_pos: int, right_pos: int) -> int:
        ...
        while left_pos <= right_pos:
            if left_pos & 1:
                llen, lpref, lsuff, lcnt = merge(llen, lpref, lsuff, lcnt, seg_len[left_pos], prefix[left_pos], suffix[left_pos], count[left_pos])
                left_pos += 1
            if not right_pos & 1:
                rlen, rpref, rsuff, rcnt = merge(seg_len[right_pos], prefix[right_pos], suffix[right_pos], count[right_pos], rlen, rpref, rsuff, rcnt)
                right_pos -= 1
            left_pos >>= 1
            right_pos >>= 1
