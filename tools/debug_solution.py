...
def main() -> int:
    ...
    parser.add_argument("--custom-input-json", default="")
    parser.add_argument(
        "--no-hold",
        action="store_true",
    ...
    solution_path = active_solution_path(args.challenge_id, "python")
    ...
    custom_input = json.loads(args.custom_input_json) if args.custom_input_json else None
    ...
    selected = ", ".join(case.id for case in run_cases)
    print(f"running {args.challenge_id} (mode={args.mode}, cases={selected})")
