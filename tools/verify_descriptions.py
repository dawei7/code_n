import json
import re
from pathlib import Path

LEETCODE_ROOT = Path("dsa/leetcode")
INDEX_PATH = LEETCODE_ROOT / "index.json"

with open(INDEX_PATH, "r", encoding="utf-8") as f:
    questions = json.load(f)["questions"]

total_questions = len(questions)
descriptions_count = 0
placeholders_count = 0
descs_with_images = 0
total_local_images = 0
missing_files = []
broken_image_refs = []
unresolved_external_urls = []

for q in questions:
    fid = int(q["frontend_id"])
    slug = q["slug"]
    folder_name = f"{fid:04d}_{slug}"
    ref_dir = LEETCODE_ROOT / folder_name / "reference"
    desc_file = ref_dir / "description.md"
    images_dir = ref_dir / "images"
    
    if not desc_file.is_file():
        missing_files.append(folder_name)
        continue

    content = desc_file.read_text(encoding="utf-8")
    if "An official LeetCode description is not available for this problem" in content:
        placeholders_count += 1
    else:
        descriptions_count += 1

    # Find image references
    md_imgs = re.findall(r'!\[.*?\]\((.*?)\)', content)
    html_imgs = re.findall(r'<img\s+[^>]*?src=["\']([^"\']+)["\']', content)
    all_imgs = md_imgs + html_imgs

    has_local = False
    for img_ref in all_imgs:
        clean_ref = img_ref.split()[0].strip()
        if clean_ref.startswith("images/"):
            has_local = True
            fname = clean_ref[len("images/"):]
            target_file = images_dir / fname
            if not target_file.is_file():
                broken_image_refs.append((folder_name, clean_ref))
        elif clean_ref.startswith("http://") or clean_ref.startswith("https://") or "leetcode.com" in clean_ref:
            unresolved_external_urls.append((folder_name, clean_ref))

    if has_local:
        descs_with_images += 1

    if images_dir.is_dir():
        total_local_images += len(list(images_dir.iterdir()))

print("=== DESCRIPTIONS AUDIT REPORT ===")
print(f"Total problems in index: {total_questions}")
print(f"Description files present: {descriptions_count + placeholders_count}/{total_questions}")
print(f"  - Converted Descriptions: {descriptions_count}")
print(f"  - Description Placeholders: {placeholders_count}")
print(f"Descriptions with local image galleries: {descs_with_images}")
print(f"Total local image files saved across reference/images/: {total_local_images}")
print(f"Missing description files: {len(missing_files)}")
print(f"Broken local image references: {len(broken_image_refs)}")
print(f"Unresolved external image URLs remaining: {len(unresolved_external_urls)}")

if broken_image_refs:
    print("\nSample broken image references:", broken_image_refs[:10])
if unresolved_external_urls:
    print("\nSample unresolved URLs:", unresolved_external_urls[:10])
