...
def run_with_trace(func: Callable, kwargs: dict[str, Any],
                   ...
                   capture: bool = True) -> tuple[Any, ExecutionTrace]:
    ...
    step_count = 0
    last_step_update = 0
    code_matches_target: dict[object, bool] = {}

    def tracer(frame, event, arg):
        nonlocal step_count, last_step_update
        frame_code = frame.f_code
        matches_target = code_matches_target.get(frame_code)
        if matches_target is None:
            frame_file = os.path.normcase(os.path.abspath(frame_code.co_filename))
            matches_target = frame_file == target_file
            code_matches_target[frame_code] = matches_target
        if matches_target and event in {"call", "line", "return"}:
            if event == "line":
                step_count += 1
...
def _load_inline_breakpoints(path: str) -> set[int]:
    markers = (
        "engine: breakpoint",
        "engine breakpoint",
        # Backward compatibility for existing user files.
        "code_n: breakpoint",
        "code_n breakpoint",
    )
