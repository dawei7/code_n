...
class CountOpsTest(unittest.TestCase):
    ...
    def test_loop_early_return_linear_call_is_counted_once(self) -> None:
        source = (
        ...
        # The index() scan is behind a return, so it can happen at most once.
        # It should not be multiplied by every loop iteration.
        self.assertEqual(count_ops(source, 16), 87)
        self.assertLess(count_ops(source, 32), 200)
