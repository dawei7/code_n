...
"""
    ...
    def test_cpp_hardcoded_initializer_list_is_flagged(self) -> None:
        source = (
            "class Solution { public: "
            "vector<int> solve(vector<int> data) { return vector<int>{1, 2, 3}; } "
            "};"
        )
        r = check(source, user_ops=100, reference_ops=100, language="cpp")
        self.assertEqual(r.reason, Reason.HARDCODED)
        self.assertTrue(r.flagged)

    def test_cpp_raw_string_fake_return_is_ignored(self) -> None:
        source = (
            "class Solution { public: "
            'vector<int> solve(vector<int> data) { string note = R"RAW(" ; return vector<int>{1, 2}; )RAW"; '
            "return data; } "
            "};"
        )
        r = check(source, user_ops=100, reference_ops=100, language="cpp")
    ...
    def test_java_hardcoded_list_factory_is_flagged(self) -> None:
        source = (
            "class Solution { "
            "List<Integer> solve(List<Integer> data) { return Arrays.asList(1, 2, 3); } "
            "}"
        )
        r = check(source, user_ops=100, reference_ops=100, language="java")
        self.assertEqual(r.reason, Reason.HARDCODED)
        self.assertTrue(r.flagged)

    def test_java_empty_container_factories_are_flagged(self) -> None:
        sources = [
            (
                "class Solution { "
                "List<Integer> solve(List<Integer> data) { return Collections.emptyList(); } "
                "}"
            ),
            (
                "class Solution { "
                "List<Integer> solve(List<Integer> data) { return new ArrayList<>(); } "
                "}"
            ),
            (
                "class Solution { "
                "Set<Integer> solve(List<Integer> data) { return Set.of(); } "
                "}"
            ),
        ]
    ...
    def test_cpp_empty_container_constructor_is_flagged(self) -> None:
        source = (
            "class Solution { public: "
            "vector<int> solve(vector<int> data) { return vector<int>(); } "
            "};"
        )
        r = check(source, user_ops=100, reference_ops=100, language="cpp")
        self.assertEqual(r.reason, Reason.HARDCODED)
    ...
    def test_csharp_hardcoded_collection_initializer_is_flagged(self) -> None:
        source = (
            "public class Solution { "
            "public List<int> Solve(List<int> data) { return new List<int> { 1, 2, 3 }; } "
            "}"
        )
        r = check(source, user_ops=100, reference_ops=100, language="csharp")
        self.assertEqual(r.reason, Reason.HARDCODED)
        self.assertTrue(r.flagged)

    def test_csharp_empty_container_factories_are_flagged(self) -> None:
        sources = [
            (
                "public class Solution { "
                "public List<int> Solve(List<int> data) { return new List<int>(); } "
                "}"
            ),
            (
                "public class Solution { "
                "public int[] Solve(List<int> data) { return Array.Empty<int>(); } "
                "}"
            ),
        ]
    ...
    def test_javascript_hardcoded_array_literal_is_flagged(self) -> None:
        source = (
            "class Solution { "
            "solve(data) { return [1, 2, 3]; } "
            "}"
        )
        r = check(source, user_ops=100, reference_ops=100, language="javascript")
        self.assertEqual(r.reason, Reason.HARDCODED)
        self.assertTrue(r.flagged)

    def test_go_hardcoded_composite_literal_is_flagged(self) -> None:
        source = (
            "package main\n"
            "func solve(data []int) []int { return []int{1, 2, 3} }\n"
        )
        r = check(source, user_ops=100, reference_ops=100, language="go")
        self.assertEqual(r.reason, Reason.HARDCODED)
        self.assertTrue(r.flagged)

    def test_kotlin_hardcoded_list_factory_is_flagged(self) -> None:
        source = (
            "class Solution { "
            "fun solve(data: MutableList<Int>): List<Int> { return listOf(1, 2, 3) } "
            "}"
        )
        r = check(source, user_ops=100, reference_ops=100, language="kotlin")
        self.assertEqual(r.reason, Reason.HARDCODED)
        self.assertTrue(r.flagged)

    def test_c_family_computed_collection_initializer_is_ok(self) -> None:
        source = (
            "public class Solution { "
            "public List<int> Solve(List<int> data) { int answer = data.Count; return new List<int> { answer }; } "
            "}"
        )
        r = check(source, user_ops=100, reference_ops=100, language="csharp")
        self.assertEqual(r.reason, Reason.OK)

    def test_c_family_literal_fallback_with_computed_return_is_ok(self) -> None:
        source = (
            "public class Solution { "
            "public List<int> Solve(List<int> data) { "
            "for (int i = 0; i < data.Count; i++) { "
            "if (data[i] == 7) return new List<int> { i, data[i] }; "
            "} "
