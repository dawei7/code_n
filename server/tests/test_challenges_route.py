...
class ChallengesRouteTest(conftest._Base):
    ...
    def test_1502_exposes_canonical_solution_tab(self) -> None:
        ...
        optimal = detail["solution_variants"][0]
        self.assertIn("class Solution", optimal["leetcode_sources"]["python"])

    def test_1502_default_paths_resolve_to_canonical_solution(self) -> None:
        package = leetcode_package_dir("lc_1502")
        self.assertIsNotNone(package)
        assert package is not None

        sol_path = leetcode_solution_path("lc_1502", "python")
        self.assertIsNotNone(sol_path)
        self.assertTrue(sol_path and sol_path.is_file())
        self.assertEqual(sol_path, package / "solution.py")
