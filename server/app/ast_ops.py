...
Why this exists
---------------
The engine's runtime ``OperationCounter`` (in ``engine.counter``)
...
def _count_stmt(stmt: ast.stmt, multiplier: int, n: int) -> int:
    ...
    if isinstance(stmt, ast.If):
        ...
        then_multiplier = 1 if multiplier > 1 and _block_returns(stmt.body) else multiplier
        else_multiplier = 1 if multiplier > 1 and _block_returns(stmt.orelse) else multiplier
        then_ops = _count_block(stmt.body, then_multiplier, n)
        else_ops = _count_block(stmt.orelse, else_multiplier, n) if stmt.orelse else 0
...
def _block_returns(body: list[ast.stmt]) -> bool:
    """Return true when entering ``body`` always exits the function.

    This is conservative by design. It handles straight-line returns and simple
    if/else blocks where both arms return. Loops are not treated as guaranteed
    exits because they may run zero times.
    """
    for stmt in body:
        if isinstance(stmt, (ast.Return, ast.Raise)):
            return True
        if isinstance(stmt, ast.If) and stmt.orelse:
            if _block_returns(stmt.body) and _block_returns(stmt.orelse):
                return True
    return False
