...
* eager challenge-summary preparation so every selector view is immediately
  available after the desktop window appears
...
from server.app.routes import challenges, custom_problem_sets, debug, docs, health, leetcode_submission, practice_export, progress, run, solutions, profiles
...
def prepare_runtime() -> None:
    """Prepare writable data and preload the complete challenge library."""
    ensure_data_dirs()
    from server.app.user_solutions import migrate_legacy_solutions

    migrated = migrate_legacy_solutions()
    if migrated:
        logging.info("Migrated %d legacy user solution files", migrated)
    loaded = challenges.warm_challenge_summaries()
    logging.info("Preloaded %d challenge summaries for instant view switching", loaded)
...
def create_app() -> FastAPI:
    ...
    app.include_router(custom_problem_sets.router, prefix="/api")
    app.include_router(solutions.router, prefix="/api")
    app.include_router(profiles.router, prefix="/api")
    app.include_router(leetcode_submission.router, prefix="/api")
