...
Sei $f: \mathcal{T} \to \mathbb{Z}$ eine Funktion, die die Höhe eines balancierten Baumes berechnet und $-1$ zurückgibt, falls der Baum unausgeglichen ist:
$$ f(T) = \begin{cases}
...
- **Zeitkomplexität:** Da $f(T)$ sowohl die Höhe als auch den Balancestatus gleichzeitig in einem einzigen Post-Order-Durchlauf berechnet, ist die Zeitkomplexität strikt $O(|V|)$.
