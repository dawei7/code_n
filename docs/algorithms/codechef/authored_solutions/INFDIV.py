...
def expected_value(n: int, m: int) -> int:
    ...
    # For x < n:
    #   E[x] = 1 + (sum_{r=1..x} E[x % r] + (n-x)E[x]) / n
    # hence
    #   x E[x] = n + sum_{r=1..x} E[x % r].
    #
    # The sum over x % r is grouped by q = floor(x / r).  For small q we use
    # prefix sums over arithmetic progressions modulo q; for large q the groups
    # are short and are summed directly.
    limit = n - 1
