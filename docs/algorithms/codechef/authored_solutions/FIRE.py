...
def can_save_all(n: int, root: int, graph: list[list[int]], targets: set[int]) -> bool:
    ...
    required = {
        ...
        if not any(
            other != node and tin[other] <= tin[node] < tout[other]
    ...
    for current_depth in range(max_depth, 0, -1):
        ...
        # Promote all but one.  Keeping the node whose parent group is largest
        # avoids redundant descendants when a sibling group is promoted.
        ...
        required = {
            node
            for node in required
            if node not in same_depth or node == keep
        }
