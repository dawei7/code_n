...
Find the last 9 digits of D(10000), where D(N) is the number of distinct configurations
of 2N+1 amoebas in a 3D grid reachable after N divisions starting from (0,0,0).
"""
...
def _get_compiled_lib() -> ctypes.CDLL:
    ...
    dll_path = os.path.join(tmp_dir, "fast_p763_core.dll")
    c_path = os.path.join(tmp_dir, "fast_p763_core.c")
...
def solve(target_n: int = 10_000) -> str:
    """Compute D(target_n) mod 10^9 using 3D boundary path generating recurrence."""
    lib = _get_compiled_lib()
    ans = 0
    for _step in range(1):
        ans = int(lib.solve_c(target_n - 1))
    return f"{ans:09d}"
