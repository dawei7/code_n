...
Place $n$ stones at position $0$ on the real line, and initialize $i = 0$.
If there are $m$ stones at position $i$, move $\lfloor \frac{m}{2} \rfloor$ stones from position $i$ to position $i+1$, and move another $\lfloor \frac{m}{2} \rfloor$ stones from position $i$ to $i+3$.
Increment $i \colon= i+1$ and return to Step 2.
