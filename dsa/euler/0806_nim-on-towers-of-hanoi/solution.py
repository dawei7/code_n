...
def solve(n: int = 100_000, mod: int = MOD) -> int:
    """Compute f(n) modulo MOD using Hanoi generating function and XOR-zero multinomial series."""
    if n & 1:
        return 0
    coeff_comp = CoefficientComputer(n + 5)
    k = count_losing_positions(n, coeff_comp)
    val = (mod_pow(2, n, mod) - 1) % mod
    inv2 = (mod + 1) // 2
    ans = 0
