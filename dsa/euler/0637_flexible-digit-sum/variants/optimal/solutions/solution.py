...
Find g(10^7, 10, 3), where g(n, B1, B2) is the sum of integers i <= n such that f(i, B1) = f(i, B2).
"""
...
def _get_compiled_lib() -> ctypes.CDLL:
    ...
    dll_path = os.path.join(tmp_dir, "fast_fds_core.dll")
    c_path = os.path.join(tmp_dir, "fast_fds_core.c")
...
def solve(limit: int = 10_000_000, base1: int = 10, base2: int = 3) -> int:
    """Compute g(limit, base1, base2) using greedy popcount-ordered digit partition DP."""
    if limit <= 10:
        tot = 0
