...
We trace the dynamic Array, Hash Table, Sliding Window, Sorting, Counting sliding window on a representative input instance.
...
The objective for **Longest Harmonious Subsequence** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
