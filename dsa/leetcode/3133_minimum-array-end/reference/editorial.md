...
![DetailedBinaryOperationAnalysis](images/3133_Approach2.png)
...
- Extract the `i`-th bit of `x` and store it in $\text{binaryX}[i]$.
- Extract the `i`-th bit of `n-1` and store it in $\text{binaryN}[i]$.
...
- Traverse the binary representation of `x` (`binaryX`):
  ...
  - Copy the corresponding bit from $\text{binaryN}[posN]$ into $\text{binaryX}[posX]$.
...
- If $\text{binaryX}[i]$ is `1`, convert the bit back to its decimal value using $2^i$ and add it to `result`.
