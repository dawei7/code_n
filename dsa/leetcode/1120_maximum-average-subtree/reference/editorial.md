...
1. $ValueSum(node) = ValueSum(\text{node.left}) + ValueSum(\text{node.right}) + Value(node)$
2. $NodeCount(node) = NodeCount(\text{node.left}) + NodeCount(\text{node.right}) + 1$

Also, for any leaf node `leaf`, we know that:

1. $ValueSum(leaf) = \text{node.val}$
2. $NodeCount(leaf) = 1$
