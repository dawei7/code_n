...
The core task in **Minimum Operations to Write the Letter Y on a Grid** is to a **0-indexed** `n x n` grid where `n` is odd, and $\text{grid}[r][c]$ is `0`, `1`, or `2`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
