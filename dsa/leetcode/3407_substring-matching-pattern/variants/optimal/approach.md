...
The core task in **Substring Matching Pattern** is to a string `s` and a pattern string `p`, where `p` contains **exactly one** `'*'` character. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
