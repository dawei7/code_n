...
Keep `node` and an explicit stack of ancestors. While `node` exists, push it and move to its left child. Each pushed node is deferred: its own visit and right subtree must wait until every node reachable down its left side has been handled.

**A pop marks the exact inorder visit moment**

When no further left child exists, pop the nearest waiting ancestor into `node`. Its left subtree is now complete, so append its value and set `node` to its right child. The outer loop then descends that right subtree's left spine before any later ancestor is visited.

**Result, stack, and node divide completed and pending work**

The result contains precisely the nodes whose inorder visit is complete. The stack contains ancestors awaiting their own visit after left-side work. `node` begins the next subtree whose left spine has not yet been pushed. These three regions contain all reached nodes without duplication.
...
- Empty input leaves both `node` and the stack empty and returns an empty list.
