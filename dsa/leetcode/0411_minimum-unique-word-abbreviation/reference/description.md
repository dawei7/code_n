...
Note that `"s55n"` (`"s <u>ubsti</u> <u>tutio</u> n"`) is not a valid abbreviation of `"substitution"` because the replaced substrings are adjacent.
...
- `target`: The lowercase English word to abbreviate.
- `dictionary`: The lowercase English words that the returned abbreviation must not match.

**Return value**

Return any abbreviation of `target` having minimum token length among those that abbreviate no word in
`dictionary`.
...
- $\text{log}_{2}(n) + m \le 21$ if `n > 0`

- `target` and $\text{dictionary}[i]$ consist of lowercase English letters.
