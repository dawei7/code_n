...
- Check if $N \le 1$, if it is, then we should return `N`.
...
* Time complexity: $O(\log N)$. We do not use loops or recursion, so the time required equals the time spent performing the calculation using `Binet's formula`. However, raising the $\text{golden}_{ratio}$ to the power of $N$ requires $O(\log N)$ time.
