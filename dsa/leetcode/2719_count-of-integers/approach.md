...
$$
\texttt{min\_sum}\le s\le\texttt{max\_sum},
$$
...
- **Prune sums above `max_sum`:** Returning zero immediately can reduce practical states and supports a bound using $\min(9L,\texttt{max\_sum})$.
