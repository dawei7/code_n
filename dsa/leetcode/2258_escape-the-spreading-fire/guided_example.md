...
We trace the logarithmic Array, Binary Search, Breadth-First Search, Matrix search on a representative problem instance.
...
The objective for **Escape the Spreading Fire** is to pinpoint the target value or optimal threshold in logarithmic $O(\log N)$ time.
