...
First, we need some function or array that represents the answer to the problem from a given state. For many solutions on LeetCode, you will see this function/array named "dp". For this problem, let's say that we have an array `dp`. As just stated, this array needs to represent **the answer to the problem for a given state**, so let's say that $\text{dp}[i]$ represents the length of the **longest increasing subsequence** that **ends with the $i^{th}$ element**. The "state" is one-dimensional since it can be represented with only one variable - the index `i`.

Second, we need a way to transition between states, such as $\text{dp}[5]$ and $\text{dp}[7]$. This is called a **recurrence relation** and can sometimes be tricky to figure out. Let's say we know $\text{dp}[0]$, $\text{dp}[1]$, and $\text{dp}[2]$. How can we find $\text{dp}[3]$ given this information? Well, since $\text{dp}[2]$ represents the length of the longest increasing subsequence that ends with $\text{nums}[2]$, if $\text{nums}[3] > \text{nums}[2]$, then we can simply take the subsequence ending at $i = 2$ and append $\text{nums}[3]$ to it, increasing the length by 1. The same can be said for $\text{nums}[0]$ and $\text{nums}[1]$ if $\text{nums}[3]$ is larger. Of course, we should try to maximize $\text{dp}[3]$, so we need to check all 3. Formally, the recurrence relation is: $\text{dp}[i] = max(\text{dp}[j] + 1) for all j where \text{nums}[j] < \text{nums}[i] and j < i$.
...
![Slide 1](images/slideshow_300_LIS_300_1.png)

![Slide 2](images/slideshow_300_LIS_300_2.png)

![Slide 3](images/slideshow_300_LIS_300_3.png)

![Slide 4](images/slideshow_300_LIS_300_4.png)

![Slide 5](images/slideshow_300_LIS_300_5.png)

![Slide 6](images/slideshow_300_LIS_300_6.png)

![Slide 7](images/slideshow_300_LIS_300_7.png)

![Slide 8](images/slideshow_300_LIS_300_8.png)

![Slide 9](images/slideshow_300_LIS_300_9.png)

![Slide 10](images/slideshow_300_LIS_300_10.png)

![Slide 11](images/slideshow_300_LIS_300_11.png)

![Slide 12](images/slideshow_300_LIS_300_12.png)

![Slide 13](images/slideshow_300_LIS_300_13.png)

![Slide 14](images/slideshow_300_LIS_300_14.png)

![Slide 15](images/slideshow_300_LIS_300_15.png)

![Slide 16](images/slideshow_300_LIS_300_16.png)

![Slide 17](images/slideshow_300_LIS_300_17.png)

![Slide 18](images/slideshow_300_LIS_300_18.png)

**Algorithm**

1. Initialize an array `dp` with length `nums.length` and all elements equal to 1. $\text{dp}[i]$ represents the length of the longest increasing subsequence that ends with the element at index `i`.

2. Iterate from $i = 1$ to $i = \text{nums.length} - 1$. At each iteration, use a second for loop to iterate from $j = 0$ to $j = i - 1$ (all the elements before i). For each element before `i`, check if that element is smaller than $\text{nums}[i]$. If so, set $\text{dp}[i] = max(\text{dp}[i], \text{dp}[j] + 1)$.
...
- At the second element `1`, we can't increase the length of the subsequence since $8 \ge 1$, so we have to choose only one element to keep. Well, this is an easy decision, let's take the `1` since there may be elements later on that are greater than `1` but less than `8`, now we have $sub = [1]$.

- At the third element `6`, we can build on our subsequence since `6 > 1`, now $sub = [1, 6]$.

- At the fourth element `2`, we can't build on our subsequence since $6 \ge 2$, but can we improve on it for the future? Well, similar to the decision we made at the second element, if we replace the `6` with `2`, we will open the door to using elements that are greater than `2` but less than `6` in the future, so $sub = [1, 2]$.
