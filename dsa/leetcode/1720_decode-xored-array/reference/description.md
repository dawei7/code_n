...
It was encoded into another integer array `encoded` of length $n - 1$, such that $\text{encoded}[i] = \text{arr}[i] XOR arr[i + 1]$. For example, if `arr = [1,0,2,1]`, then $encoded = [1,2,3]$.
