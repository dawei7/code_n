...
- `shift`: a nonempty matrix whose row $\text{shift}[i]$ is $[\text{direction}_{i}, \text{amount}_{i}]$.

For each operation, $\text{direction}_{i} = 0$ means left and $\text{direction}_{i} = 1$ means right. $\text{amount}_{i}$ is the requested number of cyclic positions and may be zero.
