## General
**Optimal Approach — Last Person to Fit in the Bus**

The query executes a structured relational pipeline for **Last Person to Fit in the Bus**. It uses `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
