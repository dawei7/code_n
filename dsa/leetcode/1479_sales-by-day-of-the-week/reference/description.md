...
- $Orders(\text{order}_{id}, \text{customer}_{id}, \text{order}_{date}, \text{item}_{id}, quantity)$: dated item
  order quantities;
- $Items(\text{item}_{id}, \text{item}_{name}, \text{item}_{category})$: item identities and categories.
