...
Now we can find the day of the week for each order using the function $\text{day}_{name}()$. We also convert the day names to uppercase using `str.upper()`. Additionally, we map the column type of the frame we created above ($\text{all}_{weekdays}$) to this column to make sure it contains all values in the same order.
...
Almost there! Now we only need to reshape the table and pivot the values in `dayofweek` to columns. There are different ways to do this; in this approach, we choose the function $\text{pivot}_{table}$ to calculate the aggregate total of `quantity` in the step. If you want to use `pivot()` or `unstack()`, you need to calculate the aggregate total first before reshaping the results.
...
<!-- Describe your approach to solving the problem. -->
Since the column $\text{item}_{category}$ is stored in the table `Items` and the column `quantity` is stored in the table `Orders`, we `LEFT JOIN` the table `Items` to the table `Orders` to make sure the result includes all $\text{item}_{category}$ no matter if there is an order associated with the $\text{item}_{category}$ or not.

To identify the day of the week from an $\text{order}_{date}$, we use the function `DAYOFWEEK()`. Since there is no existing function to pivot a table in MySql, we leverage the function `SUM(CASE WHEN)` to pivot the rows (the day of the week for each order) to columns (the total quantities sold for the day of the week from all orders). For the days that have no orders for any $\text{item}_{category}$, we add the function `IFNULL()` to return 0 for such days. Lastly, we have all the aggregate total quantities grouped by the $\text{item}_{category}$ and rename the column to `CATEGORY` for the final output.
