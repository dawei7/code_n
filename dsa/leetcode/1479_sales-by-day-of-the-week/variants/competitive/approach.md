## General
**Competitive Approach — Sales by Day of the Week**

The query executes a structured relational pipeline for **Sales by Day of the Week**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(I+O+C\log C)$ — Operation count proportional to input scale.
