...
We trace the dynamic String, Sliding Window, Rolling Hash, Hash Function sliding window on a representative input instance.
...
The objective for **Find Substring With Given Hash Value** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
