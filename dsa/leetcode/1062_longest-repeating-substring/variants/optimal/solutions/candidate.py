"""Optimal solution for LeetCode 1062: Longest Repeating Substring."""
...
def solve(s: str) -> int:
    ...
    for modulus_index, modulus in enumerate(moduli):
        for source_position, character in enumerate(s):
            powers[modulus_index][source_position + 1] = powers[modulus_index][source_position] * base % modulus
            prefixes[modulus_index][source_position + 1] = (prefixes[modulus_index][source_position] * base + ord(character)) % modulus

    def window_hash(start: int, window_length: int) -> tuple[int, int]:
    ...
    def has_repeat(window_length: int) -> bool:
        starts_by_hash: dict[tuple[int, int], list[int]] = {}
