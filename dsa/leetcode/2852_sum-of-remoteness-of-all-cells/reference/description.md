...
- If the cell `(i, j)` is a **non-blocked** cell, $R[i][j]$ is the sum of the values $\text{grid}[x][y]$ such that there is **no path** from the **non-blocked** cell `(x, y)` to the cell `(i, j)`.
...
- $1 \le \text{grid}[i][j] \le 10^{6}$ or $\text{grid}[i][j] = -1$
