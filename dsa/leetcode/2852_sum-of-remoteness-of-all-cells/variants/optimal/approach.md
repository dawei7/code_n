...
The core task in **Sum of Remoteness of All Cells** is to a **0-indexed** matrix `grid` of order $n * n$. Each cell in this matrix has a value $\text{grid}[i][j]$, which is either a **positive** integer or `-1` representing a blocked cell. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
