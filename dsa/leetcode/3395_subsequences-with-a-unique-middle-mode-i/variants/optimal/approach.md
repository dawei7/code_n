...
where $Q_L=\sum_y\binom{L_y}{2}$ and $T_L=\sum_y L_y^2R_y$, again excluding $x$.
...
The benchmark defines `size` as $n$ and uses 32, 64, and 128 elements, spanning 4x. Each tier repeats $n/4$ distinct values four times, leaving many values represented on both sides of typical middles. The accepted aggregate method remains linear, while a correct slower formulation scans every distinct value for each middle and fails only the scaling verdict.

## Alternatives and edge cases

- **Enumerate five indices:** Direct enumeration costs $O(n^5)$ and becomes infeasible well below the maximum length.
- **Scan every distinct value per middle:** The same combinatorial families can be counted in $O(nd)$ time, but the maintained moments eliminate this inner scan.
...
- **Negative and large values:** Hash-map keys handle the entire value range without coordinate-sized storage.
