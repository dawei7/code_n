...
Let's say that we take `x` as the base and we do not yet know the relation between `x` and the total cost `F(x)`. Naturally, we want to try all possible bases to find the one that brings the minimum cost, however, there are infinite bases. Fortunately, we don't need to try every `x` which is impractical. **We just need to try every $\text{nums}[i]$ as the base**. The proof will be given later!

If we take $\text{nums}[i]$ as the base, we need to increase those numbers that are smaller than $\text{nums}[i]$ and decrease those numbers that are larger than $\text{nums}[i]$. Let's first roughly estimate the time complexity of this approach:

Assume there are $n$ integers in `nums`, for each base $\text{nums}[i]$, we want to get the cost of making every element equal $\text{nums}[i]$, this step needs an iteration over `nums` to add up each individual cost. Therefore, this approach needs $n$ iterations for each element in the input and takes $O(n ^ 2)$ time at least. It might not pass the time limit.

We could refine it by sorting `nums` first, don't forget to match $\text{cost}[i]$ with its original $\text{nums}[i]$.

![img](images/1.png)

This way, instead of iterating through `nums` every time, we only need to get the **difference** between the cost made by the current base $\text{nums}[i]$ and that made by the previous case $nums[i - 1]$. How does that happen? Let's look at the following picture, the cost consists of two parts:

- The cost of increasing those numbers smaller than $\text{nums}[i]$ (we call it 'prefix' part).
- The cost of decreasing those numbers larger than $\text{nums}[i]$ (we call it 'suffix' part).

![img](images/3.png)

Take a closer look at the 'prefix' part. Assume the current base `x` is a value between $\text{nums}[2]$ and $\text{nums}[3]$, if we increase it by `Δx`, the 'prefix' cost is increased by $Δx * (\text{cost}[0] + \text{cost}[1] + \text{cost}[2])$, in other words, the increment of the cost is proportional to the prefix sum of `cost`. Similarly, the decrement of the cost is proportional to the suffix sum of `cost`. It implies that we shall build the prefix sum array of the costs, then these prefix and suffix terms, as well as the change of the total cost, can be calculated in constant time.

![img](images/2.png)

Recall that we have left an unproven conclusion: we only need to try every $\text{nums}[i]$ as the base. The reason is that the total cost `F(x)` is **monotonic** when the `x` is in the range of two adjacent elements($\text{nums}[i]$ and $nums[i + 1]$).

As shown in the picture below, the current base `x` is within $\text{nums}[2]$ and $\text{nums}[3]$, if we increase `x` by `Δx`, the change of the total cost is $Δx * (prefix - suffix)$, which is monotonic. Therefore, either $\text{nums}[2]$ or $\text{nums}[3]$ is the base that brings the minimum cost (within the range `[nums[2], nums[3]]`).

![img](images/5.png)

Therefore, we only need to try each $\text{nums}[i]$ as the base, starting with $\text{nums}[0]$. Then we will iterate over the rest of the sorted `nums`, for $\text{nums}[i]$, we don't bother iterating over `nums` again, but use the prefix sum array to get the cost difference between $F(\text{nums}[i])$ and $F(nums[i - 1])$.
...
1) Collect every pair of $\text{nums}[i]$ and $\text{cost}[i]$ in the array `numsAndCost` and sort it according to $\text{nums}[i]$.

2) Build a prefix sum array `prefixCost` to record the prefix sum of the cost in the sorted `numsAndCost`.

3) Start with $\text{nums}[0]$, calculate the cost of making every element equal $\text{nums}[0]$ as `totalCost`.

4) Iterate over `nums` starting from $\text{nums}[1]$, for every base $\text{nums}[i]$:
- Calculate the different between the current base and the previous base as $gap = \text{nums}[i] - nums[i - 1]$.

- The current `totalCost` made by $\text{nums}[i]$, compared with the previous cost made by $nums[i - 1]$, is increased by `gap` times the prefix sum of costs $prefixCost[i - 1]$ and decreased by `gap` times the suffix sum of costs $prefixCost[n - 1] - prefixCost[i - 1]$.
...
Here, we define $f_i(x)$ as the cost function if `nums` only contains one element $\text{nums}[i]$.

If `nums` only contains $\text{nums}[i]$, the cost function $f_i(x)$ is convex, as shown in the picture below.
...
Let $n$ be the length of the input array `nums` and $k$ be the difference between the maximum and minimum value of $\text{nums}[i]$.
