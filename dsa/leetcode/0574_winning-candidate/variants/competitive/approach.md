## General
**Competitive Approach — Winning Candidate**

The query executes a structured relational pipeline for **Winning Candidate**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(V + C \log C)$ — Operation count proportional to input scale.
