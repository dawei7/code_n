...
- The corresponding characters of two strings match: $ch = \text{secret}[idx]$.

- Then it's time to update the bulls: `bulls += 1`.

- The update of the cows is needed if the count for the current character in the hashmap is negative or equal to zero. That means that before it was already used for cows, and the cows counter should be decreased: $cows -= int(h[ch] \le 0)$.

- The corresponding characters of two strings don't match: $ch \neq \text{secret}[idx]$. Then increase the cows counter: $cows += int(h[ch] > 0)$.
