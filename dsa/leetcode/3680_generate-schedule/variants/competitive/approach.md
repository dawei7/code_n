...
The core task in **Generate Schedule** is to an integer `n` representing `n` teams. You are asked to generate a schedule such that:. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
