...
7. Whenever we visit a child of a $\text{parent}_{node}$ we push the $(\text{parent}_{node}, updated_{parent\_state})$ onto the stack. We update the state of the parent since a child/branch has been visited/processed and accordingly the state changes.
