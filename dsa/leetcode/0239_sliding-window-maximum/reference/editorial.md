...
By maintaining the monotonic decreasing property, the largest element in the window must always be the first element in the deque, which is $nums[\text{dq}[0]]$.

We initialize the first window with the first `k` elements. Then we iterate over the indices `i` in the range `[k, n - 1]`, and for each element, we add its index to `dq` while maintaining the monotonic property. We also remove the first element in `dq` if it is too far to the left ($\text{dq}[0] = i - k$). After these operations, `dq` will correctly hold the indices of all useful elements in the current window in decreasing order. Thus, we can push $nums[\text{dq}[0]]$ to the answer.

Here's an animation visually showing how the approach works:

![Slide 1](images/slideshow_239-slides_239-1.png)

![Slide 2](images/slideshow_239-slides_239-2.png)

![Slide 3](images/slideshow_239-slides_239-3.png)

![Slide 4](images/slideshow_239-slides_239-4.png)

![Slide 5](images/slideshow_239-slides_239-5.png)

![Slide 6](images/slideshow_239-slides_239-6.png)

![Slide 7](images/slideshow_239-slides_239-7.png)

![Slide 8](images/slideshow_239-slides_239-8.png)
...
- While `dq` is not empty and the current element $\text{nums}[i]$ is greater or equal to `nums[dq.peekLast()]`, continue to pop the last element.
...
- While `dq` is not empty and the current element $\text{nums}[i]$ is greater or equal to `nums[dq.back()]`, continue to pop the last element.
- Push `i` at the end of `dq`.
