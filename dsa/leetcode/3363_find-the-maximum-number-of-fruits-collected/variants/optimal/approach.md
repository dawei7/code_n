...
The core task in **Find the Maximum Number of Fruits Collected** is to There is a game dungeon comprised of `n x n` rooms arranged in a grid. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
