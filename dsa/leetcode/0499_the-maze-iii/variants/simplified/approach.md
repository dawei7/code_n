# Simplified Approach - The Maze III
...
## Complexity detail
- **Time Complexity**: $O(rows \cdot cols \cdot p \log(rows \cdot cols))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(rows \cdot cols \cdot p)$ — Utilizes standard auxiliary storage proportional to the active computation state.
