...
## Complexity detail
- **Time Complexity**: $O(rows \cdot cols \cdot p \log(rows \cdot cols))$ — High-efficiency runtime performance.
- **Space Complexity**: $O(rows \cdot cols \cdot p)$ — Minimal auxiliary memory overhead.
