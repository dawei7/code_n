...
Let's try to check separately whether each word $\text{words}[i]$ is a subsequence of `S`.

For every such `word`, we try to find the first occurrence of $\text{word}[0]$ in `S`, then from that point on try to find the first occurrence of $\text{word}[1]$, and so on.
...
* Time Complexity:  $O(\text{words.length} * S\text{.length} + \sum_i \text{\text{words}[i].length})$.  For each word, our `subseq` check on word $\text{words}[i]$ may take time complexity $O(S\text{.length} + \text{\text{words}[i].length})$.

* Space Complexity: $O(1)$.  (In Java, our space complexity is $O(S\text{.length} + \text{max(\text{words}[i].length)})$, but can be made to be $O(1)$ with a pointer based implementation.)
...
* Time Complexity:  $O(S\text{.length} + \sum_i \text{\text{words}[i].length})$.

* Space Complexity: $O(\text{words.length})$.  (In Java, our additional space complexity is $O(\sum_i \text{\text{words}[i].length})$, but can be made to be $O(\text{words.length})$ with a pointer based implementation.)