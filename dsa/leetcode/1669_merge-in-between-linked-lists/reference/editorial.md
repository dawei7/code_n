...
We traverse `list1` with the pointer `end`, which starts at the head of `list1` and is progressed using $end = \text{end.next}$ until `end` points to the node at index `b` of `list1`. Inside the loop, we set `start` to `end` if $index = a - 1$.
...
4. Find the tail of `list2` by traversing the list with $list2 = \text{list2.next}$ until the last node is reached.
