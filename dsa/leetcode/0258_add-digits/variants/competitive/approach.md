...
The core task in **Add Digits** is to an integer `num`, repeatedly add all its digits until the result has only one digit, and return it. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
