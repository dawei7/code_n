...
- Iterate through each element in `nums`:
  - If the current element $\text{nums}[i]$ is not zero:
- Place $\text{nums}[i]$ at index `lastNonZeroFoundAt`.
...
- Iterate through each element in `nums` using `cur` as the index:
  - If $\text{nums}[cur]$ is not zero:
- Swap $\text{nums}[lastNonZeroFoundAt]$ with $\text{nums}[cur]$ to move the non-zero element to the correct position.
