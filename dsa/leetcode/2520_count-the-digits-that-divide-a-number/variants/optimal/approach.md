...
The core task in **Count the Digits That Divide a Number** is to an integer `num`, return *the number of digits in `num` that divide *`num`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
