...
By keeping track of `balance` (the number of `(` parentheses minus the number of `)` parentheses), we can partition `S` into primitive substrings $S = P_{1} + P_{2} + ... + P_{n}$.  Then, $score(S) = score(P_{1}) + score(P_{2}) + ... + score(P_{n})$, by definition.
