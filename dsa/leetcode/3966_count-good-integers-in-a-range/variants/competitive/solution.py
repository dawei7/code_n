...
class Solution:
    def goodIntegers(self, l, r, k):
        ...
        def count(n):
            ...
            result = 1
            bound = next((i for i in range(1, len(digits)) if abs(digits[i]-digits[i-1]) > k), len(digits))
            if bound == len(digits):
                result += 1
            for i in reversed(range(len(digits))):
                dp = [prefix[min(d+k, 9)+1]-prefix[max(d-k, 0)] for d in range(10)] if i != len(digits)-1 else [1]*10
                prefix = [0]*(len(dp)+1)
