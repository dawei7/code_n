...
4) If `queue` has nodes, get the first node $\text{curr}_{node}$ from `queue`. Return `true` if $\text{curr}_{node}$ is `destination`, otherwise, go to step 5.
5) Add all **unvisited** neighbor nodes of $\text{curr}_{node}$ to `queue` and mark them as **visited**, then repeat step 4.
...
4) While `stack` has nodes, get the top node $\text{curr}_{node}$ from `stack`.
- If $\text{curr}_{node}$ equals `destination`, return `true`.
- Otherwise, add **unvisited** neighbor nodes of $\text{curr}_{node}$ to `stack` and mark them as **visited** and repeat step 4.
...
2) Iterate over all edges. For each edge $edge = [\text{node}_{a}, \text{node}_{b}]$, use the DSU data structure we initialized to connect $\text{node}_{a}$ and $\text{node}_{b}$.
