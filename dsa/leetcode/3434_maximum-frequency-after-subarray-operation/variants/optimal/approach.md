...
The core task in **Maximum Frequency After Subarray Operation** is to an array `nums` of length `n`. You are also given an integer `k`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
