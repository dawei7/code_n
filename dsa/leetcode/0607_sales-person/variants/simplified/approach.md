# Simplified Approach - Sales Person
...
## Complexity detail
- **Time Complexity**: $O((S + C + O) \log(S + C + O))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(S + C + O)$ — Utilizes standard auxiliary storage proportional to the active computation state.
