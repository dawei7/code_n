...
The core task in **Longest Unequal Adjacent Groups Subsequence I** is to a string array `words` and a **binary** array `groups` both of length `n`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
