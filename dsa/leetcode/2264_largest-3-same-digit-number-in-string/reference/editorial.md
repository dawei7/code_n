...
- In this method, iterate over string `num` from index $idx = 0$ till $\text{num.size}() - 3$ and return `true` if for any index `idx`, characters at indices `idx`, $(idx + 1)$, and $(idx + 2)$ are `sameDigitNumber`. Otherwise, return `false`.
...
2. Iterate on string `num` from index $idx = 0$ till $\text{num.size}() - 3$.
- For any index `idx`, if the `idx`, $(idx + 1)$, and $(idx + 2)$ index characters are the same then store the maximum of `maxDigit` and $\text{num}[idx]$ in `maxDigit`.
