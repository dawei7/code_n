...
This brings us to our solution. We will use a sliding window over the sorted `nums`. For each element $\text{nums}[right]$, we will treat `target` as this element and try to make every element in our window equal to `target`.
...
However, if we add an element $\text{nums}[right]$ to the window and the window is valid, then the if statement will not trigger, and `left` will not be incremented. Thus, we will increase our window size by `1`. In this scenario, it implies the current window `[left, right]` is the best window we have seen so far.
...
Given an index `i`, if we treat $\text{nums}[i]$ as `target`, we are concerned with how many elements on the left we can take. In the earlier approaches, we used a sliding window. In this approach, we will directly find the left-most index of these elements using binary search.

Let's say that `best` is the index of the furthest element to the left that we could increment to $target = \text{nums}[i]$. Note that here, `best` is analogous to what `left` was after the while loop finished in the first approach. How do we find `best`?
...
- $target = \text{nums}[i]$, the current target.
...
- Calculate $originalSum = \text{prefix}[i] - \text{prefix}[mid] + \text{nums}[mid]$.
