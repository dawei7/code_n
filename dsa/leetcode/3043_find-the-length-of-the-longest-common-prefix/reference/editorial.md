...
- Iterate over each character `digit` in `numStr`:
      ...
      - If $node.\text{children}[idx]$ is null, create a new `TrieNode` and assign it to $node.\text{children}[idx]$.
      - Move to the child node at $node.\text{children}[idx]$.
...
- Iterate over each character `digit` in `numStr`:
      - Convert `digit` to its integer index `idx`.
      - If $node.\text{children}[idx]$ exists, increment `len` and move to the child node at $node.\text{children}[idx]$.
      - If $node.\text{children}[idx]$ is null, break the loop as the prefix match ends.
