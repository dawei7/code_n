...
![Few examples of inorder successors](images/img1.png)

*Figure 1. Few examples of inorder successors.*

![Inorder example](images/img2.png)
...
2. Inside the function `inorderSuccessor`, we first check which of the two cases we need to handle. For that, we simply check for the presence of the right child.
   * *The right child exists*
       ...
       In this case, we assign the right child to a node called `leftmost` and we iterate until we reach a node (`leftmost`) that doesn't have a left child. We iteratively assign $leftmost = \text{leftmost.left}$ and that's how we will get the leftmost node in the subtree.
...
* For case 1, we might have a scenario where the root node has a right subtree that is left-skewed. Something like the following.
        ...
        ![Time complexity skewed](images/img6.png)
...
* For case 2, we might have to process the entire tree before finding the in-order successor. Let's look at an example tree to understand when that might happen.
        ...
        ![Time complexity skewed](images/img7.png)
...
![BST property depiction](images/img8.png)
...
2. At each step we compare the value of node `p` with that of `node`.
   1. If $\text{p.val} \ge \text{node.val}$ that implies we can safely discard the left subtree since all the nodes there including the current `node` have values less than `p`.

       ![Skipping the left subtree](images/img9.png)
