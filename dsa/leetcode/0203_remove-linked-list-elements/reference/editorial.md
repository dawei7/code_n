...
![bla](images/to_delete2.png)

Here sentinel node will be used as a pseudo-head.

**Algorithm**

- Initiate the sentinel node as `ListNode(0)` and set it to be the new head: $\text{sentinel.next} = head$.
...
- If the values are equal, delete the current node: $\text{prev.next} = \text{curr.next}$.

- Otherwise, set the predecessor to be equal to the current node.

- Move to the next node: $curr = \text{curr.next}$.
