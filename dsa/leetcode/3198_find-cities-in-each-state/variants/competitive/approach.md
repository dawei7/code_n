## General
**Competitive Approach — Find Cities in Each State**

The query executes a structured relational pipeline for **Find Cities in Each State**. It uses `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
