# Simplified Approach - Power Grid Maintenance
...
## Complexity detail
- **Time Complexity**: $O((c + m) \alpha(c) + q + c log c)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(c + q)$ — Utilizes standard auxiliary storage proportional to the active computation state.
