...
The core task in **Minimum Time to Make Rope Colorful** is to Alice has `n` balloons arranged on a rope. You are given a **0-indexed** string `colors` where $\text{colors}[i]$ is the color of the $$i^{\text{th}}$$ balloon. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
