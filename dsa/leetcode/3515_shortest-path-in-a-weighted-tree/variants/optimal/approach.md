...
When the exit marker is eventually popped, every descendant has already been entered and assigned a preorder number. The current `timer` is stored as `exit_time[node]`.
