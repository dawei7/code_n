...
![graph example](images/399_graph_example.png)
...
- We build a graph out the equations. In the worst case where there is no overlapping among the equations, we would have $N$ edges and $2N$ nodes in the graph.
    Therefore, the space complexity of the graph is $\mathcal{O}(N + 2N) = \mathcal{O}(3N) = \mathcal{O}(N)$.
...
The entry is defined as $key -> (\text{group}_{id}, weight)$.
...
- Initially, the entries for each variable would look like the following, where the $\text{group}_{id}$ of each variable is the variable itself and the `weight` of each variable is `1`.
Each variable forms a group on its own, since there is no relationship among them at the moment.

![init state](images/399_union_find_init.png)
...
![second state](images/399_union_find_ab.png)
...
![third state](images/399_union_find_bc.png)

- As one might notice, there is some inconsistency in the above graph, _i.e._ the $\text{group}_{id}$ of the variable of `a` should then be `c` and the weight of the variable `a` should be `6` rather than `2`.
Indeed, these inconsistencies are expected.
The **magic** happens when we invoke the **Find** operation on the variable `a`, where a _chain_ reaction would be triggered to update the $\text{group}_{id}$ and `weight` along the chain, as follows:

![final state](images/399_union_find_ac.png)
...
- `find(variable)`: the function will return the $\text{group}_{id}$ that the variable belongs to. Moreover, the function will update the states of variables along the chain, if there is any discrepancy.
...
- case 2): If both variables are valid, we then apply the `find(variable)` to obtain the tuple of $(\text{group}_{id}, weight)$ for each variable. If they are not of the same group, _i.e._ there is no chain of division between them, we then return `-1.0` as the result.
