...
To address this, we use a dynamic programming approach, recording the minimum distance from the source to each node in an array as we progress through the graph. To account for discounts on edge weights, we employ a 2D array `DP`, where $\text{DP}[v][d]$ represents the minimum distance from the source to node `v` using exactly `d` discounts.

At each step, we can either use a discount on the current edge or not. If we use a discount, we update $\text{DP}[v][d + 1]$ with the halved toll cost. If we don't use a discount, we update $\text{DP}[v][d]$ with the full toll cost, ensuring our array captures the minimum distances for both scenarios.

Ultimately, the minimum cost to reach the target node with any number of discounts will be the smallest value in $\text{DP}[target][d]$ for all valid `d`. While this approach works for smaller graphs, it may cause a Time Limit Exceeded (TLE) error for larger graphs due to the exhaustive checking of every node and discount combination. To avoid this, we need a more efficient method to reduce the number of explorations and speed up the process, which we will discuss next.
...
7. While `pq` is not empty:
   ...
   - For each neighbor pair `(neighbor, toll)` in the adjacency list of `city`:
     - If traveling to `neighbor` without using a discount results in a lower cost:
       - Update $\text{dist}[neighbor][discountsUsed]$ to $currentCost + toll$.
     ...
     - If `discountsUsed` is less than `discounts` and using a discount results in a lower cost:
       ...
       - Update $\text{dist}[neighbor][discountsUsed + 1]$ to `discountedCost`.
...
- Time Complexity: $O((N \cdot K + E) \cdot \log(N \cdot K))$
    ...
    $
    $\mathcal{O}(E + (N \cdot K)$ \log(N \cdot K) + E \log(N \cdot K)) = $\mathcal{O}((N \cdot K + E)$ \cdot \log(N \cdot K))
    $
...
For example, consider a graph with nodes `A`, `B`, and `C`, where the edges are as follows: $\text{highways}[0] = [A, B, 5]$, $\text{highways}[1] = [B, C, 10]$, and $\text{highways}[2] = [A, C, 12]$. Suppose we have one discount available.

If we find a path from `A` to `B` to `C` with a total cost of `15` (`5` from `A` to `B` and `10` from `B` to `C`), and record this in $\text{dist}[C][0]$, we can later skip expanding any path to `C` with a higher accumulated cost, such as `20`, since it exceeds the recorded minimum cost of `15`.
...
- Time Complexity: $O((N \cdot K + E) \cdot \log(N \cdot K))$
    ...
    Combining these steps, the overall time complexity is,
    $O(E + (N \cdot K) \log(N \cdot K) + E \log(N \cdot K)) =$\mathcal{O}((N \cdot K + E)$\log(N \cdot K))$
