...
2. Let's create a function `getSumOfKthPowerOfDigits()` that takes in the parameters `n` and `k` and returns the sum of taking the k'th power of each digit. The function works as explained further.
   ...
   2. Get the $\text{last}_{digit}$ of `n` by using the formula `n % 10`.
   3. Add $\text{last}_{digit}$ raised to the power of `k` to the result.
   4. Remove the $\text{last}_{digit}$ of `n` by using the formula `n /= 10`.
   5. Repeat steps 1 - 4 until $n \neq 0$.
...
5. Repeat steps 3 and 4 while $tempN \neq 0$.
