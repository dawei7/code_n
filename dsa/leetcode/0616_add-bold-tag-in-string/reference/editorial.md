...
If we can figure out which characters in the string need to be bold, then it's relatively easy to add the tags. So how do we figure out which characters should be bold? We can use a boolean array `bold` with the same length as `s`. If $\text{bold}[i] = true$, it means that the $i^{th}$ character of `s` should be bold.
...
2. While $start \neq -1$:
- Iterate over the indices of the substring $[start, start + \text{word.length})$ and mark them as `true` in `bold`.
- After marking all the indices, set $start = \text{s.find}(word, start + 1)$. We will look for another occurrence of `word` in `s` that comes after the previous occurrence we found.
...
We can iterate over the indices of `s` and at each index `i`, if $\text{bold}[i]$:
...
1. Initialize $n = \text{s.length}$ and a boolean array `bold` of length `n`, with values initially set to `false`.
2. Iterate over `words`. For each `word`, use the process described above to mark characters in `bold`:
- Set $start = \text{s.find}(word)$.
- While $start \neq -1$, iterate `i` from `start` until $start + \text{word.length}$ and set $\text{bold}[i] = true$. Then, set $start = \text{s.find}(word, start + 1)$.
3. Build the answer `ans`. Iterate over the indices of `s` using `i`.
- If $\text{bold}[i]$ and either $i = 0$ or $bold[i - 1] = false$, add `<b>` to the answer.
- Add $s[i]$ to the answer.
- If $\text{bold}[i]$ and either $i = n - 1$ or $bold[i + 1] = false$, add `</b>` to the answer.
