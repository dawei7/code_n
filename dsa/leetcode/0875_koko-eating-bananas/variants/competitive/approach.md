...
The core task in **Koko Eating Bananas** is to Koko loves to eat bananas. There are `n` piles of bananas, the $$i^{\text{th}}$$ pile has $\text{piles}[i]$ bananas. The guards have gone and will come back in `h` hours. Instead of scanning every element one by one in $O(n)$ time, this solution uses **Binary Search**. Think of looking up a word in a dictionary: you open it in the middle, see if your word comes before or after, and discard half of the remaining pages. By halving the candidate window at each step, we find the answer in fast logarithmic $O(\log n)$ time.
