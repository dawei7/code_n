...
Where $\text{HASH}_{MULTIPLIER}$ is a constant slightly larger than twice the maximum possible coordinate value. In this case, we choose `60013`.
...
- Create a constant $\text{HASH}_{MULTIPLIER}$ to use in the hashing function.
...
- Time complexity: $O(m + n)$
    ...
    Thus, the overall time complexity of the algorithm is $O(n) +$\mathcal{O}(m)$= O(m + n)$.
