...
![prefix sum formula](images/Slide1.png)
...
![prefix sum formula2](images/Slide2.png)
...
We iterate over all the elements from the beginning of `nums`. We set $prefixMod = (prefixMod + \text{nums}[i]) \% k$ for each element to find the remainder of the prefix sum when divided by `k`.
...
1. Initialize an integer $prefixMod = 0$ and a hashmap `modSeen`. Initialize $\text{modSeen}[0]$ with -1 to account for the initial value of prefixMod.
2. Iterate over all the elements of `nums`:
   - Compute the `prefixMod` as $prefixMod = (prefixMod + \text{nums}[i]) \% k$.
