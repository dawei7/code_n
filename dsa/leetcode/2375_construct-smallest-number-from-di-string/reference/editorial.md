...
- If $\text{pattern}[i] = 'I'$, then the digit at position `i` in the number should be smaller than the digit at position $i + 1$.
- If $\text{pattern}[i] = 'D'$, then the digit at position `i` should be larger than the digit at position $i + 1$.
...
- If the pattern character is `'I'`, ensure that the corresponding number in the sequence is in increasing order ($\text{sequence}[patternIndex] < sequence[patternIndex + 1]$).
- If the pattern character is `'D'`, ensure that the corresponding number in the sequence is in decreasing order ($\text{sequence}[patternIndex] > sequence[patternIndex + 1]$).
...
##### `smallestNumber` Function:
  ...
  - Use the $\text{next}_{permutation}$ function to generate successive permutations of the sequence.
...
- If $\text{pattern}[currentIndex]$ is 'I', increment `currentCount` and recurse with the next index.
- If $\text{pattern}[currentIndex]$ is 'D', recurse without incrementing `currentCount` and recurse with the next index.
...
Instead of modifying an existing sequence as we traverse the pattern, we first scan the pattern **backward** to compute an array $\text{arrD}[i]$, where each entry represents the number of consecutive `'D'`s that appear after the corresponding position. This precomputed information allows us to determine the exact digit that should be placed in each position without needing to reverse segments manually.
...
When encountering an `'I'`, we simply assign the smallest available number, which is $maxSoFar + 1$. However, when encountering a `'D'`, we need to ensure that the digits form a descending order. To achieve this, we use $\text{arrD}[i]$ to determine how far the descending sequence extends. Instead of constructing the decreasing sequence step by step, we calculate the correct number directly:
...
![Slide 1](images/slideshow_optimized_greedy_fix_slide1_fix.png)

![Slide 2](images/slideshow_optimized_greedy_fix_slide2_fix.png)

![Slide 3](images/slideshow_optimized_greedy_fix_slide3_fix.png)

![Slide 4](images/slideshow_optimized_greedy_fix_slide4_fix.png)

![Slide 5](images/slideshow_optimized_greedy_fix_slide5_fix.png)

![Slide 6](images/slideshow_optimized_greedy_fix_slide6_fix.png)

![Slide 7](images/slideshow_optimized_greedy_fix_slide7_fix.png)

![Slide 8](images/slideshow_optimized_greedy_fix_slide8_fix.png)

![Slide 9](images/slideshow_optimized_greedy_fix_slide9_fix.png)

![Slide 10](images/slideshow_optimized_greedy_fix_slide10_fix.png)

![Slide 11](images/slideshow_optimized_greedy_fix_slide11_fix.png)
...
- If the current character is `'D'`, calculate the appropriate digit from `maxSoFar` and $\text{arrD}[position]$, append it to `result`, and update `currMax`.
