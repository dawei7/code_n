...
The core task in **Set Matrix Zeroes** is to an `m x n` integer matrix `matrix`, if an element is `0`, set its entire row and column to `0`'s. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
