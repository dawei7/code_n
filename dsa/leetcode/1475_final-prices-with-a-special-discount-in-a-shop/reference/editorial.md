...
- If $\text{prices}[j]$ is less than or equal to $\text{prices}[i]$:
      - Calculate the discounted price by subtracting $\text{prices}[j]$ from $\text{prices}[i]$.
      - Store the calculated discounted price in $\text{result}[i]$.
