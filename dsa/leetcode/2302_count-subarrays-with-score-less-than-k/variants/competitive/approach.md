...
The core task in **Count Subarrays With Score Less Than K** is to The **score** of an array is defined as the **product** of its sum and its length. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
