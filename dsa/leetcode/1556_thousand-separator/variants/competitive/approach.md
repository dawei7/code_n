...
The core task in **Thousand Separator** is to an integer `n`, add a dot (".") as the thousands separator and return it in string format. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
