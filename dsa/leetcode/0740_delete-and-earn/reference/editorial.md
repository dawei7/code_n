...
![Slide 1](images/slideshow_740_Delete_and_Earn_740_1.png)

![Slide 2](images/slideshow_740_Delete_and_Earn_740_2.png)

![Slide 3](images/slideshow_740_Delete_and_Earn_740_3.png)
...
4. In `maxPoints`, first check for the base cases. If $num = 0$, return `0`. If $num = 1$, return $\text{points}[1]$. Otherwise, check if `num` is already in `cache`. If it is, just return $\text{cache}[num]$.

5. Otherwise, apply the recurrence relation. Find the answer for `num` with $max(maxPoints(num - 1), maxPoints(num - 2) + \text{points}[num])$. Store this answer in `cache`, and then return it.
...
In the top-down approach, we start at the "top" (`maxNumber`), and move towards our base cases ($\text{maxPoints}[0] = 0, \text{maxPoints}[1] = \text{nums.count}(1)$). With bottom-up, we will start at the base cases and iterate away from them towards the result we want (`maxNumber`).

For this approach, instead of having `maxPoints` be a recursive function, we will have it be an array where $\text{maxPoints}[num]$ stores the maximum points we can gain if we consider only numbers from `0` to `num`. As you can see, this is just a different way to format `maxPoints`, but in terms of functionality, it will be the same as the previous approach.
...
3. Initialize an array `maxPoints` of size $maxNumber + 1$, where $\text{maxPoints}[num]$ will store the maximum amount of points we can gain if we only considered numbers from `0` to `num` (inclusive). Set all the values to `0` initially, except for $\text{maxPoints}[1]$ - it should be set to $\text{points}[1]$ (our base case).

4. Iterate from `2` to `maxNumber`. At each iteration, apply the recurrence relation $\text{maxPoints}[num] = max(maxPoints[num - 1], maxPoints[num - 2] + \text{points}[num])$ to populate `maxPoints`.

5. At the end, return $\text{maxPoints}[maxNumber]$.
...
In our recurrence relation, for a given `num`, we refer to $num - 1$ and $num - 2$. To find $\text{maxPoints}[10]$, we only need $\text{maxPoints}[9]$ and $\text{maxPoints}[8]$. While it is true that we also need to calculate `maxPoints` for `0` to `7`, by the time we get to `10`, we don't need those values anymore. Because of this, instead of using an array to store the answers to all states, we can improve to constant space by only storing the previous two values.

Let's use two integer variables, `twoBack` and `oneBack`, that at any given `num` will represent $maxPoints[num - 2]$ and $maxPoints[num - 1]$ respectively. In the second approach, we started iterating from `2`. This means that `twoBack` should be initialized the same as $\text{maxPoints}[0] = 0$, and `oneBack` should be initialized the same as $\text{maxPoints}[1] = \text{nums.count}(1)$.

Our recurrence relation remains the same, as long as we appropriately substitute the variables. Now, it is $\text{maxPoints}[num] = max(oneBack, twoBack + \text{points}[num])$. Once we calculate this value, we need to update `twoBack` and `oneBack`. When we go to the next number, the previous value (`oneBack`) will become `twoBack`, and the current value will become `oneBack`. At the very end, `oneBack` will be our answer.
...
If we find that adjacent elements have a difference of `1`, that means we can only take the points associated with one of them, and we need to use the same recurrence relation as in the previous approach ($oneBack = max(oneBack, twoBack + \text{points}[num])$). However, if two elements do not differ by `1`, then that means we don't need to worry about deletions, and we should always take the points. In this case, we just add $\text{points}[num]$ to `oneBack`.

<img src="images/740_4.png" width="960"> <br>
...
4. Initialize $twoBack = 0$ and $oneBack = points[\text{elements}[0]]$. These are the base cases.
...
- If they differ by `1`, then we need to apply the normal recurrence relation $oneBack = max(oneBack, twoBack + \text{points}[currentElement]$.
- If they don't, then it is always beneficial to take the points. Do $oneBack += \text{points}[currentElement]$.
...
* Time complexity: $O(N \cdot log(N))$
    ...
    This gives us a time complexity of $O(N + N \cdot log(N) + N) =$\mathcal{O}(N \cdot \log(N)$)$.
