# Simplified Approach - The Earliest Moment When Everyone Become Friends
...
## Complexity detail
- **Time Complexity**: $O(n + m \log m + m\alpha(n))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(m + n)$ — Utilizes standard auxiliary storage proportional to the active computation state.
