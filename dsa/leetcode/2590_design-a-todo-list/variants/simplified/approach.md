# Simplified Approach - Design a Todo List
...
## Complexity detail
- **Time Complexity**: $O(q^2 log q)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(q + S)$ — Utilizes standard auxiliary storage proportional to the active computation state.
