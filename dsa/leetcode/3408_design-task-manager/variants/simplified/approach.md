# Simplified Approach - Design Task Manager
...
## Complexity detail
- **Time Complexity**: $O((t + q) log(t + q))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(t + q)$ — Utilizes standard auxiliary storage proportional to the active computation state.
