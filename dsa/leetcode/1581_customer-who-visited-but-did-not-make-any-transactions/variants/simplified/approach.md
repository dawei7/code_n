# Simplified Approach - Customer Who Visited but Did Not Make Any Transactions
...
## Complexity detail
- **Time Complexity**: $O((V+T)\log(V+T))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(V+T)$ — Utilizes standard auxiliary storage proportional to the active computation state.
