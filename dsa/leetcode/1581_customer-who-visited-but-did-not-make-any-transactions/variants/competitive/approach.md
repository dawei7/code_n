## General
**Competitive Approach — Customer Who Visited but Did Not Make Any Transactions**

The query executes a structured relational pipeline for **Customer Who Visited but Did Not Make Any Transactions**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O((V+T)\log(V+T))$ — Operation count proportional to input scale.
