...
To represent the gas gain and consumption at each station using a single array, we can define $\text{gain}[i] = \text{gas}[i] - \text{cost}[i]$. The value of $\text{gain}[i]$ represents the **net change** in the amount of gas in our tank when moving from station `i` to $i + 1$. At each station `i`, the amount of gas in the tank is changed by $\text{gain}[i]$. If, at any point, the gas in our tank drops below zero, we cannot reach the next station nor finish the circular route.
...
Upon adopting the given idea, we start our journey from $i = 0$ and verify if station 0 is a valid starting station by accumulating gas $\text{gain}[i]$ at each station along the way.
...
$\text{gain}_1 \ge -(\text{gain}_2 + \text{gain}_3 + ... )$
...
1) Initialize $\text{curr}_{gain}$, $\text{total}_{gain}$ and `answer` to 0.

2) Traverse through `gas` and `cost`. For each index `i`, increment $\text{total}_{gain}$ and $\text{curr}_{gain}$ by  $\text{gas}[i] - \text{cost}[i]$.

3) If $\text{curr}_{gain}$ is smaller than 0, we will test if station $i + 1$ is a valid starting station by setting `answer` as $i + 1$, resetting $\text{curr}_{gain}$ to 0, and repeating step 2.

4) When the iteration is complete, if $\text{total}_{gain}$ is smaller than 0, return `-1`. Otherwise, return `answer`.
...
- The algorithm traverses through `gas` and `cost` once. Updating $\text{total}_{gain}$, $\text{curr}_{gain}$ and `answer` takes $O(1)$ time at each step.

* Space complexity: $O(1)$

- We only need to keep track of some parameters $\text{total}_{gain}$, $\text{curr}_{gain}$, and `answer`, which take $O(1)$ space.
