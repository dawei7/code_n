...
2) Traverse through `gas` and `cost`. For each index `i`, increment `total_gain` and `curr_gain` by  `gas[i] - cost[i]`.
...
4) When the iteration is complete, if `total_gain` is smaller than 0, return `-1`. Otherwise, return `answer`.
