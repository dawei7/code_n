...
3. Use a for loop to iterate a variable `start` starting from `0` until and including $\text{s.length} - length$. This variable represents the starting point of the substring we are currently considering.
...
Let's use a table `dp` with dimensions of $n * n$. $\text{dp}[i][j]$ is a boolean representing if the substring with inclusive bounds `i, j` is a palindrome. We initialize $\text{dp}[i][i] = true$ for the substrings of length 1, and then $\text{dp}[i][i + 1] = (s[i] = s[i + 1])$ for the substrings of length 2.
...
If this condition is true, then the substring with inclusive bounds `i, j` must be a palindrome. We set $\text{dp}[i][j] = true$.
...
1. Initialize $n = \text{s.length}$ and a boolean table `dp` with size $n * n$, and all values to `false`.
2. Initialize $ans = [0, 0]$. This will hold the inclusive bounds of the answer.
3. Set all $\text{dp}[i][i] = true$.
4. Iterate over all pairs $i, i + 1$. For each one, if $s[i] = s[i + 1]$, then set $\text{dp}[i][i + 1] = true$ and update $ans = [i, i + 1]$.
...
- In that case, set $\text{dp}[i][j] = true$ and $ans = [i, j]$
...
This is very promising - we can lower the minimum iterations required if we focus on the centers instead of on the bounds. Let's use a helper method `expand(i, j)` that starts two pointers $left = i$ and $right = j$. In this method, we will consider `i, j` as a center. When $i = j$, we are considering odd-length palindromes. When $i \neq j$, we are considering even-length palindromes. We will expand from the center as far as we can to find the longest palindrome, and then return the length of this palindrome.
...
- However, when the while loop ends, it implies $s[left] \neq s[right]$. Therefore, we need to subtract `2`. Return $right - left - 1$.
2. Initialize $ans = [0, 0]$. This will hold the inclusive bounds of the answer.
3. Iterate `i` over all indices of `s`.
- Find the length of the longest odd-length palindrome centered at `i`: $oddLength = expand(i, i)$.
- If `oddLength` is the greatest length we have seen so far, i.e. $oddLength > \text{ans}[1] - \text{ans}[0] + 1$, update `ans`.
