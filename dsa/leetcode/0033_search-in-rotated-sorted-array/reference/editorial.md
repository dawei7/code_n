...
After identifying the middle element in the searching space `[left ~ right]`, we compare $\text{nums}[mid]$ with `nums[-1]`.
- If $\text{nums}[mid] > nums[-1]$, it suggests that the pivot value lies on the right of $\text{nums}[mid]$. We will then proceed with the right half of the search space, which is `[mid + 1 ~ right]`.
- Otherwise, the pivot value is $\text{nums}[mid]$ or it's situated to the left of $\text{nums}[mid]$, we continue with the left half of the searching space, which is `[left ~ mid - 1]`.
...
- If $\text{nums}[mid] > nums[n - 1]$, this suggests that `pivot` is located to the right of `mid`, hence we set $left = mid + 1$. Otherwise, `pivot` could be either at `mid` or to the left of `mid`, in which case we should set $right = mid - 1$.
...
Our key task is to locate `pivot`, the index of the smallest value in `nums`. Notably, $\text{nums}[pivot]$ would have been at index 0 in the unrotated, original array. Hence, if we were to rotate it to the right by $n - pivot$ steps (taking the modulus of `n` into account), it would return to its original position, index 0.
...
Let's take a step back and consider a regular binary search. Why are we able to confidently discard half of the array after comparing `target` with the middle value $\text{nums}[mid]$? The reason is that both halves of the array are sorted. Hence, if `target` is less than the middle value, it's assured to be smaller than every value in the right half. If `target` is larger than the middle value, it's guaranteed to be larger than every value in the left half. Therefore, we can safely discard one half of `nums` in either case.
...
> - If $A[l] \le target \le A[r]$, then `A[l ~ r]` might contain `target`, which needs to be verified by binary search, we will continue with this subarray.
...
To sum up, there are 3 possible cases after comparing `target` with $\text{nums}[mid]$:

Case 1. If $\text{nums}[mid] = target$, which denotes that we have found `target`, return `mid` as its index.

Case 2. If $\text{nums}[mid] \ge \text{nums}[left]$. It implies that the left subarray `nums[left ~ mid]` is sorted. We can determine whether to proceed with this subarray by comparing `target` with the boundary elements:

- If $\text{nums}[left] \le target$ and $target < \text{nums}[mid]$, it suggests that the sorted left half might include `target` while the other half does not contain `target`. Consequently, we focus on the left half for further steps.
- Otherwise, the left half is guaranteed not to contain `target`, and we will move on to the right half.

![img](images/q1.png)

Case 3. If $\text{nums}[mid] < \text{nums}[left]$, it implies that the left subarray is rotated and the right subarray `nums[mid ~ right]` is sorted. Therefore, we can determine whether to proceed with the right subarray by comparing the target with its boundary elements:

- If $\text{nums}[mid] < target$ and $target < \text{nums}[right]$, it implies that the sorted right half might contain `target`. As a result, we will move on with the right half.
...
- Case 1: Check if the middle element $\text{nums}[mid]$ is equal to `target`.
      - If true, return `mid` as the index of `target`.

- Case 2: Check if the subarray from `left` to `mid` is sorted ($\text{nums}[mid] \ge \text{nums}[left]$).
      - If `target` is within the range $[\text{nums}[left], \text{nums}[mid])$:
- Adjust the search range by setting `right` to $mid - 1$.
      - Otherwise:
- Adjust the search range by setting `left` to $mid + 1$.

- Case 3: If the subarray from `mid` to `right` is sorted ($\text{nums}[mid] > \text{nums}[right]$):
      - If `target` is within the range $(\text{nums}[mid], \text{nums}[right]]$:
- Adjust the search range by setting `left` to $mid + 1$.
      - Otherwise:
- Adjust the search range by setting `right` to $mid - 1$.
