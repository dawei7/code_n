...
Now, the indices of $arr$ can be used instead of storing the elements again. Thus, we make use of $arr$ in such a way that, $\text{arr}[i]$ is used to store
...
Now, using this idea as the base, let's take it a step forward and use it for the current problem. By taking the XOR of all the elements of the given $nums$ array with all the numbers from $1$ to $n$, we will get a result, $xor$, as $x \oplus y$. Here, $x$ and $y$ refer to the repeated and the missing term in the given $nums$ array. This happens on the same grounds as in the first problem discussed above.
...
![XOR](images/645_Set_Mismatch.PNG)
