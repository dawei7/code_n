...
The core task in **Flower Planting With No Adjacent** is to You have `n` gardens, labeled from `1` to `n`, and an array `paths` where $\text{paths}[i] = [x_{i}, y_{i}]$ describes a bidirectional path between garden $x_{i}$ to garden $y_{i}$. In each garden, you want to plant one of 4 types of flowers. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
