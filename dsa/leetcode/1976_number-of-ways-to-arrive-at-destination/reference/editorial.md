...
To achieve this, we introduce an additional array, `pathCount`, where $\text{pathCount}[i]$ keeps track of the number of ways to reach node `i` in the shortest time possible. This modification allows us to not only compute the shortest travel time but also count all valid paths that follow this time constraint.

Initially, $\text{pathCount}[0] = 1$, since there is exactly one way to start at node `0`. When we find a new shorter path to a node, we reset its path count to be the same as the number of ways we could reach the previous node, since we have discovered a new optimal route.
...
This problem is notorious for its edge cases, which often cause issues when submitted. A common mistake is using $\text{INT}_{MAX}$ (or similar equivalent in the language of your choice) as the initial value, assuming it is large enough to represent an unreachable node. However, for this problem, using $\text{INT}_{MAX}$ causes incorrect results or even integer overflow in certain test cases.
...
To avoid this issue, we should initialize the `shortestTime` array with $\text{LLONG}_{MAX}$, which is $9.2 × 10^18$, or use a sufficiently large constant like `1e12`. Both options ensure that our algorithm can correctly compute distances without encountering overflow. This small but crucial adjustment is necessary to handle the problem’s constraints correctly.

The algorithm is visualized below:

![Slide 1](images/slideshow_dijikstra_dijikstra_slide1.png)

![Slide 2](images/slideshow_dijikstra_dijikstra_slide2.png)

![Slide 3](images/slideshow_dijikstra_dijikstra_slide3.png)

![Slide 4](images/slideshow_dijikstra_dijikstra_slide4.png)

![Slide 5](images/slideshow_dijikstra_dijikstra_slide5.png)

![Slide 6](images/slideshow_dijikstra_dijikstra_slide6.png)

![Slide 7](images/slideshow_dijikstra_dijikstra_slide7.png)

![Slide 8](images/slideshow_dijikstra_dijikstra_slide8.png)

![Slide 9](images/slideshow_dijikstra_dijikstra_slide9.png)

![Slide 10](images/slideshow_dijikstra_dijikstra_slide10.png)

![Slide 11](images/slideshow_dijikstra_dijikstra_slide11.png)

#### Algorithm

- Define $MOD = 1e9 + 7$ for modular arithmetic.
- Build an adjacency list `graph` where $\text{graph}[i]$ stores `{neighbor, travelTime}` pairs.

- Initialize a min-heap (`minHeap`) for Dijkstra's algorithm.
- Create `shortestTime` array to store the shortest time to each node, initialized to $\text{LLONG}_{MAX}$ (or its equivalent in other preferred languages).
- Create `pathCount` array to store the number of shortest paths to each node, initialized to `0`.
- Set $\text{shortestTime}[0] = 0$ and $\text{pathCount}[0] = 1$ (starting node has distance `0` and one valid path).
- Push `{0, 0}` into `minHeap` to start processing.

- While `minHeap` is not empty:
  - Extract the node `currNode` with the current shortest known time `currTime`.
  - If $currTime > \text{shortestTime}[currNode]$, skip outdated distances.
  - Iterate over neighbors of `currNode`:
- If a new shortest path is found:
      - Update $\text{shortestTime}[neighborNode]$.
      - Reset $\text{pathCount}[neighborNode]$ to match $\text{pathCount}[currNode]$.
      - Push ${\text{shortestTime}[neighborNode], neighborNode}$ into `minHeap`.
- If an equally short path is found:
      - Add $\text{pathCount}[currNode]$ to $\text{pathCount}[neighborNode]$, modulo `MOD`.
...
- Time Complexity: $O(N + E \log E)$
    ...
    Combining both parts, the overall time complexity is: $O(E) +$\mathcal{O}(E \\log E)$= O(N + E \log E)$.
...
To implement this, we define a three-dimensional dynamic programming table $\text{dp}[src][dest][x]$. The first value, $\text{dp}[src][dest][0]$, stores the shortest time required to travel from `src` to `dest`, while $\text{dp}[src][dest][1]$ keeps track of how many different ways this shortest time can be achieved. At the beginning, the shortest time between any two distinct nodes is set to a very large value, representing that they are initially unreachable. The number of ways is set to `0` because no path has been established yet. The only exception is when `src` and `dest` are the same, in which case the shortest time is `0` and the number of ways is `1`, as staying at the node is trivially possible in exactly one way.
...
Once all direct edges are accounted for, we use Floyd-Warshall to iteratively improve our shortest paths by considering each node `mid` as a possible bridge between every pair of nodes `(src, dest)`. For every such pair, we check whether traveling through `mid` results in a smaller total travel time than the best-known value stored in $\text{dp}[src][dest][0]$. If a strictly shorter path is found, we update $\text{dp}[src][dest][0]$ to reflect this new shortest time and reset $\text{dp}[src][dest][1]$ to be the product of $\text{dp}[src][mid][1]$ and $\text{dp}[mid][dest][1]$, which accounts for all possible ways to reach `mid` from `src` and then travel from `mid` to `dest`. If the new path through `mid` results in the same shortest time that was already recorded, we do not update $\text{dp}[src][dest][0]$, but we add the newly found paths to $\text{dp}[src][dest][1]$, since they provide additional routes that achieve the minimum distance.
...
- Initialize a 3D DP table $\text{dp}[n][n][2]$ where:
  - $\text{dp}[src][dest][0]$ stores the minimum time to reach `dest` from `src`.
  - $\text{dp}[src][dest][1]$ stores the number of ways to achieve the minimum time.
...
- If $src \neq mid$ and $dest \neq mid$:
          - Calculate `newTime` as $\text{dp}[src][mid][0] + \text{dp}[mid][dest][0]$.
          - If $newTime < \text{dp}[src][dest][0]$ (current time):
- Update $\text{dp}[src][dest][0]$ to `newTime`.
- Update the number of ways $\text{dp}[src][dest][1]$ to the number of ways to reach `mid` from `src` ($\text{dp}[src][mid][1]$) multiplied by the number of ways to reach `dest` from `mid` ($\text{dp}[mid][dest][1]$).
          - If $newTime = \text{dp}[src][dest][0]$ (current time):
- Increment the number of ways $\text{dp}[src][dest][1]$ by the number of ways to reach `mid` from `src` ($\text{dp}[src][mid][1]$) multiplied by the number of ways to reach `dest` from `mid` ($\text{dp}[mid][dest][1]$).
