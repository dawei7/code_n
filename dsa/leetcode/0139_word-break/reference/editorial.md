...
- If $start = \text{s.length}$, return `true`.
...
For example, given `s = "leetcode"` and $wordDict = ["leet", "code"]$, `dp(3)` would return `true`. `s` up to index `3` is `"leet"`, and we can build `"leet"` using the words in `wordDict`. The answer to the problem would be $dp(\text{s.length} - 1)$, which represents if we can build `s`.
...
1. First, there needs to be a `word` from `wordDict` that **ends** at index `i`. Given a `word`, the substring of `s` from indices $i - \text{word.length} + 1$ up to and including `i` should match `word`. We can check every `word` for this.
2. If we manage to find a `word` that **ends** at index `i`, we would need to add it on top of another string (since we are building `s` by joining words together one by one). We need to make sure that the string we are adding onto is also buildable. If we find a `word` that passes the first check, it means `word` would start at index $i - \text{word.length} + 1$. The index before that is $i - \text{word.length}$. To check if the string ending at that index is buildable, we can refer to $dp(i - \text{word.length})$.
...
![Slide 1](images/slideshow_139_1.png)

![Slide 2](images/slideshow_139_2.png)

![Slide 3](images/slideshow_139_3.png)
...
- Check the substring of `s` ending at `i` with the same length as `word`. If the substring matches, and $dp(i - \text{word.length})$ is `true`, return `true`.
- If no `word` satisfying the criteria was found, return `false`.
3. Return $dp(\text{s.length} - 1)$.
...
>
> In Java and C++, we will use an array `memo` to save values. $\text{memo}[i] = -1$ if we haven't calculated yet, $\text{memo}[i] = 0$ if $dp(i) = false$, and $\text{memo}[i] = 1$ if $dp(i) = true$.
...
The same algorithm can be implemented iteratively. Instead of using a function `dp(i)`, we will use an array `dp` where $\text{dp}[i]$ represents the same thing that `dp(i)` did. We can use the same recurrence relation:

$\large{\text{\text{dp}[i]} = \text{any}(\text{s}[\text{i - word.length + 1, i}] = \text{word \&\& dp[i - \text{word.length}]})}$

In top-down, we started at the top ($\text{s.length} - 1$) and work our way down to the base cases. In bottom-up, we start at the bottom $(i = 0)$ and work our way up to the top.

Before we check $dp[i - \text{word.length}]$, we should check if $i = \text{word.length} - 1$. This would mean that the current `word` we are placing to end at index `i` is the first word. $i - \text{word.length}$ would be negative, so we need to separately check this case.
...
- Check if $i = \text{word.length} - 1$ or $dp[i - \text{word.length}] = true$.
- If so, and the substring of `s` ending at `i` with the same length as `word` matches, set $\text{dp}[i] = true$ and `break`.
3. Return $dp[\text{s.length} - 1]$.
...
In the previous approach, we iterated over each state `i` and then calculated $\text{dp}[i]$. To calculate a given $\text{dp}[i]$, we did the following:

- Iterate over every `word` in `wordDict`
- Check if each `word` ended at the current index

This cost us $O(m \cdot k)$. In the problem constraints, we can see that the maximum value of $m \cdot k$ is `20,000`, so this is expensive. We can optimize the time it takes to calculate a given $\text{dp}[i]$ by using a trie.
...
- Otherwise, we see if $\text{dp}[i]$ can be `true`. Initialize a node `curr` at the `root` of the trie.
...
- If we can traverse the trie, we move to the child node. We check the child's `isWord` attribute. If it is `true`, it means there is a word in `wordDict` starting at index `i` and ending at index `j`. We set $\text{dp}[j] = true$.
...
- If `curr.isWord`, set $\text{dp}[j] = true$.
4. Return $dp[\text{s.length} - 1]$.
...
* Time complexity: $O(n^2 + m \cdot k)$
    ...
    Once we build the trie, we calculate `dp`. For each `i`, we iterate over all the indices after `i`. We have a basic nested for loop which costs $O(n^2)$ to handle all $\text{dp}[i]$.
...
Here, we let $\text{dp}[i]$ hold the answer to the question: "is it possible to form `s` up to a length of `i`? To find the answer for each index, instead of iterating over the words in `wordDict` and checking if a `word` ends at the current index `i`, we will instead iterate over **all substrings that end before index `i`**. If we find one of these substrings is in `wordDict` **and** we can form the string prior to the substring, then $\text{dp}[i] = true$.
...
- If $\text{dp}[j]$ is true AND the substring `s[j:i]` is in `words`, set $\text{dp}[i] = true$ and break. Note that `s[j:i]` represents the substring starting at `j` and ending at $i - 1$.
4. Return $\text{dp}[n]$.
