...
![fig](images/89_Gray_Code_Table.png)
...
* Time complexity: $O(n* 2 ^ n)$
    ...
    In our backtracking algorithm, at every function call we iterate over a loop of length $n$ and try to find out all possible successors of the last added number in the present sequence. We continue our search with the first value that succeeds (not present in the `isPresent` set). Since we are using `HashSet` and $\text{unordered}_{set}$ which have an amortized runtime of $O(1)$ in all operations, use of sets doesn't increase the time complexity.
...
![Slide 1](images/slideshow_89_Gray_Code_89_Gray_Code_1.png)

![Slide 2](images/slideshow_89_Gray_Code_89_Gray_Code_2.png)

![Slide 3](images/slideshow_89_Gray_Code_89_Gray_Code_3.png)
...
2. Since the Gray code sequence for $i$ bits (`G(i)`) can be constructed from the Gray code sequence for $i - 1$ bits ($G(i - 1)$), we run a for loop from $\text{i} \hspace{3pt} = 1 \hspace{3pt} \text{to} \hspace{3pt} n$ to find Gray code sequences with increasing bits.
...
![fig](images/89_Gray_Code_Tree.png)
...
![fig](images/89_Gray_Code_Table2.png)
...
\text{On} \hspace{3pt}\text{doing} \hspace{3pt}\text{bitwise} \hspace{3pt}\text{comparison} ,\\
    ...
    b^{n - 2} \oplus b^{n - 1} = 0 => b^{n - 2} = 0\\
    b^{n - 3} \oplus b^{n - 2} = 0 => b^{n - 3} = 0\\
    .\\
.\\
    . \\
