...
<center>
<img src="images/473_Matchsticks-In-Square-Diag-1.png" height="400"></center>
...
<center>
<img src="images/473_Matchsticks-In-Square-Diag-2.png" width="500"></center>
...
It is very clear from the pseudo-code above that the state of a recursion is defined by two variables $\text{matchsticks}_{used}$ and $\text{sides}_{formed}$. Hence, these are the two variables that will be used to **memoize** or cache the results for that specific subproblem.
...
* Time Complexity: $O(N \times 2^N)$. At max $2^N$ unique bit masks are possible and during every recursive call, we iterate our original matchsticks array to sum up the values of matchsticks used to update the $\text{sides}_{formed}$ variable.
...
- The size of the cache is defined by the two variables $\text{sides}_{formed}$ and `mask`. The number of different values that $\text{sides}_{formed}$ can take = 4 and number of unique values of `mask` = $2^N$.
