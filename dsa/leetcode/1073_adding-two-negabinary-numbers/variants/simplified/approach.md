# Simplified Approach - Adding Two Negabinary Numbers
...
## Complexity detail
- **Time Complexity**: $O(A+B)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(A+B)$ — Utilizes standard auxiliary storage proportional to the active computation state.
