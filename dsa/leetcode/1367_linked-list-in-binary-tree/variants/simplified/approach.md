# Simplified Approach - Linked List in Binary Tree
...
## Complexity detail
- **Time Complexity**: $O(N+LU)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(LU+H)$ — Utilizes standard auxiliary storage proportional to the active computation state.
