...
Write a solution to count the number of **overlapping shifts** for each employee. Two shifts are considered overlapping if one shift’s $\text{end}_{time}$ is **later than** another shift’s $\text{start}_{time}$.
