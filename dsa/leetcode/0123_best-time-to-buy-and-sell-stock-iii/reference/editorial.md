...
- If later the price stays the same or even plunges, _i.e._ $p2 \le p1$, then a good trader should just hold the money in the pocket, neither to buy nor sell any stock.
...
First, we denote a sequence of prices as $\text{Prices}[i]$, with index starting from `0` to `N-1`. Then we define two arrays, namely $\text{left}_{profits}[i]$ and $\text{right}_{profits}[i]$.

- As suggested by the name, each element in the $\text{left}_{profits}[i]$ array would hold the maximum profits that one can gain from doing one single transaction on the left subsequence of prices from the index zero to `i`, (_i.e._ $\text{Prices}[0], \text{Prices}[1], ... \text{Prices}[i]$).
For instance, for the subsequences of `[7, 1, 5]`, the corresponding $\text{left}_{profits}[2]$ would be 4, which is to buy the price of 1 and sell it at the price of 5.
<br/>

- And each element in the $\text{right}_{profits}[i]$ array would hold the maximum profits that one can gain from doing one single transaction on the right subsequence of the prices from the index `i` up to `N-1`, (_i.e._ $\text{Prices}[i], Prices[i+1], ... Prices[N-1]$).
For example, for the right subsequence of `[3, 6, 4]`, the corresponding $\text{right}_{profits}[3]$ would be 3, which is to buy at the price of 3 and then sell it at the price of 6.
...
- We pad the $\text{right}_{profits}[i]$ array with an additional zero, which indicates the maximum profits that we can gain from an empty right subsequence, so that we can compare the result of having only one transaction (_i.e._ $\text{left}_{profits}[N-1]$) with the profits gained from doing two transactions.
...
Just when we think that the space complexity of $\mathcal{O}(N)$ is the best we can get for this problem, many users in the Discussion forum proposed a more optimized solution that reduced the space complexity to $O(1)$, (just to name a few of them [@weijiac](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-iii/discuss/39611/Is-it-Best-Solution-with-$\mathcal{O}(n)$-$\mathcal{O}(1)$.), [@shetty4l](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-iii/discuss/149383/Easy-DP-solution-using-state-machine-$\mathcal{O}(n)$-time-complexity-$\mathcal{O}(1)$-space-complexity)).
...
- $\text{t1}_{cost}$: the _minimal_ cost of buying the stock in transaction #1. The minimal cost to acquire a stock would be the minimal price value that we have seen so far at each step.
<br/>

- $\text{t1}_{profit}$: the _maximal_ profit of selling the stock in transaction #1. Actually, at the end of the iteration, this value would be the answer for the first problem in the series, _i.e._ [Best Time to Buy and Sell Stock](https://leetcode.com/problems/best-time-to-buy-and-sell-stock/).
<br/>

- $\text{t2}_{cost}$: the _minimal_ cost of buying the stock in transaction #2, while taking into account the profit gained from the previous transaction #1. One can consider this as the cost of reinvestment. Similar with $\text{t1}_{cost}$, we try to find the lowest price so far, which in addition would be partially compensated by the profits gained from the first transaction.
<br>

- $\text{t2}_{profit}$: the _maximal_ profit of selling the stock in transaction #2. With the help of $\text{t2}_{cost}$ as we prepared so far, we would find out the maximal profits with at most two transactions at each step.
