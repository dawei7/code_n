...
- Case 1, $x \ge k$: we can safely take this interval because it won't cause an overlap. We should update $k = y$ since this interval is now the most recent interval we are keeping.
...
2. Initialize an answer variable $ans = 0$ and an integer `k` to represent the most recent end time. `k` should be initialized to a small value like $\text{INT}_{MIN}$.
