# Simplified Approach - Before and After Puzzle
...
## Complexity detail
- **Time Complexity**: $O(S+G+R\log R)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(S+G)$ — Utilizes standard auxiliary storage proportional to the active computation state.
