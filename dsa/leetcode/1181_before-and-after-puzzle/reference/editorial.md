...
2. Perform a double traversal of the array `sp`. If $\text{sp}[i][0] = \text{sp}[j][1]$, then the phrases at indices `i` and `j` can be concatenated. Store the concatenated phrase in the hash table.
