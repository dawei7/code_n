...
We are given a string `s` and a substring `part`, and we need to repeatedly remove the first occurrence of `part` from `s` until it no longer appears. Since the constraints are relatively small ($\text{s.length} \le 1000$ and $\text{part.length} \le 1000$), we can try a brute force approach.
...
- Run a `while` loop to repeatedly check if the string `s` contains the substring `part`.
  ...
  - Use the substring method to extract the portion of `s` before `part` (`s.substring(0, partStartIndex)`) and the portion after `part` ($\text{s.substring}(partStartIndex + \text{part.length}())$).
...
![Slide 1](images/slideshow_p_slideshow_p_slide1.png)

![Slide 2](images/slideshow_p_slideshow_p_slide2.png)

![Slide 3](images/slideshow_p_slideshow_p_slide3.png)

![Slide 4](images/slideshow_p_slideshow_p_slide4.png)

![Slide 5](images/slideshow_p_slideshow_p_slide5.png)

![Slide 6](images/slideshow_p_slideshow_p_slide6.png)

![Slide 7](images/slideshow_p_slideshow_p_slide7.png)

![Slide 8](images/slideshow_p_slideshow_p_slide8.png)

![Slide 9](images/slideshow_p_slideshow_p_slide9.png)
...
- If the characters at current and `prefixLength` match, we increment both pointers and set $\text{lps}[current] = prefixLength$.
- If they don’t match and `prefixLength` is not zero, we backtrack `prefixLength` to $lps[prefixLength - 1]$.
- If they don’t match and `prefixLength` is zero, we set $\text{lps}[current] = 0$ and increment `current`.
...
- Declare an array `patternIndexes` of size $\text{s.length}() + 1$ to keep track of the pattern index for each character in the stack.
...
- Increment `prefixLength` and store it in $\text{lps}[current]$.
...
- Backtrack to the previous longest prefix-suffix using the LPS array.
  - If no match is found and `prefixLength` is zero, set $\text{lps}[current]$ to zero and proceed to the next character.
