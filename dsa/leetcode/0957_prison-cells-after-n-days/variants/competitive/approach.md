...
The core task in **Prison Cells After N Days** is to There are `8` prison cells in a row and each cell is either occupied or vacant. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
