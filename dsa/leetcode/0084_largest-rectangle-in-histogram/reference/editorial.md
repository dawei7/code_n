...
We start with the leftmost bar and keep pushing the current bar's index onto the stack until we get two successive numbers in descending order, i.e. until we get $\text{heights}[i] < heights[i-1]$. Now, we start popping the numbers from the stack until we hit a number $\text{stack}[j]$ on the stack such that $heights\big[\text{stack}[j]\big] \leq \text{heights}[i]$.
...
i.e. if we pop an element $\text{stack}[top]$ and i is the current index to which we are pointing in the original array, the current area of the rectangle will be considered as:

$(i-stack[top-1]-1) \times heights\big[\text{stack}[top]\big].$
...
Further, if we reach the end of the array, we pop all the elements of the stack and at every pop, this time we use the following equation to find the area:
  $(heights.length - stack[top-1] - 1) \times heights\big[\text{stack}[top]\big]$, where $\text{stack}[top]$ refers to the element just popped. Thus, we can get the area of the of the largest rectangle by comparing the new area found everytime.
