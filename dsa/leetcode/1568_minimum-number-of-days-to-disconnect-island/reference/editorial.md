...
- Set $\text{visited}[row][col]$ to `true`.
- For each `direction` in `DIRECTIONS`:
  - Set `newRow` to $row + \text{direction}[0]$.
  - Set `newCol` to $col + \text{direction}[1]$.
...
- Return `true` if the cell is within the `grid` bounds, $\text{grid}[row][col]$ is `1` and has not been visited yet.
- Else, return `false`.
...
- If the cell has not been visited ($\text{discoveryTime}[i][j]$ = `-1`):
...
- To explore adjacent cells, loop through each `direction` in `DIRECTIONS`:
  - Calculate `newRow` as $row + \text{direction}[0]$.
  - Calculate `newCol` as $col + \text{direction}[1]$.
...
- If the `discoveryTime` of the new cell is `-1`:
      ...
      - Update the `lowestReachable` time for the current cell to the minimum of $\text{lowestReachable}[row][col]$ and $\text{lowestReachable}[newRow][newCol]$.
...
- Set `lowestReachable` time of `(row, col)` to the minimum of $\text{lowestReachable}[row][col]$ and $\text{discoveryTime}[newRow][newCol]$.
...
* Space complexity: $O(m \cdot n)$
    ...
    Thus, the total space complexity of the algorithm is $O(m \cdot n) +$\mathcal{O}(m \cdot n)$= O(m \cdot n)$.
