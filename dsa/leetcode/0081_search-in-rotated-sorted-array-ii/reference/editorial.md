...
- Case 1: If $target > \text{arr}[start]$: `target` exists in the first array `F`.

![rotating a sorted array](images/4.png)

- Case 2: If $target < \text{arr}[start]$: `target` exists in the second array `S`.

![rotating a sorted array](images/5.png)

- Case 3: If $target = \text{arr}[start]$: `target` obviously exists in the first array `F`, but it might also be present in the second array `S`.

![rotating a sorted array](images/6.png)
...
By identifying the positions of both $\text{arr}[mid]$ and `target` in `F` and `S`, we can reduce search space in the very same way as in standard binary search:

- Case 1: $\text{arr}[mid]$ lies in `F`, `target` lies in `S`: Since `S` starts after `F` ends, we know that element lies here:`(mid, end]`.

![rotating a sorted array](images/7.png)

- Case 2: $\text{arr}[mid]$ lies in the `S`, `target` lies in `F`: Similarly, we know that element lies here: `[start, mid)`.

![rotating a sorted array](images/8.png)

- Case 3: Both $\text{arr}[mid]$ and `target` lie in `F`: since both of them are in same sorted array, we can compare $\text{arr}[mid]$ and `target` in order to decide how to reduce search space.

![rotating a sorted array](images/9.png)

- Case 4: Both $\text{arr}[mid]$ and `target` lie in `S`: Again, since both of them are in same sorted array, we can compare $\text{arr}[mid]$ and `target` in order to decide how to reduce search space.

![rotating a sorted array](images/10.png)

But there is a catch, if $\text{arr}[mid]$ equals $\text{arr}[start]$, then we know that $\text{arr}[mid]$ might belong to both `F` and `S` and hence we cannot find the relative position of `target` from it.

![rotating a sorted array](images/11.png)
...
* Time complexity : $O(N)$ worst case, $O(\log N)$ best case, where $N$ is the length of the input array.
    ...
    Worst case: This happens when all the elements are the same and we search for some different element. At each step, we will only be able to reduce the search space by 1 since $\text{arr}[mid]$ equals $\text{arr}[start]$ and it's not possible to decide the relative position of `target` from $\text{arr}[mid]$.
