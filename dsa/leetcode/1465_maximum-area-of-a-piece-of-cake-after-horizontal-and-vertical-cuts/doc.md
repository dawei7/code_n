...
- `horizontalCuts`: the horizontal cut coordinates. Its length $H$ satisfies $1 \le H \le \min(h-1,10^5)$; every coordinate is distinct and lies strictly between `0` and `h`.
- `verticalCuts`: the vertical cut coordinates. Its length $V$ satisfies $1 \le V \le \min(w-1,10^5)$; every coordinate is distinct and lies strictly between `0` and `w`.
