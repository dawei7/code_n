# Simplified Approach - Count Student Number in Departments
...
## Complexity detail
- **Time Complexity**: $O((D + S) \log D)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(D + S)$ — Utilizes standard auxiliary storage proportional to the active computation state.
