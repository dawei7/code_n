...
![representation](images/Slide1.PNG)
...
![two_occurrences](images/Slide2.PNG)
...
Still, we can call `get` for indices varying from `0` to $\text{mountainArr.length}() - 1$, and find the index of the `target` element.
...
>
> While the basic algorithm sounds simpler, backed by in-built functions such as $bisect.\text{bisect}_{left}$, $\text{upper}_{bound}$, $\text{lower}_{bound}$, etc., the implementation has a good number of corner cases to handle, particularly off-by-one errors.
>
...
- The array is **strictly decreasing** from index $peakIndex + 1$ to $\text{mountainArr.length}() - 1$. Thus, we can use **Binary Search** to find the `target` element in the range `[peakIndex + 1, mountainArr.length() - 1]`.
...
- **What's the possible range of `peakIndex`?**
    ...
    Looking at the problem description, we are sure that index-`0` and index-$\text{mountainArr.length}() - 1$ are not the peak indices. Hence, the lowest possible value of `peakIndex` is `1`, and the highest possible value of `peakIndex` is $\text{mountainArr.length}() - 2$.

    Hence, we can set
- $low = 1$

- $high = \text{mountainArr.length}() - 2$
...
- `p` for the `peakIndex`
    ...
    ![arbitrarypoints](images/Slide3.PNG)

- For all `i`, we have $\text{mountainArr.get}(i) < \text{mountainArr.get}(i + 1)$.

- For all `d`, we have $\text{mountainArr.get}(d) > \text{mountainArr.get}(d + 1)$.
- For `p`, we have $\text{mountainArr.get}(p) > \text{mountainArr.get}(p + 1)$.

    Thus,

- if $\text{mountainArr.get}(testIndex) < \text{mountainArr.get}(testIndex + 1)$, then `testIndex` is `i` only.
...
- the case $\text{mountainArr.get}(testIndex) = \text{mountainArr.get}(testIndex + 1)$ is not possible.

- if $\text{mountainArr.get}(testIndex) > \text{mountainArr.get}(testIndex + 1)$, then `testIndex` is either `d` or `p`.
        ...
        Note that failure of the first `if` condition ($\text{mountainArr.get}(testIndex) < \text{mountainArr.get}(testIndex + 1)$) implies passing of this `if` condition ($\text{mountainArr.get}(testIndex) > \text{mountainArr.get}(testIndex + 1)$). Hence, we can use `else` instead of the `if` condition. This will prevent unnecessary comparison calls to `get`.
...
- **When to stop the search?**
    ...
    > - If $\text{mountainArr.get}(testIndex) < \text{mountainArr.get}(testIndex + 1)$, then in the next iteration, the search space will reduce to a single element `[h]`.
    > - If $\text{mountainArr.get}(testIndex) > \text{mountainArr.get}(testIndex + 1)$, then in the next iteration, the search space will reduce to `[f, g]`.
    ...
    > - If $\text{mountainArr.get}(testIndex) < \text{mountainArr.get}(testIndex + 1)$, then in the next iteration, the search space will reduce to a single element `[g]`.
    > - If $\text{mountainArr.get}(testIndex) > \text{mountainArr.get}(testIndex + 1)$, then in the next iteration, the search space will reduce to a single element `[f]`.
...
- **How to test if `testIndex` is the `targetIndex`?**
    ...
    ![increasing](images/Slide4_1.PNG)
...
- Otherwise, it means $\text{mountainArr.get}(testIndex) \ge target$, then we are sure that all elements at indices greater than `testIndex` are greater than or equal to `target`. Here, we can discard the right half of the search space, and search in the left half of the search space. This can be done by setting $high = testIndex$. We cannot discard `testIndex` as it is a candidate for `targetIndex`. Hence, by setting $high = testIndex$, we are not discarding candidates for `targetIndex`.
...
- **When to stop the search?**
    ...
    > - If $\text{mountainArr.get}(testIndex) \ge target$, then in the next iteration, the search space will reduce to a single element `[f, g]`.
    >
    ...
    > - If $\text{mountainArr.get}(testIndex) \ge target$, then in the next iteration, the search space will reduce to a single element `[f]`.
...
- If $\text{mountainArr.get}(low) = target$, then `low` is the `targetIndex`. Hence, we will return `low`.

- Otherwise, if $\text{mountainArr.get}(low) \neq target$, then `target` is not present in the strictly increasing part of the array. In this case, we will search in the strictly decreasing part of the array.
...
- **How to test if `testIndex` is the `targetIndex`?**
    ...
    ![decreasing](images/Slide4_2.PNG)
...
- Otherwise, it means $\text{mountainArr.get}(testIndex) \le target$, then we are sure that all elements at indices greater than `testIndex` are less than or equal to `target`. Here, we can discard the right half of the search space, and search in the left half of the search space. This can be done by setting $high = testIndex$. We cannot discard `testIndex` as it is a candidate for `targetIndex`. Hence, by setting $high = testIndex$, we are not discarding the candidate for `targetIndex`.
...
- **When to stop the search?**
    ...
    > - If $\text{mountainArr.get}(testIndex) \le target$, then in the next iteration, the search space will reduce to a single element `[r, q]`.
    ...
    > - If $\text{mountainArr.get}(testIndex) \le target$, then in the next iteration, the search space will reduce to a single element `[r]`.
...
- Otherwise, if $\text{mountainArr.get}(low) \neq target$, then `target` is not present in the strictly decreasing part of the array. Searching in the strictly decreasing part of the array implies that `target` was not present in the strictly increasing part of the array. Hence, `target` is not present in the `mountainArr`. In this case, we will return `-1`.
...
- While $low \neq high$, do the following:
- Find the middle index of the search space `[low, high]`. Let's call the index `testIndex`.
- If $\text{mountainArr.get}(testIndex) < \text{mountainArr.get}(testIndex + 1)$, then set $low = testIndex + 1$.
...
- Otherwise, set $high = testIndex$.
- If $\text{mountainArr.get}(low) = target$, then return `low`.
...
- if `mountainArr.get(testIndex) > target`, then we can set $high = testIndex - 1$. Because we know that `testIndex` is no longer a candidate for `targetIndex`. In [Approach-1](#approach-1-binary-search), we were setting $high = testIndex$ because it was a potential candidate. The condition there was $\text{mountainArr.get}(testIndex) \ge target$. Here, the condition is `mountainArr.get(testIndex) > target`. Hence, we can set $high = testIndex - 1$.
    ...
    Thus, the condition of the stop of the search will be `low > high`, and while loop condition will be $low \le high$.
...
>
>    - If $\text{mountainArr.get}(testIndex + 1)$ is in the cache, then use the cached value. Otherwise, call $\text{mountainArr.get}(testIndex + 1)$ and cache the value. Call this `next`. If $next = target$, check if it is in the strictly increasing part of the array. If it is, then return $testIndex + 1$ as `targetIndex`. Otherwise, continue the search for `peakIndex`.
>
> 2. **SEARCH IN STRICTLY INCREASING PART OF THE ARRAY**
>
...
- While $low \neq high$, do the following:
- Find the middle index of the search space `[low, high]`. Let's call the index `testIndex`.
- If `testIndex` is in the `cache`, then set $curr = \text{cache.get}(testIndex)$. Otherwise, call `mountainArr.get(testIndex)` and set $curr = \text{mountainArr.get}(testIndex)$. Cache the value of `curr` in the `cache`.
- If $testIndex + 1$ is in the `cache`, then set $next = \text{cache.get}(testIndex + 1)$. Otherwise, call $\text{mountainArr.get}(testIndex + 1)$ and set $next = \text{mountainArr.get}(testIndex + 1)$. Cache the value of `next` in the `cache`.
...
- Set $low = 0$ and $high = peakIndex$.
- While $low \le high$, do the following:
- Find the middle index of the search space `[low, high]`. Let's call the index `testIndex`.
- If `testIndex` is in the `cache`, then set $curr = \text{cache.get}(testIndex)$. Otherwise, call `mountainArr.get(testIndex)` and set $curr = \text{mountainArr.get}(testIndex)$.
...
- Getting the value of $\text{mountainArr.get}(testIndex + 1)$ from the `cache` or from the `mountainArr`. Caching if not present in the `cache`. This takes $O(1)$ time.
