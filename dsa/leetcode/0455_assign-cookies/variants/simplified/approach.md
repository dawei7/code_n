# Simplified Approach - Assign Cookies
...
## Complexity detail
- **Time Complexity**: $O(g \log g + s \log s)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(g + s)$ — Utilizes standard auxiliary storage proportional to the active computation state.
