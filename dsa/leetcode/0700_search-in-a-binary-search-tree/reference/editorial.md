...
- If the tree is empty $root = null$ or the value to find is here $val = \text{root.val}$ - return root.
...
- While the tree is not empty $root \neq null$ and the value to find is _not_ here $val \neq \text{root.val}$:

- If `val < root.val` - go to search into the left subtree $root = \text{root.left}$.

- If `val > root.val` - go to search into the right subtree $root = \text{root.right}$.
