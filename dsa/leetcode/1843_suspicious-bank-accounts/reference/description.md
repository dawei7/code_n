...
A bank account is **suspicious** if the **total income** exceeds the $\text{max}_{income}$ for this account for **two or more consecutive** months. The **total income** of an account in some month is the sum of all its deposits in that month (i.e., transactions of the type `'Creditor'`).
