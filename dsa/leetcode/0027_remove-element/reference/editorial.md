...
When $\text{nums}[j]$ equals to the given value, skip this element by incrementing $j$. As long as $\text{nums}[j] \neq val$, we copy $\text{nums}[j]$ to $\text{nums}[i]$ and increment both indexes at the same time. Repeat the process until $j$ reaches the end of the array and the new length is $i$.
...
When we encounter $\text{nums}[i] = val$, we can swap the current element out with the last element and dispose the last one. This essentially reduces the array's size by 1.
