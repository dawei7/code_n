# Simplified Approach - Minimum Time to Transport All Individuals
...
## Complexity detail
- **Time Complexity**: $O(m n 3^n log(m 2^n))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(m n 3^n)$ — Utilizes standard auxiliary storage proportional to the active computation state.
