...
- For each pair `(i, j)`:
  - Check if $\text{nums}[i]$ is less than or equal to $\text{nums}[j]$.
...
In Approach 1, comparing $\text{nums}[i]$ with $\text{nums}[j]$ needed to be done for all pairs since `nums` is not sorted in order. This leads to an inefficient solution.
...
- Iterate over the sorted `indices`:
  - Update `maxWidth` to be the maximum of its current value and the difference between the current index ($\text{indices}[i]$) and `minIndex`.
  - Update `minIndex` to be the minimum of its current value and the current index ($\text{indices}[i]$).
...
- We check if the condition $\text{nums}[left] \leq \text{rightMax}[right]$ holds. If true, we calculate the ramp width as $right - left$ and update our maximum width if this is the largest we’ve seen.
- If the condition is not satisfied, it means the value at $\text{nums}[left]$ is too large to form a ramp with $\text{rightMax}[right]$, so we increment the `left` pointer to try and find a smaller value.
...
- For each index `i`, set $\text{rightMax}[i]$ to the maximum of $rightMax[i + 1]$ and $\text{nums}[i]$.
...
- Move the `left` pointer forward while the current value at $\text{nums}[left]$ exceeds the corresponding value in $\text{rightMax}[right]$.
...
- If `indicesStack` is empty or the value at the top index of the stack is greater than the current value $\text{nums}[i]$, push `i` onto the stack.
...
- While the stack is not empty and the value at the index on the top of the stack is less than or equal to $\text{nums}[j]$:
- Update `maxWidth` to the maximum of its current value and the width calculated as $j - \text{indicesStack.top}()$.
