...
The core task in **Alternating Digit Sum** is to a positive integer `n`. Each digit of `n` has a sign according to the following rules:. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
