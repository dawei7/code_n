...
The core task in **Minimum Operations to Sort a Permutation** is to an integer array `nums` of length `n`, where `nums` is a permutation of the integers from 0 to $n - 1$. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
