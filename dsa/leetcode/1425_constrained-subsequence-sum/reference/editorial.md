...
We will iterate over the input from left to right. At each index `i`, we will consider the maximum possible sum of a subsequence that **includes and ends at nums[i]**. Let's call this value `curr`. How do we calculate `curr` for a given index `i`? We want the maximum possible sum of a subsequence that ends within the last `k` indices. We will then add $\text{nums}[i]$ to this sum.

We could solve this using dynamic programming - let $\text{dp}[i]$ represent the maximum possible sum of a subsequence that includes and ends at $\text{nums}[i]$. We can calculate $\text{dp}[i]$ by taking the maximum $\text{dp}[j]$ for all `j` in the range `[i - k, i - 1]` (the last `k` indices), then adding $\text{nums}[i]$ to it.

However, we would be iterating up to `k` times to calculate each state. As `k` can be large, this approach is too slow. We need a faster way to find the maximum $\text{dp}[j]$ for all indices `j` in the range `[i - k, i - 1]`.

Because we are only concerned with the maximum sum, we could use a max heap. The max heap would store $\text{dp}[j]$ for all `j` in the last `k` indices. We can easily calculate `curr` by simply checking the top of this heap.
...
1. Initialize a max `heap` with $(\text{nums}[0], 0)$. Also initialize the answer $ans = \text{nums}[0]$.
...
- Set `curr` to the value (first element) at the top of `heap`, plus $\text{nums}[i]$. Note that if the value at the top of `heap` is negative, we should take `0` instead.
...
As we saw in the previous approach, the crux of the dynamic programming idea was finding the maximum value of `dp` in the last `k` indices. We accomplished this in $O(\log{}n)$ time with a heap, but we could achieve $O(\log{}k)$ with a tree map data structure (like a red-black tree). Because $k \le n$, this is a slight improvement in terms of big O.

Let's actually use the `dp` array that we spoke of in the previous approach this time. We will have a data structure `window` that holds all values of `dp` in the last `k` indices. We can easily calculate $\text{dp}[i]$ as $\text{nums}[i]$ plus the maximum value in `window`. Then, we can add $\text{dp}[i]$ to `window`.
...
- Set $\text{dp}[i]$ to $\text{nums}[i]$ plus the maximum key in `window`.
- Increment the frequency of $\text{dp}[i]$ in `window`.
- If $i \ge k$:
...
To maintain this data structure, we need to make sure that whenever we push a new element, it will be the smallest value. Before we push an element $\text{dp}[i]$, we check the last element. If it is less than $\text{dp}[i]$, we must pop it, otherwise, the monotonic property would be broken. Since there may be multiple elements less than $\text{dp}[i]$, we need to use a while loop to "clean" the data structure before pushing $\text{dp}[i]$.

Only once there are no elements in the data structure less than $\text{dp}[i]$ will we push $\text{dp}[i]$. Additionally, we will only push positive values of $\text{dp}[i]$ to `queue`.

The reason we want to remove elements that are less than $\text{dp}[i]$ is because $\text{dp}[i]$ comes after those elements. Thus, those elements will be out of range before $\text{dp}[i]$, and because $\text{dp}[i]$ is greater than them, there is no chance those elements will ever be the maximum value in the last `k` indices anymore.
...
- To check if the max value is out of range, we check if $i - \text{queue.front}() > k$.
...
> Note that we could also store pairs $(\text{dp}[i], i)$ on the queue.
...
- Set $\text{dp}[i]$ to $dp[\text{queue.front}()] + \text{nums}[i]$. If `queue` is empty, use `0` instead of `dp[queue.front()]`.
- While `dp[queue.back()]` is less than $\text{dp}[i]$, pop from the back of `queue`.
- If $\text{dp}[i] > 0$, push `i` to the back of `queue`.
