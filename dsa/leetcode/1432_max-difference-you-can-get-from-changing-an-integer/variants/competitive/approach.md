...
The core task in **Max Difference You Can Get From Changing an Integer** is to an integer `num`. You will apply the following steps to `num` **two** separate times:. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
