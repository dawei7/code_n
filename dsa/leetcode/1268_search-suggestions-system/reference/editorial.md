...
* Time complexity : $O(nlog(n)) +$\mathcal{O}(mlog(n)$)$. Where `n` is the length of `products` and `m` is the length of the search word. Here we treat string comparison in sorting as  $O(1)$.  $O(nlog(n))$ comes from the sorting and $O(mlog(n))$ comes from running `binary search` on products `m` times.
