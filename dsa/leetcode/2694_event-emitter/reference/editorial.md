...
* Inside the subscribe method:
  ...
  * We check if there are any existing handlers for the current event by accessing $this.\text{events}[event]$.
  ...
  * We can then push the provided callback function `(cb)` to the array of handlers $(this.\text{events}[event])$.
...
* Inside the `emit` method:
  ...
  * We check if there are any handlers subscribed to the current event by accessing $this.\text{events}[event]$.

  * If there are no handlers, we return an empty array `[]` indicating that no callbacks were executed.

  * If there are handlers:

* We use the `map` method to iterate over the array of handlers $(this.\text{events}[event])$.
...
* **Time complexity:** For subscribe & unsubscribe: $O(1)$, For emit: $O(n)$ , where `n` represents the number of callbacks subscribed to the event
