...
Each of the $n$ indices is processed once with constant-time state updates, so the running time is $O(n)$. The per-value occurrence state uses $O(n)$ auxiliary space in the worst case, which supports the II limit of $10^5$ values.

## Alternatives and edge cases

- **Enumerate all index pairs:** Scanning possible first and last equal-value indices and checking whether another occurrence lies between them is correct but takes $O(n^2)$ time, which is too slow for the II bound.
