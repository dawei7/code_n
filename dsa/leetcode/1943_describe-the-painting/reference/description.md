...
<ul>
	<li>For example, if colors `2`, `4`, and `6` are mixed, then the resulting mixed color is `{2,4,6}`.</li>
</ul>
...
<ul>
	<li>For example, the painting created with `segments = [[1,4,5],[1,7,7]]` can be described by `painting = [[1,4,12],[4,7,7]]` because:

	<ul>
		<li>`[1,4)` is colored `{5,7}` (with a sum of `12`) from both the first and second segments.</li>
		<li>`[4,7)` is colored `{7}` from only the second segment.</li>
	</ul>
	</li>
</ul>
