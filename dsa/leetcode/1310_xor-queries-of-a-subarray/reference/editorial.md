...
Here, $prefixXOR[right + 1]$ gives the XOR of elements from the start up to `right`, and $\text{prefixXOR}[left]$ gives the XOR from the start up to $left - 1$. XORing these two values gives the result for the subarray from `left` to `right`.

When we XOR $prefixXOR[right + 1]$ with $\text{prefixXOR}[left]$, we effectively remove the XOR of elements from the start to left - 1 from the XOR of elements from the start to right.
...
- Build the `prefixXOR` array:
  - Iterate through each element $\text{arr}[i]$:
- Compute $prefixXOR[i + 1]$ as $\text{prefixXOR}[i] ^ \text{arr}[i]$ (XOR current element with previous prefix XOR value).
...
- Update each element by XOR-ing it with the previous element ($\text{arr}[i] ^= arr[i - 1]$).
