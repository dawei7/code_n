...
1. Group rows into subgroups specific to each player by means of `GROUP BY
   player_id`.
2. Retrieve the *earliest* $\text{event}_{date}$ for each subgroup by means of `MIN()`.
3. Return the result set in any order (`ORDER BY` is not necessary here).

We can use `groupby()` function to group the data in `activity` by the $\text{player}_{id}$ column, it allows us to perform operations on subsets of the data based on the unique player ID. This step ensures that we can analyze the data on a per-player basis. Once the data is grouped by $\text{player}_{id}$, we select the $\text{event}_{date}$ column using and apply the function `min()` to this column, by doing so, we find the minimum (earliest) date within each group of $\text{player}_{id}$. This will give us the first login date for each player.

```python
df = activity.groupby('player_id')['event_date'].min().reset_index()
```

We will have the following DataFrame `df`, where each $\text{event}_{date}$ is the first login in date of the corresponding player:
...
Note that the final output is a DataFrame with two columns: $\text{player}_{id}$ and $\text{first}_{login}$. Hence, we need to rename the column $\text{event}_{date}$ as $\text{first}_{login}$.
...
by $\text{player}_{id}$, essentially we have the following information at our disposal
for each group (i.e., each $\text{player}_{id}$ in this case):

- `1`:
  + $\text{device}_{id}$: `2`, `2`
  + $\text{event}_{date}$: `2016-03-01`, `2016-05-02`
  + $\text{games}_{played}$: `5`, `6`
- `2`:
  + $\text{device}_{id}$: `3`
  + $\text{event}_{date}$: `2017-06-25`
  + $\text{games}_{played}$: `1`
- `3`:
  + $\text{device}_{id}$: `1`, `4`
  + $\text{event}_{date}$: `2016-03-02`, `2018-07-03`
  + $\text{games}_{played}$: `0`, `5`

The breakdown above makes it clear we have three groups in total (one for each
distinct $\text{player}_{id}$). For players with $\text{player}_{id}$ values of `1`, `2`, and
...
aggregate function to essentially scan every $\text{event}_{date}$ subgroup value for
each group to find the smallest one. The smallest value for each group will be
the $\text{first}_{login}$ value reported:
...
The $\text{FIRST}_{VALUE}()$ window function can be used in crafting a
...
problem using the $\text{LAST}_{VALUE}()$ window function, but care must be taken in
effectively defining the [window function frame
specification](https://dev.mysql.com/doc/refman/8.0/en/window-functions-frames.html).
...
docs](https://dev.mysql.com/doc/refman/8.0/en/window-functions-frames.html),
...
Hence, an appropriate frame specification when using $\text{LAST}_{VALUE}()$ would look
...
An inline view can be used in conjunction with `RANK()`, $\text{DENSE}_{RANK}()$, or
$\text{ROW}_{NUMBER}()$. The choice will not impact the outcome because `(player_id,
event_date)` is the primary key of the `Activity` table (i.e., we do not have
to worry about the possibility of more than a single row having a `rnk` value
of `1` since partitions are created by $\text{player}_{id}$ and rows are ordered by
$\text{event}_{date}$, thus guaranteeing unique `rnk` values).
