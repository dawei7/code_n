...
If an element $\text{majority}_{element}$ occurs more than $\lfloor \dfrac{n}{2} \rfloor$ times, then there are at least $\lfloor \dfrac{n}{2} \rfloor$ elements of identical values with `num` at each bit. That is, we can reconstruct the exact value of `num` by combining the most frequent value (0 or 1) at each bit.
...
where $\text{suf}[0]$ is the majority element in that suffix. To do this, we
