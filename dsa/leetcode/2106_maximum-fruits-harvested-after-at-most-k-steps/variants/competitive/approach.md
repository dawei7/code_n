...
The core task in **Maximum Fruits Harvested After at Most K Steps** is to Fruits are available at some positions on an infinite x-axis. You are given a 2D integer array `fruits` where $\text{fruits}[i] = [\text{position}_{i}, \text{amount}_{i}]$ depicts $\text{amount}_{i}$ fruits at the position $\text{position}_{i}$. `fruits` is already **sorted** .... Instead of using nested loops that inspect every pair in $O(n^2)$ time, this algorithm uses the **Two-Pointer technique**. We place two markers (pointers)—typically one at the start (`left`) and one at the end (`right`) of a sorted array—and move them toward each other based on clear comparison rules, completing the search in a single efficient pass.
