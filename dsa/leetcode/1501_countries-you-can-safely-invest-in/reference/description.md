...
| $\text{phone}_{number}$ | varchar | Ten-character number `xxx-yyyyyyy`; the prefix is a country code and digits may have leading zeroes. |
...
| $\text{country}_{code}$ | varchar | Unique three-digit code, including any leading zeroes. |

**`Calls`**

| Column | Type | Meaning |
|---|---|---|
| $\text{caller}_{id}$ | int | Person who placed the call. |
| $\text{callee}_{id}$ | int | Different person who received it. |
