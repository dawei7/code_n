...
The core task in **Find Closest Person** is to three integers `x`, `y`, and `z`, representing the positions of three people on a number line:. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
