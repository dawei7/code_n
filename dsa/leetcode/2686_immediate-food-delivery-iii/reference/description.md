...
Write a solution to find the percentage of immediate orders on each unique $\text{order}_{date}$, **rounded to 2 decimal places**.

Return *the result table ordered by* $\text{order}_{date}$ *in **ascending** order.*
