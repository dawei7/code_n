...
3. Otherwise $s \neq goal$. We keep two variables `firstIndex` and `secondIndex` both initialized to `-1`, to store the indices of string `s` having different characters than string `goal` at the same index.
4. We iterate over each index `i` in string `s`:
- If characters at $s[i]$ and $\text{goal}[i]$ are not the same, then we store the current index:
- If $firstIndex = -1$, it means this is the first index with a different character, thus we update $firstIndex = i$. If $firstIndex \neq -1$, but $secondIndex = -1$, it means this is the second index with a different character, thus we update $secondIndex = i$.
...
6. Now, all characters of both strings are the same except at two indices. So, $s[firstIndex]$ must be equal to $\text{goal}[secondIndex]$ and $s[secondIndex]$ must be equal to $\text{goal}[firstIndex]$, to make both strings equal after swapping.
