# Guided Example: Get Equal Substrings Within Budget

We trace the dynamic String, Binary Search, Sliding Window, Prefix Sum sliding window on a representative input instance.
...
The objective for **Get Equal Substrings Within Budget** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
