# Simplified Approach - Design a File Sharing System
...
## Complexity detail
- **Time Complexity**: $O(k + \log U + p \log p)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(U + H)$ — Utilizes standard auxiliary storage proportional to the active computation state.
