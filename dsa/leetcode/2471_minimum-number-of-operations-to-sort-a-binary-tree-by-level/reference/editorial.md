...
- Time complexity: $O(n \log n)$
    ...
    The position mapping and swap calculations take $O(w)$ time. In the worst case when the tree is a complete binary tree, $w$ could be $n/2$, making the complexity $2 \cdot$\mathcal{O}(n)$+$\mathcal{O}(n \\log n)$= O(n \log n)$.
...
- Time complexity: $O(n \log n)$
    ...
    The swapping phase at each level takes $O(w)$ time. In the worst case of a complete binary tree, $w$ could be $n/2$, making the complexity $O(n) +$\mathcal{O}(n \\log n)$= O(n \log n)$.
