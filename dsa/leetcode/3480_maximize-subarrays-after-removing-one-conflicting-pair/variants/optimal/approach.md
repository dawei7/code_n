...
The core task in **Maximize Subarrays After Removing One Conflicting Pair** is to an integer `n` which represents an array `nums` containing the numbers from 1 to `n` in order. Additionally, you are given a 2D array `conflictingPairs`, where $\text{conflictingPairs}[i] = [a, b]$ indicates that `a` and `b` form a conflicting pair. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
