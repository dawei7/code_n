...
The core task in **Maximum Number of Matching Indices After Right Shifts** is to two integer arrays, `nums1` and `nums2`, of the same length. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
