...
1. Reducing the number of balls in a bag: We can split a bag with $\text{nums}[i]$ balls into smaller bags. After $operations_i$ splits, the original bag is replaced with $operations_i + 1$ smaller bags.
...
-   Calculate the operations needed for the `i`-th bag: $operations = ceil(\text{nums}[i] / maxBallsInBag) - 1$.
...
-   Initialize the boundaries of the binary search: $left = 1$ and $right = max(\text{nums}[i])$.
-   While `left < right`:
