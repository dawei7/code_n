...
Keeping the entire stream sorted makes the middle element easy to read, but inserting into the middle of an array
shifts $O(n)$ values. A stream of $n$ insertions then costs $O(n^2)$. The query does not need the full sorted order:
it only needs the largest value in the lower half and the smallest value in the upper half.
...
## Complexity detail
Let $n$ be the number of values in the app-local stream. Each insertion performs a constant number of heap operations
on at most $n$ values, costing $O(\log n)$, and reading a median costs $O(1)$. Producing all $n$ running medians
therefore costs $O(n \log n)$ time. The two heaps together store exactly $n$ values, so the auxiliary space is
$O(n)$ apart from the returned medians.
...
- **Duplicates and negative values:** need no special placement rule; either heap may receive values equal to the lower
  boundary before rebalancing.
- **Even stream length:** average the two boundary roots and return a floating-point result.
