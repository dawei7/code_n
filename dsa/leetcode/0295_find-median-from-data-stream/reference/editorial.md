...
* Time complexity: $O(n\log n) +$\mathcal{O}(1)$\simeq O(n\log n)$.
...
* Time complexity: $O(n) +$\mathcal{O}(\\log n)$\approx O(n)$.
...
* Time complexity: $O(5 \cdot \log n) +$\mathcal{O}(1)$\approx O(\log n)$.
...
+ Two iterators/pointers $\text{lo}_{median}$ and $\text{hi}_{median}$, which iterate over the `data` multiset.
...
+ Finding the median is easy! It is simply the **mean** of the elements pointed to by the two pointers $\text{lo}_{median}$ and $\text{hi}_{median}$.
...
* Time complexity: $O(\log n) +$\mathcal{O}(1)$\approx O(\log n)$.
