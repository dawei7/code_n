...
3. For each palindromic $word$ do the following. If $\text{count}[word]$ is even,
increase $answer$ by $\text{count}[word]$. Otherwise, if $\text{count}[word]$ is odd,
increase $answer$ by $\text{count}[word] - 1$ and set $central=true$ (we can use the
$word$ as a central word).

4. For each non-palindrome $word$ such that $\text{word}[0] < \text{word}[1]$
...
increase $answer$ by $2 \cdot \min (\text{count}[word], \text{count}[reversedWord])$
(we use $\min (\text{count}[word], \text{count}[reversedWord])$ pairs of the corresponding words).
...
When a $word$ occurs, increase $count[\text{word}[0]-‘a’][\text{word}[1]-‘a’]$
...
3. Iterate over $0 \le i < |\Sigma|$. If $\text{count}[i][i]$ is even,
increase $answer$ by $\text{count}[i][i]$. Otherwise, if $\text{count}[i][i]$ is odd,
increase $answer$ by $\text{count}[i][i] - 1$ and set $central=true$ (we can use the
corresponding word as central word).

4. For each pair $(i, j)$ such that $0 \le i < j < |\Sigma|$
increase $answer$ by $2 \cdot \min (\text{count}[i][j], \text{count}[j][i])$
(we use $\min (\text{count}[i][j], \text{count}[j][i])$ pairs of the corresponding words).
