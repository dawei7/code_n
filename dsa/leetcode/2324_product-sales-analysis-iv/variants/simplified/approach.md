# Simplified Approach - Product Sales Analysis IV
...
## Complexity detail
- **Time Complexity**: $O((s + p) log s)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(s + p)$ — Utilizes standard auxiliary storage proportional to the active computation state.
