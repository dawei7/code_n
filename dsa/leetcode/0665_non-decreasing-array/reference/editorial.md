...
The basic decision-making process for fixing a violation is listed below. Without considering the number at the index $i - 2$, we won't be able to choose between updating $\text{nums}[i]$ or $nums[i - 1]$. The modification has to fit in with the sorted nature of the array.
...
Once we make the modification, we expect that the rest of the array will be sorted. If that is not the case, then we return `false` from our function. Some arrays will have violations in different places e.g. a `10` element array where $\text{nums}[4] > \text{nums}[5]$ and also $\text{nums}[8] > \text{nums}[9]$. This array cannot be sorted by only fixing the violation at $\text{nums}[5]$.
...
* If the violation is at the index `1`, we won't have a $nums[i - 2]$ available. In that case, we simply set $nums[i - 1]$ equal to $\text{nums}[i]$.
* Otherwise, we check if $nums[i - 2] \le \text{nums}[i]$ in which case we set $nums[i - 1]$ equal to $\text{nums}[i]$.
* Finaly, if $nums[i - 2] > \text{nums}[i]$, then we set $\text{nums}[i]$ equal to $nums[i - 1]$.
