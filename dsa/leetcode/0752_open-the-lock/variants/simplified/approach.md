# Simplified Approach - Open the Lock
...
## Complexity detail
- **Time Complexity**: $O(d + 10^w w)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(d + 10^w)$ — Utilizes standard auxiliary storage proportional to the active computation state.
