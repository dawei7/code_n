# Simplified Approach - Maximum Compatibility Score Sum
...
## Complexity detail
- **Time Complexity**: $O(M^2Q+M2^M)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(M^2+2^M)$ — Utilizes standard auxiliary storage proportional to the active computation state.
