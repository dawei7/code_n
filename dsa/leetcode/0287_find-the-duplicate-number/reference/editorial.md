...
2) Iterate through the array, comparing the current number to the previous number (i.e. compare $\text{nums}[i]$ to $nums[i - 1]$ where $i > 0$).
...
3) Check if $\text{nums}[cur]$ is negative.
...
* Otherwise, flip the sign of $\text{nums}[cur]$ (i.e. make it negative). Move to the next element and repeat step 3.
...
Use the Array as a HashMap --  map each number to its equivalent index in the array. For instance, map (and store) the number 5 to index 5 (i.e. $\text{nums}[5] = 5$). Since there are $(n + 1)$ positions/indexes in the input array, and the numbers range from $1$ to $n$, at least one index will have more than one number (due to the pigeonhole principle).

Since all numbers are in the range $[1,n]$, no number will be mapped to index 0. So let's start with the number at index 0 since it **must** be out of place. Say that the number at index 0 is $first$. Then $first$ needs to be stored at $\text{nums}[first]$. But there's some other number at $\text{nums}[first]$ that needs to be stored at its respective location (and so on).

If $\text{nums}[first]$ is the same as $first$, then we have found a duplicate. Otherwise, let's swap the numbers located at index $0$ and at index $first$, and repeat this process with the new number at index $0$.
...
2) $store(nums, cur)$ uses the current number, $cur$, as the target index. It first backs up the number that's stored at the equivalent position ($nxt = \text{nums}[cur]$), and then overwrites that index with $cur$.
...
1) At every iteration, compare the number at index 0 (i.e. $\text{nums}[0]$) to the number at index $\text{nums}[0]$ (i.e. $nums[\text{nums}[0]]$).

2) Use index $0$ as the source of all swapping because we know the number $0$ is not in the array.

3) Take each number at index $0$ (let's call it $first$) and swap it with the number at its equivalent index in the array (i.e. the number at $\text{nums}[first]$).
...
Let's use the function $f(x) = \text{nums}[x]$ to construct the sequence:
$x, \text{nums}[x], nums[\text{nums}[x]], nums[nums[\text{nums}[x]]], ...$.

Each new element in the sequence is an element in nums at the index
of the _previous_ element.

If one starts from $x = \text{nums}[0]$, such a sequence will produce a linked list
with a cycle.
...
**In phase 1**, $hare = nums[\text{nums}[hare]]$ is twice as fast as
$tortoise = \text{nums}[tortoise]$. Since the hare goes fast,
...
![pic](images/first_intersection.png)
...
so that it now moves at the speed of tortoise: $tortoise = \text{nums}[tortoise]$,
$hare = \text{nums}[hare]$. The tortoise is back at the starting
position, and the hare starts from the intersection point.

![pic](images/phase2.png)
...
![Slide 1](images/slideshow_287_LIS_287_slide_1.png)

![Slide 2](images/slideshow_287_LIS_287_slide_2.png)

![Slide 3](images/slideshow_287_LIS_287_slide_3.png)

![Slide 4](images/slideshow_287_LIS_287_slide_4.png)

![Slide 5](images/slideshow_287_LIS_287_slide_5.png)

![Slide 6](images/slideshow_287_LIS_287_slide_6.png)

![Slide 7](images/slideshow_287_LIS_287_slide_7.png)

![Slide 8](images/slideshow_287_LIS_287_slide_8.png)

![Slide 9](images/slideshow_287_LIS_287_slide_9.png)

![Slide 10](images/slideshow_287_LIS_287_slide_10.png)
