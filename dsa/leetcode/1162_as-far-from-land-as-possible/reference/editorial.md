...
1. Create a 2D array `dist` of the same size as the given `grid` to store the minimum distance from any land cell. Initialize all the cells to $\text{MAX}_{DISTANCE}$, the maximum possible manhattan distance in the matrix + 1.
2. Iterate over the matrix from top-left to bottom-right and for each cell at `(i, j)`:

1. If the cell is a land cell, i.e., with value `1`, store the minimum distance in the array `dist` as `0`.
2. If the cell is a water cell, update the distance as the minimum of $grid[i - 1][j]$ and $\text{grid}[i][j - 1]$ plus `1` (If these cells don't exist, use the $\text{MAX}_{DISTANCE}$ ).

3. Iterate over the matrix from bottom-right to top-left, and for each cell at `(i, j)`, update the distance as the minimum of $grid[i + 1][j]$ and $\text{grid}[i][j + 1]$ plus `1`.
4. Store the maximum of all the values in the array `dist` in the variable `ans`.
5. Return `ans`. If the value of `ans` is `0` or $\text{MAX}_{DISTANCE}$, then return `-1` as these values imply that the matrix doesn't have a water or land cell.
...
1. If the cell is a land cell, i.e., with value `1`, store the minimum distance in  `grid` as `0`.
2. If the cell is a water cell, first initialize it to $\text{MAX}_{DISTANCE}$ and then update the distance as the minimum of $grid[i - 1][j]$ and $\text{grid}[i][j - 1]$ plus `1` (If these cells don't exist, use the $\text{MAX}_{DISTANCE}$ ).

2. Iterate over the matrix from bottom-right to top-left, and for each cell at `(i, j)`, update the distance as the minimum of $grid[i + 1][j]$ and $\text{grid}[i][j + 1]$ plus `1`. Also, keep updating the variable `ans` if the updated value in $\text{grid}[i][j]$ is greater than `ans`.
3. Return `ans`. If the value of `ans` is `0` or $\text{MAX}_{DISTANCE}$, then return `-1` as these values imply that the matrix doesn't have a water or land cell.
