# Simplified Approach - Maximum Number of Moves to Kill All Pawns
...
## Complexity detail
- **Time Complexity**: $O(p B + p^2 2^p)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(B + p 2^p)$ — Utilizes standard auxiliary storage proportional to the active computation state.
