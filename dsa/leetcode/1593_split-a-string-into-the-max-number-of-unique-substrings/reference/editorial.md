...
Suppose we’re currently examining the substring starting at `start` (our current index). Our goal is to split the remaining substring from `start` to $\text{s.size}() - 1$ into smaller substrings such that:
...
- For each `end` from $start + 1$ to the size of `s`, extract the substring $\text{s.substr}(start, end - start)$.
...
More technically: If we're currently at `start`, and we've counted `count` unique substrings so far (stored in the `seen` set), and we take `s[start:end]` as a new unique substring, then there are at most $\text{s.size}() - end$ unique substrings possible from `end` to the end of `s`.

This gives us a total of $count + 1 + (\text{s.size}() - end)$ as the best possible result for the current choice of `s[start:end]`. For this to potentially improve our maximum so far, it must be greater than `maxCount`. If it’s not, we would be wasting work by exploring options that can, at best, only match the current `maxCount`.

So this let us:
- Return early from recursive calls if we can’t improve on the current best.
- When iterating over `end`, use the condition $if (count + (\text{s.size}() - start) \le maxCount) return;$ to determine an upper bound for `end`, dynamically limiting the range and avoiding unnecessary recursion.
