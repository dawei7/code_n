...
The *taxicab* distance is the number of moves required to get from point `A` to point `B` in our grid.  It is calculated as $dist(A, B) = abs(\text{A.x} - \text{B.x}) + abs(\text{A.y} - \text{B.y})$.

Let's say we start at `S`, the ghost starts at `G`, the target is `T`, and the ghost first meets us at `X`.  This implies $dist(G, X) \le dist(S, X)$, as the ghost must reach `X` before or at the time that we do.

Now, if the ghost travels from `G` to `X` and then to `T`, it will reach `T` at time $dist(G, T) \le dist(G, X) + dist(X, T) \le dist(S, X) + dist(X, T)$.  The first inequality is because of the *triangle inequality* that all distance metrics satisfy.
