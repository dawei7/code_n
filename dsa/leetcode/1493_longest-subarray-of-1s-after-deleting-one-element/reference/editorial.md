...
2. Iterate over the array from index `i` to $\text{array.length} - 1$ (inclusive), and keep counting the zeroes in the variable `zeroCount`.
3. After every element, check if the `zeroCount` exceeds `1`; if yes, keep removing elements from the left until the value of `zeroCount` becomes $\le 1$.
