...
You are given two integer arrays of length `n`, `aliceValues` and `bobValues`. Each $\text{aliceValues}[i]$ and $\text{bobValues}[i]$ represents how Alice and Bob, respectively, value the $$i^{\text{th}}$$ stone.
...
- `aliceValues`: Input parameter (`List[int]`).
- `bobValues`: Input parameter (`List[int]`).
...
- $1 \le \text{aliceValues}[i], \text{bobValues}[i] \le 100$
