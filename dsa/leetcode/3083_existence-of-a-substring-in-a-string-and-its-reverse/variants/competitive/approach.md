...
The core task in **Existence of a Substring in a String and Its Reverse** is to a** **string `s`, find any substring of length `2` which is also present in the reverse of `s`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
