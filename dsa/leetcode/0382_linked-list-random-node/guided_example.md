...
We execute the single-pass Linked List, Math, Reservoir Sampling, Randomized pointer manipulation on a representative linked list instance.

- **Input:** `{"head": [1, 2, 3], "draws": 5}`
- **Required output:** `[1, 2, 3, 1, 2]`
...
The objective for **Linked List Random Node** is to transform the linked structure by strictly updating `next` references in place.
