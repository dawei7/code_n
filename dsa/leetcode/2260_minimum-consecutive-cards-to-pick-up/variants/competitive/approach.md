...
The core task in **Minimum Consecutive Cards to Pick Up** is to an integer array `cards` where $\text{cards}[i]$ represents the **value** of the $$i^{\text{th}}$$ card. A pair of cards are **matching** if the cards have the **same** value. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
