...
The idea behind this approach is that if we can reach some position in $x$ moves, we can reach all its adjacent positions in $x+1$ moves. Based on this idea, we make use of a 2-D $dp$ array to store the number of ways in which a particular position can be reached. $\text{dp}[i][j]$ refers to the number of ways the position corresponding to the indices $(i,j)$ can be reached given some particular number of moves.

Now, if the current $dp$ array stores the number of ways the various positions can be reached by making use of $x-1$ moves, in order to determine the number of ways the position $(i,j)$ can be reached by making use of $x$ moves, we need to update the corresponding $dp$ entry as $\text{dp}[i][j] = dp[i-1][j] + dp[i+1][j] + \text{dp}[i][j-1] + \text{dp}[i][j+1]$ taking care of boundary conditions. This happens because we can reach the index $(i,j)$ from any of the four adjacent positions and the total number of ways of reaching the index $(i,j)$ in $x$ moves is the sum of the ways of reaching the adjacent positions in $x-1$ moves.
...
Thus, we start off by considering zero move available for which we make an initial entry of $\text{dp}[x][y] = 1$($(x,y)$ is the initial position), since we can reach only this position in zero move. Then, we increase the number of moves to 1 and update all the $dp$ entries appropriately. We do so for all the moves possible from 1 to N.

In order to update $count$, which indicates the total number of possible moves which lead an out of boundary path, we need to perform the update only when we reach the boundary. We update the count as $count = count + \text{dp}[i][j]$, where $(i,j)$ corresponds to one of the boundaries. But, if $(i,j)$ is simultaneously a part of multiple boundaries, we need to add the $\text{dp}[i][j]$ factor multiple times(same as the number of boundaries to which $(i,j)$ belongs).

After we are done with all the $N$ moves, $count$ gives the required result.

The following animation illustrates the process:

![Slide 1](images/slideshow_576_Boundary_Paths_576_Boundary_PathsSlide1.PNG)

![Slide 2](images/slideshow_576_Boundary_Paths_576_Boundary_PathsSlide2.PNG)

![Slide 3](images/slideshow_576_Boundary_Paths_576_Boundary_PathsSlide3.PNG)

![Slide 4](images/slideshow_576_Boundary_Paths_576_Boundary_PathsSlide4.PNG)

![Slide 5](images/slideshow_576_Boundary_Paths_576_Boundary_PathsSlide5.PNG)

![Slide 6](images/slideshow_576_Boundary_Paths_576_Boundary_PathsSlide6.PNG)

![Slide 7](images/slideshow_576_Boundary_Paths_576_Boundary_PathsSlide7.PNG)
