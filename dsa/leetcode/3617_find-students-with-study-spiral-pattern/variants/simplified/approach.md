# Simplified Approach - Find Students with Study Spiral Pattern
...
## Complexity detail
- **Time Complexity**: $O(R log R + S log S)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(R + S)$ — Utilizes standard auxiliary storage proportional to the active computation state.
