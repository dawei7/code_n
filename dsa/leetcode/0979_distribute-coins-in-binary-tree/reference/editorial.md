...
- Calculate the number of coins the left subtree needs to exchange: $leftCoins = dfs(\text{root.left})$.
- Calculate the number of coins the right subtree needs to exchange: $rightCoins = dfs(\text{root.right})$.
- Return the number of coins the current node has available to exchange with its parent: $(\text{current.val} - 1) + leftCoins + rightCoins$.
