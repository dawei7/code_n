## General
**Competitive Approach — Find All Unique Email Domains**

The query executes a structured relational pipeline for **Find All Unique Email Domains**. It uses `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(S + g log g)$ — Operation count proportional to input scale.
