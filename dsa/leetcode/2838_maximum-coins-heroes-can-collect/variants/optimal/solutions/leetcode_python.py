...
class Solution:
    def maximumCoins(
        ...
        for hero_power, hero_index in sorted(
            (power, index) for index, power in enumerate(heroes)
        ):
            while (
                monster_index < len(monster_rewards)
                and monster_rewards[monster_index][0] <= hero_power
            ):
