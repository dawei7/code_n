...
There is a battle and `n` heroes are trying to defeat `m` monsters. You are given two **1-indexed** arrays of **positive** integers `<font face="monospace">heroes</font>` and `<font face="monospace">monsters</font>` of length `n` and `m`, respectively. `<font face="monospace">heroes</font>[i]` is the power of `i^th` hero, and `<font face="monospace">monsters</font>[i]` is the power of `i^th` monster.
