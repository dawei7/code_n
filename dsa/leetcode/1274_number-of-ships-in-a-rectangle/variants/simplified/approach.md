# Simplified Approach - Number of Ships in a Rectangle
...
## Complexity detail
- **Time Complexity**: $O(s \log C)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(\log C)$ — Utilizes standard auxiliary storage proportional to the active computation state.
