...
![ships](images/Ships_Introduction.png)
...
![ships](images/Divide_And_Conquer_Technique.png)
...
![ships](images/Recursion_Levels.png)
...
![ships](images/Ships_SubRectangles.png)
...
Let $M$ be the range of possible x-coordinate values between $\text{bottomLeft}[0]$ and $\text{topRight}[0]$ and let $N$ be the range of possible y-coordinate values between $\text{bottomLeft}[1]$ and $\text{topRight}[1]$. Thus, the maximum possible number of points in the rectangle is $M \cdot N$. Finally, let $S$ be the maximum number of ships in the sea.
