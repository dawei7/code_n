...
![contradiction](images/contradiction.png)
...
![greedy_works](images/greedy_works.png)
...
3. Calculate `maxFreq` as $\text{freq}[25] - 1$. In this case, $maxFreq = 3 - 1 = 2$.
...
3. Identify the number of tasks to be placed into those slots: $availableTasks = \text{tasks.length} - count(A)$.
...
- $availableTasks = \text{tasks.length} - count(A) * count of tasks with the highest frequency$
...
> **Note:** A more concise way of calculating the return value is $max(\text{tasks.length}, (n + 1) * (max-1) + maxCount)$. We have used the below method instead for the sake of readability.
...
![math_approach](images/math_approach.png)
