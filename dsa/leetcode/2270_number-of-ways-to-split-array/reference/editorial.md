...
- $\text{prefix}[0]$ = 2 (sum of the first element).
- $\text{prefix}[1]$ = 2 + 3 = 5 (sum of the first two elements).
- $\text{prefix}[2]$ = 2 + 3 + 5 = 10 (sum of all three elements).

Using the prefix sum array, we can calculate the sum of any section of the array in constant time. For example, the sum of elements between index `start` (exclusive) and `end` (inclusive) is simply $\text{prefix}[end] - \text{prefix}[start]$. This avoids recalculating sums repeatedly for different splits.

Now that we understand prefix sums, let's create a prefix sum array `prefSum`. The first element will be $\text{nums}[0]$ since no prefix exists for the `0`th element. For each subsequent index, we'll add the current value in `nums` to the prefix sum of the previous element.
...
- Iterate from index `1` to $n - 1$ to build the prefix sum array:
  ...
  - Store this value in $\text{prefSum}[i]$.
