...
The core task in **Count of Smaller Numbers After Self** is to an integer array `nums`, return* an integer array *`counts`* where *$\text{counts}[i]$* is the number of smaller elements to the right of *$\text{nums}[i]$. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
