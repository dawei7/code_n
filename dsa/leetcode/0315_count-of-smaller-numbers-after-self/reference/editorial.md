...
For a particular element in `nums`, located at index `i`, we want to count how many of the numbers on the right side of index `i` are smaller than $\text{nums}[i]$. Notice that the value of the smaller numbers must be in the range $(-\infty, \text{\text{nums}[i]}-1]$.

Hence, if we can find the count of **each number** in the range $(-\infty, \text{\text{nums}[i]}-1]$ on the right side of index `i`, then the answer will be the sum of those counts.
...
Since we need counts of values, we can use an approach similar to [bucket sort](https://en.wikipedia.org/wiki/Bucket_sort), where we have buckets of values and $\text{buckets}[value]$ stores the count of `value`. For each value, we increment $\text{buckets}[value]$ by 1. With this approach, the number of elements smaller than $\text{nums}[i]$ is the range sum of $(-\infty, \text{num}-1]$ in buckets.
...
With the given constraint $-10^{4} \le \text{nums}[i] \le 10^{4}$, we can initialize buckets from $-10^{4}$ to $10^{4}$.
...
- Time Complexity: $O(N\log(M))$.
  We need to iterate over `nums`. For each element, we spend $O(\log(M))$ to find the number of smaller elements after it, and spend $O(\log(M))$ time to update the counts. In total, we need $O(N \cdot \log(M)) =$\mathcal{O}(N\\log(M)$)$ time.
...
- Merge sort `nums`, store the number of elements jumping from right to left in `result`.
  ...
  - Alternatively, one can sort the _indices_ with corresponding values in `nums`. That is to say, we are going to sort list `[0, 1, ..., n-1]` according to the comparator $\text{nums}[i]$. This helps to track the indices and update `result`. You can find additional details in the implementations below.
