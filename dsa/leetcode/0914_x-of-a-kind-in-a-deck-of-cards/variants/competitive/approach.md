...
The core task in **X of a Kind in a Deck of Cards** is to an integer array `deck` where $\text{deck}[i]$ represents the number written on the $$i^{\text{th}}$$ card. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
