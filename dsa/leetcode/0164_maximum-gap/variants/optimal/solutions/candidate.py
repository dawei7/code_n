def solve(nums: list[int]) -> int:
    ...
    for value in nums:
        i = (value - lowest) // width
        current_min = bucket_min[i]
        current_max = bucket_max[i]
        bucket_min[i] = value if current_min is None else min(current_min, value)
        bucket_max[i] = value if current_max is None else max(current_max, value)
