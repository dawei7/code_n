...
The core task in **Plus One** is to a **large integer** represented as an integer array `digits`, where each $\text{digits}[i]$ is the $$i^{\text{th}}$$ digit of the integer. The digits are ordered from most significant to least significant in left-to-right order. The large integer does not contain any leading `.... The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
