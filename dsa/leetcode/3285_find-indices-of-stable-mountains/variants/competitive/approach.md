...
The core task in **Find Indices of Stable Mountains** is to There are `n` mountains in a row, and each mountain has a height. You are given an integer array `height` where $\text{height}[i]$ represents the height of mountain `i`, and an integer `threshold`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
