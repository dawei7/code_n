...
The core task in **Find Right Interval** is to an array of `intervals`, where $\text{intervals}[i] = [\text{start}_{i}, \text{end}_{i}]$ and each $\text{start}_{i}$ is **unique**. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
