...
The core task in **Maximum White Tiles Covered by a Carpet** is to a 2D integer array `tiles` where $\text{tiles}[i] = [l_{i}, r_{i}]$ represents that every tile `j` in the range $l_{i} \le j \le r_{i}$ is colored white. Instead of using nested loops that inspect every pair in $O(n^2)$ time, this algorithm uses the **Two-Pointer technique**. We place two markers (pointers)—typically one at the start (`left`) and one at the end (`right`) of a sorted array—and move them toward each other based on clear comparison rules, completing the search in a single efficient pass.
