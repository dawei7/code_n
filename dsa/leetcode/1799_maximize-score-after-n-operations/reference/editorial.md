...
- If we had already solved the same sub-problem earlier, i.e. $\text{memo}[mask] \neq -1$, then we return the stored result from the `memo` array.
...
Also, let's keep a `dp` array, where $\text{dp}[i]$ will store the maximum score we can get after we have picked elements represented by `i` (in binary).

As we can't get more score after picking all the numbers.
Thus, our base case will be: $\text{\text{dp}[finalMask] = 0}$, where $\text{finalMask = 1111...111}$ (in binary).
...
> $\text{\text{dp}[state] = \text{dp}[stateAfterPickingCurrPair] + operationNumber * gcd(\text{nums}[i], \text{nums}[j])}$,
...
- If we have picked all numbers, we know we can't get more score as no number is remaining. Therefore, we set $\text{dp}[state]$ to `0` and continue to the next iteration. Otherwise, we count the number of numbers already picked `numbersTaken` using built-in STL methods and calculate the number of pairs formed `pairsFormed` by dividing `numbersTaken` by `2`.
...
- We mark them as picked in `stateAfterPickingCurrPair`, calculate the current score `currentScore` by multiplying the number of pairs formed after picking current pair, $pairsFormed + 1$ by the gcd of these two picked numbers, and find the score of the remaining numbers `remainingScore` by looking up $\text{dp}[stateAfterPickingCurrPair]$.
- If the $\text{dp}[state]$ is smaller than $currentScore + remainingScore$, we update $\text{dp}[state]$ with it.
3. Return the maximum score we can get by picking all numbers, which is stored in $\text{dp}[0]$.
