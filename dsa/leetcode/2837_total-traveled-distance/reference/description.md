...
Write a solution to calculate the `distance` traveled by **each user**. If there is a user who hasn't completed any rides, then their `distance` should be considered as `0`. Output the $\text{user}_{id}$, `name` and total traveled `distance`.
