...
2) Add all the cells of island `A` to a list $\text{bfs}_{queue}$.
3) While $\text{bfs}_{queue}$ is not empty, we build an empty list $\text{new}_{bfs}$ as the candidate cells for the next BFS round, then we iterate over every cell `(x, y)` in $\text{bfs}_{queue}$.
4) Check the four neighbors of `(x, y)` (up, down, left, and right). If a valid neighbor has value of `0`, we can mark it as visited by setting the value as `-1`, then we can add this cell to the list $\text{new}_{bfs}$. If a neighbor cell has a value of `1`, it means that we have found a land cell of the second island (island `B`). Since we are traversing water cells in BFS approach, it means that the first cell of island `B` we found has the shortest distance from island `A` among all cells on island `B`.
5) Once the iteration (current round) ends, if we still haven't reached island `B`, it means that we should look for cells that have a longer distance from island `A`. Therefore, we increment `distance` by 1, set $\text{bfs}_{queue} = \text{new}_{bfs}$, and repeat step 3.
...
1) Iterate over the grid `grid` until we find a land cell, suppose it is $grid[\text{first}_{x}][\text{first}_{y}]$.

2) Start from $grid[\text{first}_{x}][\text{first}_{y}]$ and use depth-first search to find and set the values of all cells of the same island (island `A`) to `2`.

3) Create a list $\text{bfs}_{queue}$ and add all cells on island `A` to it, starting with $distance = 0$.

4) While $\text{bfs}_{queue}$ is not empty, we create another list $\text{new}_{bfs}$ to collect the water cells we need to visit in the next round. Iterate over cells in $\text{bfs}_{queue}$, for each cell `(x, y)`:
- if $\text{grid}[x][y] = 1$, it means we have reached the second island, return `distance`.
- Otherwise, we look for its unvisited water neighbors (cells with value `0`), mark them as `-1`, and add them to $\text{new}_{bfs}$.

5) Once the iteration ends, set $\text{bfs}_{queue} = \text{new}_{bfs}$, increment `distance` by 1, and start the next round by repeating step 4.
...
In this approach, we will use the same strategy as in the previous approach, but we will use BFS instead of DFS to search for all cells of island `A`. Again, we will first traverse `grid`, take the first land found (assume it is $grid[\text{first}_{x}][\text{first}_{y}]$) and treat it as a land cell of Island `A`. Then, we BFS over all cells of island `A` and set their values to `2` to distinguish them from the other island.
...
1) Iterate over the `grid` until we find the first land cell, suppose it is $grid[\text{first}_{x}][\text{first}_{y}]$.

2) Create:
- a list $\text{bfs}_{queue}$ and add $grid[\text{first}_{x}][\text{first}_{y}]$ on island `A` to it.
- an empty list $\text{new}_{bfs}$ for the next round's search.
- an empty list `second_bfs_queue` for searching the distance between two islands later.

3) Iterate over $\text{bfs}_{queue}$, for each cell $\text{grid}[x][y]$, if $\text{grid}[x][y] = 1$:
- set $\text{grid}[x][y] = 2$
- add `(x, y)` to $\text{new}_{bfs}$ for the next round's search.
- add `(x, y)` to `second_bfs_queue` for searching over water cells later.

4) If $\text{new}_{bfs}$ is not empty, we set $\text{bfs}_{queue} = \text{new}_{bfs}$ and repeat step 3. Otherwise, move on to step 5.

5) Set $distance = 0$.

6) Now we start BFS on water cells. While `second_bfs_queue` is not empty, we create an empty list $\text{new}_{bfs}$ to collect the cells we need to visit in the next round. Iterate over cells in `second_bfs_queue`, for each cell `(x, y)`:
- if $\text{grid}[x][y] = 1$, it means we have reached the second island, return `distance`.
...
- The maximum number of land cells of island `A` that we need to check with BFS is $n^2$, thus the space used by $\text{bfs}_{queue}$ is $O(n^2)$.
