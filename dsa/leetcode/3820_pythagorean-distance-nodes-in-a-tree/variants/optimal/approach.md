...
The core task in **Pythagorean Distance Nodes in a Tree** is to an integer `n` and an undirected tree with `n` nodes numbered from 0 to $n - 1$. The tree is represented by a 2D array `edges` of length $n - 1$, where $\text{edges}[i] = [u_{i}, v_{i}]$ indicates an undirected edge between $u_{i}$ and $v_{i}$. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
