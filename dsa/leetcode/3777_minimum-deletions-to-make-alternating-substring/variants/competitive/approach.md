...
The core task in **Minimum Deletions to Make Alternating Substring** is to a string `s` of length `n` consisting only of the characters `'A'` and `'B'`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
