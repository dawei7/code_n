...
- Time complexity: $O(n \log n)$
    ...
    The time complexity is dominated by the sorting operation, which takes $O(n \log n)$ time. We call the `checkCuts` function twice (once for horizontal cuts and once for vertical cuts), and each call performs sorting and a linear scan through the rectangles. This gives us $2 \cdot ($\mathcal{O}(n \\log n)$+$\mathcal{O}(n)$)$, which simplifies to $O(n \log n)$.
