...
$\text{(original value of \text{nums}[i])} + \text{(number of active flips affecting i)} \mod 2$

- If the result is `0`, it means that $\text{nums}[i]$ is currently `0`, so we must flip it.
...
- Iterate through `nums`:
  ...
  - Check if $\text{nums}[i]$ needs flipping using $(\text{nums}[i] + len(flipQueue)) \% 2 = 0$.
...
We iterate through the array, ensuring that at every position `i`, we can check the last element of a full triplet ($nums[i - 2]$). If $nums[i - 2]$ is `0`, we immediately flip $nums[i - 2], nums[i - 1], and \text{nums}[i]$, and we increase the flip count.
...
Instead of checking the last element of a triplet (`nums[i-2]`), here we directly iterate through the array from left to right and flip any `0` we encounter at $\text{nums}[i]$. Additionally, flipping $\text{nums}[i]$ also forces us to flip the next two elements, $nums[i + 1]$ and $nums[i + 2]$. This ensures that the `0` at $\text{nums}[i]$ is turned into `1` while maintaining correctness for future elements.

To achieve this, whenever we find a `0` at $\text{nums}[i]$, we perform the following operation and increase the count of operations:
- Flip $\text{nums}[i]$ (turning it into `1`).
...
- Iterate through `nums` up to $n - 3$:
  - If $\text{nums}[i]$ is `0`, perform a triplet flip starting at `i`:
- Flip $\text{nums}[i]$ to `1`.
