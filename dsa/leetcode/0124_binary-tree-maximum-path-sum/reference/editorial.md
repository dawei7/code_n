...
![Some examples of valid paths](images/124_valid_path_examples.png)

![Some examples of invalid paths](images/124_invalid_path_examples.png)
...
![An example of maximum path sum in a binary tree](images/124_example.png)
...
![When root is not a part of the maximum path sum](images/124_example_2.png)
...
1. The function returns the path sum gain contributed by the subtree.
    ...
    The recursive function compares $\text{left}_{gain}$ and $\text{right}_{gain}$ and adds the maximum of the two to the value of the root. The sum, `gain_from_subtree` is then returned to the caller.
...
1. Initialize a global variable $\text{max}_{sum}$ to `-Infinity`.
2. Call the function `gain_from_subtree` on the tree's root.
3. Return the value of $\text{max}_{sum}$.
...
4. Update the maximum path sum ($\text{max}_{sum}$) seen so far. To do so, compare $\text{max}_{sum}$ with the sum of the following, and update it if it is smaller.
