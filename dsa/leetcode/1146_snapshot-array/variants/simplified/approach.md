# Simplified Approach - Snapshot Array
...
## Complexity detail
- **Time Complexity**: $O(\log u)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(\texttt{length}+s)$ — Utilizes standard auxiliary storage proportional to the active computation state.
