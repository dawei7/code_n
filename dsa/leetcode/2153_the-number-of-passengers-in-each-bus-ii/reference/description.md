...
Buses and passengers arrive at the LeetCode station. If a bus arrives at the station at a time $t_{bus}$ and a passenger arrived at a time $t_{passenger}$ where $t_{passenger} \le t_{bus}$ and the passenger did not catch any bus, the passenger will use that bus. In addition, each bus has a capacity. If at the moment the bus arrives at the station there are more passengers waiting than its capacity `capacity`, only `capacity` passengers will use the bus.
