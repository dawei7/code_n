...
The core task in **Tuple with Same Product** is to an array `nums` of **distinct** positive integers, return *the number of tuples *`(a, b, c, d)`* such that *$a * b = c * d$* where *`a`*, *`b`*, *`c`*, and *`d`* are elements of *`nums`*, and *$a \neq b \neq c \neq d$*.*. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
