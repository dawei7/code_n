...
Before processing counts, reject when `len(hand) % groupSize != 0`, because complete equal-sized groups cannot cover a non-divisible number of cards.
...
- **Non-divisible hand size:** If $n$ is not a multiple of `groupSize`, a complete partition is impossible immediately.
