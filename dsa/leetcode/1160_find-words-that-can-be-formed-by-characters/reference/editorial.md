...
- Set $good = true$.
- Iterate over each key `c` in `wordCount`. Let $freq = \text{wordCount}[c]$.
- If $\text{counts}[c] < freq$, set $good = false$ and break from the loop.
...
Now, we let `counts` and `wordCount` be an array of length `26`. We let $\text{counts}[x]$ represent the frequency of `x` in `chars`, where `x` is the letter at position `x` in the alphabet. `wordCount` functions similarly.
...
- If $\text{counts}[i] < \text{wordCount}[i]$, set $good = false$ and break from the loop.
- If $good = true$, add the length of `word` to `ans`.
5. Return `ans`.
