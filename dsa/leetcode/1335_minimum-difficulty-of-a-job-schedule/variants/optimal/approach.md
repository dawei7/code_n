...
- **Recompute every segment maximum:** Calling `max(jobDifficulty[i:cut + 1])` for each transition is correct but raises the worst-case time to $O(dn^3)$.
