...
![separate n job into d days](images/1335_1BruteForce.JPG)
...
In this example, we want to get the result for $\text{min}_{diff}(0, 3)$, which represents the minimum job difficulty when starting from the $0^{th}$ index with 3 days remaining. Refer to the top panel of the image below to see the backtracking process.
Each edge signifies how the state from a top row can be derived from the state in a bottom row. For instance, the edge connecting `dp(0,3)` and `dp(1,2)` means the state $\text{min}_{diff}(0, 3)$ (starting from the $0^{th}$ index with 3 days remaining) can be derived from $\text{min}_{diff}(1, 2)$ (starting from the $1^{st}$ index with 2 days remaining, if only the $0^{th}$ job is performed on that day). Notice that some states, $\text{min}_{diff}(5, 1)$ for instance, were calculated many times. Therefore, to improve the time complexity, we can use memoization to avoid such repeated calculations. Adding memoization to DFS is an example of dynamic programming and is illustrated in the bottom panel of the image below.

![dfs to dp](images/1335_2DFS_to_dp.JPG)
...
That is, to solve for $\text{min}_{diff}(i, d)$, we must first calculate $\text{min}_{diff}(j, d - 1)$ for all `j > i`, and then take the minimum job difficulty of all these possibilities as the output.
...
1. State definition:
     ...
     Index `i` (the index of the **first** task for **today** in the `jobDifficulty` array) and day `d` (number of remaining days) will define the DP state. $\text{min}_{diff}(i, d)$ refers to the minimum difficulty when starting at the $i^{th}$ job with `d` days left.

     $\text{min}_{diff}(0, d)$ will be the final answer since it represents starting at the beginning of the job array and finishing all jobs in exactly `d` days.
...
2. State transition function:
     ...
     $\text{min}_{diff}(i, d) = \text{min}_{diff}(j, d - 1) + max(jobDifficulty[i:j]) for all j > i$
...
![Bottom-up dp illustration](images/1335_3Bottom-up_dp.JPG)

There is no dependency between $\text{min}_{diff}[d][i]$ and $\text{min}_{diff}[d][j]$ if $i \neq j$, because the value of $\text{min}_{diff}[d][i]$ only depends on the results of $\text{min}_{diff}[d - 1][j]$ when `j > i`. By identifying such relationships, we can draw viable edges for state transfer between different cells in the DP matrix.

**Algorithm**

1. State definition:

     Index `i` (starting index in the `jobDifficulty` array) and day `d` (number of remaining days) will define the DP state. $\text{min}_{diff}[d][i]$ refers to the minimum difficulty when starting at the $i^{th}$ job with `d` days left.

     $\text{min}_{diff}[d][0]$ will be the final answer since it represents starting at the beginning of the job array and finishing all jobs in `d` days.
...
2. State transfer function:
     ...
     The value of $\text{min}_{diff}[d][i]$ only depends on the results of $\text{min}_{diff}[d - 1][j]$ when `j > i`. The statements can be expressed as:

     $\text{min}_{diff}[d][i] = min(daily_{max\_job\_diff} + \text{min}_{diff}[d - 1][j])$ for all `j > i`,
...
Since the result in $\text{min}_{diff}[d][i]$ only depends on $\text{min}_{diff}[d - 1][j]$, we only need to store the states of the current day and the next day as two DP arrays of size `n`, where `n` is the number of jobs.

This approach is very similar to Approach 2, the few changes made include:  $\text{min}_{diff}[d][i]$ has been replaced by with $\text{min\\_diff\\_curr\\_day}[i]$, and $\text{min}_{diff}[d - 1][j]$ has been replaced with $\text{min\\_diff\\_next\\_day}[j]$.
$\text{min\\_diff\\_curr\\_day}[i]$ records all the results for the current day, while $\text{min\\_diff\\_next\\_day}[j]$ records all the results for the next day (with one less remaining day).
...
Remember our conclusion in the previous approaches that the value of $\text{min}_{diff}[d][i]$ only depends on the results of $\text{min}_{diff}[d - 1][j]$ when `j > i`. From the state transfer function $\text{min\\_diff\\_curr\\_day}[i] = min(\text{min\\_diff\\_curr\\_day}[i], dailyMaxJobDiff + \text{min\\_diff\\_next\\_day}[j])$, it is evident that the job difficulty of the current day is solely determined by the maximum job difficulty seen so far, on the current day (between index `i` and `j`). Therefore, when $\text{jobDifficulty}[i]$ and $\text{jobDifficulty}[j]$ are the two jobs with the highest job difficulty between `i` and `j`, then the values of other job difficulties between `i` and `j` do not affect the difficulty of the current day as shown in the figure below.

![Ignore jobs with lower job difficulties](images/1335_lower_job_difficulties.JPG)
...
- Scenario 1: $\text{jobDifficulty}[j] \le \text{jobDifficulty}[i]$. After we include the $i^{th}$ job on the current day, the minimum job schedule difficulty on the current day will increase by at most $\text{jobDifficulty}[i] - \text{jobDifficulty}[j]$ compared with when the $j^{th}$ job is the **last** job scheduled for the current day. Since job `i` has higher job difficulty than job `j`, the value for $\text{jobDifficulty}[j]$ becomes irrelevant when scheduling any future jobs on the current day, so index `j` can be popped from the stack.

![Scenario 1](images/1335_Scenario_1.JPG)

- Scenario 2: $\text{jobDifficulty}[j] > \text{jobDifficulty}[i]$. The minimum job schedule difficulty on the current day only depends on the most difficult job scheduled on that day, which is $\text{jobDifficulty}[j]$. So after we include the $i^{th}$ job on the current day, the minimum job schedule difficulty will be no higher than that of when the $j^{th}$ job is the **last** job scheduled for the current day. We still need to keep job `j` in the stack for scheduling future jobs on the current day.

![Scenario 2](images/1335_Scenario_2.JPG)

To recap, in the above scenarios, every time a new job (`i`) is added to the current day's schedule, if any prior job (`j`) has a lower or equal job difficulty, then we can discard the job `j` as it becomes irrelevant for the job schedule difficulty of the current day (as in scenario 1). Otherwise, we will need to keep the value the job `j` when $\text{jobDifficulty}[j] > \text{jobDifficulty}[i]$ (as in scenario 2).  As a result, at any given moment, the difficulty of the jobs we keep in the stack will be monotonically decreasing. The monotonically decreasing relationship is a hint that we may be able to solve this problem more efficiently by using a monotonic stack. Notice that when the $i^{th}$ job is added to the current day's schedule, we focus on whether previous jobs should be popped out from the stack and how to update the current state of job difficulty using the popped out job.
...
Contrary to the previous approach, where we iterated over the $\text{days}_{remaining}$ from `1` to `d`. Here, we will iterate over the current day from `day` equals 0, which is the start of the job schedule, to `day` equals $d - 1$, which is the last day of the job schedule.

Each day, we will iterate over the indices `i` of all jobs that could be scheduled on the current day. We start at `i` equals `day` because at least one job per day must have been scheduled on previous days. For each index `i`, we will say this will be the index of the **last** job scheduled today. Thus, the current day's job minimum schedule difficulty will start as $\text{jobDifficulty}[i]$ for $i = day$ because, so far, the job at `i` is the only job scheduled today, and the job schedule difficulty ($\text{min\\_diff\\_curr\\_day}[i]$) is the difficulty of the job at index `i` plus the minimum difficulty of the job schedule that ended yesterday at index $i - 1$, which is $min_{diff\_prev\_day}[i - 1]$.

Now, let's try to schedule more jobs for today by increasing `i` and see how previous jobs at index `j` where $day \le j < i$ will impact the minimum job difficulty of the current day. Here's the trick, as long as the difficulty of the job at index `i` is greater than the difficulty of all jobs between `j` and `i`, then extending a job schedule to include all jobs up to and including `i` will change the schedule difficulty by $\text{jobDifficulty}[i] - \text{jobDifficulty}[j]$. This is because $\text{jobDifficulty}[j]$ is the most difficult job scheduled today so far, and by extending the schedule to include the job at `i`, the schedule difficulty for today will increase to $\text{jobDifficulty}[i]$ from $\text{jobDifficulty}[j]$.
...
So what makes this approach more efficient? The key insight is that after extending the job schedule that ends at index `i` once, we never need to consider extending it again. Here's why. Consider the case where we update the job schedule difficulty at `i` ($\text{min\\_diff\\_curr\\_day}[i]$) by extending the job schedule that ends at index `j` like so: $\text{min\\_diff\\_curr\\_day}[i] \le \text{min\\_diff\\_curr\\_day}[j] + (\text{jobDifficulty}[i]- \text{jobDifficulty}[j])$. Then at a later index (say $i + 1$), we update the job schedule difficulty at $i + 1$ by extending the job schedule that ends at index `i` following the same equation. Let's take a close look at $min_{diff\_curr\_day}[i + 1]$ when we extend the job schedule ending at `i` and $\text{jobDifficulty}[j] < \text{jobDifficulty}[i]$.
...
Notice that extending the job schedule ending at `i` to include all jobs up to $i + 1$ has a equal or tighter bound than extending the job schedule ending at `j` to include all jobs up to $i + 1$ given $\text{jobDifficulty}[j] < \text{jobDifficulty}[i]$. For this reason, we only need to consider extending the job schedule that ends at `j` once, then the difficulty of the job schedule that ends at `j` can be forgotten.
...
> "Consider extending the schedule" means updating $\text{min\\_diff\\_curr\\_day}[i]$ to $\text{min\\_diff\\_curr\\_day}[j] + \text{jobDifficulty}[i] - \text{jobDifficulty}[j]$ if it is less than the value of $\text{min\\_diff\\_curr\\_day}[i]$.

After popping all `j` from the stack where $\text{jobDifficulty}[j] \le \text{jobDifficulty}[i]$ (scenario 1), if the stack is not empty, we will check scenario 2 once. Finally, we can push `i` onto the stack since we may consider extending the job schedule that ends at `i` later.
...
For instance, $\text{min\\_diff\\_curr\\_day}[i]$ records the minimum total job difficulty after completing the $i^{th}$ job on the current day.  Since we need to complete at lease one job on each day, when we are at day 0 (with `d` remaining days), we can only perform the $0^{th}$ job, so $\text{min\\_diff\\_curr\\_day}[i]$ is $\text{jobDifficulty}[0]$. When we are not at the very first day, then we must have previously completed at least $day-1^{th}$ jobs. On the current day, by completing the $i = day^{th}$ job, $\text{min\\_diff\\_curr\\_day}[i]$ will be $min_{diff\_prev\_day}[i-1] + \text{jobDifficulty}[i]$.
...
Since $\text{jobDifficulty}[j] \le \text{jobDifficulty}[i]$, by additionally completing jobs from index $j + 1$ to index `i`, the total job difficulty $\text{min\\_diff\\_curr\\_day}[i]$ will be no more than $\text{min\\_diff\\_curr\\_day}[j] + \text{jobDifficulty}[i] - \text{jobDifficulty}[j]$. So, we can update $\text{min\\_diff\\_curr\\_day}[i]$ as $min(\text{min\\_diff\\_curr\\_day}[i], \text{min\\_diff\\_curr\\_day}[j] + \text{jobDifficulty}[i] - \text{jobDifficulty}[j])$

3. On each day, after we iterate through each indices `i`, popping all indices `j` off the stack where $\text{jobDifficulty}[j] \le \text{jobDifficulty}[i]$, and updating $\text{min\\_diff\\_curr\\_day}[i]$, there may be some remaining indices in the monotonically decreasing stack. Since we are using a monotonically decreasing stack, we know that $\text{jobDifficulty}[j] > \text{jobDifficulty}[i]$ for all of the remaining indices in the stack. Until now, while updating $\text{min\\_diff\\_curr\\_day}[i]$, we only considered scenario 1 (when $\text{jobDifficulty}[j] \le \text{jobDifficulty}[i]$). Before we move to the next day, we must consider scenario 2 as well. Thus, if the stack is not empty, we compare the minimum difficulty job schedule ending at job `j` where `j` is on the top of the stack, and the total job difficulty in the current day $\text{min\\_diff\\_curr\\_day}[i]$ will be no more than $\text{min\\_diff\\_curr\\_day}[j]$. So, we can update $\text{min\\_diff\\_curr\\_day}[i]$ as $min(\text{min\\_diff\\_curr\\_day}[i], \text{min\\_diff\\_curr\\_day}[j])$.

4. At the end, we will return the total minimum job difficulty on the $d^{th}$ day after completing the $n-1^{th}$ job, which is $min_{diff\_prev\_day}[n - 1]$.
