...
1. Create a character array of input string `s` to allow modification. (In languages like C++ where strings are mutable, we can sort `s` in-place without this step.)
...
3. Concatenate the character array into a string. (Skip if sorting was done in-place.)
...
- In Python, the sorted() function uses Timsort, which has a space complexity of $O(N)$. The Python solution also creates a list of length $N$ from the input string.
...
2. Populate the frequency table by incrementing $\text{freq}[letter]$ for each letter in `s`.
