...
1. Fill the array `prefixSum`. The $i^{th}$ index of `prefixSum` will have the sum of integers in `nums` in the range `[0, i - 1]` with $\text{prefix}[0] = 0$. (We need `prefixSum` because  each time we reach a base case, we must return the sum of the remaining elements, and a prefix sum array allows us to do this in constant time.)
...
**Algorithm**
1. Fill the array `prefixSum`. The $i^{th}$ index of `prefixSum` will have the sum of integers in `nums` in the range `[0, i - 1]` with $\text{prefix}[0] = 0$.
2. Initialize an array `memo` where $\text{memo}[currIndex][subarrayCount]$ will store the result for the subproblem (`currIndex`, `subarrayCount`).
3. We need to find the value of $\text{memo}[0][m]$ which represents the minimum largest subarray sum starting at index `0` with `m` subarrays. But we only know what the result will be for the base cases. To fill the `memo` array, we will iterate `subarrayCount` over the range `[1, m]` (starting at 1 because that is our base case) and iterate `currIndex` over the range `[0, n - 1]`.
4. For each value of `subarrayCount` and `currIndex`, we will update $\text{memo}[subarrayCount][currIndex]$:
   - As the sum of the elements between `currIndex` and the end of the array if we are at a base case (`subarrayCount` equals 1).
   - Otherwise, we will use the recurrence relation and the results from previously solved subproblems to calculate $\text{memo}[subarrayCount][currIndex]$.
5. Return the value stored at $\text{memo}[0][m]$.
...
![Slide 1](images/slideshow_410_Split_Array_Largest_Sum_410B.png)

![Slide 2](images/slideshow_410_Split_Array_Largest_Sum_410C.png)

![Slide 3](images/slideshow_410_Split_Array_Largest_Sum_410D.png)

![Slide 4](images/slideshow_410_Split_Array_Largest_Sum_410E.png)

![Slide 5](images/slideshow_410_Split_Array_Largest_Sum_410F.png)

![Slide 6](images/slideshow_410_Split_Array_Largest_Sum_410G.png)

![Slide 7](images/slideshow_410_Split_Array_Largest_Sum_410H.png)

![Slide 8](images/slideshow_410_Split_Array_Largest_Sum_410I.png)
