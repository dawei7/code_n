...
- `restaurants`: Input parameter (`List[List[int]]`).
- `veganFriendly`: Input parameter (`int`).
- `maxPrice`: Input parameter (`int`).
...
- $1 \le \text{id}_{i}, \text{rating}_{i}, \text{price}_{i}, \text{distance}_{i} \le 10^{5}$

- $1 \le maxPrice, maxDistance \le 10^{5}$

- $\text{veganFriendly}_{i}$ and `veganFriendly` are 0 or 1.

- All $\text{id}_{i}$ are distinct.
