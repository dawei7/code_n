...
## Complexity detail
- **Time Complexity**: $O((n + h)^{h + 1})$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O((n + h)^h)$ — Utilizes standard auxiliary storage proportional to the active computation state.
