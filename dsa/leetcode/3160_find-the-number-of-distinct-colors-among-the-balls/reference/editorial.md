...
1. Initialize:
   ...
   * an array `result` of length `n`, where $\text{result}[i]$ denotes the number of distinct colors after the `ith` query.
...
2. Iterate from index `0` to $n - 1$ to traverse the queries. For each query, $\text{query}[i]$:
   * Initialize:
       * an integer `ball` equal to $\text{query}[i][0]$, denoting the current ball that will be colored.
       * an integer `color` equal to $\text{query}[i][1]$, denoting the color that the ball will be colored.
   * If $\text{ballArray}[ball]$ is not `0`, meaning the ball is already colored:
   ...
   * Update $\text{ballArray}[ball]$ to color.
   * Increase the count of `color` in `colorMap` by one.
   * Set $\text{result}[i]$ to the size of `colorMap`.
...
A significant portion of our memory usage comes from the array of size $limit + 1$. When we look at the constraints, we can see that the value of `limit` can be extremely large, with the range $1 \le limit \le 10^{9}$. Contrarily, the queries only range from $1 \le n \le 10^{5}$, where `n` is the length of `queries`. As we navigate through the queries, we can see that not all of the ball labels are guaranteed to be accessed by the queries, leading to unnecessary memory usage.
...
2. Iterate from index `0` to `n-1` to traverse the queries. For each query, $\text{query}[i]$:
   ...
   * If `ball` already exists in `ballMap`, meaning it is already colored:
       ...
       * Decrement the count of `prevColor` in `colorMap`.
       * If the count becomes `0`, remove `prevColor` from `colorMap`.
   * Update $\text{ballMap}[ball]$ to `color`.
   * Increase the count of `color` in `colorMap` by one.
   * Set $\text{result}[i]$ to the current size of `colorMap`.
3. Return the `result` array.
