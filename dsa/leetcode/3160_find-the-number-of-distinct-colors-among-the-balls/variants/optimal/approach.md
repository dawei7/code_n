...
The core task in **Find the Number of Distinct Colors Among the Balls** is to an integer `limit` and a 2D array `queries` of size `n x 2`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
