...
To Table: $\text{customer}_{transactions}$, this database query builds a step-by-step SQL pipeline.
...
## Complexity detail
- **Time Complexity**: $O(N log N + C log C)$ — Detailed Analysis: The time complexity corresponds directly to the total number of operations required by the step-by-step execution loop described above.
