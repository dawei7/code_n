## General
**Competitive Approach — Find Loyal Customers**

The query executes a structured relational pipeline for **Find Loyal Customers**. It uses `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(N log N + C log C)$ — Operation count proportional to input scale.
