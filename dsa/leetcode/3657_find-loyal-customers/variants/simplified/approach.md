# Simplified Approach - Find Loyal Customers
...
## Complexity detail
- **Time Complexity**: $O(N log N + C log C)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(N + C)$ — Utilizes standard auxiliary storage proportional to the active computation state.
