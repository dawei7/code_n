...
- Set $\text{bobPath}[sourceNode]$ to `time` and $\text{visited}[sourceNode]$ to `true` to mark the current node as explored.
- If `sourceNode` is 0, return `true`.
- Iterate through `adjacentNode` of `sourceNode`:
- If $\text{visited}[adjacentNode]$ is set to `false`, recursively call `findBobPath` for the child node and return `true`.
...
- If Alice reaches the node first (`sourceNode` is not in `bobPath` or $\text{bobPath}[sourceNode] > time$), add $\text{amount}[sourceNode]$ to `income`.
- If Alice and Bob reach the node at the same time ($\text{bobPath}[sourceNode] = time$), add half of $\text{amount}[sourceNode]$ to `income`.
- If Alice reached a leaf node ($\text{tree}[sourceNode]$ only has one value and `sourceNode` is not `0`), set `maxIncome` to the maximum of `maxIncome` and `income`.
- Iterate through `adjacentNode` of `sourceNode`:
- If $\text{visited}[adjacentNode]$ is set to `false`:, push an array consisting of `adjacentNode`, $time + 1$, and `income` into `nodeQueue`.
- Set $\text{visited}[sourceNode]$ as `true` to mark the current node as explored.
...
- Set $\text{visited}[sourceNode]$ to `true` to mark the current node as explored.
- If Alice and Bob reach the node at the same time ($\text{bobPath}[sourceNode] = time$), add half of $\text{amount}[sourceNode]$ to `income`.
- If Alice reached a leaf node ($\text{tree}[sourceNode]$ only has one value and `sourceNode` is not `0`), set `maxIncome` to the maximum of `maxIncome` and `income`.
- If Alice reached a leaf node ($\text{tree}[sourceNode]$ only has one value and `sourceNode` is not `0`), set `maxIncome` to the maximum of `maxIncome` and `income`.
- Iterate through `adjacentNode` of `sourceNode`:
- If $\text{visited}[adjacentNode]$ is set to `false`, recursively call `findAlicePath` for the child node.
...
- Define a Depth-First Search (DFS) function `findPaths`:
  - Initialize `maxIncome` to 0 and `maxChild` to $\text{INT}_{MIN}$ to track the maximum income path.
...
- Update $\text{distanceFromBob}[sourceNode]$ as the minimum of its current value and the child's distance plus one.
  - If Alice reaches the node first ($\text{distanceFromBob}[sourceNode] > time$), add the node’s `amount` to `maxIncome`.
  - If Alice and Bob reach the node at the same time ($\text{distanceFromBob}[sourceNode] = time$), add half of $\text{amount}[sourceNode]$ to `maxIncome`.
  - If `maxChild` remains $\text{INT}_{MIN}$, return `maxIncome` (indicating a leaf node).
