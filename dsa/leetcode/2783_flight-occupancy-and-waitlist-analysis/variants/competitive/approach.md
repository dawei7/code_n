## General
**Competitive Approach — Flight Occupancy and Waitlist Analysis**

The query executes a structured relational pipeline for **Flight Occupancy and Waitlist Analysis**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(F + P)$ — Operation count proportional to input scale.
