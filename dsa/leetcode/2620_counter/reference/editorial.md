...
One key difference is that closures allow for true ***encapsulation***. In the class example, there is nothing stopping you from writing $\text{addTo2.a} = 3;$ and breaking it's expected behavior. However, in the closure example, it is theoretically impossible to access `a`. Note that as of 2022, true encapsulation is achievable in classes with [# prefix syntax](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes/Private_class_fields).
