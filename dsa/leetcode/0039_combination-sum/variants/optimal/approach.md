...
Sort candidates and carry a `start` position. At a state, choose any candidate at position `i` from `start` onward. Recurse with `search(i, remaining - value)` because the chosen value may be reused. Never return to a smaller position, so every multiset combination has exactly one nondecreasing representation instead of one path for every permutation.
...
**One nondecreasing position path per multiset**

Every recursive path chooses only candidate values and is emitted only when its accumulated sum reaches the target, so every result is valid. Reuse passes the current position `i` again, allowing any positive multiplicity of that candidate, while later recursion never returns to a smaller position.

Sort any valid multiset combination by candidate position. It defines one nondecreasing sequence that the search follows exactly, including repeated positions. No different recursion path can encode the same multiset because changing the choice order is forbidden. Every combination is therefore reachable once and only once.
