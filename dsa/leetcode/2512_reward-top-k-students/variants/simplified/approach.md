# Simplified Approach - Reward Top K Students
...
## Complexity detail
- **Time Complexity**: $O(F + n log n)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(F + n)$ — Utilizes standard auxiliary storage proportional to the active computation state.
