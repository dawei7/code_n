...
We are given that $n \le 6$. Typically, problems that ask you to find **all** of something with low bounds can be solved with backtracking.
...
To generate a permutation one element at a time, we will use an array `curr` that represents the current permutation we are building. To start, we add the first element in `nums`. We have $curr = [\text{nums}[0]]$. We are **locking** in this first value and we will now find all permutations that start with $\text{nums}[0]$.

To find all permutations that start with $\text{nums}[0]$, we start by adding the next element, which is $\text{nums}[1]$. We now have $curr = [\text{nums}[0], \text{nums}[1]]$. We are **locking** in this second element and we will now find all permutations that start with $\text{nums}[0], \text{nums}[1]$.

This continues until we use all elements, i.e. $\text{curr.length} = \text{nums.length}$. Let's say that we have finished finding all permutations that start with `[nums[0], nums[1]]`. Now what? We **backtrack** by removing the $\text{nums}[1]$, and we have $curr = [\text{nums}[0]]$ again. Now, we add the second element that comes after $\text{nums}[0]$, which is $\text{nums}[2]$. We have $curr = [\text{nums}[0], \text{nums}[2]]$, and now we need to find all permutations that start with `[nums[0], nums[2]]`.

Once we find all the permutations that start with `[nums[0]]`, we backtrack by removing $\text{nums}[0]$ from `curr` and adding the next element. We have $curr = [\text{nums}[1]]$, and now we need to find all permutations that start with $\text{nums}[1]$.
...
- If $\text{curr.length} = \text{nums.length}$, add a copy of `curr` to `ans` and return.
