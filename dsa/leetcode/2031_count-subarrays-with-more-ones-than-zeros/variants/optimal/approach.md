...
The core task in **Count Subarrays With More Ones Than Zeros** is to a binary array `nums` containing only the integers `0` and `1`. Return* the number of **subarrays** in nums that have **more** *`1`'*s than *`0`*'s. Since the answer may be very large, return it **modulo** *$10^{9} + 7$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
