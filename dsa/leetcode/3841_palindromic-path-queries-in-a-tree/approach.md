...
When the leaving marker is popped, `exit\_time[node]` receives the current timer. By then, every node in this subtree has been entered and no outside node was entered between them.

Therefore the subtree of `node` corresponds exactly to the half-open Euler interval:

$$
[\texttt{entry[node]},\texttt{exit\_time[node]}).
$$
...
`apply_from(index, mask)` performs a Fenwick point update. The source calls it at `entry[node]` and, when the interval does not end at `n`, at `exit\_time[node]`.
