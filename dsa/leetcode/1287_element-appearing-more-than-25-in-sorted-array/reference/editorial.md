...
2. Iterate over each element in `arr`. For each element `num`, increment $\text{counts}[num]$.
3. Set $target = \text{arr.length} / 4$.
...
We first calculate a value $size = n / 4$ (floor division). We then iterate `i` over the indices of `arr` until $n - size$. At each index `i`, we check if $\text{arr}[i] = arr[i + size]$. If it is, $\text{arr}[i]$ must be the answer!
...
2. Iterate `i` from `0` until $\text{arr.length} - size$:
- If $\text{arr}[i] = arr[i + size]$, return $\text{arr}[i]$.
