# Simplified Approach - Word Break II
...
## Complexity detail
- **Time Complexity**: $O(S + n + R)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(D + n + R)$ — Utilizes standard auxiliary storage proportional to the active computation state.
