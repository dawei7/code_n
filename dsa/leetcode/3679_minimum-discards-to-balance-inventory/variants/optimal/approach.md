...
The core task in ** Minimum Discards to Balance Inventory** is to two integers `w` and `m`, and an integer array `arrivals`, where $\text{arrivals}[i]$ is the type of item arriving on day `i` (days are **1-indexed**). To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
