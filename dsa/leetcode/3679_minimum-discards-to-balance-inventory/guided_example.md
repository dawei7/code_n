# Guided Example:  Minimum Discards to Balance Inventory

We trace the dynamic Array, Hash Table, Sliding Window, Simulation, Counting sliding window on a representative input instance.
...
The objective for ** Minimum Discards to Balance Inventory** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
