...
- Choose two indexes `i` and `j` and increment $\text{nums1}[i]$ by `k` and decrement $\text{nums1}[j]$ by `k`. In other words, $\text{nums1}[i] = \text{nums1}[i] + k$ and $\text{nums1}[j] = \text{nums1}[j] - k$.
