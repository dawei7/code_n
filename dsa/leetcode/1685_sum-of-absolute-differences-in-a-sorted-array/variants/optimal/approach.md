...
The core task in **Sum of Absolute Differences in a Sorted Array** is to an integer array `nums` sorted in **non-decreasing** order. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
