...
The core task in **Maximum Subarray Sum After One Operation** is to an integer array `nums`. You must perform **exactly one** operation where you can **replace** one element $\text{nums}[i]$ with $\text{nums}[i] * \text{nums}[i]$. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
