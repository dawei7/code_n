...
![img](images/twoPointerApproach.svg)
...
class Solution:
    def maximumUniqueSubarray(self, nums: List[int]) -> int:
        max_sum = current_window_sum = window_start_ptr = 0
        window_unique_elem = set()

        for elem in nums:
            while elem in window_unique_elem:
                current_window_sum -= nums[window_start_ptr]
                window_unique_elem.remove(nums[window_start_ptr])
                window_start_ptr += 1

            window_unique_elem.add(elem)
            current_window_sum += elem
...
![img](images/booleanArrayExample.svg)

**Algorithm**

- Build a boolean array `isPresent` to track the elements present in current subarray. If an element $i$ is added to the current subarray, mark $\text{isPresent}[i] = true$. Similarly, if an element $i$, is removed from current subarray, mark $\text{isPresent}[i] = false$.
...
- Time Complexity: We spend $\mathcal{O}(k)$ time to initialize the `isPresent` array. After that, the sliding window moves each element at most twice (once when added, once when removed), which takes $O(n)$ So overall time complexity is $O(n + k)$
...
- Increment the `end` pointer by $1$ and update the element's frequency in `countMap`. Let the newly added element be `currentElement`, increment $\text{countMap}[currentElement]$ by $1$.

- If the value of $\text{countMap}[currentElement]$ is greater than $1$, it is not unique in the current subarray. Remove elements from current subarray by incrementing `start` pointer until the frequency of `currentElement` is $1$.
...
- Time Complexity: We first initialize a frequency vector of size $k$, which takes $O(k)$. After that, the two-pointer traversal ensures each element is added and removed at most once, which is $O(n)$. Combining both gives us a total of $O(n + k)$
...
There is another idea for that. We can calculate the prefix sum of the array. The prefix sum stores sum of all elements till the current index starting from the $0^{th}$ index. Example, the value at position $\text{prefixSum}[i]$ would be the sum of all the values in array` nums` from $0^{th}$ index to $i^{th}$ index.

So, in the above example, to get the sum of subarray `[3, 5, 4]` starting at index `2` and ending at index `4` we could simply calculate $\text{prefixSum}[4] - \text{prefixSum}[1]$.

The following figure illustrates the idea for `nums = [2, 4, 3, 5, 4, 1]`.

![img](images/lastIndexApproach.svg)
...
- Build an integer array `lastIndexes` of size $10^{4}$ to store the last index of any element in the array. If `currentElement` is the last added element to the subarray having index `end`, we would update $\text{lastIndexes}[currentElement]$ to `i`.
...
- Time Complexity: $O(n + k)$. We need $O(k)$ time to initialize `lastIndexes` of size $k$ and then iterate through all $n$ elements once.

- Space Complexity: $O(n + k)$, since we store `prefixSum` of size $n$ and `lastIndexes` of size $k$.
