...
The core task in **Minimum Number of Steps to Make Two Strings Anagram II** is to two strings `s` and `t`. In one step, you can append **any character** to either `s` or `t`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
