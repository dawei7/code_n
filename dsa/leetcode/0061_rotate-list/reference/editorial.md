...
> We were assuming that `k < n`. What about the case of $k \ge n$?
...
* Find the old tail and connect it with the head $\text{old}_{tail}.next = head$ to close the ring. Compute the length of the list `n` at the same time.

* Find the new tail, which is $(n - k \% n - 1)$th node from the `head`, and the new head, which is $(n - k \% n)$th node.

* Break the ring $\text{new}_{tail}.next = None$ and return $\text{new}_{head}$.
