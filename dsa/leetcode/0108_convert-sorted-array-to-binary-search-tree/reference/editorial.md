...
- Initiate the root: $root = TreeNode(\text{nums}[p])$.

- Compute recursively left and right subtrees: $\text{root.left} = helper(left, p - 1)$, $\text{root.right} = helper(p + 1, right)$.
