...
The core task in **Encode String with Shortest Length** is to a string `s`, encode the string such that its encoded length is the shortest. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
