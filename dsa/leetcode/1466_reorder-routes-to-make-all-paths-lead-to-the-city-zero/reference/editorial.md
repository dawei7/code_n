...
2. Create an adjacency list `adj` that contains a list of pairs of integers such that $\text{adj}[node]$ contains all the neighbors of `node` in the form of `(neighbor, sign)` where `neighbor` is the neighboring node of `node` and `sign` denotes the direction of the edge i.e., whether its an "original" or "artificial" edge.
...
- Iterate over all the neighbors of the `node` (nodes that share an edge) using $\text{adj}[node]$. For every `neighbor, sign` in $\text{adj}[node]$, check if `neighbor` is equal to `parent`. If `neighbor` is equal to `parent`, we will not visit it again.
...
- While the queue is not empty, we dequeue the first element `node` from the queue and iterate over all its neighbors using $\text{adj}[node]$. For each `neighbor, sign` in $\text{adj}[node]$, we check if `neighbor` has been visited already. If `neighbor` has not yet been visited, we mark it visited, perform `count += sign`, and push `neighbor` into the queue.
4. Return `count`.
