...
**Build primality once and locate each next prime**
...
Materialize the sorted prime values from the sieve. At each even index, binary search for the first prime not smaller than the element and add their difference. At each odd index, consult the sieve flag and apply the zero-, one-, or two-operation rule above.
...
Let $N$ be the array length and $M$ its maximum value. The sieve and prime-list construction take $O(M\log\log M)$ time. Each of at most $N$ even indices performs an $O(\log M)$ binary search, while odd indices take constant time, for a total of $O(M\log\log M+N\log M)$. The sieve and prime list use $O(M)$ auxiliary space.

The benchmark defines size as $N$ while holding $M=99991$ fixed across its three tiers. The accepted-class method pays the same sieve-domain cost and performs one binary search per even position. A correct slower control tests divisors independently for every element, adding $\Theta(N\sqrt M)$ work on these prime-heavy inputs.

## Alternatives and edge cases

- **Backward next-prime table:** Scanning the sieve from right to left can store the nearest prime for every value and reduce the array pass to $O(N)$ time, at the same $O(M)$ space cost. The packaged native Accepted source uses this equivalent formulation.
