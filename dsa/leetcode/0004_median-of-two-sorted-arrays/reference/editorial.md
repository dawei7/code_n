...
However, we do not really need to merge and sort these arrays. Note that both arrays are already sorted, so the smallest element is either the first element of `nums1` or the first element of `nums2`. Therefore, we can set two pointers `p1` and `p2` at the start of each array, then we can get the smallest element from the `nums1` and `nums2` by comparing the values $\text{nums1}[p1]$ and $\text{nums2}[p2]$.

Please refer to the following slide as an example:

![Slide 1](images/slideshow_s1_b1.png)

![Slide 2](images/slideshow_s1_b2.png)

![Slide 3](images/slideshow_s1_b3.png)

![Slide 4](images/slideshow_s1_b4.png)

![Slide 5](images/slideshow_s1_b5.png)

![Slide 6](images/slideshow_s1_b6.png)

![Slide 7](images/slideshow_s1_b7.png)

![Slide 8](images/slideshow_s1_b8.png)

![Slide 9](images/slideshow_s1_b9.png)

![Slide 10](images/slideshow_s1_b10.png)

![Slide 11](images/slideshow_s1_b11.png)

![Slide 12](images/slideshow_s1_b12.png)
...
- If $\text{nums1}[p1]$ is smaller than $\text{nums2}[p2]$, we move `p1` one place to the right.
...
Similarly, we can get and compare their middle values $A_{mid}$ and $B_{mid}$. Without loss of generality in this example we assume $A_{mid} \le B_{mid}$ initially, as shown in the yellow boxes.
...
Now we know that $A_{left}$ is smaller than two halves: $A_{right}$ and $B_{right}$. Although we still don't know where exactly these elements are, what we do know is **$A_{left}$ doesn't intersect with $A_{right} + B_{right}$**! There is an invisible boundary between the $A_{left}$ segment and the mixed segment $A_{right} + B_{right}$. As shown in the picture below, the dashed line divides all sorted elements into two halves.
...
1) If the range (for example, a range of `A`) is empty, in other words $a_{start} > a_{end}$, it means all elements in `A` are passed, we just return the $(k - a_{start})$-th element from the other array `B`. Vice versa if $b_{start} > b_{end}$.

2) Otherwise, get the middle indexes of the two ranges: $a_{index} = (a_{start} + a_{end}) / 2$, $b_{index} = (b_{start} + b_{end}) / 2$.
3) Get the middle values of the two ranges: $a_{value} = A[a_{index}]$, $b_{value} = B[b_{index}]$.
4) Cut one array in half, according to:
- If $a_{index} + b_{index} < k$, cut one smaller half.
- If $a_{value} < b_{value}$, cut the smaller half of `A`.
- Otherwise, cut the smaller half of `B`.
- Otherwise, cut one larger half.
- If $b_{value} < a_{value}$, cut the larger half of `B`.
...
- Therefore, the time complexity is $O(\log m + \log n)$.
     $O(\log m + \log n) =$\mathcal{O}(\\log (m\cdot n)$)$
...
- However, during the recursive self-call, we only need to maintain 4 pointers: $a_{start}$, $a_{end}$, $b_{start}$ and $b_{end}$. The last step of the function is to call itself, so if tail call optimization is implemented, the call stack always has $O(1)$ records.
...
If both $maxLeftA \le minRightB$ and $maxLeftB \le minRightA$ hold, it means that we have partitioned arrays at the correct place.

- The smaller half consists of two sections $A_{left}$ and $B_{left}$
- THe larger half consists of two sections $A_{right}$ and $B_{right}$
...
3) While $left \le right$ holds, do the following.
...
- Determine the maximum value of the section $A_{left}$ as $maxLeftA = nums1[partitionA - 1]$. If $partitionA - 1 < 0$, set it as $maxLeftA = float(-inf)$.
- Determine the minimum value of the section $A_{right}$ as $minRightA = \text{nums1}[partitionA]$. If $partitionA \ge m$, set it as $minRightA = float(inf)$.
- Determine the maximum value of the section $B_{left}$ as $maxLeftB = nums2[partitionB - 1]$. If $partitionB - 1 < 0$, set it as $maxLeftB = float(-inf)$.
- Determine the maximum value of the section $B_{right}$ as $minRightB = \text{nums2}[partitionB]$. If $partitionB \ge n$, set it as $minRightB = float(inf)$.
...
7) When both $maxLeftA \le minRightB$ and $maxLeftB \le minRightA$ are true:
