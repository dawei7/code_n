# Simplified Approach - Median of Two Sorted Arrays
...
## Complexity detail
- **Time Complexity**: $O(\log(\min(m, n)))$ — Every element is visited in a predictable number of passes.
- **Space Complexity**: $O(1)$ — Uses auxiliary space proportional to the problem state without intricate pointer packing.
