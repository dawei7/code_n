                ...
                def dp(
                    ...
                    for digit in range(limit + 1):
                        if not started and digit == 0:
                            total += dp(position + 1, digit_sum, missing_factor, tight and digit == limit, False)
                        elif digit != 0 and digit_sum + digit <= target_sum:
                            total += dp(position + 1, digit_sum + digit, missing_factor // gcd(missing_factor, digit), tight and digit == limit, True)
                    return total
...
def solve(l: int, r: int) -> int:
    return Solution().beautifulNumbers(l, r)
