## General
**Competitive Approach — Find Cutoff Score for Each School**

The query executes a structured relational pipeline for **Find Cutoff Score for Each School**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(SE)$ — Operation count proportional to input scale.
