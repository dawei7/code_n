...
Let us define $\text{sum}[k]$ as the cumulative sum for $nums[0 \cdots k-1]$ (inclusive):

$\text{sum}[k] = \left\{ \begin{array}{rl} \sum_{i=0}^{k-1}\text{nums}[i] \& , k > 0 \\ 0 \&, k = 0 \end{array} \right.$

Now, we can calculate *sumRange* as following:

$sumRange(i, j) = sum[j + 1] - \text{sum}[i]$
