...
The core task in **Print Words Vertically** is to a string `s`. Return all the words vertically in the same order in which they appear in `s`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
