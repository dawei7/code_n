# Guided Example: Minimum Subarray Length With Distinct Sum At Least K

We trace the dynamic Array, Hash Table, Sliding Window sliding window on a representative input instance.
...
The objective for **Minimum Subarray Length With Distinct Sum At Least K** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
