...
- Check if the root's value is greater than the `target` value:
  ...
  - Attach the right part of the split ($\text{result}[1]$) to the root's left subtree ($root->left = \text{result}[1]$).
  - Return an array containing the left part of the split ($\text{result}[0]$) and the current root.
- If the root's value is less than or equal to the `target` value:
  ...
  - Attach the left part of the split ($\text{result}[0]$) to the root's right subtree ($root->right = \text{result}[0]$).
  - Return an array containing the left part of the split ($\text{result}[0]$) and the current root.
...
- Process nodes in reverse order from the stack to perform the split:
  ...
  - If the node's value is greater than the `target` value:
     - Assign the node's left child to the subtree containing nodes greater than the target ($curr->left = \text{ans}[1]$).
     - Update the root of the subtree containing nodes greater than the target ($\text{ans}[1] = curr$).
  - Otherwise:
     - Assign the node's right child to the subtree containing nodes less than or equal to the target ($curr->right = \text{ans}[0]$).
     - Update the root of the subtree containing nodes less than or equal to the target ($\text{ans}[0] = curr$).
