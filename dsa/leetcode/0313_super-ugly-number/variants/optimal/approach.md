...
Store one pointer into the generated sequence for each prime. Its current product is the first candidate from that stream not yet passed. At each new sequence position `i`, choose the minimum current candidate across all streams.

**Advance every stream tied at the chosen value**

The same number may have several representations, such as `2*3` and `3*2`. After writing the minimum candidate at `ugly[i]`, scan each prime stream `j` and advance every pointer whose product equals it. Advancing only one would present the same value again on the next iteration and violate strict sequence order.
