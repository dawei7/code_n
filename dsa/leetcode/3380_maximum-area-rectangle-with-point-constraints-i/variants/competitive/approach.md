...
The core task in **Maximum Area Rectangle With Point Constraints I** is to an array `points` where $\text{points}[i] = [x_{i}, y_{i}]$ represents the coordinates of a point on an infinite plane. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
