# Simplified Approach - Grand Slam Titles
...
## Complexity detail
- **Time Complexity**: $O(C + K)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(C + K)$ — Utilizes standard auxiliary storage proportional to the active computation state.
