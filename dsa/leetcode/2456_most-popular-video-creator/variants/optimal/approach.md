...
The core task in **Most Popular Video Creator** is to two string arrays `creators` and `ids`, and an integer array `views`, all of length `n`. The $$i^{\text{th}}$$ video on a platform was created by $\text{creators}[i]$, has an id of $\text{ids}[i]$, and has $\text{views}[i]$ views. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
