...
- Increment $\text{frequencies}[num]$ by `1`.
...
Let $n$ be the length of `arrays` and $m$ be the average length of $\text{arrays}[i]$.
...
During each iteration, we compare the values of $\text{nums1}[first]$ and $\text{nums2}[second]$. There are three possibilities.
...
2. $\text{nums1}[first] < \text{nums2}[second]$. Because `nums2` is sorted, every element after `second` will also be greater than $\text{nums1}[first]$. However, there is a chance that an element in `nums1` after `first` will be equal to $\text{nums2}[second]$. Thus, we should increment `first`.

3. $\text{nums1}[first] > \text{nums2}[second]$. The logic works the other way visa versa. We should increment `second`.
...
1. Initialize an array `longestCommonSubseq` to $\text{arrays}[0]$.
...
- If $\text{nums1}[first]$ is less than $\text{nums2}[second]$, increment `first` by `1` because we need a larger value from `nums1` to match the value at $\text{nums2}[second]$.
- If $\text{nums1}[first]$ is greater than $\text{nums2}[second]$, increment `second` by `1` because we need a larger value from `nums2` to match the value at $\text{nums1}[first]$.
- Otherwise, $\text{nums1}[first]$ must equal $\text{nums2}[second]$, so append $\text{nums1}[first]$ to the `longestCommonSeq`, and increment both `first` and `second`.
...
- Set `mid` to $left + (right - left) / 2$, which is the middle of this section of `nums`. We will compare $\text{nums}[mid]$ to `target`.
- If $\text{nums}[mid]$ is greater than `target`, set `right` to $mid - 1$; we will continue to search in the left half `nums`.
- If $\text{nums}[mid]$ is less than `target`, set `left` to $mid + 1$; we will continue to search in the right half `nums`.
- Otherwise, $\text{nums}[mid]$ equals `target`, return `true`.
...
Let $n$ be the length of `arrays` and $m$ be the average length of $\text{arrays}[i]$. Let $s$ be the length of the shortest array in `arrays`.
