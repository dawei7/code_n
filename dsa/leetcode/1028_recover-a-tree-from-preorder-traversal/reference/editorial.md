...
We traverse the input while keeping track of depth using dashes. Whenever we encounter a digit, we extract the node value directly and create a new node. Instead of using a stack, we maintain a `levels` list where $\text{levels}[depth]$ always holds the last node at that depth.
...
- Iterate while `index < n`:
  ...
  - If `depth` is smaller than `levels.size()`, replace $\text{levels}[depth]$ with the new node.
...
- Return $\text{levels}[0]$ as the root of the reconstructed tree.
