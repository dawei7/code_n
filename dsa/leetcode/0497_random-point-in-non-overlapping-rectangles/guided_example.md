...
We trace the logarithmic Array, Math, Binary Search, Reservoir Sampling, Prefix Sum, Ordered Set, Randomized search on a representative problem instance.
...
The objective for **Random Point in Non-overlapping Rectangles** is to pinpoint the target value or optimal threshold in logarithmic $O(\log N)$ time.
