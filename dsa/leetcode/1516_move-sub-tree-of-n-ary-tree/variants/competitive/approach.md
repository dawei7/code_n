...
The core task in **Move Sub-Tree of N-Ary Tree** is to the `root` of an N-ary tree of unique values, and two nodes of the tree `p` and `q`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
