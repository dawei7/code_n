...
- Compare each word with all the following words one by one. If two words have no common letters, update the maximum product `maxProd`. Perform "no common letters" check in a constant time with the help of precomputed `masks` array: $(\text{masks}[i] \& \text{masks}[j]) = 0$.
