...
Actually, as an implementation level detail, if `x` has frequency `f`, then we'll have `x` in all $\text{group}[i] (i \le f)$, not just the top.  This is because each $\text{group}[i]$ will store information related to the `i`th copy of `x`.
