...
The array `position` lists the positions (in km) of signs in **strictly** increasing order (with $\text{position}[0] = 0$ and $position[n - 1] = l$).

Each $\text{time}[i]$ represents the time (in minutes) required to travel 1 km between $\text{position}[i]$ and $position[i + 1]$.

You **must** perform **exactly** `k` merge operations. In one merge, you can choose any **two** adjacent signs at indices `i` and $i + 1$ (with `i > 0` and $i + 1 < n$) and:

- Update the sign at index $i + 1$ so that its time becomes $\text{time}[i] + time[i + 1]$.
...
- $\text{position}[0] = 0$ and $position[n - 1] = l$
