...
This problem is a successor to another LeetCode problem [78. Subsets](https://leetcode.com/problems/subsets/). We are given an array `nums` of size `n` where $1 \le n \le 10$. We need to find the power set (all possible subsets) excluding duplicate subsets. The maximum number of subsets exists when `nums` contains unique numbers only. In a subset, each number could be present or absent. Thus, for `n` numbers, the power set will consist of no more than $2^n$ subsets.
...
6. We run an inner `for` loop from $j = 0$ to $n - 1$ to check the position of set bits in `subsetIndex`. If at the $j^{th}$ position bit is set, add $\text{nums}[j]$ to the current subset `currentSubset` and append $\text{nums}[j]$ to `hashcode` string.
