## General
**Competitive Approach — Consecutive Transactions with Increasing Amounts**

The query executes a structured relational pipeline for **Consecutive Transactions with Increasing Amounts**. It uses Common Table Expressions (CTEs) to isolate intermediate data transformations into modular steps, PostgreSQL window functions for analytical ranking and offset calculations, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
