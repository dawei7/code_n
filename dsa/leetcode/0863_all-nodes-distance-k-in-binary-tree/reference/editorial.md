...
1) Define a recursive function $\text{add}_{parent}(cur, parent)$ to recursively add a parent pointer to node `cur`: If `cur` is not empty, add a pointer from `cur` to `parent`: $\text{cur.parent} = parent$. Then recursively call $\text{add}_{parent}$ on the left and right children of `cur`:
- $\text{add}_{parent}(\text{cur.left}, cur)$
- $\text{add}_{parent}(\text{cur.right}, cur)$

2) Call $\text{add}_{parent}(root, None)$ to add all parent pointers, note that the root node does not have a parent node.
...
- Both $\text{add}_{parent}$ and `dfs` recursively call themselves to process the left and right subtrees of the current node `cur`. Each node is visited once by each function.
...
1) We will build a hash map `graph`. Define a recursive function $\text{build}_{graph}(cur, parent)$ to recursively build the equivalent graph: If both `cur` and `parent` are not empty, add an edge that connects `cur` and `parent` in the hash map `graph`. Then recursively call $\text{build}_{graph}$ on the left and right children of `cur`:

- If `cur.left` is not empty, call $\text{build}_{graph}(\text{cur.left}, cur)$
- If `cur.right` is not empty, call $\text{build}_{graph}(\text{cur.right}, cur)$

2) Call $\text{build}_{graph}(root, None)$ to build the equivalent graph, note that the root node does not have a parent node.
...
* Time complexity: $O(n)$
- $\text{build}_{graph}$ recursively calls itself to process the left and right subtrees of the current node `cur`. Each node is visited once.
...
1) Define a recursive function $\text{build}_{graph}(cur, parent)$ to recursively build the equivalent graph: If both `cur` and `parent` are not empty, add an edge that connects `cur` and `parent` in the hash map `graph`. Then recursively call $\text{build}_{graph}$ on the left and right children of `cur`:
- If `cur.left` is not empty, call $\text{build}_{graph}(\text{cur.left}, cur)$
- If `cur.right` is not empty, call $\text{build}_{graph}(\text{cur.right}, cur)$
...
4) Initialize an empty queue `queue` to store the nodes to be visited. Enqueue the target node, in the format of $(\text{target.value}, distance = 0)$
