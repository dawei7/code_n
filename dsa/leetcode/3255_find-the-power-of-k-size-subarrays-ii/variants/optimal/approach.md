...
The core task in **Find the Power of K-Size Subarrays II** is to an array of integers `nums` of length `n` and a *positive* integer `k`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
