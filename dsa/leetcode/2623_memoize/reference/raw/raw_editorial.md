...
4. `(cache[args.join()] = fn(...args))` sets the key on the `cache` to the output of the provided function. It then returns that value. This code gets hit if there is a cache-miss.
