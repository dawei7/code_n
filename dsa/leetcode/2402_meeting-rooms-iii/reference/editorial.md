...
$\text{used}_{rooms}$ is a priority queue that contains elements in the form of ${room_{availability\_time}, \text{room}_{number}}$. Here, `room_availability_time` signifies the time at which this room becomes unused. This priority queue is ordered in ascending order based on both `room_availability_time` and $\text{room}_{number}$. This ensures that when an element is popped from it, the room returned is the one that becomes unused earliest. This assists in adhering to rules 2 and 3 while allocating a meeting to the room that becomes unused earliest when all rooms are currently in use.
