...
- An asynchronous function `queryMultiple` which accepts an array of string keys `input`. It will resolve with an array of values that is the same length as the input array. Each index corresponds to the value associated with $\text{input}[i]$. You can assume the promise will never reject.
...
- $0 \le \text{calls.length} \le 10$

- $1 \le \text{key.length} \le 100$
