...
1. As a base case, if $\text{arr.length} = k$, return `arr`.
...
3. While the window's size is less than `k`, check which number is closer to `x`: $\text{arr}[left]$ or $\text{arr}[right]$. Whichever pointer has the closer number, move that pointer towards the edge to include that element in our output.
...
We can actually find the bounds of our sliding window much faster - and independent of `k`! First of all, what is the biggest index the left bound could be? If there needs to be `k` elements, then the left bound's upper limit is $\text{arr.length} - k$, because if it were any further to the right, you would run out of elements to include in the final answer.
