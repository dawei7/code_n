# Simplified Approach - Find Trending Hashtags II
...
## Complexity detail
- **Time Complexity**: $O(S + h + g log g)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(S + h + g)$ — Utilizes standard auxiliary storage proportional to the active computation state.
