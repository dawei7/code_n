...
Count, for each row, entries no greater than `distanceThreshold`, excluding the zero-distance entry for the city itself. Scan city indices in ascending order and replace the answer whenever the new count is less than or equal to the best count. Allowing equality deliberately retains the greatest index required by the tie rule.
...
- **Inclusive threshold:** A shortest path exactly equal to `distanceThreshold` qualifies.
