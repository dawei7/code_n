...
The core task in **Number of Students Doing Homework at a Given Time** is to two integer arrays `startTime` and `endTime` and given an integer `queryTime`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
