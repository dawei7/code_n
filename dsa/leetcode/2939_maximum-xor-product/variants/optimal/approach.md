...
The core task in **Maximum Xor Product** is to three integers `a`, `b`, and `n`, return *the **maximum value** of* $(a XOR x) * (b XOR x)$ *where* $0 \le x < 2^n$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
