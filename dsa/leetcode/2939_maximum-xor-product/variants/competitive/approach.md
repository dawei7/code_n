...
The core task in **Maximum Xor Product** is to three integers `a`, `b`, and `n`, return *the **maximum value** of* $(a XOR x) * (b XOR x)$ *where* $0 \le x < 2^n$. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
