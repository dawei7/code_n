...
The core task in **Binary Trees With Factors** is to an array of unique integers, `arr`, where each integer $\text{arr}[i]$ is strictly greater than `1`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
