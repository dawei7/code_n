...
- `penalty`: An $m \times n$ matrix where $\text{penalty}[i][j]$ is the extra cost of waiting in zero-based cell `(i, j)` or leaving it in a direction not permitted at the current second.
