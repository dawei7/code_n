...
Let $r$ be the number of rows and $c$ be the number of columns. Every one of the $r \times c$ cells performs constant
work, giving $O(rc)$ time. The vertical, previous-diagonal, current-diagonal, and anti-diagonal arrays each have $c$
entries, so auxiliary space is $O(c)$.
