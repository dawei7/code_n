...
In this case, $\text{b.age} - \text{a.age}$ sorts the people array in descending order of `age`.
...
* Can you sort an array of numbers in descending order using `Array.prototype.sort()`? How?
  * Yes, you can sort an array of numbers in descending order using `Array.prototype.sort()` by providing a comparator function that sorts `b` before `a` if `b` is greater than `a`. Here is an example: $let arr = [1, 5, 2]; \text{arr.sort}((a, b) => b - a);$