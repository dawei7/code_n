...
The core task in **Minimum Number of Pushes to Type Word I** is to a string `word` containing **distinct** lowercase English letters. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
