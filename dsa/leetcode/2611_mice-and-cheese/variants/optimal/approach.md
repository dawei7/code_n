...
The core task in **Mice and Cheese** is to There are two mice and `n` different types of cheese, each type of cheese should be eaten by exactly one mouse. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
