...
In the sliding window approach, we typically start with an initial window containing just the first element. Then, we try to expand our sliding window by incrementing `end` to cover more elements of `nums`. For each new element $\text{nums}[end]$ that we add to our window, there are two possible cases:
...
To handle constraint 1, we can use a hash map to track each element's last occurrence index. This allows us to check if the newly added element $\text{nums}[end]$ already exists in the current window. Specifically, if the index of the last occurrence of $\text{nums}[end]$ is greater than or equal to `begin`, then it is already in the window. To fix this, we have to shrink our window and adjust `begin` so that it excludes the existing occurrence of $\text{nums}[end]$. This means `begin` has to be greater than the last occurrence index. As we shrink our window to meet this condition, we also have to update the current sum of the window by subtracting the values of the excluded elements. After these adjustments, the window can include $\text{nums}[end]$ without violating constraint 1.
...
* Get the current number we are adding to our window: $currNum = \text{nums}[end]$
* Get its last occurrence: $lastOccurrence = \text{numToIndex.getOrDefault}(currNum, -1)$
* While the window still contains this current number $begin \le lastOccurrence$ or our window size is too large $end - begin + 1 > k$:
* Update the current sum: $currentSum -= \text{nums}[begin]$
* Shrink our window by 1: `begin++`
* Our window is good now. So add the newly added element to our map: `numToIndex.put(currNum, end)`
* Update current sum $currentSum += \text{nums}[end]$
