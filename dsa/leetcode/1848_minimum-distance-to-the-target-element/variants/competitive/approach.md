...
The core task in **Minimum Distance to the Target Element** is to an integer array `nums` **(0-indexed)** and two integers `target` and `start`, find an index `i` such that $\text{nums}[i] = target$ and $abs(i - start)$ is **minimized**. Note that `abs(x)` is the absolute value of `x`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
