...
All the friends are divided into pairs. The pairings are given in a list `pairs`, where $\text{pairs}[i] = [x_{i}, y_{i}]$ denotes $x_{i}$ is paired with $y_{i}$ and $y_{i}$ is paired with $x_{i}$.
...
- `preferences`: Input parameter (`List[List[int]]`).
...
- $\text{preferences}[i]$ does not contain `i`.

- All values in $\text{preferences}[i]$ are unique.

- $\text{pairs.length} = n/2$
