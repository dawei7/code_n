...
4. Initialize $\text{dp}[start]$ by $\text{dp}[start] + 1$ to consider the case where the character at index `start` is an extra character.
...
6. If the substring from `start` to `end` is found in the `dictionary` set, update $\text{dp}[start]$ by taking the minimum of its current value and $dp[end + 1]$.
7. Finally, return the value at $\text{dp}[0]$.
...
1. Start by defining a `TrieNode` class with `children` and $\text{is}_{word}$ attributes. Each node represents a character in the trie.
...
8. If a valid substring is found in the trie ($node.\text{is}_{word} = true$), call $dp(end + 1)$.
...
1. Define a `TrieNode` class with `children` and $\text{is}_{word}$ attributes. Each node represents a character in the trie.
2. The `buildTrie` function is used to construct the trie by iterating through each word in the dictionary and adding it to the trie character by character.
...
8. Initialize $\text{dp}[start]$ with $dp[start + 1] + 1$.
...
11. If a valid substring is found in the trie ($node.\text{is}_{word} = true$), update $\text{dp}[start]$ by taking the minimum of its current value and $dp[end + 1]$.
12. Finally, return the value at $\text{dp}[0]$.
