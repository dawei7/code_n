...
Let $\text{tags}[i]$ denote the component ID of node $i$.

During the traversal:
* If $\text{nums}[i] - nums[i - 1] > maxDiff$, then node $i$ starts a new connected component, so we set $\text{tags}[i] = tags[i - 1] + 1$.
* Otherwise, node $i$ belongs to the same connected component as node $i - 1$, so we set $\text{tags}[i] = tags[i - 1]$.
