# Simplified Approach - Sort Integers by The Power Value
...
## Complexity detail
- **Time Complexity**: $O(U + R log R)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(U + R)$ — Utilizes standard auxiliary storage proportional to the active computation state.
