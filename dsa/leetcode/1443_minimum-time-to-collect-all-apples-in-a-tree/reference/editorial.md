...
Let's say we have a node `c1` which is a child of a node `p`. We first find the time it takes to collect all the apples in the subtree of `c1`, let's call it `t`. If $t = 0$ and $\text{hasApple}[c1] = false$, that means there are no apples in this subtree and there is no point in visiting it.
...
4. At the end of `dfs` traversal for node `0`, we have the time required to collect the apples in the subtree of node `0`.
