...
We are given a 2D grid of size `m x n`. Each cell of grid represents a water or land denoted by `0` and `1` respectively. Initially, all cells of the grid are marked as `0`.

We are also given an array `positions` where $\text{positions}[i]$ = [$r_i$, $c_i$] is the position ($r_i$, $c_i$) which is converted from water to land at the $i^{th}$ operation.

Our task is to return an array of integers `answer` where $\text{answer}[i]$ is the number of islands after turning the cell ($r_i$, $c_i$) into land.
...
![Slide 1](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.001.png)

![Slide 2](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.002.png)

![Slide 3](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.003.png)

![Slide 4](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.004.png)

![Slide 5](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.005.png)

![Slide 6](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.006.png)

![Slide 7](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.007.png)

![Slide 8](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.008.png)

![Slide 9](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.009.png)

![Slide 10](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.010.png)

![Slide 11](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.011.png)

![Slide 12](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.012.png)

![Slide 13](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.013.png)

![Slide 14](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.014.png)

![Slide 15](images/slideshow_305_number_of_islands_ii_unionfind_unionfind.015.png)

<br>

#### Algorithm

1. Initialize arrays $x[] = { -1, 1, 0, 0 }$ and $y[] = { 0, 0, -1, 1 }$ to be used to find neighbors of a cell.
2. Create an instance of `UnionFind`, say $dsu(m * n)$. Initialize all parents to `-1`. We will use union by rank, so initialize all ranks to `0`. Lastly, initialize $count = 0$.
3. Create a list of integers `answer` where $\text{answer}[i]$ would store the number of islands formed after converting $\text{position}[i]$ cell to land.
4. Iterate through `positions` and for every `position` in `positions`;
- Perform a linear mapping to convert a 2D cell `position` to $landPosition = \text{position}[0] * n + \text{position}[1]$.
...
- Initializing `UnionFind` takes $O(m \cdot n)$ time beacuse we are initializing the `parent` and `rank` arrays of size $m * n$ each.
...
- Obtaining the number of islands for each position and pushing it to `answer` takes $O(1)$ per position. For $l$ positions, it would take $O(l)$ time.
- As a result, the total time required is $O(m \cdot n + l)$.

* Space complexity: $O(m \cdot n)$

- We are using the `parent` and `rank` arrays, both of which require $O(m \cdot n)$ space.
- Other integers, such as `count`, and arrays, such as `x` and `y` take up $O(1)$ space.