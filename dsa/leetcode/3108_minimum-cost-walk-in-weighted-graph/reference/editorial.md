...
We are given an undirected weighted graph, represented by an array `edges`, where $\text{edges}[i] = [u, v, w]$ indicates an edge between vertices `u` and `v` with weight `w`. Additionally, we are given an array `queries`, where $\text{queries}[i] = [s, t]$ represents a pair of nodes in the graph.
...
-   Update the component cost by performing a bitwise AND: $\text{componentCost}[root] \&= weight$.
-   Initialize an array `answer` to store the answer for each query.
-   For each $query = [start, end]$ in `queries`:
-   If the two nodes belong to different connected components, i.e. $find(start) \neq find(end)$, push `-1` into `answer`.
-   Otherwise:
-   Find the root of their component: $root = find(start)$.
-   Push $\text{componentCost}[root]$ into `answer`.
-   Return `answer`.

##### `find(node)` function:
- If $\text{parent}[node] = -1$, `node` is the representative of its group, so return `node`.
- Otherwise, return $find(\text{parent}[node])$ and store the result in $\text{parent}[node]$ (path compression).
...
- Otherwise, if $\text{depth}[root1] < \text{depth}[root2]$, swap the two roots to ensure that `root1` has greater depth.
- Merge the two groups, by setting $\text{parent}[root2] = root1$.
- If the groups had the same depth, increment the depth of the merged group by `1` ($\text{depth}[root1]++$).
...
We start a BFS traversal from each unvisited node, marking it as part of a new component with a unique `componentId`. During the traversal, we mark every node we visit as part of the current component by setting $\text{components}[node] = componentId$. As we explore, we calculate the component's cost by performing a bitwise AND on the weights of the edges we visit. After finishing the traversal of all nodes and edges in the component, we store the calculated cost in a map, where the key is the `componentId` and the value is the component's cost.
...
-   For each $edge = [node1, node2, weight]$ in edges:
-   Push `[node2, weight]` to $\text{adjList}[node1]$.
-   Push `[node1, weight]` to $\text{adjList}[node2]$.
-   Initialize:
...
-   For each $query = [start, end]$ in `queries`:
-   If $\text{components}[start] = \text{components}[end]$, meaning that the two nodes belong to the same component:
-   Push the cost of the component ($componentCost[\text{components}[start]]$) into `answer`.
...
-   Mark that `node` belongs to this component by setting $\text{components}[node] = componentId$.
-   For each `[neighbor, weight]` in $\text{adjList}[node]$:
...
-   Set $\text{components}[node] = componentId$ to mark the `node` as part of the current component.
