# Simplified Approach - Friendly Movies Streamed Last Month
...
## Complexity detail
- **Time Complexity**: $O(P + C + T \log T)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(C + T)$ — Utilizes standard auxiliary storage proportional to the active computation state.
