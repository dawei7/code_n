...
We trace the dynamic Array, Sliding Window, Sorting, Bucket Sort, Ordered Set sliding window on a representative input instance.
...
The objective for **Contains Duplicate III** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
