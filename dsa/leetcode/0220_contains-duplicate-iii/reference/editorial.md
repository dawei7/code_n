...
* C++'s `std::set`, $std::set::\text{upper}_{bound}$ and $std::set::\text{lower}_{bound}$ are equivalent to Java's `TreeSet`, `TreeSet::ceiling` and `TreeSet::floor`, respectively. Python does not provide a Self-balancing BST through its library.
