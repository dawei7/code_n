...
The natural way to represent the time is as an integer `t` in the range $0 \le t < 24 * 60$. Then the hours are $t / 60$, the minutes are `t % 60`, and each digit of the hours and minutes can be found by $hours / 10, hours \% 10$ etc.
...
Also, we should make sure to handle $\text{cand}_{elapsed}$ carefully. When our current candidate time `cur` is equal to the given starting time, then $\text{cand}_{elapsed}$ will be `0` and we should handle this case appropriately.
