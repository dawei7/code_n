## General
**Optimal Approach — Top Travellers**

The query executes a structured relational pipeline for **Top Travellers**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(U + R + U\log U)$ — Operation count proportional to input scale.
