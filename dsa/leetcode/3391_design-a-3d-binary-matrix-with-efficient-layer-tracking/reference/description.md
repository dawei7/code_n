...
- `void setCell(int x, int y, int z)` Sets the value at $\text{matrix}[x][y][z]$ to 1.
...
matrix3D.setCell(0, 0, 0); // Sets $\text{matrix}[0][0][0]$ to 1.

matrix3D.largestMatrix(); // Returns 0. $\text{matrix}[0]$ has the most number of 1's.

matrix3D.setCell(1, 1, 2); // Sets $\text{matrix}[1][1][2]$ to 1.

matrix3D.largestMatrix(); // Returns 1. $\text{matrix}[0]$ and $\text{matrix}[1]$ tie with the most number of 1's, but index 1 is bigger.
...
matrix3D.setCell(2, 1, 1); // Sets $\text{matrix}[2][1][1]$ to 1.

matrix3D.largestMatrix(); // Returns 2. $\text{matrix}[2]$ has the most number of 1's.
