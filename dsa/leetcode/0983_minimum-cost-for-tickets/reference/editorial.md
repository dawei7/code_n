...
1. Ticket valid for `1` day at $\text{costs}[0]$ dollars.
2. Ticket valid for `7` days at $\text{costs}[1]$ dollars.
3. Ticket valid for `30` days at $\text{costs}[2]$ dollars.
...
In the recursive function, we will start with `currDay` as `1`. The base condition would be when we have covered all the days, which can be identified as $currDay > days[\text{days.length} - 1]$. Now we need to decide if we need to travel on this day or not; for this, we will check if the `currDay` is present in `days`. If not, then we don't need to buy a ticket, and hence we simply move on to the next day by returning $solve(currDay + 1)$. To efficiently find if `currDay` is present in `days` or not, we will create a hash set  `isTravelNeeded` which will have all the days on which we need to travel.

If we need to buy a ticket on this day, we have three choices:
1. Buy a 1-day pass. We incur a cost of $\text{costs}[0]$ and move on to the next day. The total cost is $\text{cost}[0] + solve(currDay + 1)$.
2. Buy a 7-day pass. We incur a cost of $\text{costs}[1]$ and don't need to worry about the next seven days. The total cost is $\text{cost}[1] + solve(currDay + 7)$.
3. Buy a 30-day pass. We incur a cost of $\text{costs}[2]$ and don't need to worry about the next thirty days. The total cost is $\text{cost}[2] + solve(currDay + 30)$.
...
- Find the cost for the three tickets we can take for this day, add the corresponding cost, and update $\text{dp}[currDay]$ accordingly in the recursive call.
...
We will start with the previous approach's base case and build up the answers for the remaining states using the recursive equation. In this approach, $\text{dp}[day]$ represents the minimum cost to travel until `day`. For each value of `day`, we got here in one of three ways:

1. We bought a one-day ticket on $day - 1$ with cost $\text{costs}[0]$.
2. We bought a seven-day ticket on $day - 7$ with cost $\text{costs}[1]$.
3. We bought a thirty-day ticket on $day - 30$ with costs $\text{costs}[2]$.
...
1. If the current `day` is less than $\text{days}[i]$, the cost for $\text{dp}[day]$ would be the same as $dp[day - 1]$ as we don't need to travel on this day.
2. Otherwise, store the minimum of three options per the recursive equation in the array as $\text{dp}[day]$. Also, increment the variable `i` as we have bought the ticket for this index and now need to focus on the next index.
4. Return $\text{dp}[lastDay]$; `lastDay` is the last value in the array `days`.
