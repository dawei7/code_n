...
- Check if both initial moves (right and down) in the grid require more than 1 second:
  - If both $\text{grid}[0][1] > 1$ and $\text{grid}[1][0] > 1$, return `-1` because it’s impossible to proceed.
...
- Add the starting point (top-left cell) to the priority queue with its initial time ($\text{grid}[0][0]$).
