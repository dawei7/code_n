...
The core task in **Minimum Increment to Make Array Unique** is to an integer array `nums`. In one move, you can pick an index `i` where $0 \le i < \text{nums.length}$ and increment $\text{nums}[i]$ by `1`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
