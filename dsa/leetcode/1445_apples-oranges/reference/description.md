...
- $Sales(\text{sale}_{date}, fruit, \text{sold}_{num})$ records a daily quantity for `apples` or `oranges`;
- ($\text{sale}_{date}$, `fruit`) uniquely identifies a row.

Let $R$ be the number of rows in `Sales`, and let $D$ be the number of distinct sale dates.

**Return value**

Return $\text{sale}_{date}$ and `diff` for every recorded date, where
