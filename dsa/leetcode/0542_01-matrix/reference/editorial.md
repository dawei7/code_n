...
![Slide 1](images/slideshow_542_1.png)

![Slide 2](images/slideshow_542_2.png)

![Slide 3](images/slideshow_542_3.png)

![Slide 4](images/slideshow_542_4.png)

![Slide 5](images/slideshow_542_5.png)

![Slide 6](images/slideshow_542_6.png)

![Slide 7](images/slideshow_542_7.png)

![Slide 8](images/slideshow_542_8.png)
...
- If so, set $\text{matrix}[nextRow][nextCol] = steps + 1$ and push $nextRow, nextCol, steps + 1$ onto the `queue`. Also mark `nextRow, nextCol` in `seen`.
5. Return `matrix`.
...
$\text{dp}[row][col] = 1 + min(dp[row - 1][col], dp[row + 1][col], \text{dp}[row][col - 1], \text{dp}[row][col + 1])$

Where $\text{dp}[x][y]$ is the minimum distance for the cell $\text{mat}[x][y]$ and $\text{mat}[x][y] \neq 0$.
...
$\text{dp}[row][col] = 1 + min(dp[row - 1][col], \text{dp}[row][col - 1])$
...
- Set $\text{dp}[row][col] = minNeighbor + 1$.
3. Iterate over `dp` again but in reverse order. At each `row, col`:
- Initialize `minNeighbor` to a large value.
- Making sure to stay in bounds, check the two cells we could have arrived from: $row + 1, col$ and $row, col + 1$.
- Update `minNeighbor` with their `dp` values.
- If $minNeighbor + 1$ is less than $\text{dp}[row][col]$ (the minimum distance if we only considered down-right paths), then update it.
4. Return `dp`.
