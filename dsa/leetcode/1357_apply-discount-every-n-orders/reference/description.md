...
- $1 \le \text{products}[i] \le 200$

- $1 \le \text{prices}[i] \le 1000$

- The elements in `products` are **unique**.

- $1 \le \text{product.length} \le \text{products.length}$

- $\text{amount.length} = \text{product.length}$

- $\text{product}[j]$ exists in `products`.

- $1 \le \text{amount}[j] \le 1000$
