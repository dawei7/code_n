...
The core task in **Find Nearest Point That Has the Same X or Y Coordinate** is to two integers, `x` and `y`, which represent your current location on a Cartesian grid: `(x, y)`. You are also given an array `points` where each $\text{points}[i] = [a_{i}, b_{i}]$ represents that a point exists at $(a_{i}, b_{i})$. A point is **valid** if it shares the same x-.... To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
