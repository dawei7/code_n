...
You are also given a **0-indexed** 2D integer array `queries` where $\text{queries}[i] = [k_{i}, \text{trim}_{i}]$. For each $\text{queries}[i]$, you need to:

- **Trim** each number in `nums` to its **rightmost** $\text{trim}_{i}$ digits.
...
- **Output:** `[3,0]`
- **Explanation:** 1. Trimmed to the last digit, nums = ["4","7","6","4"]. The 2^nd smallest number is 4 at index 3.
...
- All $\text{nums}[i].length$ are **equal**.

- $1 \le \text{queries.length} \le 100$

- $\text{queries}[i].length = 2$

- $1 \le k_{i} \le \text{nums.length}$

- $1 \le \text{trim}_{i} \le \text{nums}[i].length$
