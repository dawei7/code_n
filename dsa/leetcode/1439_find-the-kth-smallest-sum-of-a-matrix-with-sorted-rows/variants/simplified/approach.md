# Simplified Approach - Find the Kth Smallest Sum of a Matrix With Sorted Rows
...
## Complexity detail
- **Time Complexity**: $O(mn+mklog k)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(k)$ — Utilizes standard auxiliary storage proportional to the active computation state.
