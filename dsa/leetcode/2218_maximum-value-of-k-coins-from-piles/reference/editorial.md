...
Let $\text{dp}[i][coins]$ be the maximum total value of coins you can have in your wallet if you choose at most `coins` coins from the leftmost `i` piles optimally.

>For example, $\text{dp}[4][7]$ is the maximum total value when one takes at most seven coins from the leftmost four piles. Since all coins have positive denomination, if the leftmost four piles contain at least seven coins in total, it is optimal to take exactly seven coins. In other words, it is never optimal to take less coins than we are allowed.

The base case of this DP is $i = 0$ – no piles are considered, so one didn't take any coins from any piles. Since the total value of $0$ coins is zero, $\text{dp}[0][coins] = 0$.
...
When the value of `currentCoins` is optimal, $\text{dp}[i][coins] = dp[i - 1][coins - currentCoins] + currentSum$, because $dp[i - 1][coins - currentCoins]$ gives the optimal answer to the smaller subproblem of size $i - 1$.
...
Finally, we can formulate the DP transitions: $\text{dp}[i][coins]$ is the maximum $dp[i - 1][coins - currentCoins] + currentSum$ over `currentCoins` between `0` and $min(piles[i - 1].length, coins)$ inclusively.

Since it is never optimal to take less than `k` coins if it is allowed to take `k`, the answer to the problem is $\text{dp}[n][k]$ – one takes at most `k` coins (in the optimal solution we will take **exactly** `k`) from `n` piles.
...
* Update the value of $\text{dp}[i][coins]$ with $dp[i - 1][coins - currentCoins] + currentSum$.
3. Return $\text{dp}[n][k]$.
...
For specified `i` and `coins`, the number of iterations of the `for currentCoins` loop is $\min (\text{len}(\text{piles}[i - 1]), \text{coins}) + 1 =$\mathcal{O}(\text{len}(\text{piles}[i - 1])$)$.

To find the total number of iterations, we need to calculate the sum of this value over all possible values of `i` and `coins`: $\sum_{i=1}^n \sum_{\text{coins}=0}^k$\mathcal{O}(\text{len}(\text{piles}[i - 1])$) = \sum_{i=1}^n (k + 1) \cdot$\mathcal{O}(\text{len}(\text{piles}[i - 1])$) = (k + 1) \cdot \sum_{i=1}^n$\mathcal{O}(\text{len}(\text{piles}[i - 1])$) = (k + 1) \cdot$\mathcal{O}(s)$= O(k \cdot s)$.
...
We will use the recursive function `f(i, coins)` that returns the value of $\text{dp}[i][coins]$.
...
The answer to the problem is $f(n, k) = \text{dp}[n][k]$.
...
For example, we call `f(4, 7)` for the first time, calculate the result for $i = 4$, $coins = 7$ and write this result into $\text{dp}[4][7]$. When we call `f(4, 7)` for the second time, we don't compute the same result again, but return the value of $\text{dp}[4][7]$.

This is how we achieve calculating the value of `f` for each state (each pair of parameters `(i, coins)`) only once.

There remains one small technical question: how to know whether we call `f(i, coins)` for the first time and need to compute the result, or we call it later and can return $\text{dp}[i][coins]$ which was computed earlier? One can handle this by initializing the `dp` matrix with `-1`. Then $\text{dp}[i][coins] = -1$ will mean that `f(i, coins)` was not calculated yet. As soon as we find the result `f(i, coins)`, we will write it in into $\text{dp}[i][coins]$ and this value will not be `-1` anymore.

#### Algorithm

The function `f` takes two parameters: `i` and `coins`.
1. If $i = 0$, return $0$.
2. If $\text{dp}[i][coins] \neq -1$ (which means that we found this value earlier), return $\text{dp}[i][coins]$.
3. Initialize $currentSum = 0$.
4. Iterate `currentCoins` from `0` to $min(piles[i - 1].length, coins)$.
* If `currentCoins > 0`, increase `currentSum` by $piles[i - 1][currentCoins - 1]$.
* Update the value of $\text{dp}[i][coins]$ with $f(i - 1, coins - currentCoins) + currentSum$.
5. Return $\text{dp}[i][coins]$.
...
Even though we changed the order in which we calculate DP, the time complexity is the same as in the previous approach: for each pair `(i, coins)` we calculate $\text{dp}[i][coins]$ in $O(\text{len}(\text{piles}[i - 1]))$. Since we store the results in the memory, we will compute $\text{dp}[i][coins]$ only once.
