...
**The actual distance is a distraction**
...
**How the exact code counts the groups**
...
**Why the approach is optimal**

Choose any proposed final position \(t\). If \(t\) is even, every initially odd chip must undergo an odd number of parity-changing unit moves, so it must cost at least one; this gives a lower bound equal to the odd count. That bound is attainable by using free two-step moves plus exactly one unit move for each odd chip. The minimum cost for an even \(t\) is therefore exactly the odd count. Symmetrically, the minimum for an odd \(t\) is exactly the even count. Taking the smaller of the only two possible destination classes proves global optimality.

This reasoning also explains why no simulation is needed. The problem asks only for total cost, not the chosen coordinate or a sequence of moves. Parity contains every fact that can affect that cost.

## Complexity detail

Let \(n\) be the number of chips, which is `len(position)`. The generator inside `sum` examines every position once and performs one remainder operation per chip, so the running time is \(O(n)\). Computing the length, subtracting, and taking a minimum are each \(O(1)\). Reading every chip is necessary in the worst case because changing the parity of one unseen position can change the answer, so the linear time bound is asymptotically optimal.

The generator is consumed lazily by `sum` and does not build a separate list. Apart from generator iteration state and the two integer counts, the algorithm uses \(O(1)\) auxiliary space. The input list is not modified and is not counted as extra space.
