...
Note that $\text{orders}[i]$ represents a batch of $\text{amount}_{i}$ independent orders with the same price and order type. All orders represented by $\text{orders}[i]$ will be placed before all orders represented by `orders[i+1]` for all valid `i`.
...
- `orders`: Input parameter (`List[List[int]]`).
...
- $1 \le \text{price}_{i}, \text{amount}_{i} \le 10^{9}$

- $\text{orderType}_{i}$ is either `0` or `1`.
