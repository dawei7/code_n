...
The core task in **Count All Valid Pickup and Delivery Options** is to `n` orders, each order consists of a pickup and a delivery service. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
