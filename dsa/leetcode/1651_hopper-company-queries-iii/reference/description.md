...
The `average_ride_distance` is calculated by summing up the total $\text{ride}_{distance}$ values from the three months and dividing it by `3`. The `average_ride_duration` is calculated in a similar way.
