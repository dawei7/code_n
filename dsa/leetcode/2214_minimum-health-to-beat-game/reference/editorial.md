...
The problem presents a game with levels `0` to `n-1`. We need to visit every level, and visiting each level causes some damage. For a level `i`, the damage caused is $\text{damage}[i]$. We are also given an armor that can be used at most once during the game at any level. It will shield us from at most `armor` damage.
