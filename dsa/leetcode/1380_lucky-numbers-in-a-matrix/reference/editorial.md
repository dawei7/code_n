...
3. Iterate over each integer in the matrix and for each integer at `(i, j)`, check if the integer is equal to $\text{rowMin}[i]$ and $\text{colMax}[j]$. If yes, add it to the list `luckyNumbers`.
