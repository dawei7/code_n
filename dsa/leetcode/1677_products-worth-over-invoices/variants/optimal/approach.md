## General
**Optimal Approach — Product's Worth Over Invoices**

The query executes a structured relational pipeline for **Product's Worth Over Invoices**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(I + P \log P)$ — Operation count proportional to input scale.
