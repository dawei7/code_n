...
![pic](images/430_list_step_1.png)
...
![pic](images/430_list_step_2.png)

![pic](images/430_list_step_3.png)
...
- First of all, we define our recursive function as $\text{flatten}_{dfs}(prev, curr)$ which takes two pointers as input and returns the pointer of tail in the _flattened_ list. The `curr` pointer leads to the sub-list that we would like to flatten, and the `prev` pointer points to the element that should precede the `curr` element.
<br/>

- Within the recursive function $\text{flatten}_{dfs}(prev, curr)$, we first establish the double links between the `prev` and `curr` nodes, as in the _**preorder**_ DFS we take care of the **current state** first before looking into the children.
<br/>

- Further in the function $\text{flatten}_{dfs}(prev, curr)$, we then go ahead to flatten the **left subtree** (_i.e._ the sublist pointed by the `curr.child` pointer) with the call of $\text{flatten}_{dfs}(curr, \text{curr.child})$, which returns the `tail` element to the flattened sublist. Then with the `tail` element of the previous sublist, we then further flatten the **right subtree** (_i.e._ the sublist pointed by the `curr.next` pointer), with the call of $\text{flatten}_{dfs}(tail, \text{curr.next})$.
<br/>
...
- We should make a copy of the `curr.next` pointer before the first recursive call of $\text{flatten}_{dfs}(curr, \text{curr.child})$, since the `curr.next` pointer might be altered within the function.
...
![Slide 1](images/slideshow_430_LIS_430_slide_2.png)

![Slide 2](images/slideshow_430_LIS_430_slide_3.png)

![Slide 3](images/slideshow_430_LIS_430_slide_4.png)

![Slide 4](images/slideshow_430_LIS_430_slide_5.png)

![Slide 5](images/slideshow_430_LIS_430_slide_6.png)

![Slide 6](images/slideshow_430_LIS_430_slide_7.png)

![Slide 7](images/slideshow_430_LIS_430_slide_8.png)

![Slide 8](images/slideshow_430_LIS_430_slide_9.png)

![Slide 9](images/slideshow_430_LIS_430_slide_10.png)

![Slide 10](images/slideshow_430_LIS_430_slide_11.png)

![Slide 11](images/slideshow_430_LIS_430_slide_12.png)
