# Simplified Approach - Minimum Absolute Difference in Sliding Submatrix
...
## Complexity detail
- **Time Complexity**: $O(R C k^2 log k)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(k^2 + R C)$ — Utilizes standard auxiliary storage proportional to the active computation state.
