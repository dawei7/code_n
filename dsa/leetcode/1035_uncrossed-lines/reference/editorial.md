...
2. Create a 2D array called `memo` having $n1 + 1$ rows and $n2 + 1$ columns where $\text{memo}[i][j]$ contains the maximum number of lines we can draw by choosing the first `i` numbers from `nums1` and the first `j` numbers from `nums2`. Initialize it to `-1`.
...
- If $i \le 0 || j \le 0$, it indicates that we don't have any number in one of the two arrays under consideration. We return `0`.
- If $\text{memo}[i][j] \neq -1$, it indicates that we have already solved this subproblem, so we return $\text{memo}[i][j]$.
- If $nums[i - 1] = nums[j - 1]$, we add `1` to include the line between these numbers and recursively solve the problem ignoring the last number of both arrays. We perform $\text{memo}[i][j] = 1 + solve(i - 1, j - 1, nums1, nums2, memo)$.
- Otherwise, if the last numbers do not match, we recursively search for the maximum number of lines that can be drawn ignoring the last number from both arrays. We pick the maximum of these two. We perform $\text{memo}[i][j] = max(solve(i, j - 1, nums1, nums2, memo), solve(i - 1, j, nums1, nums2, memo))$.
- Return $\text{memo}[i][j]$.
...
Using the same method as before, we create a 2D-array `dp`, where $\text{dp}[i][j]$ contains the maximum number of lines we can draw by choosing the first `i` numbers from `nums1` and the first `j` numbers from `nums2`. Our answer would be $\text{dp}[n1][n2]$, $n1$ is the length of `nums1` and $n2$ is the length of `nums2`. The state transition would be as follows:

> 1. If $nums1[i - 1] = nums2[j - 1]$, perform $\text{dp}[i][j] = 1 + dp[i - 1][j - 1]$.
> 2. Otherwise, perform $\text{dp}[i][j] = max(\text{dp}[i][j - 1], dp[i - 1][j]$.

#### Algorithm

1. Create two integer variables `n1` and `n2`. Initialize them to the size of `nums1` and `nums2`.
2. Create a 2D array called `dp` having $n1 + 1$ rows and $n2 + 1$ columns where $\text{dp}[i][j]$ contains the maximum number of lines we can draw by choosing the first `i` numbers from `nums1` and the first `j` numbers from `nums2`. It is initialized to `0`.
...
- If the last number from both the arrays under consideration are equal, i.e., $nums1[i - 1] = nums2[j - 1]$, we draw a line between two numbers and add it to the maximum number of lines that can be drawn ignoring the last number from both the arrays. We perform $\text{dp}[i][j] = 1 + dp[i - 1][j - 1]$. We already have the answer for $dp[i - 1][j - 1]$ which was computed in the previous iteration of the outer loop.
- Otherwise, if the last numbers do not match, we look for the maximum number of lines that can be drawn ignoring the last number from both arrays. We pick the maximum of these two. We perform $\text{dp}[i][j] = max(\text{dp}[i][j - 1], dp[i - 1][j])$.
...
If we look closely at this transition, to fill $\text{dp}[i][j]$ for a particular `i` and all possible values of `j`, we only need the values from the current and previous rows. To fill row `i` in the `dp` grid, we need the values from row `i` ($\text{dp}[i][j - 1]$) and previously computed value in the $(i - 1)^{th}$ row ($dp[i - 1][j - 1]$ and $dp[i - 1][j]$). Values in rows $i - 2$, `i- 3`, and so on are no longer needed.
...
Now, on iteration `i`, $\text{dp}[j]$ stores the maximum number of lines we can draw by choosing the first `i` numbers from `nums1` and the first `j` numbers from `nums2`. It is similar to what $\text{dp}[i][j]$ stored in the previous approach.

The other array `dpPrev` is important to understand. It helps us to remember the previous state that we completed previously. On iteration `i`, $\text{dpPrev}[j]$ stores the maximum number of lines we can draw by choosing the first $i - 1$ numbers from `nums1` and the first `j` numbers from `nums2`. It is analogous to $dp[i - 1][j]$ in the previous approach.
...
3. We iterate using two loops. The outer loop iterates from $i = 1$ to $i = n1$ incrementing `i` by `1` after each iteration. We start an inner loop that iterates from $j = 1$ to $j = n2$ and perform the following:
- If the last number from both the arrays under consideration are equal, i.e., $nums1[i - 1] = nums2[j - 1]$, we draw a line between two numbers and add it to the maximum number of lines that can be drawn ignoring the last number from both the arrays. We perform $\text{dp}[j] = 1 + dpPrev[j - 1]$.
- Otherwise, if the last numbers do not match, we look for the maximum number of lines that can be drawn ignoring the last number from both arrays. We pick the maximum of these two. We perform $\text{dp}[j] = max(dp[j - 1], \text{dpPrev}[j])$.
- After the completion of the inner loop, we copy `dp` to `dpPrev`.
4. Return $\text{dp}[n2]$ (or $\text{dpPrev}[n2]$ as both are similar).
