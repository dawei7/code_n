...
This gives us a linear time way to count strings with all characters appearing an even number of times: maintain the parity mask of the current prefix, and compare it with previous prefixes of the same value in a frequency map. The key is a mask, which corresponds to a prefix of the string, and the value is the frequency of the key mask. To count substrings with all even letters ending at some index $r$, take the prefix ending at $r$ with parity mask $m$, and add $\text{freq}[m]$ to the answer. The difference of two prefixes with the same bitmask will equal `0`, which corresponds to strings with all even frequency letters.
...
![figA](images/1915_acadac_revised.png)
...
6. Iterate through each possible character that appears an odd number of times, and add the frequency of $mask ^ (1 << \text{odd}_{c})$, where $^$ is the XOR function.
