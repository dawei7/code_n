...
The core task in **Number of Ways to Split a String** is to a binary string `s`, you can split `s` into 3 **non-empty** strings `s1`, `s2`, and `s3` where $s1 + s2 + s3 = s$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
