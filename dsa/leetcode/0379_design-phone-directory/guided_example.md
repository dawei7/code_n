...
We execute the single-pass Array, Hash Table, Linked List, Design, Queue pointer manipulation on a representative linked list instance.

- **Input:** `{"max_numbers": 3, "operations": [["get"], ["get"], ["check", 2], ["get"], ["check", 2], ["release", 2], ["check", 2]]}`
...
The objective for **Design Phone Directory** is to transform the linked structure by strictly updating `next` references in place.
