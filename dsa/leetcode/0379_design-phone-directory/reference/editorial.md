...
Each slot `i` assigned by `get()` will be marked as $\text{isSlotAvailable}[i] = false$ to indicate the slot is already occupied. Each slot `i` released by `release()` will be marked as `true` to indicate it's available.

While performing the `get()` operation, we can iterate on all slots from `0` till `isSlotAvailable.size()` one by one, and search for an unoccupied slot (i.e. $\text{isSlotAvailable}[i] = true$), if found, then we mark that slot as occupied (i.e. $\text{isSlotAvailable}[i] = false$) and return the slot `i`.
If no slot is available then in that case we return `-1`.

To check if a slot `number` is occupied or not, we can check if $\text{isSlotAvailable}[number]$ is `true` (available) or `false` (not available).

To release a slot `number`, we need to set $\text{isSlotAvailable}[number] = true$, indicating that the slot is available.
...
2. Implementing the `get()` method:
    Iterate through our `isSlotAvailable` array and if $\text{isSlotAvailable}[i] = true$ at any index, mark $\text{isSlotAvailable}[i] = false$ and return `i`.
    Otherwise, return `-1`.

3. Implementing the `check(number)` method:
    Return $\text{isSlotAvailable}[number]$.

4. Implementing the `release(number)` method:
    Mark $\text{isSlotAvailable}[number] = true$.
...
Searching if a `number` is present in the queue or not will take linear time, so to avoid searching in the queue to check whether the slot is available or not, we will use the previously used boolean array `isSlotAvailable` where we will mark $\text{isSlotAvailable}[number] = true$ if `number` is present in the queue. Thus, we can quickly check if the slot `number` is present in the queue (i.e., is available) or not in constant time.
...
While performing the `get()` operation, we need to return any available slot, so if `slotsAvailableQueue` is not empty, we pop its first element `slot`, mark it as not available (i.e. $\text{isSlotAvailable}[number] = false$), and return `slot`.
Otherwise, if `slotsAvailableQueue` is empty, all slots are already occupied, then we return `-1`.

To check if a slot `number` is occupied or not, we can check if $\text{isSlotAvailable}[number]$ is `true` (indicating it's available), or `false` (indicating it's not available).

While releasing a slot `number`, if $\text{isSlotAvailable}[number]$ is already `true` it means it is already available and is present in the queue so there is no need to push it again.
Otherwise, the `number` was earlier occupied and is now released, so, we will push the `number` in the `slotsAvailableQueue` queue and mark it as available $\text{isSlotAvailable}[number] = true$.
...
2. Implementing the `get()` method:
    If `slotsAvalaiableQueue` is not empty, we pop its first element `slot`, mark $\text{isSlotAvailable}[slot] = false$, and return `slot`.
    Otherwise, return `-1`.
...
4. Implementing the `release(number)` method:
    If $\text{isSlotAvailable}[number] = true$, then return.
    Otherwise, mark $\text{isSlotAvailable}[number] = true$ and push `number` in `slotsAvalaiableQueue`.
