...
Now, suppose `q` is the least number for which $len(B) \le len(A * q)$. We only need to check whether `B` is a substring of $A * q$ or $A * (q+1)$. If we try `k < q`, then `B` has a larger length than $A * q$ and therefore can't be a substring. When $k = q+1$, $A * k$ is already big enough to try all positions for `B`; namely, $A[i:i+len(B)] = B$ for $i = 0, 1, ..., len(A) - 1$.
