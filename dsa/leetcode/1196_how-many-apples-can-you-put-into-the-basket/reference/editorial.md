...
> **Note:** We will creat a heap using the heapify method. To create a heap using the heapify method requires $\mathcal{O}(N)$ time. More details can be found [here](https://stackoverflow.com/questions/9755721/how-can-building-a-heap-be-on-time-complexity).
...
Notably, this question has the constraint that $1 \leq \text{\text{arr}[i].length} \leq 10^{3}$, and taking advantage of this we can improve the time complexity to $\mathcal{O}(n)$ using [Counting Sort](https://leetcode.com/explore/learn/card/sorting/695/non-comparison-based-sorts/4437/). However, this approach should be used with caution. It is safe to use here because the heaviest apple is guaranteed to weigh no more than $10^{3}$, but this approach would not perform well on a test case such as $\text{weight} = [1, 2, 3, 10^{9}]$.

**Algorithm**

- Create an array `counts` with the length of $max(arr) + 1$, where $\text{counts}[i]$ represents the number of apples with weight `i`.
- Iterate through `arr`:
- for each $\text{arr}[i]$, increment $counts[\text{arr}[i]]$ by `1`;
...
- to make sure it will not exceed `5000` units of weight, the number of apples we take is  $take = min{\text{counts}[i], (5000-units)/i}$;
