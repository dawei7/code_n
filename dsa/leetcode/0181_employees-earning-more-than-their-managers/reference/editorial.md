...
Next, because we have the employee salary $\text{salary}_{e}$ and their manager's salary $\text{salary}_{m}$, we can use `loc` to select rows of interest. In addition, we only select one column, which is the name of the employee $\text{name}_{e}$.
