...
![one example robbing](images/337_overview.png)
...
function helper(node, the parent is robbed or not?) {
    ...
    if the parent is not robbed:
        two choices: rob this node or not?
        calculate `rob` and $\text{not}_{rob}$...
        return max(rob, not_rob)
}
</pre>
...
For example, when calculating `rob` and $\text{not}_{rob}$:
...
Here we create two arrays: $\text{dp}_{rob}$ and `dp_not_rob`.
...
![the example mapping](images/337_approach_2_1.png)
...
![the example mapping with graph](images/337_approach_2_2.png)
