...
**Only the largest project can create a scheduling bottleneck**
...
**Why projects among “rest” can supply the separators**
...
**Read the inequality at the boundary**

The condition in the source is `mx > rest + 1`, not `mx >= rest + 1`. When equality holds, a schedule can begin and end with the largest project:

largest, separator, largest, separator, $\ldots$, largest.

It uses every milestone and has one more largest-project occurrence than all other occurrences combined. Changing the comparison to greater-than-or-equal would enter the imbalanced branch unnecessarily. The numeric answer would happen to remain the same at exact equality because $s=mx+rest=2rest+1$, but the logical classification would be wrong.

**Why choosing a largest project is sufficient**

If the maximum count satisfies the separator condition, every smaller count does too because it has at least as many milestones outside its own project. If the maximum violates it, no smaller project can be the limiting one. Thus the complete feasibility question reduces to only `max` and `sum`; project identities and the order of smaller counts need not be stored.

For `[10,1,1,1]`, `rest=3`. The best sequence uses four milestones from the largest project and all three separators, for seven weeks. Six dominant milestones remain, but none can be scheduled after the final dominant week without repeating the same project.

## Complexity detail

Let $N$ be the number of projects.

`max(milestones)` scans the array once and `sum(milestones)` scans it once more. Two linear passes remain $O(N)$ time. The arithmetic afterward is constant time.

Only `mx`, `s`, and `rest` are stored, so auxiliary space is $O(1)$. The input is not modified.

Python integers safely hold totals beyond fixed 32-bit ranges.
...
- **Several projects tied for maximum:** None can dominate the sum of all others excessively, so the balanced branch handles them.
