...
<center>
<img src="images/Diag_1.png" width="800"></center>
...
6. When all of the parentheses in the original expression have been processed, we simply check if the expression represented by `expr` i.e. the expression formed till now is valid one or not. The way we check if the final expression is valid or not is by looking at the values in $\text{left}_{count}$ and $\text{right}_{count}$. For an expression to be valid $\text{left}_{count} = \text{right}_{count}$. If it is indeed valid, then it could be one of our possible solutions.
* Even though we have a valid expression, we also need to keep track of the number of removals we did to get this expression. This is done by another variable passed in recursion called $\text{rem}_{count}$.
* Once recursion finishes we check if the current value of $\text{rem}_{count}$ is < the least number of steps we took to form a valid expression till now i.e. the global minima. If this is not the case, we don't record the new expression, else we record it.
...
2. $\text{left}_{count}$ which represents the number of left parentheses that have been added to the expression we are building.
3. $\text{right}_{count}$ which represents the number of right parentheses that have been added to the expression we are building.
4. $\text{left}_{rem}$ is the number of left parentheses that remain to be removed.
5. $\text{right}_{rem}$ represents the number of right parentheses that remain to be removed. Overall, for the final expression to be valid, $\text{left}_{rem} = 0$ and $\text{right}_{rem} = 0$.
* When we decide to not consider a parenthesis i.e. delete a parenthesis, be it a left or a right parentheses, we have to consider their corresponding remaining counts as well. This means that we can only discard a left parentheses if $\text{left}_{rem} > 0$ and similarly for the right one we will check for $\text{right}_{rem} > 0$.
* There are no changes to checks for **considering** a parenthesis. Only the conditions change for **discarding** a parenthesis.
* Condition for an expression being valid in the base case would now become $\text{left}_{rem} = 0 and \text{right}_{rem} = 0$. Note that we don't have to check if $\text{left}_{count} = \text{right}_{count}$ anymore because in the case of a valid expression, we would have removed all the misplaced or invalid parenthesis by the time the recursion ends. So, the only check we need if $\text{left}_{rem} = 0 and \text{right}_{rem} = 0$.

> The most important thing here is that we have completely gotten rid of checking if the number of parentheses removed is lesser than the current minimum or not. The reason for this is we always remove the same number of parentheses as defined by $\text{left}_{rem} + \text{right}_{rem}$ at the start of recursion.
...
* Time Complexity : The optimization that we have performed is simply a better form of pruning. Pruning here is something that will vary from one test case to another. In the worst case, we can have something like `(((((((((` and the $\text{left}_{rem} = len(S)$ and in such a case we can discard all of the characters because all are misplaced. So, in the worst case we **still** have 2 options per parenthesis and that gives us a complexity of $O(2^N)$.
