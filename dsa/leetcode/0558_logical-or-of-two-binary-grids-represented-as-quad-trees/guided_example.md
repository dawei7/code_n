...
We trace the hierarchical Divide and Conquer, Tree traversal and subtree aggregation on a representative binary tree.

- **Input:** `{"quadTree1": {"leaf": true, "value": 0}, "quadTree2": {"leaf": true, "value": 1}}`
- **Required output:** `{"leaf": true, "value": 1}`
...
The objective for **Logical OR of Two Binary Grids Represented as Quad-Trees** is to evaluate tree properties by visiting nodes in topological hierarchy (post-order, pre-order, or level-order).
