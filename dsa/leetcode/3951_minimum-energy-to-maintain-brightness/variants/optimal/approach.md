...
The core task in **Minimum Energy to Maintain Brightness** is to an integer `n`, representing `n` light bulbs arranged in a line and indexed from 0 to $n - 1$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
