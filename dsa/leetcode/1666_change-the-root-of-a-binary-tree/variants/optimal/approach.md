...
The core task in **Change the Root of a Binary Tree** is to the `root` of a binary tree and a `leaf` node, reroot the tree so that the `leaf` is the new root. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
