...
The core task in **Change the Root of a Binary Tree** is to the `root` of a binary tree and a `leaf` node, reroot the tree so that the `leaf` is the new root. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
