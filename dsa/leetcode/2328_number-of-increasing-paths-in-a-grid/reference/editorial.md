...
Let's build an auxiliary array `dp` of the same size as `grid` to represent the number of paths that end at each cell. Initially, the value of each $\text{dp}[i][j]$ cell is `1`, which stands for the path made by $\text{grid}[i][j]$ cell itself.

![img](images/2.png)

Then, for each cell $\text{grid}[i][j]$, we need to look for its neighbor cells in 4 directions, if there exists a neighbor cell (let's say $grid[i + 1][j]$) that is larger than $\text{grid}[i][j]$, it means every path that ends at $\text{grid}[i][j]$ can be extended to $grid[i + 1][j]$. Therefore, the number of paths ending at $grid[i + 1][j]$ should be incremented by $\text{grid}[i][j]$.
...
3) For each cell $\text{grid}[i][j]$, check its 4-direction neighbor cells, if a neighbor cell $grid[\text{curr}_{i}][\text{curr}_{j}]$ has a larger value, then increment $dp[\text{curr}_{i}][\text{curr}_{j}]$ by $\text{dp}[i][j]$.
...
We define a function `dfs(i, j)` to calculate the number of increasing paths ending at $\text{grid}[i][j]$. `dfs(i, j)` consists of **at most** 5 parts:

- 1, the path consisting of $\text{grid}[i][j]$ itself.
- $dfs(i - 1, j)$, if $grid[i - 1][j]$ exists and is smaller than $\text{grid}[i][j]$.
- $dfs(i, j - 1)$, if $\text{grid}[i][j - 1]$ exists and is smaller than $\text{grid}[i][j]$.
- $dfs(i + 1, j)$, if $grid[i + 1][j]$ exists and is smaller than $\text{grid}[i][j]$.
- $dfs(i, j + 1)$, if $\text{grid}[i][j + 1]$ exists and is smaller than $\text{grid}[i][j]$.

If a neighbor cell with a smaller value (For example, $grid[i + 1][j] < \text{grid}[i][j]$) exists, we can get the number of paths ending at this cell as $dfs(i + 1, j)$, and for all those paths we can extend the path with the current element, so we increment `dfs(i, j)` by $dfs(i + 1, j)$.

![img](images/3.png)

Remember to use the memoization method to avoid repeated computation. Similarly, we create an array `dp` of the same size as `grid`. Initially, each cell $\text{dp}[i][j]$ is set to `-1`, which means **unvisited**.

![img](images/4.png)

Once we get the number of paths ending at cell $\text{grid}[i][j]$ and update $\text{dp}[i][j]$, it means the cell $\text{grid}[i][j]$ is **visited** and the value of `dfs(i, j)` calculated for the first time. If we need `dfs(i, j)` in the further iteration, we don't bother repeating the DFS process to calculate `dfs(i, j)`, but return $\text{dp}[i][j]$ since $\text{dp}[i][j] = dfs(i, j)$.

<br>

As shown in the picture below, suppose we have visited $\text{grid}[0][0]$ and updated $\text{dp}[0][0]$. Now we are visiting $\text{grid}[1][0]$, $\text{grid}[0][0]$ is smaller than $\text{grid}[1][0]$, according to the previous definition, we need a step `dfs(1, 0) += dfs(0, 0)`. However, there is no need to recalculate `dfs(0, 0)`, we can just return $\text{dp}[0][0]$ which will take constant time!

![img](images/5.png)

We can tell that each cell in $\text{dp}[i][j]$ is only calculated by once.
...
2) Iterate over every cell $\text{grid}[i][j]$ and get `dfs(i, j)`, the number of paths end at it:
- If $\text{dp}[i][j]$ is non-zero, it means we have visited this cell, just return $\text{dp}[i][j]$ and repeat step 2.
- Otherwise, set $dfs(i, j) = 1$, the path consisting of the cell itself.
- Check its 4-direction neighbor cells, if a neighbor cell $(\text{prev}_{i}, \text{prev}_{j})$ has a smaller value, increment `dfs(i, j)` by $dfs(\text{prev}_{i}, \text{prev}_{j})$
- Update $\text{dp}[i][j]$ as `dfs(i, j)`.
