...
The column index is calculated as $\text{Math.floor}(i / rowsCount)$. This gives us the integer quotient of `i` divided by `rowsCount`, which corresponds to the column index. We use `Math.floor()` to round down to the nearest integer because the quotient could be a floating-point number.
