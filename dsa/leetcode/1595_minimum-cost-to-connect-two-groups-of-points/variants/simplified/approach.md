# Simplified Approach - Minimum Cost to Connect Two Groups of Points
...
## Complexity detail
- **Time Complexity**: $O(mn2^n)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(m2^n)$ — Utilizes standard auxiliary storage proportional to the active computation state.
