...
<img src="images/847_1.png" width="960"><br>
...
<img src="images/847_2.png" width="960"><br>
...
<img src="images/847_3.png" width="960"><br>
...
$dp(node, mask) = 1 + min(dp(neighbor, mask), dp(neighbor, mask ^ (1 << node))), for all neighbors in \text{graph}[node]$.
...
- Now, apply the recurrence relation. For all `neighbor` in $\text{graph}[node]$, explore both mask options: already visited (`dp(neighbor, mask)`) or visiting for the first time ($dp(neighbor, mask ^ (1 << node))$). Choose the best option out of all adjacent states, add `1` to it, and update the entry in the cache for the current state.
...
More formally, for any given state `(node, mask)`, we can traverse to `(neighbor, mask | (1 << neighbor))` for all `neighbor`s in $\text{graph}[node]$. We will still need to use some space to ensure that we don't revisit states and create an infinite cycle, but this time we don't need to associate the states with any values, just a flag to indicate if it has been visited yet or not.
