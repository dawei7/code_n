...
1. The last two elements $\text{nums}[i]$ and $nums[i - 1]$ form a subarray of two equal elements. In this case, if $prefixIsValid(i - 2)$ is true, it indicates that `prefixIsValid(i)` is also true. Since the valid partition for $nums[0 ~ i - 2]$ can be appended by the subarray `[nums[i - 1], nums[i]]` to form a valid partition for `nums[0 ~ i]`.

![img](images/r4.png)

2. The last three elements $\text{nums}[i]$, $nums[i - 1]$, and $nums[i - 2]$ form a subarray of three equal elements. In this case, if $prefixIsValid(i - 3)$ is true, it indicates that `prefixIsValid(i)` is also true. Since the valid partition for $nums[0 ~ i - 3]$ can be appended by the subarray `[nums[i - 2], nums[i - 1], nums[i]]` to form a valid partition for `nums[0 ~ i]`.

![img](images/r3.png)

3. The last three elements $\text{nums}[i]$, $nums[i - 1]$, and $nums[i - 2]$ form a subarray of three consecutive increasing elements. In this case, if $prefixIsValid(i - 3)$ is true, it indicates that `prefixIsValid(i)` is also true. Since the valid partition for $nums[0 ~ i - 3]$ can be appended by the subarray `[nums[i - 2], nums[i - 1], nums[i]]` to form a valid partition for `nums[0 ~ i]`.
...
- If `i` is stored in `memo`, return $\text{memo}[i]$.
- Otherwise, set $ans = false$.
- If `i > 0` and $\text{nums}[i] = nums[i - 1]$, we update `ans` as $ans |= prefixIsValid(i - 2)$.
- If `i > 1` and $\text{nums}[i] = nums[i - 1] = nums[i - 2]$, update $ans |= prefixIsValid(i - 3)$.
- If `i > 1` and $\text{nums}[i] = nums[i - 1] + 1 = nums[i - 2] + 2$, update $ans |= prefixIsValid(i - 3)$.
- Set $\text{memo}[i] = ans$ and return `ans`.
...
We will initialize an array `dp` of size $n + 1$ with `false` (indicating no valid partition is found yet). Here, $\text{dp}[i]$ represents if the prefix of length `i` can form a valid partition. Note that $\text{dp}[i] = prefixIsValid(i - 1)$ from the previous approach. We set $\text{dp}[0] = true$ since it represents a valid partition for an empty array.

Considering this base case $\text{dp}[0] = true$, for the same prefix array, `dp` and `nums` have different indices, specifically $\text{dp}_{index} = i + 1$. Hence, $dp[\text{dp}_{index}]$ denotes whether there is a valid partition for the prefix array `nums[0 ~ i]`. We will iterate over `nums` and update `dp` as we go along.

![img](images/2.png)

We check the same three possibilities at each index `i` as in the recursive approach. If any of the conditions is true, it represents the existence of a valid partition, and we update $dp[\text{dp}_{index}]$ as `true`.

![img](images/6.png)

In the end, we check the last index of `dp`. If $\text{dp}[n]$ is true, it means we can form a valid partition of the whole array, we return `true`. Otherwise, we return `false`.

<br>

#### Algorithm

1) Create an array `dp` of length $n + 1$, initialized with all `false` values. Set $\text{dp}[0] = true$.

2) Iterate over `nums`, for each index `i`:
- Get the corresponding index to `dp` as $\text{dp}_{index} = i + 1$.
- If `i > 0` and $\text{nums}[i] = nums[i - 1]$, we update $dp[\text{dp}_{index}]$ as $dp[\text{dp}_{index}] |= dp[\text{dp}_{index} - 2]$.
- If `i > 1` and $\text{nums}[i] = nums[i - 1] = nums[i - 2]$, update $dp[\text{dp}_{index}] |= dp[\text{dp}_{index} - 3]$.
- If `i > 1` and $\text{nums}[i] = nums[i - 1] + 1 = nums[i - 2] + 2$, update $dp[\text{dp}_{index}] |= dp[\text{dp}_{index} - 3]$.

3) Return $\text{dp}[n]$ once the iteration is complete.
...
How do we continuously update the stored values using an array of length 3? We use a method called the "rolling index", as shown in the picture below. Let's assume that $\text{long}_{dp}$ is the long table we used in the previous approach, and `dp` is the short array of length `3` for this approach.

- The first three indices can be directly saved in `dp`.

- Upon reaching $\text{dp}_{index} = 3$, we obtain the value of $\text{long}_{dp}[\text{dp}_{index}]$. Recall that we only need the most recent three indices. Therefore, we can directly use $\text{dp}[0]$ to store $\text{long}_{dp}[3]$. Although the original value $\text{dp}[0] = \text{long}_{dp}[0]$ is overwritten, it does not affect the calculation because we no longer need that value in the following iterations.

- Upon reaching $\text{dp}_{index} = 4$, similarly, we use $\text{dp}[1]$ to store $\text{long}_{dp}[4]$.

- Upon reaching $\text{dp}_{index} = 5$, we use $\text{dp}[2]$ to store $\text{long}_{dp}[5]$.

![img](images/better2.png)

We can use the modulo operation on $\text{dp}_{index}$ with `3` to map each element $\text{long}_{dp}[\text{dp}_{index}]$ to $dp[\text{dp}_{index} \% 3]$.

In the end, we check the last element $\text{long}_{dp}[n]$, which is `dp[n % 3]` in our case. If `dp[n % 3]` is true, it means we can form a valid partition of the whole array, we return `true`. Otherwise, we return `false`.

<br>

#### Algorithm

1) Create an array `dp` of length `3`, initialized with all `false` values. Set $\text{dp}[0] = true$.

2) Iterate over `nums`, for each index `i`:
- Get the corresponding index to `dp` as $\text{dp}_{index} = i + 1$.
- If `i > 0` and $\text{nums}[i] = nums[i - 1]$, we update $dp[\text{dp}_{index} \% 3] |= dp[(\text{dp}_{index} - 2) \% 3]$.
- If `i > 1` and $\text{nums}[i] = nums[i - 1] = nums[i - 2]$, update $dp[\text{dp}_{index} \% 3] |= dp[(\text{dp}_{index} - 3) \% 3]$.
- If `i > 1` and $\text{nums}[i] = nums[i - 1] + 1 = nums[i - 2] + 2$, update $dp[\text{dp}_{index} \% 3] |= dp[(\text{dp}_{index} - 3) \% 3]$.
