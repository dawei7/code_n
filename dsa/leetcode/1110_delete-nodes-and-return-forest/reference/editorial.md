...
We are given a binary tree `root` where each node has a unique value, and an array $\text{to}_{delete}$ containing values of nodes to delete. The goal is to delete all nodes with values in $\text{to}_{delete}$ and return all the remaining root nodes.
...
![Slide 1](images/slideshow_slideshow_1110_Slides_1.png)

![Slide 2](images/slideshow_slideshow_1110_Slides_2.png)

![Slide 3](images/slideshow_slideshow_1110_Slides_3.png)

To optimize lookup time for deletions, $\text{to}_{delete}$ is converted into a set. Using a set allows constant time $O(1)$ checks to determine if a node should be deleted, which is more efficient compared to linear time array lookups.
...
1. Initialization:
- Convert the $\text{to}_{delete}$ array to a set for efficient lookups and store it as `toDeleteSet`.
...
- Time Complexity: $O(n)$
    ...
    Initializing and converting the $\text{to}_{delete}$ list into a set takes $O(m)$ time, where $m$ is the number of elements in $\text{to}_{delete}$. Since $m \leq n$, this operation is bounded by $O(n)$.
...
- Space Complexity: $O(n)$
    ...
    The `toDeleteSet` uses $O(m)$ space, where $m$ is the number of elements in $\text{to}_{delete}$. Since $m \leq n$, the space complexity is $O(n)$.
...
2. Create an unordered set `toDeleteSet` from the $\text{to}_{delete}$ list for efficient lookup.
...
- Time Complexity: $O(n)$
    ...
    The set initialization, converting the $\text{to}_{delete}$ list to a set, takes $O(m)$ time, where $m$ is the number of elements in $\text{to}_{delete}$. Since $m \leq n$, this operation is bounded by $O(n)$.
...
- Space Complexity: $O(n)$
    ...
    The `toDeleteSet` uses $O(m)$ space, where $m$ is the number of elements in $\text{to}_{delete}$. Since $m \leq n$, this is bounded by $O(n)$.
