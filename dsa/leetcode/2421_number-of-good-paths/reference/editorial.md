...
There is no point in traversing from a node to its neighbor where the $\text{vals}[neighbor] > \text{vals}[node]$. It violates the good path constraints. So, for any `node`, the only nodes that matter are the ones that have values less than $\text{vals}[node]$. Intuitively, this does give some hints about sorting the nodes according to their values, starting with the smallest value first and then going to higher values.
...
Similarly, we can count three good paths forming in component `c2`. We can also use the same formula with $N = 2$ which gives $(2 * (2 + 1) / 2) = 3$ good paths.
...
For each `node` in `nodes[]`, we check all `neighbor`. If $\text{vals}[node] \ge \text{vals}[neighbor]$, `neighbor` is already covered, and it is a node in the subgraph. It can be used as an intermediate node in a good path if formed using it. We perform `union (node, neighbor)` to add `node` to the current subgraph. Otherwise, if $\text{vals}[node] < \text{vals}[neighbor]$, the `node` is not added to the existing components, and it creates a new component with the `node` itself.
...
![Slide 1](images/slideshow_2421_number_of_good_paths_slide-1.png)

![Slide 2](images/slideshow_2421_number_of_good_paths_slide-2.png)

![Slide 3](images/slideshow_2421_number_of_good_paths_slide-3.png)

![Slide 4](images/slideshow_2421_number_of_good_paths_slide-4.png)

![Slide 5](images/slideshow_2421_number_of_good_paths_slide-5.png)
...
2. Create a map `valuesToNodes` where $\text{valuesToNodes}[X]$ is an array that contains all the nodes having the value `X`. The data structure chosen to create such a map sorts the keys in non-decreasing order, i.e., the keys are sorted.
- Iterate over all the nodes and add each `node` to $valuesToNodes[\text{vals}[node]]$.
3. Create a class `UnionFind` defining standard methods `find` and $\text{union}_{set}$.
4. Create an instance of `UnionFind`, passing the size as `n`. Also, initialize the count of good paths variable `goodPaths` with `0`.
5. Iterate over each entry `value, nodes` in `valuesToNodes` in ascending order.
- For every `node` in `nodes`, iterate over its neighbors.
- For each `neighbor` of the `node`, if $\text{vals}[node] \ge \text{vals}[neighbor]$ we perform a `union` of the `node` with the `neighbor`.
- After iterating through all the `nodes`, we create a map `group`. $\text{group}[A]$ contains the number of nodes (from the `nodes` array) that belong to the same component `A`. For every `node` in `nodes`, we find its component and increment the size of that component by `1` in `groups`, i.e., $group[find(node)] = group[find(node)] + 1$.
...
- We also need a map `valuesToNodes` having sorted keys. Each operation in such a data structure comes with a $log$ factor. We push all the `n` nodes into the map and iterate over all of them, which further adds $O(n \cdot log(n)))$ time.
