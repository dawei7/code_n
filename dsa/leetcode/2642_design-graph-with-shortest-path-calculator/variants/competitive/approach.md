...
The core task in **Design Graph With Shortest Path Calculator** is to There is a **directed weighted** graph that consists of `n` nodes numbered from `0` to $n - 1$. The edges of the graph are initially represented by the given array `edges` where $\text{edges}[i] = [\text{from}_{i}, \text{to}_{i}, \text{edgeCost}_{i}]$ meaning that there is an .... To dynamically keep track of the minimum or maximum value without sorting the entire array repeatedly, this solution uses a **Min/Max Heap (Priority Queue)**. It allows us to insert elements and extract the smallest/largest value in fast logarithmic $O(\log k)$ time.
