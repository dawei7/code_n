...
If there is a length `k` subarray common to `A` and `B`, then there is a length $j \le k$ subarray as well.
...
Since a common subarray of `A` and `B` must start at some $A[i]$ and $B[j]$, let $\text{dp}[i][j]$ be the longest common prefix of `A[i:]` and `B[j:]`. Whenever $A[i] = B[j]$, we know $\text{dp}[i][j] = dp[i+1][j+1] + 1$.  Also, the answer is $max(\text{dp}[i][j])$ over all `i, j`.
...
For every $i \ge length - 1$, we will want to record the hash of $A[i-length+1], A[i-length+2], ..., A[i]$. After, we will truncate the first element by $h = (h - A[i - (length - 1)]) * Pinv \% MOD$ to get ready to add the next element.
