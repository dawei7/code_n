...
The core task in **Sort Array by Moving Items to Empty Space** is to an integer array `nums` of size `n` containing **each** element from `0` to $n - 1$ (**inclusive**). Each of the elements from `1` to $n - 1$ represents an item, and the element `0` represents an empty space. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
