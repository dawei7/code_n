...
- Add the result of the recursive call to `sum` and take modulo `k`.
  - Add the value of `currentNode` ($\text{nodeValues}[currentNode]$) to `sum` and take modulo `k`.
...
- Otherwise, add the value of `currentNode` to $\text{values}[neighborNode]$.
...
- Otherwise, set `addValue` to the value of `currentNode`.
  ...
  - For each `neighborNode` of `currentNode`:
- If $\text{inDegree}[neighborNode]$ is already `0`, skip it (processed nodes).
- Decrement the in-degree of `neighborNode` by `1`.
- Add `addValue` to $\text{values}[neighborNode]$ to propagate the contribution of `currentNode`.
