...
The brute-force algorithm would simple consider all possible pairs of indices for ($\text{word1}_{location}$, $\text{word2}_{location}$) and see which one produces the minimum distance. Let's try and build on this idea and see if some pre-processing can help us out reduce the complexity of the brute-force algorithm.
<br>
<br>
...
The idea is to use a two pointer approach. Let's say we have a pointer `i` for the sorted list of indices of `word1` and `j` for the sorted list of indices of `word2`. At every iteration, we record the difference of indices i.e. $abs(\text{word1}[i] - \text{word2}[j])$. Once we've done that, we have two possible choices for progressing the two pointers.

<pre>
word1[i] < word2[j]
</pre>

If this is the case, that means there is no point in moving the `j` pointer forward. The location indices for the words are in a sorted order. We know that $word2[j + 1] > \text{word2}[j]$ because these indices are sorted. So, if we move `j` forward, then the difference $abs(\text{word1}[i] - word2[j + 1])$ would be even greater than $abs(\text{word1}[i] - \text{word2}[j])$. That doesn't help us since we want to find the minimum possible distance (difference) overall.
...
If this is the case, that means there is no point in moving the `i` pointer forward. We know that $word1[i + 1] > \text{word2}[j]$ because these indices are sorted. So, if we move `i` forward, then the difference $abs(word1[i + 1] - \text{word2}[j])$ would be even greater than $abs(\text{word1}[i] - \text{word2}[j])$. That doesn't help us since we want to find the minimum possible distance (difference) overall.
...
6. For a given `l1` and `l2`, we first update (if possible) the minimum difference (distance) till now i.e. $dist = min(dist, abs(\text{loc1}[l1] - \text{loc2}[l2]))$. Then, we check if $\text{loc1}[l1] < \text{loc2}[l2]$ and if this is the case, we move `l1` one step forward i.e. $l1 = l1 + 1$. Otherwise, we move `l2` one step forward i.e. $l2 = l2 + 1$.
