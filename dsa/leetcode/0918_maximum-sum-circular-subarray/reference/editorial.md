...
Imagine an array `suffixSum` where $\text{suffixSum}[i]$ represents the suffix sum starting from index `i`, namely $\text{suffixSum}[i]$ = $\text{nums}[i]$ + $nums[i + 1]$ + ... + $nums[n - 1]$ (it's like a prefix sum, but backward). We can construct an array `rightMax` where $\text{rightMax}[i] = max(\text{suffixSum}[i], suffixSum[i + 1], ...suffixSum[n - 1])$.

Namely, $\text{rightMax}[i]$ is the largest suffix sum of `nums` that comes on or after `i`.
...
* Increase `suffixSum` by $\text{nums}[i]$
* Update $\text{rightMax}[i]$ to $max(rightMax[i + 1], suffixSum)$

* Set `maxSum` and `prefixSum` to $\text{nums}[0]$.
* Iterate over `i` from `0` to $n - 2$
* Increase `prefixSum` by $\text{nums}[i]$
