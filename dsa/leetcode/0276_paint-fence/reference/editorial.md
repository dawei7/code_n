...
We know the values for `totalWays(1)` and `totalWays(2)`, now we need a formula for `totalWays(i)`, where $3 \le i \le n$. Let's think about how many ways there are to paint the $i^{th}$ post. We have two options:
...
1. Define a hash map `memo`, where $\text{memo}[i]$ represents the number of ways you can paint `i` fence posts.

2. Define a function `totalWays` where `totalWays(i)` will determine the number of ways you can paint `i` fence posts.

3. In the function `totalWays`, first check for the base cases. `return k` if $i = 1$, and $return k * k$ if $i = 2$. Next, check if the argument `i` has already been calculated and stored in `memo`. If so, $return \text{memo}[i]$. Otherwise, use the recurrence relation to calculate $\text{memo}[i]$, and then return $\text{memo}[i]$.
...
Bottom-up dynamic programming is also known as **tabulation** and is done iteratively. Instead of using a function like in top-down, let's use an array `totalWays` instead, where $\text{totalWays}[i]$ represents the number of ways you can paint `i` fence posts.

As the name suggests, we now start at the bottom and work our way up to the top (`n`). Initialize the base cases $\text{totalWays}[1] = k, \text{totalWays}[2] = k * k$, and then iterate from `3` to `n`, using the recurrence relation to populate `totalWays`.
...
1. Define an array `totalWays` of length $n + 1$, where $\text{totalWays}[i]$ represents the number of ways you can paint `i` fence posts. Initialize $\text{totalWays}[1] = k$ and $\text{totalWays}[2] = k * k$.

2. Iterate from `3` to `n`, using the recurrence relation to populate `totalWays`: $\text{totalWays}[i] = (k - 1) * (totalWays[i - 1] + totalWays[i - 2])$.

3. At the end, return $\text{totalWays}[n]$.
...
You may have noticed that our recurrence relation from the previous two approaches only cares about 2 steps below the current step. For example, if we are trying to calculate $\text{totalWays}[11]$, we only care about $\text{totalWays}[9]$ and $\text{totalWays}[10]$. While we would have needed to calculate $\text{totalWays}[3]$ through $\text{totalWays}[8]$ as well, at the time of the actual calculation for $\text{totalWays}[11]$, we no longer care about any of the previous steps.
