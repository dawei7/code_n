...
Now, we can remove the activities with the maximum and minimum number of participants. We first identify the activities that have the same number of participants in the DataFrame $\text{max}_{min}$ using `isin()`, and then remove these activities from the list of activities using `~`. For the final output, we select only the column `activity`.
...
Now we can remove the unwanted group from the main group using the left `merge`. The parameter `indicator=True` is passed to the `merge` so we can identify if the records only exist in the left table. These records will have an indicator equal to $\text{left}_{only}$. In other words, they are not in the unwanted group.
...
To get the final output, we select the records with `_merge` equal to $\text{left}_{only}$ and the column `activity`.
