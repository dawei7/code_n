## General
**Optimal Approach — Activity Participants**

The query executes a structured relational pipeline for **Activity Participants**. It uses Common Table Expressions (CTEs) to isolate intermediate data transformations into modular steps, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
