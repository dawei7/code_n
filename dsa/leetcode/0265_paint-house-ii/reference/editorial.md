...
Unlike the pseudocode above, we don't need to worry about the names of the colors. Instead, they are represented by numbers between `0` and $k - 1$. Additionally, we're also using memoization ($\text{lru}_{cache}$ in Python, and a dictionary in Java).
...
### Approach 3: Dynamic Programming with $\mathcal{O}(k)$ additional Space.
