...
- Use $\text{freq}[rank] * sqrt(mid / rank)$ to compute repairs.
...
With this optimization in mind, we keep the core idea of binary search on time. The search space remains the same: the lower bound is `1`, representing the smallest unit of time, while the upper bound is $\text{ranks}[0] * cars^2$, representing the worst-case scenario where the slowest mechanic repairs all cars alone.
...
- Set `high` to $\text{ranks}[0] * cars * cars$, the worst-case maximum repair time.
