...
- If the cell is a farmland cell, i.e. $\text{land}[row1][col1] = 1$, and hasn't been visited yet ($\text{visited}[row1][col1] = 0$), start DFS from `(row1, col1)`. Also, keep two variables `row2` and `col2` as the coordinates of the bottom right corner initialized with `0` each.
...
- If the cell is a farmland cell, i.e $\text{land}[row1][col1] = 1$ and isn't visited yet ($\text{visited}[row1][col1] = 0$), enqueue it to the queue start BFS from `(row1, col1)`.
...
- Iterate through each cell in the grid using nested loops:
  - For every cell `(row1, col1)`, check if it is part of farmland ($\text{land}[row1][col1] = 1$).
  - If farmland is found, initialize `x` to `row1` and `y` to `col1`.

- Expand the farmland boundaries:
  - Increment `x` until you find the last row where $\text{land}[x][col1] = 1$.
  - For each row in this range, increment `y` until you find the last column where $\text{land}[x][y] = 1$.
