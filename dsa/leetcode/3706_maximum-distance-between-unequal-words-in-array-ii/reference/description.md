...
For a valid pair, the indices satisfy $0\le i<j<\lvert\texttt{words}\rvert$ and $\text{words}[i] \neq \text{words}[j]$. Distance counts both endpoints, so adjacent unequal words have distance $2$, not $1$.
