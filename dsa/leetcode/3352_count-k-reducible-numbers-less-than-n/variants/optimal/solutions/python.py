...
class Solution:
    def countKReducibleNumbers(self, s: str, k: int) -> int:
        ...
        for ones in range(2, length + 1):
            reduction_steps[ones] = (
                reduction_steps[ones.bit_count()] + 1
            )
        ...
        for position, bit in enumerate(s):
            ...
            for ones in range(position + 1):
                ...
                next_counts[ones] = (
                    next_counts[ones] + ways
                ) % MODULO
                next_counts[ones + 1] = (
                    next_counts[ones + 1] + ways
                ) % MODULO

            if bit == "1":
                next_counts[exact_ones] = (
                    next_counts[exact_ones] + 1
                ) % MODULO
                exact_ones += 1
        ...
        return sum(
            ...
            if reduction_steps[ones] < k
        ) % MODULO


def solve(s: str, k: int) -> int:
    return Solution().countKReducibleNumbers(s, k)
