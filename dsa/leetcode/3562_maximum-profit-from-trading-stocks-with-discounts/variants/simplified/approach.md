# Simplified Approach - Maximum Profit from Trading Stocks with Discounts
...
## Complexity detail
- **Time Complexity**: $O(n B^2)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(n B)$ — Utilizes standard auxiliary storage proportional to the active computation state.
