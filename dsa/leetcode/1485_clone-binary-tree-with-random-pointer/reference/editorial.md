...
![random_node](images/Slide29.PNG)
...
- If `root` is `NULL`, then the respective new tree's node will also be `NULL`.
- If `root` is already present in the hash map `seen`, it was already traversed, thus we return the respective new node $\text{seen}[root]$ we created earlier.
