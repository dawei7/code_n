...
A crucial insight is that we can play a token face-up to trade any amount $\text{tokens}[i]$ for `1` score as long as we have at least that much power. Alternatively, we can play a token face-down to trade `1` score for any amount $\text{tokens}[i]$ of power.
...
1. Initializations:
- Initialize a pointer `low` to `0` and `high` to $\text{tokens.length} - 1$. `low` points to the first index of `tokens` and `high` points the the last index of `tokens`.
- Initialize a variable `score` to `0`.
2. Sort `tokens` in ascending order.
3. While `low` is less than or equal to `high`:
- If `power` is greater than or equal to $\text{tokens}[low]$, we have enough power to play a token face-up. We increment `score` by `1`, reduce `power` by $\text{tokens}[low]$, and increase `low` by `1`.
- Else if `score` is greater than `0`, and `low` is less than `high`, we play a token face-down. We decrease `score` by `1`, increase our power by $\text{tokens}[high]$, and decrease `high` by `1`.
