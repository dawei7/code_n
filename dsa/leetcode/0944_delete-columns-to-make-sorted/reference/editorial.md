...
- If the character at index `col` in the string $\text{strs}[row]$ is smaller than the character at index `col` in the string $strs[row - 1]$, then increment the counter variable `answer`. Also, we can break the inner loop here as we find the current column unsorted.
