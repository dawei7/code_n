...
<ul>
	<li>`1 <= x_i.length <= 4`</li>
	<li>`x_i` is a **hexadecimal string** which may contain digits, lowercase English letter (`'a'` to `'f'`) and upper-case English letters (`'A'` to `'F'`).</li>
	<li>Leading zeros are allowed in `x_i`.</li>
</ul>
