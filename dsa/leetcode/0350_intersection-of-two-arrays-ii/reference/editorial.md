...
![Hash Map Illustration](images/350_approach1-v2.png)
...
4. Iterate along `nums2`:
      ...
      - If the current number is in the hash map and count is positive:
          ...
          - Copy the number into $\text{nums1}[k]$, and increment `k`.
...
![Sort Illustration](images/350_approach2-v2.png)
...
3. Move indices `i` along `nums1`, and `j` through `nums2`:
      ...
      - Increment `i` if $\text{nums1}[i]$ is smaller.

      - Increment `j` if $\text{nums2}[j]$ is smaller.

      - If numbers are the same, copy the number into $\text{nums1}[k]$, and increment `i`, `j` and `k`.
...
This is similar to [Approach 2](#approach-2-sort). Instead of iterating with two pointers, we use a built-in function to find common elements. In C++, we can use $\text{set}_{intersection}$ for sorted arrays (or multisets).
...
> Note that $\text{set}_{intersection}$ returns the position past the end of the produced range, so it can be used as an input for the `erase` function. The idea is from [this solution](https://leetcode.com/problems/intersection-of-two-arrays-ii/discuss/82269/Short-Python-C%2B%2B) by [StefanPochmann](https://leetcode.com/stefanpochmann/).
