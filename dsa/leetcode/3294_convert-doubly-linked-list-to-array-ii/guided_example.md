# Guided Example: Convert Doubly Linked List to Array II

We execute the single-pass Array, Linked List, Doubly-Linked List pointer manipulation on a representative linked list instance.

- **Input:** `{"node": {"values": [1, 2, 3, 4, 5], "node_index": 4}}`
...
The objective for **Convert Doubly Linked List to Array II** is to transform the linked structure by strictly updating `next` references in place.
