# Simplified Approach - Path In Zigzag Labelled Binary Tree
...
## Complexity detail
- **Time Complexity**: $O(\log L)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(\log L)$ — Utilizes standard auxiliary storage proportional to the active computation state.
