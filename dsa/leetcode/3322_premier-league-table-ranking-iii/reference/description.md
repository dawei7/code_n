...
Goal difference is calculated as: $\text{goals}_{for} - \text{goals}_{against}$

Return *the result table ordered by* $\text{season}_{id}$ *in **ascending** order, then by* position *in **ascending** order, and finally by* $\text{team}_{name}$ *in **ascending** order.*
