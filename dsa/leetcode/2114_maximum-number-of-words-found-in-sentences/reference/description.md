...
- $\text{sentences}[i]$ consists only of lowercase English letters and `' '` only.

- $\text{sentences}[i]$ does not have leading or trailing spaces.

- All the words in $\text{sentences}[i]$ are separated by a single space.