# Simplified Approach - Split Array With Same Average
...
## Complexity detail
- **Time Complexity**: $O(2^{n/2})$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(2^{n/2})$ — Utilizes standard auxiliary storage proportional to the active computation state.
