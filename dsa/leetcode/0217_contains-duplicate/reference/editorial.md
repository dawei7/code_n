...
For our problem, we loop through all $n$ integers. For the $i$th integer $\text{nums}[i]$, we search in the previous `i-1` integers for the duplicate of $\text{nums}[i]$. If we find one, we return true; if not, we continue. Return false at the end of the program.
