...
3. Determine the left child using $construct(nums, l, \text{max}_{i})$. Doing this recursively finds the largest element in the subarray left to the current largest element.

4. Similarly, determine the right child using $construct(nums, \text{max}_{i} + 1, r)$.
