...
4. Iterate through each cell `(i, j)` in the grid:
   - If the cell is a water cell ($\text{grid}[i][j] > 0$) and has not been visited, call `countFishes(grid, visited, i, j)` to calculate the total fish in the connected component starting from `(i, j)`.
...
1. If the current cell `(r, c)`, is out of bounds, is a land cell ($\text{grid}[r][c] = 0$), or, has already been visited ($\text{visited}[r][c] = true$), return `0`.
2. Mark the current cell `(r, c)` as visited by setting $\text{visited}[r][c] = true$.
...
4. Return the sum of fish in the current cell ($\text{grid}[r][c]$) and the fish counts from all valid neighboring cells.
...
2. Iterate through the Grid:
   - For each cell `(i, j)` in the grid:
     - If the cell contains water ($\text{grid}[i][j] > 0$) and has not been visited, call `countFishes(grid, visited, i, j)` to calculate the total fish in the connected component starting from `(i, j)`.
     - Update `result` to the maximum of `result` and the fish count returned by `countFishes`.
...
- Union Operation:
   ...
   - For each water cell ($\text{grid}[row][col] > 0$), union its connected neighbors using the `unionComponents` function.
...
- If $\text{parent}[x]$ equals `x`, then `x` is its own root. Otherwise, recursively find the parent of $\text{parent}[x]$.
