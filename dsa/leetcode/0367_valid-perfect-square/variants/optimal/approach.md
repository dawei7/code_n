...
The core task in **Valid Perfect Square** is to a positive integer num, return `true` *if* `num` *is a perfect square or* `false` *otherwise*. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
