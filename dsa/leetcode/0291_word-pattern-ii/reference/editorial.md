...
- Set `symbol` to $\text{pattern}[pIndex]$, the next character in the pattern.
...
We can visualize `s` as $"\text{aa}_{aax}"$, where `"aa"` represents the known mapping of `'a'`, `x` represents a spot for the unknown mapping of `'c'`, and the empty space represents the `word` that corresponds to symbol `'b'`.
...
Then, we can identify the last possible ending index in `s` of a new word that could correspond to a given `symbol` as $\text{s.length}() - filledSpots$, which we use to narrow the search space when we generate new words.
...
- Base case: `pIndex` equals the length of `pattern`. Return `true` if `sIndex` equals the length of `s`; `s` matches the pattern. Otherwise, return `false`.
- Set `symbol` to $\text{pattern}[pIndex]$, the next character in `pattern`.
- If `symbol` is associated with a substring of `s`, save the substring as `word`, and check if the next characters in `s` match `word`.
- If the characters don't match, return `false`.
- If they do match, call `isMatch` for the next character in `pattern`.
...
- Try mapping `symbol` to new substrings of `s`, starting with the substring consisting of $s[sIndex]$ and extending until the substring reaches the last possible end index of the new word, which is $\text{s.length}() - filledSpots$.
- Save the current substring as `newWord`.
- If `newWord` already exists in `wordSet`, continue.
