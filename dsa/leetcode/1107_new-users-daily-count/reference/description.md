...
- $Traffic(\text{user}_{id}, activity, \text{activity}_{date})$: $N$ website-activity rows for $U$ distinct users. Duplicate rows are allowed, and `activity` belongs to the exact source-defined set `login`, `logout`, `jobs`, `groups`, and `homepage`.

For each user who has at least one login, first determine the minimum $\text{activity}_{date}$ over that user's complete login history. A first-login date qualifies when its difference from `2019-06-30` is between 0 and 90 days, inclusive. Thus the closed reporting interval is `2019-04-01` through `2019-06-30`; a future date does not qualify.

**Return value**

- $\text{login}_{date}$: a qualifying first-login date.
- $\text{user}_{count}$: the number of distinct users whose first login occurred on that date.
