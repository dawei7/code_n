...
- $f(2) = 2$
- $p(2) = 1$
- To avoid repeated computations, we will use 2 hashmaps ($f_{cache}$ and $p_{cache}$) to store calculated values for $f$ and $p$. In Python, the built-in `@cache` wrapper will automatically maintain these hashmaps for us.
