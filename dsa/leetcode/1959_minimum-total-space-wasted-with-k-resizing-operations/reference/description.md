...
The size of the array at time `t`, $\text{size}_{t}$, must be at least $\text{nums}[t]$ because there needs to be enough space in the array to hold all the elements. The **space wasted** at time `t` is defined as $\text{size}_{t} - \text{nums}[t]$, and the **total** space wasted is the **sum** of the space wasted across every time `t` where $0 \le t < \text{nums.length}$.
