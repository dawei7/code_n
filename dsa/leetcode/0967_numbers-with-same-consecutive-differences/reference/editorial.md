...
![tree illustration](images/967_tree_illustration.png)
...
![DFS example](images/967_dfs_example.png)
...
- Otherwise, there are still some remaining digits to be added to the current number, _e.g._ `13`. There are two potential cases to explore, based on the last digit of the current number which we denote as $\text{tail}_{digit}$.

- Adding the difference `K` to the last digit, _i.e._ $\text{tail}_{digit} + K$.
