...
| Official Link | [LeetCode](https://leetcode.com/problems/minimum-time-to-revert-word-to-initial-state-ii/) |
...
Determine the minimum positive number of seconds after which the current word can equal its initial value. The string may contain up to one million characters, so repeated full substring comparisons are not fast enough in the worst case.
...
- `word`: A string of $N$ lowercase English letters, where $1 \le N \le 10^6$.
...
- Output: `2`
- Explanation: After two seconds, the surviving `"a"` matches the initial prefix, and the appended characters can complete the original word.
