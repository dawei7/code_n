...
A key observation in this problem is that *we need to select k cards from the beginning or end of the array*. Thus no matter how many cards we choose from the beginning, in the end, we need to select two subarrays: one from the beginning, and one from the end, and their total lengths must be `k` (the only exception is when $k = \text{cardPoints.length}$, in that case, we'll select all cards). Thus after selecting the two arrays we will be left with a single subarray of length $\text{cardPoints.length} - k$. There are three ways we can select the cards:
...
![fig](images/1423_Maximum_Points_You_Can_Obtain_from_Cards_Overview_Image.png)
...
As we determined above, the `k` cards that we choose will form two contiguous subarrays: one at the start, and one at the end of the input array. If we choose `i` cards from the start (where $i \le k$) then we must choose $k - i$ cards from the end. There are `k` _different_ lengths the first array could be.
...
![Slide 1](images/slideshow_1423_Maximum_Points_You_Can_Obtain_from_Cards_1423_Maximum_Points_You_Can_Obtain_from_Cards_1.png)

![Slide 2](images/slideshow_1423_Maximum_Points_You_Can_Obtain_from_Cards_1423_Maximum_Points_You_Can_Obtain_from_Cards_2.png)

![Slide 3](images/slideshow_1423_Maximum_Points_You_Can_Obtain_from_Cards_1423_Maximum_Points_You_Can_Obtain_from_Cards_3.png)

![Slide 4](images/slideshow_1423_Maximum_Points_You_Can_Obtain_from_Cards_1423_Maximum_Points_You_Can_Obtain_from_Cards_4.png)

![Slide 5](images/slideshow_1423_Maximum_Points_You_Can_Obtain_from_Cards_1423_Maximum_Points_You_Can_Obtain_from_Cards_5.png)

![Slide 6](images/slideshow_1423_Maximum_Points_You_Can_Obtain_from_Cards_1423_Maximum_Points_You_Can_Obtain_from_Cards_6.png)
...
2. We calculate the prefix sum (sum of $0 \le i \le k$ cards) for the first `k` cards $frontSetOfCards[i + 1] = \text{frontSetOfCards}[i] + \text{cardPoints}[i]$ and the last `k` cards $rearSetOfCards[i + 1] = cardPoints[n - i - 1] + \text{rearSetOfCards}[i]$.
...
**Intuition**
 ...
 In this problem, we must draw exactly `k` cards from the array in such a way that the score (sum of the cards) is maximized. After drawing `k` cards from the array $\text{cardPoints.length} - k$ cards will remain in the array.
...
3. Initialize `requiredSubarrayLength` to $\text{cardPoints.length} - k$.
