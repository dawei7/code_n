...
2. Iterate over each `wordNum` from `0` to $\text{words.size}() - 1$, representing the number of each word:
- Then we iterate over each character `charPos` from `0` to $\text{words}[wordNum].size() - 1$:
...
- if `wordNum` is greater than or equal to $\text{words}[charPos].size()$, that is, the word represented by the `wordNum`-th column is larger than the respective row's word, or
