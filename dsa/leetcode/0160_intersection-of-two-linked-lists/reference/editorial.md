...
* Time complexity : $O(N + M)$.
    ...
    Finally, combining the two parts, we get $O(M) +$\mathcal{O}(N)$= O(M + N)$.
...
![Two linked lists with question marks on their nodes. The first is 5 nodes long, and the second is 8 nodes long.](images/image1.png)
...
![The two linked lists from above with arrows showing how the last 5 nodes of each list could be a match.](images/image2.png)
...
![The two linked lists from above with a p1 pointer at the head of the first, and a p2 pointer at the 4th node of the second.](images/image3.png)
...
![Diagram showing that one pointer could go over a + c + b while the other goes over b + c + a, and then both will end up on the intersection node.](images/image4.png)
