...
>The main idea of the algorithm is that for each node, we only take care the encoding of the node itself, and we invoke the function itself to encode each of its child node, _i.e._ $encode(node.\text{children}[i])$.
