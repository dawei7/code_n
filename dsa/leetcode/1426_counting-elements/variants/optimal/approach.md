...
The core task in **Counting Elements** is to an integer array `arr`, count how many elements `x` there are, such that $x + 1$ is also in `arr`. If there are duplicates in `arr`, count them separately. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
