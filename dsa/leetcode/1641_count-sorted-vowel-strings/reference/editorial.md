...
We maintain a 2D array , $\text{dp}$ of size $n \cdot 5$ where, $\text{\text{dp}[n][vowels]}$ denotes the total number of combinations for given $n$ and number of $\text{vowels}$. Using the recurrence relation established in _Approach 2_, we could iteratively calculate the value for $\text{\text{dp}[n][vowels]}$ as,
...
-  If $n = 1$, the number of combinations are always equal to number of $\text{vowels}$. Hence, we initialize all the values of $\text{\text{dp}[1][vowels]}$ with $\text{vowels}$.
-  If $vowels = 1$, the number of combinations are always equal to 1. Hence, we initialize all the values of $\text{\text{dp}[n][1]}$ with $1$.
