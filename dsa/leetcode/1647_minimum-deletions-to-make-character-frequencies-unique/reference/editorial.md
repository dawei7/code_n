...
- If $\text{frequency}[i] > maxFreqAllowed$, add the difference of these two in the variable `deleteCount`. Change frequency of the current character $\text{frequency}[i]$ to `maxFreqAllowed`.
- Update the `maxFreqAllowed` to $\text{frequency}[i] - 1$ (or `0` if the value is negative).
4. Return `deleteCount`
