...
1. In order to record the results of computation for every cell,  maintain a 2-dimensional matrix named `memo` where the value of $\text{memo}[row][col]$ would give the minimum falling path starting from the cell `(row, col)`.
...
3. To avoid repetitive computation of the results as in the brute force approach, we make use of stored results as follows,
   ...
   - After computing the result, store the result in the $\text{memo}[row][col]$.
...
* Space Complexity: $O(N^2)$
  ...
  The recursive call stack uses $O(N)$  space. As the maximum depth of the tree is $N$, we can’t have more than $N$ recursive calls on the call stack at any time. The 2D matrix `memo` uses $O(N^2)$ space. Thus, the space complexity is $O(N) +$\mathcal{O}(N^2)$= O(N^2)$.
...
* Time Complexity: $O(N^{2})$
  ...
  - Then, takes $N$ time to find the minimum falling path.
   - So, Time Complexity $T(n) =$\mathcal{O}(N^{2})$+$\mathcal{O}(N)$= O(N^{2})$
