...
We execute the single-pass Array, Hash Table, Linked List, Design, Hash Function pointer manipulation on a representative linked list instance.
...
The objective for **Design HashSet** is to transform the linked structure by strictly updating `next` references in place.
