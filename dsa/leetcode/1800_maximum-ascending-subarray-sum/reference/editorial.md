...
- Use an outer loop to iterate over each element in the array ($\text{nums}[startIdx]$):
  - Set `currentSubarraySum` to $\text{nums}[startIdx]$ to start a new ascending subarray from this element.

- Use an inner loop to check the next elements forming an ascending subarray:
  - Continue adding to `currentSubarraySum` while the next element ($\text{nums}[endIdx]$) is greater than the previous element ($nums[endIdx - 1]$).
