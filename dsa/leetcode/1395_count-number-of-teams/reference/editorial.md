...
- Loop over all indices from $currentIndex + 1$ to the end of the array. For each index `nextIndex`:
  - If $\text{rating}[nextIndex]$ is greater than $\text{rating}[currentIndex]$, call `countIncreasingTeams` with `nextIndex` and `teamSize` incremented by `1`.
...
- This method is exactly the same as `countIncreasingTeams` except for majorly one thing:
  - We check whether $\text{rating}[nextIndex]$ is less than $\text{rating}[currentIndex]$ to call `countDecreasingTeams`.
...
We then use three nested loops to consider all combinations of team lengths 2 and 3. For each position `(i, j)`, if we can append $\text{rating}[j]$ to the sequence ending with $\text{rating}[i]$, we update the count of teams at `j` by adding the count of teams of the previous length at `i`.
...
- Fill base case. For all `i` from `0` to `n`:
  - Set $\text{increasingTeams}[i][1]$ and $\text{decreasingTeams}[i][1]$ to `1` (as each soldier forms a sequence of length 1).
- Use 3 nested loops to fill the tables. The outer loop iterates over sequence lengths 2 and 3. The middle loop sets the middle soldier. The inner loop iterates over all soldiers as potential end points. For each pair of soldiers `i` and `j`:
  - If $\text{rating}[j] > \text{rating}[i]$, add $\text{increasingTeams}[i][count-1]$ to $\text{increasingTeams}[j][count]$.
  - If $\text{rating}[j] < \text{rating}[i]$, add $\text{decreasingTeams}[i][count-1]$ to $\text{decreasingTeams}[j][count]$.
...
Our improved solution utilizes two BITs: one to manage the left side and another for the right side of the current soldier. Each `BIT` stores frequency counts of ratings within a specific range. For instance, $\text{BIT}[5]$ keeps track of the number of soldiers with a rating of `5`, while $\text{BIT}[6]$ aggregates counts for ratings of `5` and `6`.
...
* Time complexity: $O(n \cdot \log(\text{maxRating}))$
    ...
    Thus, the overall time complexity of the algorithm comes out to be $O(n) +$\mathcal{O}(2 \cdot n \cdot \\log(\text{maxRating})$)$, which simplifies to $O(n \cdot \log(\text{maxRating}))$.
