...
In other words, given indices `i` and `j`, we need to find the first value in the `heights` array that is greater than the values at $\text{heights}[i]$ and $\text{heights}[j]$. If no such value exists, return -1.
...
1. Create a list `newQueries` where each index stores the list of queries that require this index as the maximum index of the query pair. Each query is stored as a pair containing the required height ($\text{heights}[a]$) and the query index.
...
4. Iterate over the `queries`:
   ...
   - If `a > b`, swap the indices to ensure $a \le b$.
   - If $\text{heights}[b] > \text{heights}[a]$ or $a = b$, set $\text{result}[currQuery] = b$.
   - Otherwise, add the query to $\text{newQueries}[b]$ with its required height ($\text{heights}[a]$) and the query index.
...
2. Perform a binary search:
   ...
   - If the height at $\text{monoStack}[mid]$ is greater than the required height:
...
- Loop through each query:
  - For each query `(a, b)`:
- If the height of building `a` is less than building `b` and `a` is smaller than `b`, set $\text{result}[currQuery]$ to `b` (building `b` is the answer).
- If the height of building `a` is greater than building `b` and `a` is greater than `b`, set $\text{result}[currQuery]$ to `a` (building `a` is the answer).
- If `a` is equal to `b`, set $\text{result}[currQuery]$ to `a` (both are the same building).
...
- Set the corresponding query's result in `result` and pop the element from `maxIndex` (this query is answered).
  - Push new queries from $\text{storeQueries}[index]$ into `maxIndex`, sorting them by height.
