...
The core task in **Maximize the Profit as the Salesman** is to an integer `n` representing the number of houses on a number line, numbered from `0` to $n - 1$. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
