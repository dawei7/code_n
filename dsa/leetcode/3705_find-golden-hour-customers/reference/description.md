...
- $\text{restaurant}_{orders}$: One row per order, with the six columns defined in the Description. $\text{order}_{id}$ is unique; $\text{order}_{rating}$ may be `NULL`.

Peak-hour membership depends only on the time component of $\text{order}_{timestamp}$. Both endpoints of each peak interval are included. An unrated order contributes to $\text{total}_{orders}$ and the peak-hour ratio but not to $AVG(\text{order}_{rating})$ or $COUNT(\text{order}_{rating})$.
...
- $\text{average}_{rating}$, rounded to two decimal places over rated orders only

Apply the four eligibility thresholds to their exact aggregate values, then order the retained rows by $\text{average}_{rating} DESC, \text{customer}_{id} DESC$.