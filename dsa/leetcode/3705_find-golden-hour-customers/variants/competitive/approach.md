## General
**Competitive Approach — Find Golden Hour Customers**

The query executes a structured relational pipeline for **Find Golden Hour Customers**. It uses `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(R log R + C log C)$ — Operation count proportional to input scale.
