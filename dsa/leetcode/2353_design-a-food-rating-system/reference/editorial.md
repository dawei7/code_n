...
- Store $(\text{foods}[i], \text{ratings}[i])$ and $(\text{foods}[i], \text{cuisines}[i])$ key-value pairs in `foodRatingMap` and `foodCuisineMap` respectively.
- Insert $Food(\text{ratings}[i], \text{foods}[i])$ element in the priority queue of $\text{cuisines}[i]$ key of `cuisineFoodMap`.
...
- If the rating of the top element and the rating of the corresponding food in `foodRatingMap` are not the same, i.e. $\text{highestRated.foodRating} \neq foodRatingMap[\text{highestRated.foodName}]$, then we discard and remove the current top element and fetch the next top element from the priority queue. Repeat this step until ratings are the same.
...
- Store $(\text{foods}[i], \text{ratings}[i])$ and $(\text{foods}[i], \text{cuisines}[i])$ key-value pairs in `foodRatingMap` and `foodCuisineMap` respectively.
- Insert $(-1 * \text{ratings}[i], \text{foods}[i])$ pair element in the sorted set of $\text{cuisines}[i]$ key of `cuisineFoodMap`.
