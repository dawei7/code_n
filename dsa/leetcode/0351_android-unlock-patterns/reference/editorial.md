...
- Explore all `SKIP_DOT_MOVES`:
  - For each `move`, check if it's valid using `isValidMove`.
  - Find `middleRow` as $currentRow + \text{move}[0] / 2$ and `middleCol` as $currentCol + \text{move}[1] / 2$.
...
A major inconvenience in the previous approach was hardcoding each possible move from a dot. Instead of physically creating a grid and traversing it, notice that we can reach all other dots from each dot, albeit some of those moves are skip moves. So, let's eliminate the arrays of moves, and instead maintain a matrix called `jump`. This matrix will track which moves require a jump and over which dot. For instance, moving from dot `2` to dot `8` necessitates jumping over dot `5`, so we'll set $\text{jump}[2][8]$ to `5`.
...
- Populate `jump` array with valid jump-over numbers. Each $\text{jump}[i][j]$ contains the number needed to be jumped over to reach `j` from `i`.
...
- Mark $\text{visitedNumbers}[currentNumber]$ as `true`.
...
- Initialize a 2D array `jump` to store numbers that need to be jumped over for valid moves.
- Populate `jump` array such that $\text{jump}[i][j]$ stores the number needed to be jumped over to get to `j` from `i`.
...
- For each `nextNumber` from `1` to `9`:
  - Set `jumpOverNumber` as $\text{jump}[currentNumber][nextNumber]$.
