...
The core task in **Minimum Total Operations** is to an array of integers `nums`, you can perform *any* number of operations on this array. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
