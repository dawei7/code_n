...
2. Count the Number of Unique Friends for Each User
   ...
   - We use `groupby` on $\text{primary}_{user}$ to gather all records pertaining to each user.
   - The `nunique()` function counts the number of unique $\text{friend}_{user}$ values for each $\text{primary}_{user}$. This gives us the total number of different friends each user has.

$\text{friend}_{count}$ (note that this is a `Series` and not a `DataFrame`):
...
3. Calculate the Total Number of Users
   ...
   - We extract all user IDs from $\text{primary}_{user}$ column and then use the `.nunique()` method to find all unique user IDs.
...
4. Calculate the Popularity Percentage
 ...
 $\text{popularity}_{percentage}$ (note that this is a `Series` and not a `DataFrame`):
...
5. Create the Result DataFrame
   ...
   - We create a new DataFrame where `user1` corresponds to each user (the index from our $\text{friend}_{count}$ series) and $\text{percentage}_{popularity}$ is the calculated popularity percentage.
   - We round the popularity percentage to two decimal places as requested in the problem statement.
   - The $\text{reset}_{index}(drop=True)$ ensures that the DataFrame has a clean, sequential index.
