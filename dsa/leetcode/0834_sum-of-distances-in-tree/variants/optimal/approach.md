...
The core task in **Sum of Distances in Tree** is to There is an undirected connected tree with `n` nodes labeled from `0` to $n - 1$ and $n - 1$ edges. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
