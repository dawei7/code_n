...
We trace the logarithmic Array, Hash Table, Binary Search, Greedy, Heap (Priority Queue) search on a representative problem instance.
...
The objective for **Avoid Flood in The City** is to pinpoint the target value or optimal threshold in logarithmic $O(\log N)$ time.
