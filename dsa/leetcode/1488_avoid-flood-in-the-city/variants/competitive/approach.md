...
The core task in **Avoid Flood in The City** is to Your country has $10^{9}$ lakes. Initially, all the lakes are empty, but when it rains over the $$n^{\text{th}}$$ lake, the $$n^{\text{th}}$$ lake becomes full of water. If it rains over a lake that is **full of water**, there will be a **flood**. Your goal is to avoid floods .... To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
