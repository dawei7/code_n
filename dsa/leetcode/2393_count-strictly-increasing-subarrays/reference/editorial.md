...
If we have $\text{nums}[x] > nums[x - 1]$, then we could take all `k` subarrays and simply append $\text{nums}[x]$ to it. We also consider $\text{nums}[x]$ as its own subarray, thus we have found $k + 1$ new increasing subarrays ending at index `x`.

Otherwise, if $\text{nums}[x] \le nums[x - 1]$, adding this new element will invalidate any previous subarrays. Thus we only gain one new increasing subarray, the one consisting of only $\text{nums}[x]$.
...
1. Check if $\text{nums}[i] > nums[i - 1]$, if true then increment `currSubarray`
...
1. Keep iterating over the elements while $\text{nums}[i] < nums[i + 1]$, i.e. elements are in strictly increasing order.
