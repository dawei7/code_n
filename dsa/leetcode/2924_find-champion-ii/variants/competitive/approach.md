...
The core task in **Find Champion II** is to There are `n` teams numbered from `0` to $n - 1$ in a tournament; each team is also a node in a **DAG**. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
