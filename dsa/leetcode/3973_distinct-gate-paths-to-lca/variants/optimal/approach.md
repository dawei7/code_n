...
The core task in **Distinct Gate Paths to LCA** is to an undirected tree rooted at node 0 with `n` nodes numbered from 0 to $n - 1$, represented by an array `parent` where $\text{parent}[i]$ is the parent of node `i`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
