...
The core task in **Reverse Subarray To Maximize Array Value** is to an integer array `nums`. The *value* of this array is defined as the sum of $|\text{nums}[i] - nums[i + 1]|$ for all $0 \le i < \text{nums.length} - 1$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
