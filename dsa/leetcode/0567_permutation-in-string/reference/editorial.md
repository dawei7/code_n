...
In order to generate all the possible pairings, we make use of a function $permute(\text{string}_{1}, \text{string}_{2}, \text{current}_{index})$. This function creates all the possible permutations of the short string $s1$.
...
* Space complexity: $O(l_2 - l_1)$
  ...
  Therefore, the total space complexity is: $O(26 \cdot (l_2 - l_1 + 1) + 26) =$\mathcal{O}(26 \cdot (l_2 - l_1 + 1)$)$. In simplified terms: $O(l_2 - l_1)$
...
* Time complexity: $O(l_1 + 26 \cdot (l_2 - l_1)) =$\mathcal{O}(l_1 + (l_2 - l_1)$) = O(l_2)$
  ...
  The outer loop runs $l_2 - l_1$ times, sliding the window of size $l_1$ across $s_2$. For each iteration, two operations are performed:
     - Increment the count of the new character added to the window ($s2arr[\text{s2.charAt}(i + l_{1}) - 'a']++$).
     - Decrement the count of the character leaving the window ($s2arr[\text{s2.charAt}(i) - 'a']--$).
...
* Space complexity: $O(26 + 26) = O(1)$
  ...
  Two arrays, `s1arr` and `s2arr`, are used to store character frequencies. Each array has a fixed size of 26, regardless of the lengths of $s_1$ and $s_2$. Therefore, the space used for these arrays is $O(26 + 26) =$\mathcal{O}(52)$= O(1)$.
