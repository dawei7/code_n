...
We trace the dynamic Hash Table, Two Pointers, String, Sliding Window sliding window on a representative input instance.
...
The objective for **Permutation in String** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
