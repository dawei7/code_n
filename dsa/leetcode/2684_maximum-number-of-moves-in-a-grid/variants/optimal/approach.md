...
The core task in **Maximum Number of Moves in a Grid** is to a **0-indexed** `m x n` matrix `grid` consisting of **positive** integers. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
