...
Round $\text{total}_{amount}$, `avg_transaction_amount`, and $\text{loyalty}_{score}$ to `2` decimal places.

Return *the result table ordered by* $\text{loyalty}_{score}$ *in **descending** order*, *then by *$\text{customer}_{id}$* in **ascending** order*.
