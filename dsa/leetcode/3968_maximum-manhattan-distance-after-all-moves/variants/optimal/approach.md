...
The core task in **Maximum Manhattan Distance After All Moves** is to a string `moves` consisting of the characters `'U'`, `'D'`, `'L'`, `'R'`, and `'_'`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
