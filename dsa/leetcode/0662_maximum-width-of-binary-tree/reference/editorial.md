...
- First of all, we create a `queue` data structure, which would be used to hold elements of a tuple as $(node, \text{col}_{index})$, where the `node` is the tree node and the $\text{col}_{index}$ is the corresponding index that is assigned to the node based on our indexing schema.
Also, we define a global variable called $\text{max}_{width}$ which holds the maximal width that we've seen so far.
