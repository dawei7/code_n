...
- We use a loop to go through all four directions. We create two variables $r = \text{curr}[0]$ and $c = \text{curr}[1]$ to move along a specific direction. We use a `while` loop and keep updating $r += \text{dirX}[i]$ and $c = \text{dirY}[i]$ as long as we don't go out of bounds or hit a wall. Once the `while` loop breaks, it means `r` and `c` form a cell that has a wall. Reverting the last move, i.e., $r - \text{dirX}[i]$ and $r - \text{dirY}[i]$ would give the cell to which the ball rolls. We recursively call `dfs` from cell $(r - \text{dirX}[i], c - \text{dirY}[i])$. If any DFS call returns `true`, we return `true` from the main call as we got a path to reach the `destination` cell.
...
- We use a loop to go through all four directions. We create two variables $r = \text{curr}[0]$ and $c = \text{curr}[1]$ to move along a specific direction. We use a `while` loop and keep updating $r += \text{dirX}[i]$ and $c = \text{dirY}[i]$ as long as we don't go out of bounds or hit a wall. Once the `while` loop breaks, it means `r` and `c` form a cell that has a wall. Reverting the last move, i.e., $r - \text{dirX}[i]$ and $r - \text{dirY}[i]$ would give the cell to which the ball rolls. Push the cell with coordinates $(r - \text{dirX}[i], c - \text{dirY}[i])$ into the queue if it's not already visited.
