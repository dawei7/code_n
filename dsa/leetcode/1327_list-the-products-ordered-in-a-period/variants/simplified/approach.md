# Simplified Approach - List the Products Ordered in a Period
...
## Complexity detail
- **Time Complexity**: $O(p+o+k\log k)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(p+k)$ — Utilizes standard auxiliary storage proportional to the active computation state.
