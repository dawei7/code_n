...
The core task in **Shortest Matching Substring** is to a string `s` and a pattern string `p`, where `p` contains **exactly two** `'*'` characters. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
