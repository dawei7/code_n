# Simplified Approach - Average Salary: Departments VS Company
...
## Complexity detail
- **Time Complexity**: $O((S + E) \log(S + E))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(S + E)$ — Utilizes standard auxiliary storage proportional to the active computation state.
