## General
**Competitive Approach — Average Salary: Departments VS Company**

The query executes a structured relational pipeline for **Average Salary: Departments VS Company**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
