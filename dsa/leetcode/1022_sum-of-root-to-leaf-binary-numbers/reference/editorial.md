...
If you work with decimal representation, the conversion of `1->2` into `12` is easy. You start from $\text{curr}_{number} = 1$, then shift one register to the left and add the next digit: $\text{curr}_{number} = 1 * 10 + 2 = 12$.

If you work with binaries `1 -> 1` -> `3`, it's the same. You start from $\text{curr}_{number} = 1$, then shift one register to the left and add the next digit: $\text{curr}_{number} = (1 << 1) | 1 = 3$.
