...
- Check if the i-th bit of `num` is set using the expression $(num \& (1 << i)) \neq 0$.
- If the bit is set, increment $\text{bitCount}[i]$ to track how many numbers have the i-th bit set.

- After counting set bits for all positions, find the maximum value in `bitCount` using $\text{max}_{element}$.
...
![Direct Maximum Bit Count](images/2275_approach_2.png)
