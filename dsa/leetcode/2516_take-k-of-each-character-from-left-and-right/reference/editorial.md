...
- `solve` function:
  - Base case:
- If $\text{count}[0]$, $\text{count}[1]$, and $\text{count}[2]$ (representing counts of 'a', 'b', and 'c') are each greater than or equal to `k`, update `minMinutes` with the current `minutes` and return.
...
![Example_Image](images/2516_slidning_window.png)
