...
- Calculating the final average pass ratio: This involves iterating through the heap, which takes $O(n \log n)$ time in the worst case.
    ...
    Overall, the dominant factor is the initial heap construction and the distribution of extra students, leading to a time complexity of $O(k \log n + n \log n) =$\mathcal{O}(k \cdot \\log(n)$+ n)$.
