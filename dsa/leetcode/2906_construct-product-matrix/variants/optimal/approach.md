...
The core task in **Construct Product Matrix** is to a **0-indexed** 2D integer matrix `grid` of size $n * m$, we define a **0-indexed** 2D matrix `p` of size $n * m$ as the **product** matrix of `grid` if the following condition is met:. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
