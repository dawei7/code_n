...
We trace the logarithmic Array, Hash Table, Binary Search, Binary Indexed Tree, Sorting search on a representative problem instance.
...
The objective for **Count Number of Rectangles Containing Each Point** is to pinpoint the target value or optimal threshold in logarithmic $O(\log N)$ time.
