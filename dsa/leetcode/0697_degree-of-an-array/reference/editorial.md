...
For each element in the given array, let's know `left`, the index of its first occurrence; and `right`, the index of its last occurrence. For example, with `nums = [1,2,3,2,5]` we have $\text{left}[2] = 1$ and $\text{right}[2] = 3$.

Then, for each element `x` that occurs the maximum number of times, $\text{right}[x] - \text{left}[x] + 1$ will be our candidate answer, and we'll take the minimum of those candidates.
