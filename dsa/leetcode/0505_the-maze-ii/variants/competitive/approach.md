...
## Complexity Detail
- **Time Complexity**: $O(rows \cdot cols \log(rows \cdot cols))$ — Micro-optimized algorithmic execution time.
- **Space Complexity**: $O(rows \cdot cols)$ — Low auxiliary space allocation.
