...
## Complexity detail
- **Time Complexity**: $O(rows \cdot cols \log(rows \cdot cols))$ — High-efficiency runtime performance.
- **Space Complexity**: $O(rows \cdot cols)$ — Minimal auxiliary memory overhead.
