# Simplified Approach - The Maze II
...
## Complexity detail
- **Time Complexity**: $O(rows \cdot cols \log(rows \cdot cols))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(rows \cdot cols)$ — Utilizes standard auxiliary storage proportional to the active computation state.
