# Simplified Approach - Shortest Path with Alternating Colors
...
## Complexity detail
- **Time Complexity**: $O(n + r + b)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(n + r + b)$ — Utilizes standard auxiliary storage proportional to the active computation state.
