...
1. Insert the index `i` into the list $szToGroup[\text{groupSizes}[i]]$.
2. If the size of the list becomes equal to $\text{groupSizes}[i]$, store it in the answer `ans`. Also, clear the array for the key $\text{groupSizes}[i]$ in the map `szToGroup`.
4. Return `ans`.
