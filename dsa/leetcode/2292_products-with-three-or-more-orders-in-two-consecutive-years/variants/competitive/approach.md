## General
**Competitive Approach — Products With Three or More Orders in Two Consecutive Years**

The query executes a structured relational pipeline for **Products With Three or More Orders in Two Consecutive Years**. It uses Common Table Expressions (CTEs) to isolate intermediate data transformations into modular steps, PostgreSQL window functions for analytical ranking and offset calculations, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
