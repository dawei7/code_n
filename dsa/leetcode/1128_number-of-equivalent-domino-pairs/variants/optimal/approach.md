...
The core task in **Number of Equivalent Domino Pairs** is to a list of `dominoes`, $\text{dominoes}[i] = [a, b]$ is **equivalent to** $\text{dominoes}[j] = [c, d]$ if and only if either ($a = c$ and $b = d$), or ($a = d$ and $b = c$) - that is, one domino can be rotated to be equal to another domino. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
