...
The core task in **Jump Game II** is to a **0-indexed** array of integers `nums` of length `n`. You are initially positioned at index 0. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
