...
The core task in **Minimum Time to Complete All Deliveries** is to two integer arrays of size 2: $d = [d_{1}, d_{2}]$ and $r = [r_{1}, r_{2}]$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
