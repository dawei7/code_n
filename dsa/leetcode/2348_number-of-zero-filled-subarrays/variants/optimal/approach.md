...
The core task in **Number of Zero-Filled Subarrays** is to an integer array `nums`, return *the number of **subarrays** filled with *`0`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
