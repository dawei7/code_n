...
- if we have only `2` seats, then we can divide the corridor, but in only one way, because there is only one way to divide the corridor into two sections, each containing **exactly** two `S`.
    ...
    ![twoS](images/Slide1_1.PNG){:height="75px"}
...
- Let's examine one more case before generalizing the above observations. If we have `4` seats, then we can divide the corridor. Let's focus on the illustration to understand the division.
    ...
    ![fourS](images/Slide1_2.PNG){:height="75px"}
...
- If there is `1` `P` between the second and third `S`, then we can have two ways of installing the divider. Note that out of these two blue bars, we will select only one in one division. Thus, there are two **different** ways in this example, with one divider!
        ...
        ![fourSoneP](images/Slide3_2.PNG){:height="75px"}
...
- If there are `2` `P` between the second and third `S`, then we can have three ways of installing the divider. Again, more precisely, there are three **different** ways in this example, with one divider!
        ...
        ![fourStwoP](images/Slide3_3.PNG){:height="75px"}
...
> ![fourS_2P1](images/Slide4_1.PNG){:height="75px"}
>
...
> ![fourS_2P2](images/Slide4_2.PNG){:height="75px"}
>
...
> ![fourS_2P3](images/Slide4_3.PNG){:height="75px"}
>
...
- now if we are on a valid index, and the number of seats in the current sections is **exactly** `2`, then we either can close the section, or we can keep growing the section. It depends on whether $\text{corridor}[index]$ is `S` or `P`.

- if $\text{corridor}[index]$ is `S`, then we have to close the section and start a new section from this index. Thus, we need to call `count` for the next index, with $seats = 1$. Hence, return $count(index + 1, 1)$

- if $\text{corridor}[index]$ is `P`, then we have two options, and both of these options will generate **different** ways to divide the `corridor`.
...
- if $\text{corridor}[index]$ is `S`, the number of seats in the section will be incremented. Hence, return $count(index + 1, seats + 1)$

- if $\text{corridor}[index]$ is `P`, then return $count(index + 1, seats)$
...
> ![recursionTree](images/Slide5.PNG)
>
> The same color-coded rectangles denote the same sub-problems. In general, we call two sub-problems when $\text{corridors}[index] = P$ and $seats = 2$
>
...
- If $\text{cache}[index][seats]$ is not equal to `-1`, then return $\text{cache}[index][seats]$. This implies that we have already computed the result of this sub-problem, and we can return the cached result.

- If the current section has $seats = 2$

- If $\text{corridor}[index]$ is `S`, then we have to close the section and start a new section from this index. Thus, return $count(index + 1, 1)$
- If $\text{corridor}[index]$ is `P`, then we have the option to close or to keep growing the section. Thus, return $(count(index + 1, 0) + count(index + 1, 2)) \% MOD$

- If the current section has `seats < 2`, then we have to keep growing the section.

- If $\text{corridor}[index]$ is `S`, then return $count(index + 1, seats + 1)$
- If $\text{corridor}[index]$ is `P`, then return $count(index + 1, seats)$
...
2. In the `cache`, we will store the result, which is less than `1000000007`. Now, when we are adding two such results *(in the case when $\text{corridors}[index] = P$ and $seats = 2$)*, then we need to take modulo `1000000007` again because the sum of two numbers less than `1000000007` can be greater than `1000000007`.

    It's worth noting that the sum of two numbers less than `1000000007` can be at most `2000000012` which is less than $\text{INT}_{MAX}$. Hence, we need not to worry about overflow when only two numbers are added.
...
- If $index = \text{corridor.length}$, then as per three recurrences,
  - $\text{count}[index][0] = 0$
...
- if $\text{corridor}[index]$ is `S`, then as per three recurrences,
      - $\text{count}[index][0] = count[index + 1][1]$

      - $\text{count}[index][1] = count[index + 1][2]$
      - $\text{count}[index][2] = count[index + 1][1]$

- if $\text{corridor}[index]$ is `P`, then as per three recurrences,
      - $\text{count}[index][0] = count[index + 1][0]$

      - $\text{count}[index][1] = count[index + 1][1]$
      - $\text{count}[index][2] = count[index + 1][0] + count[index + 1][2]$

        As discussed in [implementation note of Approach-1](#implementation), we need to take modulo `1000000007` while saving $\text{count}[index][2]$ in the actual implementation.

This completes the filling of the array. The result will be stored in $\text{count}[0][0]$, number of ways to divide the `corridor` starting from index `0` to the last index, with `0` number of `S` in the current section.

Moreover, we also get hints about the dimension of the `count` array. `index` can take values from `0` to `N` (where $N = \text{corridor.length}$), and `seats` can take values from `0` to `2`. Thus, the dimensions of the array will be $(N + 1) \times 3$.
...
4. Fill the array in a bottom-up fashion, for `index` from $n - 1$ to `0`
     - if $\text{corridor}[index]$ is `S`, then
       - $\text{count}[index][0] = count[index + 1][1]$
       - $\text{count}[index][1] = count[index + 1][2]$
       - $\text{count}[index][2] = count[index + 1][1]$
     - if $\text{corridor}[index]$ is `P`, then
       - $\text{count}[index][0] = count[index + 1][0]$
       - $\text{count}[index][1] = count[index + 1][1]$
       - $\text{count}[index][2] = (count[index + 1][0] + count[index + 1][2]) \% MOD$

5. Return $\text{count}[0][0]$
...
- $\text{new}_{two} = one$
    ...
    Then we can update the values of `zero`, `one`, and `two` as $zero = \text{new}_{zero}$, $one = \text{new}_{one}$, and $two = \text{new}_{two}$.
...
> ![puzzle_abc](images/Slide6_1.PNG){:height="75px"}
>
...
> ![puzzle_abcd](images/Slide6_2.PNG){:height="75px"}
>
...
![fpm_1](images/Slide7_1.PNG){:height="75px"}

As discussed in [overview](#overview), the pair of a seat `S` is fixed.

![fpm_2](images/Slide7_2.PNG){:height="75px"}
...
![fpm_3](images/Slide7_3.PNG){:height="75px"}
...
![fpm_4](images/Slide8_and_9.PNG){:height="400px"}
...
4. Initialize a variable `count` to `1`. This will store the final answer.
    ...
    >
    > Now, their product can be as large as $10^{14}$, which is greater than $\text{INT}_{MAX}$ ($2^{31} - 1$) in many programming languages.
