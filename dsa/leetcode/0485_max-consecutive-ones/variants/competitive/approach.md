...
The core task in **Max Consecutive Ones** is to a binary array `nums`, return *the maximum number of consecutive *`1`*'s in the array*. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
