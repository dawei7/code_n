...
- if `i` is smaller than all elements in `indexList`, then $i - \text{indexList}[0]$ is the shortest distance;
- else if `i` is larger than all elements in the color list, then $indexList[\text{indexList.size}() - 1] - i$ is the shortest distance;
...
An important fact is that, if $\text{color}[i]$ and $\text{color}[j]$ are `c` when `i<j` and there's no `c` between `i` and `j`, then for each index `k` between `i` and `j`:
