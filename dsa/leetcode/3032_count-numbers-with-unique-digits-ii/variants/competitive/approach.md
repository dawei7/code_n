...
The core task in **Count Numbers With Unique Digits II** is to two **positive** integers `a` and `b`, return *the count of numbers having **unique** digits in the range* `[a, b]` *(**inclusive**).*. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
