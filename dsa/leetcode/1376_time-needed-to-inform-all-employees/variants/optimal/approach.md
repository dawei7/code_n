...
The core task in **Time Needed to Inform All Employees** is to A company has `n` employees with a unique ID for each employee from `0` to $n - 1$. The head of the company is the one with `headID`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
