...
Each employee has one direct manager given in the `manager` array where $\text{manager}[i]$ is the direct manager of the `i-th` employee, $\text{manager}[headID] = -1$. Also, it is guaranteed that the subordination relationships have a tree structure.
