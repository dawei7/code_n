...
The image below is showing a tree structure with thirteen employees, each having an informing time as shown in the array. The numbers above each node are when they receive the information, hence the `headID` has the information at `0`, and its subordinates get it at $\text{informTime}[headID]$ and so on.

![fig](images/1376A.png)
</br>
...
2. Iterate over the employees from `0` to $N - 1$, and for each employee `i`, store an edge $\text{manager}[i] -> i$ if $\text{manager}[i]$ isn't `-1`.
3. Start the DFS with node `headID` and `time` as `0` for each node as `curr`:
   ...
   ii. Iterate over the adjacent nodes of `curr`, and for each adjacent node, perform DFS with time as $time + \text{informTime}[curr]$.
