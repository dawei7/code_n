...
In this problem, we have to answer `n` queries. To answer the `i`-th query, we have to find the `k` where `k` has to be less than $2^{\text{maximumBit}}$ and maximizes the $XOR$ product between `k` and $\text{nums}[0], \text{nums}[1],..., \text{nums}[i]$. We will explore two approaches that will dive into how to calculate this `k` efficiently for all queries.

### Approach 1: Prefix Array + Bit Masking

#### Intuition

For the `i-th` query, we need to first calculate the $XOR$ product for the numbers $\text{nums}[0], \text{nums}[1],..., \text{nums}[i]$. Once we have this XOR value, which we'll call `product`, we want to find a `k` that maximizes the result of `k XOR product`. We first discuss how to find the `product` for each query efficiently.

To efficiently calculate the initial `product` for each query, we can save time by precomputing these values. We do this by performing a linear scan through the `nums` array to build a prefix array called `prefixXOR`. In this array, $\text{prefixXOR}[i]$ will hold the XOR product of all numbers from $\text{nums}[0]$ to $\text{nums}[i]$. For any query at index `i`, we can easily retrieve the `product` as $\text{prefixXOR}[i]$.
...
* Initialize $\text{prefixXOR}[0]$ to $\text{nums}[0]$
* From $i = 1$ to $i = \text{nums.length} - 1$:
* $\text{prefixXOR}[i] = prefixXOR[i - 1] XOR \text{nums}[i]$
2. Define our bitmask $mask = (1 << maximumBit) - 1$
3. Initialize our answer array `ans`
4. Answer each query and populate `ans`:
* From $i = 0$ to $i = \text{nums.length} - 1$:
* The current XOR product we're dealing with is $product = prefixXOR[-i]$
* Set $\text{ans}[i]$ to be `product XOR mask`, giving us a `k` that inverts the first `maximumBit` bits of `product` to maximize our value `product XOR k`
5. Return `ans`
...
4. Answer each query and populate `ans`:
* From $i = 0$ to $i = \text{nums.length} - 1$:
* Set $\text{ans}[i]$ to be `xorProduct XOR mask`, giving us a `k` that inverts the first `maximumBit` bits of `product` to maximize our value `product XOR k`
