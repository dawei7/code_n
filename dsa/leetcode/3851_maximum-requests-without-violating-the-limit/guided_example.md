# Guided Example: Maximum Requests Without Violating the Limit

We trace the dynamic Array, Hash Table, Greedy, Sliding Window, Sorting sliding window on a representative input instance.
...
The objective for **Maximum Requests Without Violating the Limit** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
