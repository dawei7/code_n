...
The core task in **Maximum Requests Without Violating the Limit** is to a 2D integer array `requests`, where $\text{requests}[i] = [\text{user}_{i}, \text{time}_{i}]$ indicates that $\text{user}_{i}$ made a request at $\text{time}_{i}$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
