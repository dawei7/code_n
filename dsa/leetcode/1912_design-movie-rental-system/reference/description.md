...
Each movie is given as a 2D integer array `entries` where $\text{entries}[i] = [\text{shop}_{i}, \text{movie}_{i}, \text{price}_{i}]$ indicates that there is a copy of movie $\text{movie}_{i}$ at shop $\text{shop}_{i}$ with a rental price of $\text{price}_{i}$. Each shop carries **at most one** copy of a movie $\text{movie}_{i}$.

The system should support the following functions:

- **Search**: Finds the **cheapest 5 shops** that have an **unrented copy** of a given movie. The shops should be sorted by **price** in ascending order, and in case of a tie, the one with the **smaller **$\text{shop}_{i}$ should appear first. If there are less than 5 matching shops, then all of them should be returned. If no shop has an unrented copy, then an empty list should be returned.
...
- $1 \le \text{movie}_{i}, \text{price}_{i} \le 10^{4}$

- Each shop carries **at most one** copy of a movie $\text{movie}_{i}$.
