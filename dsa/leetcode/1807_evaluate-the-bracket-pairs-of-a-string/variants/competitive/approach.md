...
The core task in **Evaluate the Bracket Pairs of a String** is to a string `s` that contains some bracket pairs, with each pair containing a **non-empty** key. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
