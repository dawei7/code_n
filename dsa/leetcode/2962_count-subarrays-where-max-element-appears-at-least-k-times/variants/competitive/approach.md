...
The core task in **Count Subarrays Where Max Element Appears at Least K Times** is to an integer array `nums` and a **positive** integer `k`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
