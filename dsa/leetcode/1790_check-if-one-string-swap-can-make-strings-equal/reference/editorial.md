...
- If $s1Char \neq s2Char$, then increment `numDiffs`. If `numDiffs` is now greater than `2`, then we know one string swap will not make the strings equal, so return `false`.
- Update the frequency maps by incrementing the frequency of `s1Char` and `s2Char`: $\text{s1FrequencyMap}[s1Char]++$ and $\text{s2FrequencyMap}[s2Char]++$.
- Now, the strings are equal only if the frequency maps are equal. If `s1FrequencyMap` and `s2FrequencyMap` have the same frequencies, then return `true`. Otherwise, return `false`.
...
> Note: For this approach's implementation, observe that we do not have to explicitly check the trivial edge case where `s1` and `s2` are equal. If the two strings are equal, then no diffs are found, and $\text{s1}[firstIndexDiff] = \text{s2}[secondIndexDiff] \&\& \text{s1}[secondIndexDiff] = \text{s2}[firstIndexDiff]$ will always be true, assuming the default initialized values of `firstIndexDiff` and `secondIndexDiff` are valid (they can just be 0).
...
- Return $\text{s1}[firstIndexDiff] = \text{s2}[secondIndexDiff] \&\& \text{s1}[secondIndexDiff] = \text{s2}[firstIndexDiff]$
