## General
**Optimal Approach — Customers Who Bought Products A and B but Not C**

The query executes a structured relational pipeline for **Customers Who Bought Products A and B but Not C**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(C + O + R)$ — Operation count proportional to input scale.
