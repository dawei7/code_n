...
We trace the dynamic Math, String, Sliding Window sliding window on a representative input instance.
...
The objective for **Find the K-Beauty of a Number** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
