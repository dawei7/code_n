...
![Test cases split into 3 parts](images/shortest_subarray_to_be_removed.png)
...
- Find the longest non-decreasing subarray starting from the left:
  - While $left + 1 < n$ and $\text{arr}[left] \le arr[left + 1]$, increment `left` to expand the left subarray.

- Find the longest non-decreasing subarray starting from the right:
  - While $right - 1 \ge 0$ and $\text{arr}[right] \ge arr[right - 1]$, decrement `right` to expand the right subarray.

- If the entire array is already sorted (i.e., $left \ge right$), return `0` as no subarray removal is needed.

- Initialize `ans` to the smaller of removing the left or right part completely:
  - $ans = min(n - (left + 1), right)$

- Try to merge the left and right parts:
  - For each index `i` from 0 to `left`, use binary search (`helperBinarySearch`) to find the smallest index `j` where $\text{arr}[j] \ge \text{arr}[i]$.
...
![2 pointers](images/two_pointers.png)
...
Using this two-pointer method, for each position of `left`, we search for the smallest `right` where $\text{arr}[left] \le \text{arr}[right]$. If this condition holds, then we have found a valid subarray candidate to remove—the subarray between $\text{arr}[left]$ and $\text{arr}[right]$, which has a length of $right - left - 1$. If $\text{arr}[left] > \text{arr}[right]$, we increment `right` to find the next possible match. Once a valid `right` is found, we advance `left` to the next element, repeating the process.
...
* While `right > 0` and $\text{arr}[right] \ge arr[right - 1]$, decrement `right`
...
5. While `left < right` and `left` is still in the blue region: $left = 0 || arr[left - 1] \le \text{arr}[left]$:
* Find the right number after arr[left]:
* While `right < arr.length` and $\text{arr}[left] > \text{arr}[right]$, increment `right`
