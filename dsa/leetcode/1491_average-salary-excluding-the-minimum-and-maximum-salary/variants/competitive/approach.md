...
The core task in **Average Salary Excluding the Minimum and Maximum Salary** is to an array of **unique** integers `salary` where $\text{salary}[i]$ is the salary of the $$i^{\text{th}}$$ employee. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
