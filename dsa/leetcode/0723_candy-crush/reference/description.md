...
Given an `m x n` integer array `board` representing the grid of candy where $\text{board}[i][j]$ represents the type of candy. A value of $\text{board}[i][j] = 0$ represents that the cell is empty.
