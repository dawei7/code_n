...
The core task in **Maximum Difference Between Increasing Elements** is to a **0-indexed** integer array `nums` of size `n`, find the **maximum difference** between $\text{nums}[i]$ and $\text{nums}[j]$ (i.e., $\text{nums}[j] - \text{nums}[i]$), such that $0 \le i < j < n$ and $\text{nums}[i] < \text{nums}[j]$. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
