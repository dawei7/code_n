...
- Space complexity: $O(n + q)$
  ...
  The difference array $\textit{dif}$ has size $n + T = O(n + \sqrt{n}) = O(n)$, and the $\textit{groups}$ structure stores up to $q$ queries in total. Therefore, the overall space complexity is $O(n + q)$.
