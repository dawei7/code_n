...
1. Initialize an array `children`, where $\text{children}[X]$ contains all the children of node `X`.
...
Similar to the above approach, we initialize the answer variable $longestPath = 1$, since a single node is always an answer. We declare an array, say `childrenCount` to count the number of children of each node. We also initialize a 2D array, `longestChains` where $\text{longestChains}[node][0]$ and $\text{longestChains}[node][1]$ store the longest and second longest chains among its children. If a node, say `A` has no children or all children are covered, we update $\text{longestChains}[A][0]$ to store the length of the longest chain starting from `A` (including it) to be used by its parent.

We push the leaf nodes into the queue (nodes with $childrenCount = 0$). For each leaf node, say `A` we also set the longest chain starting from `A` to $\text{longestChains}[A][0] = 1$.

Next, we perform the BFS traversal until the queue is empty. We pop out the first element, say `currentNode` from the queue. We get the `currentNode`'s parent from $\text{parent}[currentNode]$, let's call it `par`.

We get the longest chain starting from the `currentNode` in `longestChainStartingFromCurrNode` using $longestChainStartingFromCurrNode = \text{longestChains}[currentNode][0]$. If the characters of the `currentNode` and its parent do not match, we update the $\text{longestChains}[par]$ using `longestChainStartingFromNode` if possible.

The answer variable is updated to $longestPath = max(longest, \text{longestChains}[par][0] + \text{longestChains}[par][1] + 1)$. We also reduce the child count of the `par` by one to mark that we have covered a child of the `par` node.

If the child count of a node reaches `0`, i.e., $\text{childrenCount}[par] = 0$ it means we have covered all its children, and it would behave as a leaf node. We push the `par` into the queue and also increment $\text{longestChains}[par][0]$ by `1` to store the largest chain starting from the `par` to be used for its parent.
...
1. Initialize an array `childrenCount`, that stores the number of children of a node `i` in $\text{childrenCount}[i]$.
- To populate it, we iterate over all the nodes, and for each node, increment $childrenCount[\text{parent}[node]]$ by 1. Since the root node `0` does not have a parent, we start iterating from node `1` to node $n - 1$.
2. Initialize another 2D-array $\text{longestChains}[n][2]$. For a node `i`, it stores the length of the longest and second longest chains among its children in $\text{longestChains}[i][0]$ and $\text{longestChains}[i][1]$ respectively. We initialize it with `0`.
...
- If it is a leaf node, i.e., $\text{childCount}[node] = 0 \& node \neq 0$, we push it into the queue. We also set $\text{longestChain}[node][0] = 1$ as this node does not have any children and this is the longest chain we can form including it. It will help find the answer of its parent.
5. Then, while the queue is not empty:
- Dequeue the first element `currentNode` from the queue.
- Fetch the parent `par` of the `currentNode` from $\text{parents}[currentNode]$.
- Get the longest chain starting from the `currentNode` from $longestChainStartingFromCurrNode = \text{longestChains}[currentNode][0]$ to update the longest ($\text{longestChains}[par][0]$) and second ($\text{longestChains}[par][1]$) longest chains of the `par` if possible.
- Update the `longestPath` with $max(longestPath, \text{longestChains}[par][0] + \text{longestChains}[par][1] + 1)$. `par` is an intermediate node that connects the longest and second longest chains of it.
- Because we covered the `currentNode` which is a child of `par`, we deduct $\text{childrenCount}[par]$ by `1`.
- If there are no more children of the `par` to visit, i.e., $\text{childrenCount}[par] = 0$, we push the `par` to the queue. It behaves as a leaf node. We increase $\text{longestChain}[par][0]$ by `1` to include `par` in the longest chain. We do not push root node `0` into the queue since it does not have a parent.
