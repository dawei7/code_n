...
The core task in **Minimum Adjacent Swaps to Partition Array** is to an integer array `nums` and two integers `a` and `b` such that `a < b`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
