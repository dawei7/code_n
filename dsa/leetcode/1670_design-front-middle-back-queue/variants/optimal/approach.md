...
The core task in **Design Front Middle Back Queue** is to Design a queue that supports `push` and `pop` operations in the front, middle, and back. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
