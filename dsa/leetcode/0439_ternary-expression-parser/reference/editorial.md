...
- $\text{window}[0]$ is `T` or `F`
- $\text{window}[1]$ is `?`
- $\text{window}[2]$ is `T` or `F` or `0` or `1` or `2` or `3` or `4` or `5` or `6` or `7` or `8` or `9`
- $\text{window}[3]$ is `:`
- $\text{window}[4]$ is `T` or `F` or `0` or `1` or `2` or `3` or `4` or `5` or `6` or `7` or `8` or `9`
...
- Initialize `j` to $\text{expression.size}() - 1$. This will be the rightmost index of the `window`.
...
- Initialize `questionMarkIndex` to $\text{expression.size}() - 1$. This will be the index of `?`.

- While $\text{expression}[questionMarkIndex]$ is not `?`, decrement `questionMarkIndex` by 1.

- Now, $\text{expression}[questionMarkIndex]$ is `?`. Thus, `expression[questionMarkIndex+1:]` is `E1:E2` and $expression[questionMarkIndex - 1]$ is `B`.
...
1. Initialize a stack `stack`. Also, initialize `i` to $\text{expression.size}() - 1$. This will be the index of the character we are currently processing.

2. Until we process all the characters, do the next steps.

- If $\text{expression}[i]$ is `T`, `F`, or a digit, then push it on the stack.

- If $\text{expression}[i]$ is `?`, then pop the top two elements of the stack. Let's call them `onTrue` and `onFalse`. If $expression[i - 1]$ is `T`, then push `onTrue` on the stack. Otherwise, push `onFalse` on the stack. Decrement `i` by 1 as we have already used $expression[i - 1]$.

- Decrement `i` by 1 as we have already processed $\text{expression}[i]$.
...
- if $i = j$, return $\text{expression}[i]$, since there is only one character.
...
2. Call the recursive function $solve(expression, 0, \text{expression.size}() - 1)$.
...
- If $\text{expression}[i]$ is not a boolean, then return it.

- If $\text{expression}[i]$ is the last character of the `expression`, then return it.

- If $\text{expression}[i]$ is a boolean, then

- If $expression[i + 1]$ is `:`, then return $\text{expression}[i]$. It means we have to ignore the portion after `:`, as we were processing the portion before `:`.

- If $expression[i + 1]$ is `?`, then

- If $\text{expression}[i]$ is `T`, then increment `i` by 2 to process the portion between `?` and `:`. After the `:` portion will be ignored by the above condition.

- If $\text{expression}[i]$ is `F`, then make `i` point to the character after `:` of this `?`. To have corresponding `:`, we can maintain count.
