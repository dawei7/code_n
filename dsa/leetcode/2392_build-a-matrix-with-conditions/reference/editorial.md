...
Now that we have topologically sorted arrays for both `rowConditions` and `colConditions`, how can we utilize them to construct the matrix? Each row and column should correspond to their respective sorted arrays. Therefore, the value at position $\text{matrix}[i][j]$ is derived from $\text{rowConditions}[i]$ and $\text{colConditions}[j]$.
...
- If $\text{orderRows}[i] = \text{orderColumns}[j]$, store $\text{orderRows}[i]$ in $\text{matrix}[i][j]$.
6. Return the `matrix`.
...
2. Store all the `edges` in `adj` by pushing `b` in $\text{adj}[a]$ denoting an edge from `a` to `b`.
...
1. Set $\text{visited}[node]$ to `1`.
2. Iterate over all neighbors of `node`:
- If $\text{visited}[neighbor] = 0$, perform `dfs(neighbor, adj, visited, order, hasCycle)`. If `hasCycle` is true, return.
- If $\text{visited}[neighbor] = 1$, set `hasCycle` to true, return.
4. Set $\text{visited}[node]$ as `2`.
...
2. Store all the `edges` in `adj` by pushing `b` in $\text{adj}[a]$ (denoting an edge from `a` to `b`). Also, increment the in-degree of `b` in the `deg` array.
