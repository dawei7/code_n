...
When spelling a given character $\text{key}[i]$, the number of steps it takes to spell the character will be the number of rotations made to put the character in the `"12:00"` position plus one, which represents pressing the center button to spell the character.
...
3. Return $min(\text{steps}_{around}, \text{steps}_{between})$.
...
When we reach the end of the keyword, we have spelled the whole word. Therefore, our base case is $keyIndex = \text{key.length}()$. At that point, the `keyIndex` is past the end of the keyword, and no steps need to be taken, so we return zero.
...
1. If $\text{key}_{index}$ is equal to `key.length()`, then return `0`; `key` has been spelled.
2. Iterate through each character in the `ring` using `i`:
1. If $\text{ring}[i]$ equals the current character $\text{key}[keyIndex]$:
- Calculate `totalSteps`, the steps it takes to spell `key` when we visit $\text{ring}[i]$ by adding the following three terms:
- The output of `countSteps` which finds the number of steps from the `ringIndex` to $\text{ring}[i]$.
- `1`, signifying pressing the center button.
- The output of `tryLock`, which calculates how many steps to each character in `key` are required if we choose to visit $\text{ring}[i]$.
- Save the minimum between `totalSteps` and the best so far in `minSteps`.
3. Return `minSteps`.
3. Call $tryLock(0, 0, \text{INT}_{MAX})$ as we start with the zeroth index of `ring` in the  `"12:00"` position and start spelling with the first character in `key`. The largest integer is passed as the final parameter because a path has not been determined between the zeroth index of `ring` and the first character in `key`.
...
2. Check whether the `(ringIndex, keyIndex)` pair is in `bestSteps`. If it is, return $\text{bestSteps}[ringIndex][keyIndex]$; we have already calculated the best path.
3. Iterate through each `charIndex` in `ring`:
1. If $\text{ring}[charIndex]$ equals the current character $\text{key}[keyIndex]$:
           - Calculate `totalSteps`, the steps it takes to spell `key` when we visit this occurrence of $\text{ring}[charIndex]$ by adding the following three terms:
- The output of `countSteps`, which finds the number of steps from the `ringIndex` to the $\text{ring}[charIndex]$.
- `1`, which signifies pressing the center button.
- The output of `tryLock`, which calculates the number of steps to each character in `key`, granted we chose to visit this occurrence of $\text{ring}[charIndex]$.
- Save the minimum between `totalSteps` and the best so far in `minSteps`.
- Save the `minSteps` for this `(ringIndex, keyIndex)` pair in `bestSteps`.
4. Return `minSteps`.
5. Call $tryLock(0, 0, \text{INT}_{MAX})$ as we start with the zeroth index of `ring` in the `"12:00"` position and start spelling with the first character in `key`. The largest integer is passed as the final parameter because a path has not been determined between the zeroth index of `ring` and the first character in `key`.
...
In our top-down solution, we use a map `bestSteps` to store the minimum number of steps to `keyIndex` starting from the `ringIndex` of `ring`. To generate a bottom-up solution, we will use a 2D array $\text{bestSteps}[ringIndex][keyIndex]$ to store the minimum number of steps to the `keyIndex` when the `ringIndex` of `ring` is aligned with the `"12:00"` position.
...
> $\text{bestSteps}[r][k] = min(\text{bestSteps}[r][k], 1 + countSteps[r, charIndex] + \text{bestSteps}[charIndex][\text{key}_{index} + 1])$

The terms in the recurrence relation represent:

- $\text{bestSteps}[ringIndex][keyIndex]$: the previous minimum number of steps to that occurrence of that character.
...
- $\text{bestSteps}[charIndex][keyIndex + 1]$: the number of steps it took to reach `charIndex` from the last character $key[keyIndex + 1]$ the `ring` spelled.

After iterating through both strings, we return $\text{bestSteps}[0][0]$ which stores the minimum number of steps it took to spell `key` when `ring` begins with its zeroth index in the `"12:00"` position.
...
- For each character $\text{ring}[charIndex]$ in `ring`:
- If $\text{ring}[charIndex]$ equals `key` at `keyIndex`: Use the recurrence relation $\text{bestSteps}[ringIndex][k] = min(\text{bestSteps}[ringIndex][k], 1 + countSteps[ringIndex, charIndex] + \text{bestSteps}[charIndex][keyIndex + 1])$ to calculate the minimum number of steps to find the character at `keyIndex` of the keyword when the `ringIndex` of `ring` is aligned with the `"12:00"` position.
7. Return $\text{bestSteps}[0][0]$ which stores the minimum number of steps to spell `key` when `ring` begins with its zeroth index in the `"12:00"` position.
...
The recurrence relation finds the minimum number of steps to find the $\text{key}[keyIndex]$ when the $\text{ring}[ringIndex]$ is aligned with the `"12:00"` position and stores it in $\text{curr}[ringIndex]$. The adjusted relation is:

$\text{curr}[ringIndex] = min(\text{curr}[ringIndex], 1 + countSteps[ringIndex, charIndex] + \text{prev}[charIndex])$

The terms in the recurrence relation represent:

- $\text{curr}[ringIndex]$: the current minimum number of steps to that index.
- `1`: represents selecting the character by pressing the center button.
...
- $\text{prev}[charIndex]$: the number of steps it took to reach `charIndex` from the last character the $key[keyIndex + 1]$ the `ring` spelled.

After iterating through both strings, we return $\text{prev}[0]$ which stores the minimum number of steps to spell `key` when `ring` begins with its index `0` in the `"12:00"` position.
...
- Use the recurrence relation $\text{curr}[ringIndex] = min(\text{curr}[ringIndex], 1 + countSteps[ringIndex, charIndex] + \text{prev}[charIndex])$ to calculate the minimum number of steps to find the character at `keyIndex` of key when the `ringIndex` of `ring` is aligned with the `"12:00"` position.
- Set `prev` to `curr`.
6. Return $\text{prev}[0]$ which stores the minimum number of steps to spell `key` when `ring` begins with its zeroth index in the `"12:00"` position.
...
For each occurrence of the current $\text{key}[keyIndex]$ in `ring`, `nextIndex`, we add an entry to the heap that represents turning the metal dial to the next character in the keyword. The entry consists of three parts:
- `totalSteps`: the sum of `toalSteps` and the output of $\text{count}_{steps}(ringIndex, nextIndex)$.
...
- For each occurrence `nextIndex` in `ring` of the letter $\text{key}[keyIndex]$:
...
- `totalSteps`: $totalSteps + \text{count}_{steps}(ringIndex, nextIndex)$.
- `ringIndex`: `nextIndex`.
- `keyIndex`: $keyIndex + 1$.
