...
| Column | Type | Meaning |
|---|---|---|
| $\text{candidate}_{id}$ | int | Unique candidate identifier. |
| `name` | varchar | Display name of the candidate. |
| `years_of_exp` | int | Years of professional experience. |
| $\text{interview}_{id}$ | int | Identifier for the candidate's interview. |

**`Rounds`**

| Column | Type | Meaning |
|---|---|---|
| $\text{interview}_{id}$ | int | Interview identifier; composite primary key with $\text{round}_{id}$. |
| $\text{round}_{id}$ | int | Round number within the interview. |
