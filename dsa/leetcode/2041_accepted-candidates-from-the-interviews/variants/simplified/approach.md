# Simplified Approach - Accepted Candidates From the Interviews
...
## Complexity detail
- **Time Complexity**: $O(C+R+C\log C)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(C+R)$ — Utilizes standard auxiliary storage proportional to the active computation state.
