# Simplified Approach - Minimum Cost to Merge Sorted Lists
...
## Complexity detail
- **Time Complexity**: $O(3^L + N * 2^L + N log N)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(2^L + N)$ — Utilizes standard auxiliary storage proportional to the active computation state.
