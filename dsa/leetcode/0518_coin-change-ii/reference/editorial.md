...
As we know the current state of a sub-problem depends on the index of the current under consideration and the remaining amount, we can use a 2D array `memo`. $\text{memo}[i][j]$ stores the number of ways to make up `j` using the coins beginning from index `i`.
...
- If we have already computed this problem, i.e., $\text{memo}[i][amount] \neq -1$, we return $\text{memo}[i][amount]$.
- If the value of the current coin under consideration exceeds the `amount`, we cannot use it. We recursively call $numberOfWays(i + 1, amount)$, set it equal to $\text{memo}[i][amount]$, and return it.
- Otherwise, we add the total number of ways to make up the `amount` by both picking up the current coin and ignoring it. Add up the values $numberOfWays(i, amount - \text{coins}[i])$ and $numberOfWays(i + 1, amount)$, store the result in $\text{memo}[i][amount]$, and return it.
...
We create a 2D list $\text{dp}[n][amount]$ where $\text{dp}[i][j]$ stores the number of ways to make up the `j` amount using the coins beginning from index `i`. Note that here, $\text{dp}[i][j]$ is equivalent to `numberOfWays(i, j)` from the previous approach.

We initialize $\text{dp}[i][0] = 1$ for all values of `i` from `0` to `n` since we can always make up the amount `0` by not selecting any coins. While moving from bottom to top, this serves as the base case for our solution.
...
Each iteration inside the nested loop represents a state `(i, j)`, which is equivalent to a function call from the previous approach. As such, we can apply the exact same logic to calculate $\text{dp}[i][j]$.

If $\text{coins}[i] > j$, we cannot use the current coin, so we set $\text{dp}[i][j] = dp[i + 1][j]$.

Otherwise, if we can use the current coin, we add the number of ways to make up the amount `j` by both selecting it and ignoring it. We set $\text{dp}[i][j] = dp[i + 1][j] + \text{dp}[i][j - \text{coins}[i]]$.

After computing all the `dp` states in the nested loops, $\text{dp}[0][amount]$ state stores the answer, just like `numberOfWays(0, amount)` was the answer in the previous approach.

#### Algorithm

1. Create an integer variable `n` and initialize it to the size of the `coins` array.
2. Create a 2D array called `dp` with $n + 1$ rows and $amount + 1$ columns where $\text{dp}[i][j]$ stores the number of ways to make up the `j` amount using the coins starting from index `i`.
3. Set $\text{dp}[i][0] = 1$ for all values of `i` as the base case.
...
- If the value of the current coin at index `i` exceeds `j`, we cannot use it. We set $\text{dp}[i][j] = dp[i + 1][j]$.
- Otherwise, we add the total number of ways to make up `j` by using the current coin and ignoring it. We set $\text{dp}[i][j] = dp[i + 1][j] + \text{dp}[i][j - \text{coins}[i]]$.
5. Return $\text{dp}[0][amount]$.
...
Looking closely at this transition, we can see that to fill $\text{dp}[i][j]$ for a specific `i` and all values of `j` we only need the values from $\text{dp}[i]$ and $dp[i + 1]$. Values from older rows like $i + 2$, $i + 3$, etc. are no longer relevant.

We can optimize the previous solution by using just one 1D array `dp` of size $amount + 1$.

We would have an outer loop that selects the current coin under consideration from $i = n - 1$ to `0` similar to the previous approach. After the $i^{th}$ iteration of the outer loop, $\text{dp}[j]$ would represent $\text{dp}[i][j]$ from the previous approach.

We initialize $\text{dp}[0] = 1$ since we can always make up the amount `0` by not selecting any coins. It acts as a base case. This is similar to setting $\text{dp}[i][0] = 1$ in the previous approach.

Now, consider that we have all of the values of row $i + 1$ in `dp` and that we now need to compute the values of row `i`. We can begin an inner loop that iterates from $j = \text{coins}[i]$ to `amount`. The reason we don't need to consider values from $j = 1$ to $\text{coins}[i] - 1$ is because we cannot select the $i^{th}$ coin for those values of `j`.

In such cases, as we saw in the previous approach, $\text{dp}[i][j] = dp[i + 1][j]$. As a result, we don't need to modify these values that were computed in the previous iteration.

We start an inner loop from $j = \text{coins}[i]$ to `amount`. Now, we have two cases.

We ignore the current coin. The number of ways to make up the `j` amount ignoring the current coin is already present in $\text{dp}[j]$. It is computed in the previous iteration (for row $i + 1$) and is identical to the state $dp[i + 1][j]$ of the previous approach.

When we choose the current coin, we add $dp[j - \text{coins}[i]]$. This is equivalent to adding $\text{dp}[i][j - \text{coins}[i]]$ from the previous approach.

So, we perform $\text{dp}[j] += dp[j - \text{coins}[i]]$ to add both the cases (analogous to $dp[i + 1][j]$ and $\text{dp}[i][j - \text{coins}[i]]$).

After all iterations, $\text{dp}[amount]$ stores the answer.
...
If we change the ordering of the two loops, the outer loop would run from $i = 1$ to `amount` and the inner loop would execute from $j = n - 1$ to `0`. We would perform the same operation $\text{dp}[i] += dp[i - \text{coins}[j]]$ inside the loops. After the $i^{th}$ iteration, $\text{dp}[i]$ would store all the ways to make up the amount `i` using all the coins.

Let's take an example where $coins = [1, 2]$ and $amount = 3$. The correct answer for this case is `2` ($1 + 1 + 1$ and $1 + 2$).

However, if we look at the last iteration of the outer loop when $i = 3$, we will execute $\text{dp}[3] = \text{dp}[3] + dp[3 - 1]$ when the first coin is selected using the inner loop. $\text{dp}[2]$ would be equal to `2` as there are two ways to make up amount `2` ($1 + 1$ and `2`). This way we selected two cases $1 + 1 + 1$ and $1 + 2$.

We also execute $\text{dp}[3] = \text{dp}[3] + dp[3 - 2]$ when the second coin is selected. $\text{dp}[1]$ would be equal to `1` as there is just one way to make up amount `1`. This way we counted the $1 + 2$ case. We counted the $1 + 2$ case twice. Here's a visual representation showing all the `dp` states for this approach:

![img](images/518-2.png)
...
1. Create an integer variable `n` and initialize it to the size of the `coins` array.
2. Create an array `dp` of size $amount + 1$. Initialize $\text{dp}[0] = 1$ (analogous to $\text{dp}[i][0] = 1$).
3. Fill `dp` using two loops. The outer loop runs from $i = n - 1$ to `0`. It controls the index of the current coin under consideration. The inner loop runs from $j = \text{coins}[i]$ to `amount` and controls the amount to be made up. In the nested loops, we perform the following:
- $\text{dp}[j] += dp[j - \text{coins}[i]]$.
4. Return $\text{dp}[amount]$.
