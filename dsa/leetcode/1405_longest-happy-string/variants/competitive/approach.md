...
The core task in **Longest Happy String** is to A string `s` is called **happy** if it satisfies the following conditions:. To dynamically keep track of the minimum or maximum value without sorting the entire array repeatedly, this solution uses a **Min/Max Heap (Priority Queue)**. It allows us to insert elements and extract the smallest/largest value in fast logarithmic $O(\log k)$ time.
