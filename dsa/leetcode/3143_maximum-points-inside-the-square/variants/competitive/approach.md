...
The core task in **Maximum Points Inside the Square** is to a 2D** **array `points` and a string `s` where, $\text{points}[i]$ represents the coordinates of point `i`, and $s[i]$ represents the **tag** of point `i`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
