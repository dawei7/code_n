# Simplified Approach - Find Beautiful Indices in the Given Array II
...
## Complexity detail
- **Time Complexity**: $O(N+A+B)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(N+A+B)$ — Utilizes standard auxiliary storage proportional to the active computation state.
