...
After we sort the array, iterating over it from left to right would be like "walking" up the staircase. Each time we encounter an index `i` where $\text{nums}[i] \neq nums[i - 1]$, we know that we had to take an **up step**. We can simply keep track of how many **up steps** we have taken so far in an integer `up`. We need `up` steps to reach $\text{nums}[i]$, which also means that $\text{nums}[i]$ needs `up` operations to be reduced to `min`. Therefore, at each step, we increment our answer by `up`.
...
- Check if $\text{nums}[i] \neq nums[i - 1]$. If so, increment `up`.
- Add `up` to `ans`.
4. Return `ans`.
