...
![t](images/572_root_tree_t.svg){:align="center"}
...
1. [Level Order Traversal](https://leetcode.com/problems/binary-tree-level-order-traversal/) of `t` is `[1, 2, 3, 4, 5]`, serialized as `12345`.
    ...
    ![level_fp](images/572_level_pre.svg){:align="center"}
    ...
    ![level_fn](images/572_root_tree_x.svg){:align="center"}
...
2. [Pre Order Traversal](https://leetcode.com/problems/binary-tree-preorder-traversal/) of `t` is `12345`.
    ...
    ![pre](images/572_level_pre.svg){:align="center"}
...
3. [Post Order Traversal](https://leetcode.com/problems/binary-tree-postorder-traversal/) of `t` is `24531`.
    ...
    ![post](images/572_post.svg){:align="center"}
...
4. [In Order Traversal](https://leetcode.com/problems/binary-tree-inorder-traversal/) of `t` is `21435`.
   Consider these two trees `p` and `q`
    ...
    ![in](images/572_in.svg){:align="center"}
...
1. Define a function `serialize` which takes a `node`, and a string/list $\text{tree}_{str}$ and serializes the node by appending it to that list. This function will serialize the tree in preorder traversal and will add $^$ before adding the node's value. For `null` nodes, it will add `#`.
...
- $O(N)$ integers _(Note that it will not be exactly $N$ because if any node's value is 10000, then it is contributing 5 characters, however, since $-10⁴ \le \text{root.val} \le 10⁴$, there can be at most $5N$ integer characters)_,
...
1. Define two constants $\text{MOD}_{1}$ and $\text{MOD}_{2}$ as two large prime numbers.
...
- For the first hash of the pair
      ...
      - take $\text{MOD}_{1}$ at each step to avoid overflow.
- For the second hash of the pair
      ...
      - take $\text{MOD}_{2}$ at each step to avoid overflow.
