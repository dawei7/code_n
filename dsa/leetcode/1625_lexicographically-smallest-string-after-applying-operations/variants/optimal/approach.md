...
The core task in **Lexicographically Smallest String After Applying Operations** is to a string `s` of **even length** consisting of digits from `0` to `9`, and two integers `a` and `b`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
