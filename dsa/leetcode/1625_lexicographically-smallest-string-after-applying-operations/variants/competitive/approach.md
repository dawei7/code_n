...
The core task in **Lexicographically Smallest String After Applying Operations** is to a string `s` of **even length** consisting of digits from `0` to `9`, and two integers `a` and `b`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
