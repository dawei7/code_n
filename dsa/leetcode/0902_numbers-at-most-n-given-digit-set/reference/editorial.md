...
First, call a positive integer `X` *valid* if $X \le N$ and `X` only consists of digits from `D`.  Our goal is to find the number of valid integers.
...
Let $\text{dp}[i]$ be the number of ways to write a valid number if `N` became $N[i], N[i+1], ...$.  For example, if $N = 2345$, then $\text{dp}[0]$ would be the number of valid numbers at most `2345`, $\text{dp}[1]$ would be the ones at most `345`, $\text{dp}[2]$ would be the ones at most `45`, and $\text{dp}[3]$ would be the ones at most `5`.

Then, by our reasoning above, $\text{dp}[i] = (number of d in D with d < S[i]) * ((\text{D.length}) ** (K-i-1))$, plus `dp[i+1]` if $S[i]$ is in `D`.
...
As in *Approach #1*, call a positive integer `X` *valid* if $X \le N$ and `X` only consists of digits from `D`.

Now let $B = \text{D.length}$.  There is a bijection between valid integers and so called "bijective-base-`B`" numbers.  For example, if $D = ['1', '3', '5', '7']$, then we could write the numbers `'1', '3', '5', '7', '11', '13', '15', '17', '31', ...` as (bijective-base-`B`) numbers `'1', '2', '3', '4', '11', '12', '13', '14', '21', ...`.
