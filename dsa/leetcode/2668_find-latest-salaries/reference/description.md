...
Write a solution to find the current salary of each employee assuming that salaries increase each year. Output their $\text{emp}_{id}$, `firstname`, `lastname`, `salary`, and $\text{department}_{id}$.

Return the result table ordered by $\text{emp}_{id}$ in **ascending** order*.*
