...
The composite primary key $(\texttt{emp\_id},\texttt{salary})$ prevents duplicate salary strings for the same employee. It does not by itself guarantee that name and department never change, but the narrative treats records as salary history for fixed employees.
