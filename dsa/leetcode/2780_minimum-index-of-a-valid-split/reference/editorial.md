...
- Decrement $\text{secondMap}[num]$ by `1`.
- Increment $\text{firstMap}[num]$ by `1`.
- If $\text{firstMap}[num] * 2 > index + 1$ and $\text{secondMap}[num] * 2 > n - index - 1$, return `index`, since `num` is the dominant element in both halves of the current split.
