...
1. Initialize an array `dp` of length $numPeople / 2 + 1$. Remember that $\text{dp}[i]$ is the answer to the problem if there are `2i` people. Set the base case $\text{dp}[0] = 1$.
2. Iterate `i` from `1` to $numPeople / 2$ inclusive.
* Iterate `j` from `0` to $i - 1$ inclusive.
* Increase $\text{dp}[i]$ by $\text{dp}[j] * dp[i - j - 1]$. Remember to perform operations modulo $10⁹ + 7$.
