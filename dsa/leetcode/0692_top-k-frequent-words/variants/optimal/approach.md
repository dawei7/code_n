...
The core task in **Top K Frequent Words** is to an array of strings `words` and an integer `k`, return *the *`k`* most frequent strings*. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
