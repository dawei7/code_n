...
- Time complexity : $O(N)$.
    ...
    Each node is put onto the stack *once*, by its parent node. We know each node only has one parent because this is a tree. Therefore, each node is only taken off, and processed, once. Processing a node is an $O(1)$ operation. Therefore, we get a total time complexity of $N \cdot$\mathcal{O}(1)$= O(N)$.
