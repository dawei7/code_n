...
- Add integer $\text{nums}[i]$ into the current combination `curr`.

- Proceed to add more integers into the combination: $backtrack(i + 1, curr)$.

- Backtrack by removing $\text{nums}[i]$ from `curr`.
...
where `1` on the i*th* position in bitmask means the presence of $\text{nums}[i]$
...
`1` on the i*th* position in bitmask means the presence of $\text{nums}[i]$
in the subset, and `0` means its absence.
