...
2. Otherwise, if the email has already been seen in another account, then we will union the current group (`i`) and the group the current email belongs to ($\text{emailGroup}[email]$).
