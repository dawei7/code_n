...
The elements of $x$ should be placed at indices `0, 2, 4, ...`. At $\text{nums}[i]$ we have element $x_{i + 1}$, we should place it at index $2 * i$ for all $0 \le i \lt n$.
...
- Store the element $x_{i + 1}$ , that is, $\text{nums}[i]$ at index $2 * i$,
...
- Store the element $y_{i + 1}$, that is, $\text{nums}[i]$ with $x_{i + 1}$ at index $(i - n)$, using bit manipulation as discussed previously.
