...
* After iterating add the sliced chunk to the `chunkedArray` using $\text{arr.slice}(index, index + size)$.
...
* In each iteration of the `reduce` function:
  ...
  * Checks the last chunk in the `chunkedArray` using $chunkedArray[\text{chunkedArray.length} - 1]$.
...
* The `callback` function uses $\text{arr.slice}(index * size, index * size + size)$ to extract the corresponding portion of the array for each chunk.

* Finally return the resulting chunked array.

In summary: It determines the number of chunks needed using $\text{Math.ceil}(\text{arr.length} / size)$.
