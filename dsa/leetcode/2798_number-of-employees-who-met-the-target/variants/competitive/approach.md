...
The core task in **Number of Employees Who Met the Target** is to There are `n` employees in a company, numbered from `0` to $n - 1$. Each employee `i` has worked for $\text{hours}[i]$ hours in the company. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
