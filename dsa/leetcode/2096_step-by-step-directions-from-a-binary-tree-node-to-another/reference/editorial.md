...
* Time complexity: $O(n)$
    ...
    The `findPath` method is called twice, once for the path from the LCA to `startValue` and once for the path to `destValue`. Each call can traverse up to the height of the tree, which is $n$ in the worst case (e.g., a skewed tree), making the total time complexity for both calls $O(n) +$\mathcal{O}(n)$= O(n)$.
...
- Define `findPath` with parameters: `node`, `targetValue`, and `path`.
- If `node` is `null`, return `false`.
- If $\text{node.val} = targetValue$, return `true`.
