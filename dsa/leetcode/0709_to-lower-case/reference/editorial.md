...
The first one, $\text{is}_{upper}$, simply checks if the character is between `A` and `Z` characters.

The second one, $\text{to}_{lower}$, should convert the uppercase letter into the corresponding lowercase one.
