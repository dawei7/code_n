...
The core task in **Lexicographical Numbers** is to an integer `n`, return all the numbers in the range `[1, n]` sorted in lexicographical order. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
