...
When a passenger arrives, they will wait in line for the next available bus. You can get on a bus that departs at `x` minutes if you arrive at `y` minutes where $y \le x$, and the bus is not full. Passengers with the **earliest** arrival times get on the bus first.
...
- $m = \text{passengers.length}$

- $1 \le n, m, capacity \le 10^{5}$
