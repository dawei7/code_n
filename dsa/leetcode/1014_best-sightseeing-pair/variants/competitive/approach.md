...
The core task in **Best Sightseeing Pair** is to an integer array `values` where values[i] represents the value of the $$i^{\text{th}}$$ sightseeing spot. Two sightseeing spots `i` and `j` have a **distance** $j - i$ between them. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
