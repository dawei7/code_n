...
The core task in **Longest Happy Prefix** is to A string is called a **happy prefix** if it is a **non-empty** prefix which is also a suffix (excluding itself). To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
