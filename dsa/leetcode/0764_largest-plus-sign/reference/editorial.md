...
With dynamic programming, it is either 0 if $\text{grid}[r][c]$ is zero, else it is `1` plus the count of the coordinate in the same direction.
...
For each square, we want $\text{dp}[r][c]$ to end up being the minimum of the 4 possible counts.  At the end, we take the maximum value in `dp`.
