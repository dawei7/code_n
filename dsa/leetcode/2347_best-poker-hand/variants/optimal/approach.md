...
The core task in **Best Poker Hand** is to an integer array `ranks` and a character array `suits`. You have `5` cards where the $$i^{\text{th}}$$ card has a rank of $\text{ranks}[i]$ and a suit of $\text{suits}[i]$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
