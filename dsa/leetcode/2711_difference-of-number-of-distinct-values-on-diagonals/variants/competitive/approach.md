...
The core task in **Difference of Number of Distinct Values on Diagonals** is to a 2D `grid` of size `m x n`, you should find the matrix `answer` of size `m x n`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
