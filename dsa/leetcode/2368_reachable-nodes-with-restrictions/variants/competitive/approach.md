...
The core task in **Reachable Nodes With Restrictions** is to There is an undirected tree with `n` nodes labeled from `0` to $n - 1$ and $n - 1$ edges. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
