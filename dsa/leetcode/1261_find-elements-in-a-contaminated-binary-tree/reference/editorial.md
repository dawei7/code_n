...
- For left child, we call $dfs(\text{currentNode.left}, currentValue * 2 + 1, seen)$.
- For right child, we call $dfs(\text{currentNode.right}, currentValue * 2 + 2, seen)$.
...
Once a node has been processed, we compute the values of its children based on the given rules. If the node has a left child, we use **rule 2** ($\text{n.val} * 2 + 1$) to compute its value and enqueue it for future processing. Similarly, if the node has a right child, we use **rule 3** ($\text{n.val} * 2 + 2$) and enqueue it as well. This ensures that by the time these children are processed, they already hold their correct recovered values.
...
- Pop the front element of the queue: $currentNode = \text{queue.pop}()$.
- Save the recovered value by adding `currentNode.val` into `seen`.
- If left child exists, overwrite its value $\text{currentNode.left}.val = \text{currentNode.val} * 2 + 1$ and then enqueue it.
- If right child exists, overwrite its value $\text{currentNode.right}.val = \text{currentNode.val} * 2 + 2$ and then enqueue it.
- For `find(target)` function:
- We return whether or not `seen` contains `target`: return `seen.contains(target)`.
