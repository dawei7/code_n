...
We can take advantage of the prefix sum array `presum` of the sorted `nums`, each value $\text{presum}[i]$ represents the sum of all numbers from $\text{nums}[0]$ to $\text{nums}[i]$. Therefore, we can get the sum of the range from `presum` in constant time, rather than iterating over `nums` which requires $O(n)$ time in the worst-case scenario.

![img](images/2389-ex2.png)

To build the prefix sum array for an array `nums`, we start from an empty array `presum`:
- $\text{presum}[0] = \text{nums}[0]$.
- $\text{presum}[1] = \text{nums}[0] + \text{nums}[1]$, which equals $\text{presum}[0] + \text{nums}[1]$.
- $\text{presum}[2] = \text{nums}[0] + \text{nums}[1] + \text{nums}[2]$, which equals $\text{presum}[1] + \text{nums}[2]$.
- ...

We can tell that all the terms $\text{presum}[i]$ follow $\text{presum}[i] = presum[i - 1] + \text{nums}[i]$ apart from the first term $\text{presum}[0] = \text{nums}[0]$. Therefore, we only need to iterate over `nums` once to build its prefix sum array `presum`. Moreover, since we don't need the original array `nums` once we have `presum`, thus we can build `presum` by modifying `nums` in-place to save some space:

- $\text{nums}[0] = \text{nums}[0]$.
- $\text{nums}[1] = \text{nums}[1] + \text{nums}[0]$.
- $\text{nums}[2] = \text{nums}[2] + \text{nums}[1]$.
- $\text{nums}[3] = \text{nums}[3] + \text{nums}[2]$.
- ...
