...
Suppose the number we added is $x$ then, the ranges `[1, miss)` and $[x, x + miss)$ are both covered. And since we know that $x \le miss$, the two ranges will cover the range $[1, x + miss)$. We want to choose $x$ as large as possible so that the range can cover as large as possible. Therefore, the best option is $x = miss$.
...
* While n is not covered yet
  * if the current element $\text{nums}[i]$ is less than or equal to `miss`
* extends the range to $[1, miss + \text{nums}[i])$
* increase `i` by 1
  * otherwise
...
| iteration | `miss` | covered range | `i` | $\text{nums}[i]$ | patches | comment  |
|:---------:|:------:|:-------------:|:-:|:-------:|:-------:|----------|
|     0     |    1   |     [1, 1)    | 0 |    1    |    0    |          |
|     1     |    2   |     [1, 2)    | 1 |    2    |    0    |          |
|     2     |    4   |     [1, 4)    | 2 |    3    |    0    |          |
