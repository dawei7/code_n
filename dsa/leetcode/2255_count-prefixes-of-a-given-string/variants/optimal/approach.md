...
The core task in **Count Prefixes of a Given String** is to a string array `words` and a string `s`, where $\text{words}[i]$ and `s` comprise only of **lowercase English letters**. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
