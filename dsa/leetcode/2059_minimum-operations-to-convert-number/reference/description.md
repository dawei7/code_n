...
If $0 \le x \le 1000$, then for any index `i` in the array ($0 \le i < \text{nums.length}$), you can set `x` to any of the following:

- $x + \text{nums}[i]$

- $x - \text{nums}[i]$

- $x ^ \text{nums}[i]$ (bitwise-XOR)
...
- $-10^{9} \le \text{nums}[i], goal \le 10^{9}$
