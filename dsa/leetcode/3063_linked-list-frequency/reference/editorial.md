...
Since the array is zero-indexed, the frequency of `1` is stored at $\text{frequencies}[0]$, the frequency of `2` is stored at $\text{frequencies}[1]$, and the frequency of `100` is stored at $\text{frequencies}[99]$. We will initialize `frequencies` to size $10^5$ because the maximum element in the linked list is guaranteed to be between $1$ and $10^5$ inclusive according to the constraints. To calculate the frequencies, we traverse the linked list, incrementing the frequency of each element by `1`.
...
4. Loop through the nodes in the linked list while $current \neq null$:
...
- If $\text{frequencies}[index]$ is greater than `0`:
- Set `current.next` to a new ListNode with the value $\text{frequencies}[index]$ to add a new frequency to the end of the new linked list.
