...
#### Algorithm
1. Create a sorted copy of the original array `sortedNums`.
2. Iterate through all possible rotation offsets (`rotationOffset`) from `0` to `n-1`:
   - `rotationOffset` represents the number of positions the array is rotated.

3. For each `rotationOffset`, compare the original array with a sorted version of itself:
...
- Time Complexity: $O(n^2)$
    ...
    The algorithm creates a sorted version of the array, which takes $O(n \log n)$ time. After sorting, it checks all possible rotations by iterating through the array and comparing elements for each rotation, which takes $O(n)$ for each rotation. Hence, the overall time complexity is $O(n \log n) +$\mathcal{O}(n^2)$= O(n^2)$.
...
2. Count the number of inversions (pairs where $\text{nums}[i] > nums[i + 1]$) in the array:
...
3. Compare $nums[n - 1]$ with $\text{nums}[0]$. If $\text{nums}[0] < nums[n - 1]$, increment the inversion count.
