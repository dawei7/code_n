...
Let $T=\sum \texttt{subtasks\_count}$ be the total number of valid task-subtask pairs. The recursive CTE generates exactly $T$ rows, taking $O(T)$ logical work and $O(T)$ CTE storage.
