...
We append the letter pointed to by pointer `i` i.e., $\text{word1}[i]$, and increment `i` by `1` to point to the next letter of `word1`. Because we need to add the letters in alternating order, next we append $\text{word2}[j]$ to `result`. We also increase `j` by `1`.
...
4. While `i < m || j < n`:
- If `i < m`, it means that we have not completely traversed `word1`. As a result, we append $\text{word1}[i]$ to `result`. We increment `i` to point to next index of `word1`.
- If `j < n`, it means that we have not completely traversed `word2`. As a result, we append $\text{word2}[j]$ to `result`. We increment `j` to point to next index of `word2`.
5. Return `result`.
...
Let `i` be the pointer that we'll use. We begin with $i = 0$ and progress to the size of the longer word between `word1` and `word2`, i.e., till $i = max(\text{word1.length}(), \text{word2.length}())$.

As we progress to the size of a longer word, we check each time if `i` points to an index that is in bounds of the words or not. If `i < word1.length()`, we append $\text{word1}[i]$ to `result`. Similarly if `i < word2.length()`, we append $\text{word2}[i]$ to results.
...
- If `i < m`, it means that we have not completely traversed `word1`. As a result, we append $\text{word1}[i]$ to `result`.
- If `i < n`, it means that we have not completely traversed `word2`. As a result, we append $\text{word2}[i]$ to `result`.
4. Return `result`.
