...
The core task in **Minimum Distance Excluding One Maximum Weighted Edge** is to a positive integer `n` and a 2D integer array `edges`, where $\text{edges}[i] = [u_{i}, v_{i}, w_{i}]$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
