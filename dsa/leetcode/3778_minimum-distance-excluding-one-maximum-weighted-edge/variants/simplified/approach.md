# Simplified Approach - Minimum Distance Excluding One Maximum Weighted Edge
...
## Complexity detail
- **Time Complexity**: $O((N + E) log N)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(N + E)$ — Utilizes standard auxiliary storage proportional to the active computation state.
