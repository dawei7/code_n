## General
**Competitive Approach — Second Degree Follower**

The query executes a structured relational pipeline for **Second Degree Follower**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
