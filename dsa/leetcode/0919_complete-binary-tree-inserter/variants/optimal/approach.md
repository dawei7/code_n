## General
**Optimal Approach — Complete Binary Tree Inserter**

The solution employs Binary Search over a sorted range or search space, continuously halving the candidate window. It utilizes Double-ended queue (`collections.deque`) for efficient $O(1)$ element insertion and removal, Binary Tree node traversal (`val`, `left`, `right`) to maintain optimal runtime bounds.
