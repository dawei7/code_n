...
- On the left side of the peak, there are $i + 1$ elements from $\text{nums}[0] to \text{nums}[i]$. Therefore, the number of elements to remove on the left side is $i + 1 - L1$.
- On the right side, there are $N - i$ elements from $\text{nums}[i] to nums[N - 1]$. The number of elements to remove on the right side is $N - i - L2$.
...
To find these lengths, we use dynamic programming, similar to the approach in [300. Longest Increasing Subsequence](https://leetcode.com/problems/longest-increasing-subsequence/solution/). First, we pass through the array from left to right to compute the longest increasing subsequence for each index `i`. For each element $\text{nums}[i]$, we compare it with all previous elements $\text{nums}[j]$ where `j < i`. If $\text{nums}[i] > \text{nums}[j]$, we update the subsequence length at `i` with:

> $\text{\text{lisLength}[i] = max(\text{lisLength}[i], \text{lisLength}[j] + 1)}$

Next, we perform a right-to-left pass to calculate the longest decreasing subsequence starting at each index `i`. For each element $\text{nums}[i]$, we compare it with all subsequent elements $\text{nums}[j]$ where `j > i`. If $\text{nums}[i] > \text{nums}[j]$, we update with:
> $\text{\text{ldsLength}[i] = max(\text{ldsLength}[i], \text{ldsLength}[j] + 1)}$
...
> $\text{removals = N + 1 − \text{lisLength}[i] − \text{ldsLength}[i]}$
...
- If $\text{nums}[i] > \text{nums}[j]$, update $\text{lisLength}[i]$ as $max(\text{lisLength}[i], \text{lisLength}[j] + 1)$.
3. Calculate LDS (Longest Decreasing Subsequence):
- For each index `i`, iterate through all indices `j` after `i`.
- If $\text{nums}[i] > \text{nums}[j]$, update $\text{ldsLength}[i]$ as $max(\text{ldsLength}[i], \text{ldsLength}[j] + 1)$.
4. Determine minimum removals:
- For each index `i`, if both $\text{lisLength}[i] > 1$ and $\text{ldsLength}[i] > 1$ (i.e., it's a valid mountain peak), calculate the minimum removals required as $N - (\text{lisLength}[i] + \text{ldsLength}[i] - 1)$.
...
- Update the $\text{lisLen}[i]$ to the size of `lis`.
