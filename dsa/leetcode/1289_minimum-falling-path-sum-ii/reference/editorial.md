...
In other words, if $\text{grid}[row][col]$ is chosen, then $grid[row + 1][col]$ and $grid[row - 1][col]$ cannot be chosen.
...
- If $row = n - 1$, then it means that we have reached the last row. Thus, the only path from this point is the value of the cell itself. Thus, we return $\text{grid}[row][col]$.

- Otherwise, we have to select $\text{grid}[row][col]$. Now from this point, we have $n - 1$ choices for selecting an element from the next row.
...
2. Define a recursive function `optimal`. It takes the row number `row`
   ...
   with non-zero shifts, starting from cell $\text{grid}[row][col]$.
    ...
    a. If $row = n - 1$, then there is no row left to select.
       Thus, we return $\text{grid}[row][col]$.

    b. Otherwise, initialize a variable $\text{next}_{minimum}$ with $\text{INT}_{MAX}$.
...
![Recursion Tree](images/Slide1.PNG)
...
- The value of the cell that is not yellow-highlighted will be calculated by calling `optimal` for $N - 1$ columns of the third row.
        ...
        ![after_opt_0_1](images/Slide3_1.PNG)
...
- Ensuring loop invariant of `next_min1_c`, `next_min2_c`, $\text{next}_{min1}$, and $\text{next}_{min2}$.
