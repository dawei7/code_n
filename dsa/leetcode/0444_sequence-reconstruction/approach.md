...
The central difficulty is that `sequences` does not directly give one complete ordering. Each row gives only some ordering facts. For example, `[1, 4, 7]` says that `1` must appear before `4`, and `4` must appear before `7`; it does not say where any other value belongs. The task is therefore to combine all of these partial facts and decide whether they force exactly one shortest supersequence, namely `nums`.

The optimal solution turns those ordering facts into a directed graph and then asks whether that graph has exactly one topological ordering.

**Why a directed graph represents the subsequences**
...
**Why topological order captures a shortest supersequence**
...
**Why the compact final check works here**
...
Let $V = n$ be the number of values and let $E$ be the total number of adjacent-pair occurrences across all rows. If the total number of listed elements is $S$, then $E \le S$, because a row of length $k$ contributes $k-1$ edges.

Building `g` and `indeg` visits every adjacent pair once, which costs $O(E)$ time. Creating the initial deque examines all $V$ vertices, which costs $O(V)$. During the topological process, each vertex is removed at most once, and every stored edge is traversed exactly once when its source is processed. The total time is therefore $O(V+E)$.

The adjacency lists store $E$ edge occurrences. The indegree array, deque, and outer adjacency-list structure use $O(V)$ additional entries. Total auxiliary space is $O(V+E)$.

Repeated edges do not change these bounds: they are counted as separate occurrences in $E$. The algorithm does not construct any candidate supersequence, enumerate possible orders, or compare every pair of values.
