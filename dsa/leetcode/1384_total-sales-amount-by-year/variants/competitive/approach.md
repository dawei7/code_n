## General
**Competitive Approach — Total Sales Amount by Year**

The query executes a structured relational pipeline for **Total Sales Amount by Year**. It uses Relational JOIN operations to correlate records across tables.
...
## Complexity detail
- **Time Complexity**: $O(P + S + R)$ — Operation count proportional to input scale.
- **Space Complexity**: $O(P + S)$ — Auxiliary memory allocation bound.
