...
Suppose we chose a set of deletion indices `answer` such that after deletions, the final array has its elements in **lexicographic** order (i.e., $\text{strs}[0] \le \text{strs}[1] \le \text{strs}[2] \le ... \le strs[n - 1]$). Return *the minimum possible value of* `answer.length`.
