...
The core task in **Strictly Palindromic Number** is to An integer `n` is **strictly palindromic** if, for **every** base `b` between `2` and $n - 2$ (**inclusive**), the string representation of the integer `n` in base `b` is **palindromic**. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
