...
3. Iterate over the rows from `0` to $N - 1$ and for each row `i` add the moves required to keep a rook in this row as $abs(i - \text{rooks}[i][0])$
4. Sort the list `rooks` in ascending order of their column number.
5. Iterate over the column from `0` to $N - 1$ and for each column `i` add the moves required to keep a rook in this column as $abs(i - \text{rooks}[i][1])$
6. Return `minMoves`.
...
5. Iterate over the rows/columns from `0` to $N - 1$ and for each `i`, add the rows moves required as $\text{row}[i] - 1$ and column moves required as $\text{col}[i] - 1$ to `rowMinMoves` and `colMinMoves` respectively.
6. Return `minMoves`.
