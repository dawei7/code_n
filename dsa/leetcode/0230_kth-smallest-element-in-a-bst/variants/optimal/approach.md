...
The stack procedure is exactly inorder traversal, and BST inorder order is ascending. Decrementing once per popped node counts ranks in that order, so the node returned when the counter reaches zero has the requested rank.
...
- **Skewed trees and extreme ranks:** a skewed tree has $h = n$; $k = 1$ and $k = n$ select the extreme values.
