...
1. Create an adjacency list `adj` that contains a list of pairs of integers such that $\text{adj}[node]$ contains all of node's neighbors in the form of `(neighbor, weight)` where `neighbor` is the neighboring node of `node` and `weight` denotes the weights of the edge that connects `node` and `neighbor`.
...
- Use $\text{adj}[node]$ to iterate over the neighboring nodes and edges. For each `edge`, update the `answer` to the minimum of `answer` and `edge` weight. If the node at the other end of the `edge` is not visited, we call `dfs` with that node recursively.
4. Return `answer`.
