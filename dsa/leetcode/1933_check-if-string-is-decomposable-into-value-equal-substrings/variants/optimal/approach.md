...
The core task in **Check if String Is Decomposable Into Value-Equal Substrings** is to A **value-equal** string is a string where **all** characters are the same. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
