...
The core task in **Check if String Is Decomposable Into Value-Equal Substrings** is to A **value-equal** string is a string where **all** characters are the same. Instead of using nested loops that inspect every pair in $O(n^2)$ time, this algorithm uses the **Two-Pointer technique**. We place two markers (pointers)—typically one at the start (`left`) and one at the end (`right`) of a sorted array—and move them toward each other based on clear comparison rules, completing the search in a single efficient pass.
