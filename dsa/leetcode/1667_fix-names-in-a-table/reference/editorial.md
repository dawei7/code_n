...
This task essentially boils down to updating a column of data in a table. The goal is to normalize the name column in the Users table, so that only the first letter is uppercase and the rest are lowercase, and sort the resulting values by $\text{user}_{id}$.
...
1. $SUBSTRING(\text{column}_{name}, start, length)$: This extracts a substring from a column's values, starting at the specified start position, and up to the specified length.
