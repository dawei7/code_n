...
If we observe closely, we are first traversing to the deepest node and then backtracking to the parent node to find the minimum depth for it; hence, this process is actually Depth-First Search (DFS).
...
3. If the left child of `root` is `NULL`, then we should return `1` + minimum depth for the right child of the root node, which is $1 + dfs(\text{root.right})$.
4. If the right child of `root` is `NULL`, then we should return `1` + minimum depth for the left child of the root node, which is $1 + dfs(\text{root.left})$.
5. If both child are non-null, then return $1 + min(dfs(\text{root.left}), dfs(\text{root.right}))$.
