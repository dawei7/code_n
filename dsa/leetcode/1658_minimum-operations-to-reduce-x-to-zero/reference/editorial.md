...
![Figure 1.1](images/5602_1_1.drawio.svg)
...
![Figure 1.2](images/5602_1_2.drawio.svg)
...
*Step 2:* Initialize two pointers `left` and `right` to `0`. Initialize an integer `current` to represent the sum from $\text{nums}[left]$ to $\text{nums}[right]$, inclusively. Initialize an integer `maxi` to record the maximum length that sums up to $total - x$.
...
![Figure 2.1](images/5602_2_1.drawio.svg)
...
![Figure 2.2](images/5602_2_2.drawio.svg)
...
*Step 2:* Initialize two pointers `left` and `right` to `0`. Initialize an integer `current` to represent the sum from $\text{nums}[0]$ to `nums[left-1]` and from `nums[right+1]` to $\text{nums}[last]$. Initialize an integer `mini` to record the minimum length that sums up to `x`
