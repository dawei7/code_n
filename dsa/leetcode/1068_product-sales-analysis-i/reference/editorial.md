...
Since we only need to report the columns $\text{product}_{name}$, `year`, and `price`, we create another DataFrame containing only these required columns. Double brackets are used to extract a subset of data and yield a new DataFrame.
