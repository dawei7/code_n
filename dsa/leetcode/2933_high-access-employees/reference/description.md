...
- $\text{access}_{times}[i].length = 2$

- $1 \le \text{access}_{times}[i][0].length \le 10$

- $\text{access}_{times}[i][0]$ consists only of English small letters.

- $\text{access}_{times}[i][1].length = 4$

- $\text{access}_{times}[i][1]$ is in 24-hour time format.

- $\text{access}_{times}[i][1]$ consists only of `'0'` to `'9'`.