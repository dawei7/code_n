...
We trace the dynamic Array, Math, Sliding Window, Sorting sliding window on a representative input instance.
...
The objective for **Moving Stones Until Consecutive II** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
