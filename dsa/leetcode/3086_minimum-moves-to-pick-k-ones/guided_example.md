# Guided Example: Minimum Moves to Pick K Ones

We trace the dynamic Array, Greedy, Sliding Window, Prefix Sum sliding window on a representative input instance.
...
The objective for **Minimum Moves to Pick K Ones** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
