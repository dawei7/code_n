...
> Note: In the provided CPP solution, numbers are erased from a map after insertion if they are deemed unnecessary. However, instead of using `iterator erase(iterator first, iterator last)`, which operates in $O(1)$ time, we opt for $\text{size}_{type} erase(const Key\& key)$, resulting in $O(log n)$ complexity. A micro optimization would be to obtain iterator positions from the insertion, allowing for direct erasure in constant time.
