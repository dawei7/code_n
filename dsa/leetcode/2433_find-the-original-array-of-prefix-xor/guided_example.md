...
The objective is to compute `[5, 7, 2, 3, 2]` from `{"pref": [5, 2, 0, 3, 1]}` while avoiding redundant calculations and unnecessary overhead.
...
### Step 1: Use XOR cancellation to undo a prefix
...
|---|---|---|---|
| Input Slice | `{"pref": [5, 2, 0, 3, 1]}` | Initial boundary validation | Setup completed |
| Active State | Base configuration | Apply initial state rule | Initialized |

---

### Step 2: How `pairwise` creates exactly the required neighbors
...
### Step 3: Why the recovered array is unique
...
|---|---|---|---|
| Target Output | Accumulator state | Synthesize final result | `[5, 7, 2, 3, 2]` |
...
|---|---|---|---|
| Initialization | Initial input `{"pref": [5, 2, 0, 3, 1]}` | Set up baseline structures | Holds |
| Transition | Active elements evaluated | Apply invariant transition rule | Maintained |
| Finalization | Complete sequence processed | Extract `[5, 7, 2, 3, 2]` | Verified |
...
- **Time Complexity:** $O(n)$. Let $n$ be the length of `pref`. Creating `[0] + pref` copies $n$ references into a new $n+1$ element list, taking $O(n)$ time and space. `pairwise` then yields $n$ adjacent pairs, and the comprehension performs one XOR per pair, taking another $O(n)$ time.
