...
There exists an array `arr` of $N$ integers and we are given another array `pref` in which the `ith` index has the value as $\text{pref}[i] = \text{arr}[0] ^ \text{arr}[1] ^ ... ^ \text{arr}[i]$. We need to return the original array `arr` which will generate the array `pref` using the above XOR operations.

An important property of XOR that we can use to solve this problem is $a ^ a = 0$, i.e. the `XOR` of two same integers is equal to `0`. The value in the array `pref` at index `i` is equal to $\text{arr}[0] ^ \text{arr}[1] ^ ... ^ \text{arr}[i]$, and the value in the index $i + 1$ is equal to $\text{arr}[0] ^ \text{arr}[1] ^ ... ^ \text{arr}[i] ^ arr[i + 1]$. Now, if we perform the `XOR` operation with $\text{pref}[i]$ and $pref[i + 1]$ the expression would be as shown below.
...
In the above XOR expression, each index has two terms except the $arr[i + 1]$, and hence all other terms will be evaluated to `0`. Thus the final expression will evaluate to $arr[i + 1]$. This happened because the terms $\text{pref}[i]$ and $pref[i + 1]$ have the same value expression except that $pref[i + 1]$ has one more extra term which is the only remained term when XOR operation is performed.

Therefore, the $i-\text{th}$ index in the array `arr` can be found by taking the XOR of $\text{pref}[i]$ and $pref[i - 1]$, also for index $i = 0$ the value $\text{arr}[0]$ will be equal to the $\text{pref}[0]$ as the value expression for $\text{pref}[0]$ is equal to $\text{arr}[0]$ itself.

**Algorithm**

1. Initialize an empty array `arr` of size $N$, this will store the final result.
2. Assign $\text{arr}[0]$ as $\text{pref}[0]$, because the XOR of all numbers on and before the `0th` index will be the same as the number itself.
3. Iterate over the indices from `1` to $N - 1$, and for each index `i`:
1. Assign $\text{pref}[i] ^ pref[i - 1]$ to $\text{arr}[i]$.
4. Return `arr`.
