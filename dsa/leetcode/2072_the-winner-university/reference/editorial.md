...
New York DataFrame ($\text{new}_{york}$):
...
1. Counting Excellent Students
   ...
   We use the SQL `COUNT(*)` function within a subquery for each university. The $WHERE score \ge 90$ clause ensures we only count the students with scores of 90 or above.
