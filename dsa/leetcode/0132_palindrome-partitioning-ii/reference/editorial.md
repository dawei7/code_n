...
![img](images/Approach1_iteration1_finalConfig.png)
...
![img](images/Approach1_iteration2.png)
...
![img](images/Approach1_iteration2_finalConfig.png)
...
![img](images/Approach1_bestSolution.png)
...
![img](images/Approach1_Recursion_tree.png)
...
Additionally, to check if a substring is a palindrome or not we must iterate $O(N/2)$ times within each recursive call.
   ...
   This gives us total time complexity of $O(2 ^ N) \cdot$\mathcal{O}(N/2)$= O(2 ^ N \cdot N)$.
...
![img](images/Approach2_Recursion_tree.png)
...
-  To record the results of computation on every substring, we must store the substring and its result.
   ...
   Now, instead of storing the substring, we can just store its `start` and `end` index. We will use a 2-dimensional array `memoCuts`, where the value of  $\text{memoCuts}[start][end]$ would give the minimum number of cuts for a substring starting at index start and ending at index `end`.
...
- After computing the result, store the result in $\text{memoCuts}[start][end]$.
...
* Time Complexity: $O(N^2 \cdot N)$, where `N` is the length of string `s`.
   ...
   This gives us total time complexity as, $O(N ^ 2) \cdot$\mathcal{O}(N/2)$= O(N ^ 2 \cdot N)$
...
Thus, we only need to track and update the `start` index when finding the minimum number of cuts. The $\text{memoCuts}[start]$ will determine the minimum number of cuts for a substring starting at index `start` and ending at index $length(s) - 1$.

**Algorithm**

The algorithm is similar as in _Approach 2_ with a minor difference:

- Store the results of a substring starting at index `start` in $\text{memoCuts}[start]$.

- Before calculating the result of any substring starting at index `start` and ending at index `end`, check if $\text{memoCuts}[start]$ already contains the computed result.
...
2. Build a one-dimensional array `cutsDp` to store the results of subproblems. $\text{cutsDp}[i]$ stores the minimum number of cuts for a substring ending at index `i`.
...
3. Calculating the minimum number of cuts is similar to the Memoization approach.
   ...
   Minimum cuts for substring $\text{s.substring}(0, start - 1)$ is equivalent to finding the result for substring ending at index $start - 1$ which can be given by $cutsDp[start - 1]$. So, we can say that,

       Minimum cuts for s.substring(start, end) = 1 + cutsDp[start - 1]

   In the end, we will store the results of the current calculation at $\text{cutsDp}[end]$ as every chosen substring ends at index `end`.
...
* Time Complexity: $O(N^2)$, where `N` is the length of string `s`.
   ...
   We are iterating $N \cdot N$ times to build the `palindromeDp` array and $N \cdot N$ times to find the minimum cuts in a nested for-loop. This gives us a total time complexity of $O(N \cdot N) +$\mathcal{O}(N \cdot N)$= O(N \cdot N)$.

* Space Complexity: $O(N ^ 2)$, as we are using a 2-dimensional arrays `palindromeDp` of size $N \cdot N$ and a 1-dimensional array `cutsDp` of size $N$. Thus, the space complexity can be given by, $O(N \cdot N) +$\mathcal{O}(N)$= O(N \cdot N)$.
...
- Check if the substring is a palindrome using the previously calculated values in `palindromeDp`.
   If the substring is a palindrome, update the results in the `palindromeDp` array. Keep doing this to find the minimum cut for the substring ending at index `end` and save it in $\text{cutsDp}[end]$.
...
* Space Complexity: $O(N ^ 2)$, as we are using two 2-dimensional arrays `palindromeDp` and  1-dimensional array `cutsDp` of size $N \cdot N$. Thus the space complexity can be given by, $O(N \cdot N) +$\mathcal{O}(N)$= O(N \cdot N)$ .
---
...
![img](images/Approach5_figure1.png)
...
1. Build an array `cutsDp` to store the results of every computation. Initially, the value of $\text{cutsDp}[i]$ would be the maximum possible number of cuts till index `i`, which is equivalent to `i`.

   As we progress, after every computation of minimum cuts for a substring ending at index `i`, the value of $\text{cutsDp}[i]$ would be updated.
...
![img](images/Approach5_figure2.png)
...
3. The substring will be expanded i.e `start` index would be _incremented_ and `end` index _decremented_ until the characters at those indices are the same.
    ...
    The cuts calculated so far for substring ending at `end` index can be given by $\text{cutsDp}[end]$ and minimum cuts for a substring ending at $start - 1$ index can be given by $cutsDp[start - 1]$.

![img](images/Approach5_figure3.png)

4. After every computation, update $\text{cutsDp}[end]$ with a new minimum value.
