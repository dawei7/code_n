...
There are a few points you should notice from the code above:
 ...
 1. We use the built-in function `isUpperCase` (in `Java`) and `isupper` (in `Python`) to check whether a char is upper case. You can also use the [ASCII](https://en.wikipedia.org/wiki/ASCII) to do that. Just use something like $\text{word.charAt}(i) \ge 'A' \&\& \text{word.charAt}(i) \le 'Z'$.
