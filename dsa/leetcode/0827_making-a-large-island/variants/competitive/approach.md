...
The core task in **Making A Large Island** is to an `n x n` binary matrix `grid`. You are allowed to change **at most one** `0` to be `1`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
