...
First, we initialize a DSU structure where each land cell is its own representative (each cell is its own island), meaning that  $\text{parent}[node] = node$ for every land cell node. We also initialize the `islandSize` array, where each island starts with a size of 1 (since each island is just one land cell initially). This is represented as $\text{islandSize}[node] = 1$.
...
During the merging step, we also update the size of the new island (set) by adding the size of the two merged islands. This is done by maintaining the `islandSize` array, where $\text{islandSize}[node]$ is updated after each union operation.
...
- Set $\text{parent}[node] = node$, meaning each node is initially its own parent.
- Set $\text{islandSize}[node] = 1$, indicating each island starts with size 1.
...
- Attach the smaller island to the larger one:
      - If the island of `nodeA` is smaller, set $\text{parent}[rootA] = rootB$ and update the size of `rootB`’s island.
      - If the island of `nodeB` is smaller, set $\text{parent}[rootB] = rootA$ and update the size of `rootA`’s island.
