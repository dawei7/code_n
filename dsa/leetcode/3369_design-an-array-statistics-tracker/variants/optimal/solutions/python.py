...
def solve(operations: list[str], arguments: list[list[int]]) -> list[int | None]:
    tracker: StatisticsTracker | None = None
    output: list[int | None] = []

    for operation, args in zip(operations, arguments):
        if operation == "StatisticsTracker":
            tracker = StatisticsTracker()
            output.append(None)
        elif operation == "addNumber":
            assert tracker is not None
            tracker.addNumber(args[0])
            output.append(None)
        elif operation == "removeFirstAddedNumber":
            assert tracker is not None
            tracker.removeFirstAddedNumber()
            output.append(None)
        elif operation == "getMean":
            assert tracker is not None
            output.append(tracker.getMean())
        elif operation == "getMedian":
            assert tracker is not None
            output.append(tracker.getMedian())
        elif operation == "getMode":
            assert tracker is not None
            output.append(tracker.getMode())
        else:
