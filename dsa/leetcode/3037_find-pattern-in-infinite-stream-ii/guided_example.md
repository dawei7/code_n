# Guided Example: Find Pattern in Infinite Stream II

We trace the dynamic Array, Sliding Window, Rolling Hash, String Matching, Interactive, Hash Function sliding window on a representative input instance.

- **Input:** `{"stream": {"bits": [1, 1, 1, 0, 1]}, "pattern": [0, 1]}`
- **Required output:** `3`
...
The objective for **Find Pattern in Infinite Stream II** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
