...
The core task in **Substrings of Size Three with Distinct Characters** is to A string is **good** if there are no repeated characters. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
