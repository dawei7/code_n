# Simplified Approach - Sum of Beautiful Subsequences
...
## Complexity detail
- **Time Complexity**: $O(V log log V + n sqrt(V) + T log V)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(V log V)$ — Utilizes standard auxiliary storage proportional to the active computation state.
