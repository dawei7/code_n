...
- For each index `i` from 0 to the length of `nums`:
  - Calculate the target value as $\text{nums}[i] + 2*k$, which represents the maximum possible equal value achievable for any element in the range.
  - Find the `upperBound` index where $\text{nums}[upperBound]$ is the largest element less than or equal to the target value.
...
- Time complexity: $O(n \cdot \log n)$
    ...
    Therefore, the overall time complexity is $2 \cdot$\mathcal{O}(n \cdot \\log n)$= O(n \cdot \log n)$.
...
- Time complexity: $O(n + \text{maxValue})$
    ...
    Thus, the overall time complexity is $2 \cdot$\mathcal{O}(n)$+$\mathcal{O}(\text{maxValue})$= O(n + \text{maxValue})$.
