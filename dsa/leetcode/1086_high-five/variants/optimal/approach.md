...
Sourced from `doocs/leetcode` (or refined to expert standard) in method `highFive`. This implementation is chosen for its exceptional readability, idiomatic Python 3 constructs, and clear structural separation of concerns suitable for technical software engineering interviews.

## Complexity detail
- **Time Complexity**: $O(N+S\log S)$ — Operation count proportional to input scale.
