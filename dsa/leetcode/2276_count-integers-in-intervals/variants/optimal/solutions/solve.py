    ...
    def _cover(
        ...
        if query_left <= middle:
            if node.left is None:
                node.left = _Node()
            self._cover(node.left, low, middle, query_left, query_right)
        if middle < query_right:
            if node.right is None:
                node.right = _Node()
            self._cover(node.right, middle + 1, high, query_left, query_right)
...
def solve(
    ...
    for operation, values in zip(operations, arguments, strict=True):
        if operation == "CountIntervals":
            intervals = CountIntervals()
            results.append(None)
            continue
        if intervals is None:
            raise ValueError("CountIntervals must be constructed first")
        results.append(getattr(intervals, operation)(*values))
