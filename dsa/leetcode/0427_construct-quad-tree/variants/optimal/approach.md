...
The core task in **Construct Quad Tree** is to a $n * n$ matrix `grid` of `0's` and `1's` only. We want to represent `grid` with a Quad-Tree. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
