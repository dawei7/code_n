...
The core task in **Maximum Manhattan Distance After K Changes** is to a string `s` consisting of the characters `'N'`, `'S'`, `'E'`, and `'W'`, where $s[i]$ indicates movements in an infinite grid:. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
