...
- Let's use numbers from 0 to 3 to mark the directions: $north = 0$, $east = 1$, $south = 2$, $west = 3$. In the array `directions` we could store corresponding coordinates changes, _i.e._ $\text{directions}[0]$ is to go north, $\text{directions}[1]$ is to go east, $\text{directions}[2]$ is to go south, and $\text{directions}[3]$ is to go west.
...
- If the current instruction is to move, we simply update the coordinates: $x += \text{directions}[idx][0]$, $y += \text{directions}[idx][1]$.

- After one cycle we have everything to decide.
It's a limit cycle trajectory if the robot is back to the center: $x = y = 0$ or if the robot doesn't face north: $idx \neq 0$.
...
Let's use numbers from 0 to 3 to mark the directions: $north = 0$, $east = 1$, $south = 2$, $west = 3$. After one cycle the robot is facing direction $k \neq 0$.
