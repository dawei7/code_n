...
The core task in **Robot Bounded In Circle** is to On an infinite plane, a robot initially stands at `(0, 0)` and faces north. Note that:. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
