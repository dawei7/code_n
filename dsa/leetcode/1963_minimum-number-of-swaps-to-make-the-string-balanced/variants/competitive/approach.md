...
The core task in **Minimum Number of Swaps to Make the String Balanced** is to a **0-indexed** string `s` of **even** length `n`. The string consists of **exactly** $n / 2$ opening brackets `'['` and $n / 2$ closing brackets `']'`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
