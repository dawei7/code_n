# Guided Example: Maximum Subarray With Equal Products

We trace the dynamic Array, Math, Sliding Window, Enumeration, Number Theory sliding window on a representative input instance.
...
The objective for **Maximum Subarray With Equal Products** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
