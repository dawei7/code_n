...
The core task in **Number of Smooth Descent Periods of a Stock** is to an integer array `prices` representing the daily price history of a stock, where $\text{prices}[i]$ is the stock price on the $$i^{\text{th}}$$ day. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
