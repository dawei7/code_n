## General
**Competitive Approach — Ads Performance**

The query executes a structured relational pipeline for **Ads Performance**. It uses `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(r+a\log a)$ — Operation count proportional to input scale.
