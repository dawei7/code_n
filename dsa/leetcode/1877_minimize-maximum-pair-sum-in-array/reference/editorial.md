...
3. Iterate over the array `nums` from index `0` to $\text{nums.length}() / 2 - 1$.
4. Get the sum of the current element and its corresponding pair $\text{nums}[i] + nums[\text{nums.length}() - 1 - i]$, and update `maxSum` if the sum is larger.
5. Return `maxSum`.
