...
class Solution:
    def getOrder(self, tasks: List[List[int]]) -> List[int]:
        ordered_tasks = sorted(
            (enqueue_time, processing_time, index) for index, (enqueue_time, processing_time) in enumerate(tasks)
        )
        available = []
        answer = []
        time = 0
        ...
        while next_task < len(ordered_tasks) or available:
            ...
            while next_task < len(ordered_tasks) and ordered_tasks[next_task][0] <= time:
