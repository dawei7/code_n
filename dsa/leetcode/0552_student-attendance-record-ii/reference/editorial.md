...
![all_combinations](images/Slide1.jpg)
...
We define a recursive function $\text{generate}_{combination}(n)$ that generates combinations of length `n`.
First, we choose a character for the current position (choose, `'P'`, `'A'`, and `'L'` one by one), and then we recursively get the combinations of length $n - 1$ using the function $\text{generate}_{combination}(n - 1)$ to make combinations of length `n`.
...
![recursion_image](images/Slide2.jpg)
...
![recursive_tree](images/Slide4.jpg)
