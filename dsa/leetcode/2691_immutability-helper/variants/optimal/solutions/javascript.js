...
function assignPath(proxy, path, value) {
    let target = proxy;
...
function solve(obj, mutations) {
    const helper = new ImmutableHelper(obj);
    return mutations.map(batch => helper.produce(proxy => {
        for (const assignment of batch) {
            assignPath(proxy, assignment.path, assignment.value);
        }
    }));
}
...
class Solution {
    solve(obj, mutations) {
        return solve(obj, mutations);
    }
}

module.exports = { ImmutableHelper, solve };
