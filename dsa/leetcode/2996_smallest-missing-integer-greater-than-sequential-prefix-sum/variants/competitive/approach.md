...
The core task in **Smallest Missing Integer Greater Than Sequential Prefix Sum** is to a **0-indexed** array of integers `nums`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
