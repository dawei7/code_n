...
What if the problem constraints were different and we needed to consider integer overflow when adding $\text{numbers}[low]$ and $\text{numbers}[high]$? In that case, to prevent an overflow error, we could cast our numbers from `int` data type to `long` data type before adding them together, e.g.: $long sum = \text{static}_{cast}<long>(\text{numbers}[low]) + \text{numbers}[high]$ for C++. Casting ensures that we will not get the overflow error since the signed `long` data type supports numbers up to $2^{63} - 1$. Alternatively, if we cannot use long integers, then we can check if $\text{numbers}[low] > (1 << 31) - 1 - \text{numbers}[high]$ at the beginning of each iteration. If this condition is true, then $\text{numbers}[low] + \text{numbers}[high]$ will result in integer overflow, and so we would move the larger index to the left.
...
Since the array is sorted, for each element $\text{numbers}[i]$ we can binary search for its complement $target - \text{numbers}[i]$ among the elements to its right (`numbers[i+1..n-1]`). If found, we return the two 1-indexed positions.
...
For each of the $n$ elements, we perform a binary search over at most $n - 1$ elements, which takes $O(\log n)$ time. Thus the total time complexity is $O(n \log n)$.

* Space complexity: $O(1)$.
We only use additional space to store two indices and the complement, so the space complexity is $O(1)$.