...
- `orders`: Input parameter (`List[List[str]]`).
...
- $1 \le \text{customerName}_{i}.length, \text{foodItem}_{i}.length \le 20$

- $\text{customerName}_{i}$ and $\text{foodItem}_{i}$ consist of lowercase and uppercase English letters and the space character.

- $\text{tableNumber}_{i}$is a valid integer between `1` and `500`.
