...
5. Base cases:
   - If we have traversed over all the houses i.e., $currIndex = m$, we will return the cost `0` if the `neighborhoodCount` is equal to `target`, otherwise we return the value $\text{MAX}_{COST}$ which is the maximum possible cost plus `1`.
   - If the number of neighborhoods `neighborhoodCount` is more than the target neighborhoods `neighborhoodCount > targetCount`, then the answer is not possible. Hence we return $\text{MAX}_{COST}$.
...
2. Iterate over house index `house` from `1` to $m - 1$, and neighborhoods count `neighborhoods` from `1` to the minimum of $(house + 1, target)$ (for `0` indexed `house`, $house + 1$ is the maximum number of neighborhoods possible)), and color for the current house, from `1` to `n`. For each of these iterations:
   ...
   - Initialize the cost for current parameters `currCost` to $\text{MAX}_{COST}$ which is the maximum possible cost plus `1`.
...
- Assign the value $currCost + costToPaint$ to $\text{memo}[house][neighborhoods][color - 1]$.
...
2. Iterate over house index `house` from `1` to $m - 1$,
   ...
   - Iterate over neighborhoods count `neighborhoods` from `1` to minimum of $(house + 1, target)$ and iterate over color for the current house `color` from `1` to `n`. For each iteration:
     ...
     - If the house at index `house` is already painted and the color is not the same as `color`, then continue, since we never repaint a painted house.
     - Initialize the cost for current options `currCost` to $\text{MAX}_{COST}$ which is the maximum possible cost plus `1`.
     - Iterate over the color option for the previous house `prevColor` from `1` to `n`, for each `prevColor`:
       ...
       - If the `color` and `prevColor` match, assign the minimum of `currCost` and $\text{prevMemo}[neighborhoods][color - 1]$ to `currCost`.
- Assign the cost to paint the current house at index `house` with `color` in the variable `costToPaint`.
- Assign the value $currCost + costToPaint$ to $\text{memo}[neighborhoods][color - 1]$.
