...
Let's use two pointers, `slow` and `fast`. While the slow pointer moves one step forward $slow = \text{slow.next}$, the fast pointer moves two steps forward $fast = \text{fast.next}.next$, _i.e._ `fast` traverses twice as fast as `slow`. When the fast pointer reaches the end of the list, the slow pointer should be in the middle.
...
Let's traverse the list starting from the middle node `slow` and its virtual predecessor `None`. For each current node, save its neighbors: the previous node `prev` and the next node $tmp = \text{curr.next}$.

While you're moving along the list, change the node's next pointer to point to the previous node: $\text{curr.next} = prev$, and shift the current node to the right for the next iteration: $prev = curr$, $curr = tmp$.
