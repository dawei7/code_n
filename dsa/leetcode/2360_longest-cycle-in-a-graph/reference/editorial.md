...
As we traverse the graph, suppose we need to return to `node2` from `node3` (there is an edge `node3 -> node2`) which has already been visited in this DFS traversal. It denotes the existence of a cycle. $\text{dist}[node3] - \text{dist}[node2]$ would give us the number of edges from `node2` to `node3` because we kept track of distance from the starting node `node1`. We also need to add one for the `node3 -> node2` edge that completes the cycle. As a result, this cycle would be $\text{dist}[node3] - \text{dist}[node2] + 1$ in length. This brings us to our solution.

We create a `answer` variable and initialize it to `-1`. We iterate over all the nodes and if the current node, `node`, is not visited, we start a DFS traversal. For each DFS traversal, we also create a new hash map `dist` and set $\text{dist}[node] = 0$.

In the DFS traversal, we check the `neighbor` of `node` using $neighbor = \text{edges}[node]$. If `neighbor` is not visited, we update $\text{dist}[neighbor] = \text{dist}[node] + 1$ and recursively perform the DFS traversal starting with `neighbor`.
...
If `dist` contains `neighbor`, it means we visited `neighbor` during the current DFS traversal itself and it is a formation of cycle. We update $answer = max(answer, \text{dist}[node] - \text{dist}[neighbor] + 1$ in this case.
...
2. Initialize another integer $n = \text{edges.length}$ which stores the number of nodes in the graph.
3. Create a `visit` array of length `n` to keep track of nodes that have been visited.
4. Iterate through all of the nodes and for each node `i` check if it is visited or not. If node `i` is not visited, create a hash map `dist` where `dist.get(x)` would store the distance of node `x` from starting node `i`. Begin the DFS traversal:
...
- Mark `node` as visited and get its neighbor `neighbor` using $\text{edges}[node]$.
- If `neighbor` exists and is not visited, we update $\text{dist}[neighbor] = \text{dist}[node] + 1$ and recursively call `dfs` passing `neighbor` as the node.
- Otherwise, if `neighbor` exists and is already visited, we check if it is present in `dist`. If it is present, it is a formation of cycle. We perform $answer = max(answer, \text{dist}[node] - \text{dist}[neighbor] + 1)$. Otherwise, if it not present in `dist`, we ignore it as it was visited in a previous DFS traversal.
5. Return `answer`.
...
- Get the neighbor, `neighbor`, of `node` using $\text{edges}[node]$. If $neighbor \neq -1$, we decrement $\text{indegree}[neighbor]$ by `1`.
- If $\text{indegree}[neighbor] = 0$, it means that `neighbor` behaves as a leaf node, so we push `neighbor` in the queue.
7. Iterate over unvisited nodes and for an unvisited node `i`:
- Mark node `i` as visited.
- Fetch neighbor, `neighbor`, of `i` using $\text{edge}[i]$ and create a variable `count` to count number of nodes in the cycle. Initialize $count = 1$ to count node `i` itself.
- Keep moving forward in the cycle until we reach node `i` ($neighbor \neq i$). Mark `neighbor` as visited and move to next neighbor $neighbor = \text{edges}[neighbor]$. Also, increment `count` by `1` for each node that is being visited in the cycle.
