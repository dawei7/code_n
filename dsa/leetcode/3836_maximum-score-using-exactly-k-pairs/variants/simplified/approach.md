# Simplified Approach - Maximum Score Using Exactly K Pairs
...
## Complexity detail
- **Time Complexity**: $O(N M K)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(K min(N,M))$ — Utilizes standard auxiliary storage proportional to the active computation state.
