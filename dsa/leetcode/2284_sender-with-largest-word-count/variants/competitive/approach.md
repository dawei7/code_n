...
The core task in **Sender With Largest Word Count** is to You have a chat log of `n` messages. You are given two string arrays `messages` and `senders` where $\text{messages}[i]$ is a **message** sent by $\text{senders}[i]$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
