...
We are given a total of `numCourses` courses one has to take, labeled from `0` to $numCourses - 1$. We are also given an array `prerequisites` where $\text{prerequisites}[i]$ = [$a_i$, $b_i$] indicates that course $b_i$ must be taken first if you want to take course $a_i$.
...
1. Create an array `indegree` of length `n` where $\text{indegree}[x]$ stores the number of edges entering node `x`.
2. We create an adjacency list `adj` in which $\text{adj}[x]$ contains all the nodes with an incoming edge from node `x`, i.e., neighbors of node `x`. We create this adjacency list by iterating over `prerequisites`. For every `prerequisite` in `prerequisites`, we add an edge from $\text{prerequisite}[1]$ to $\text{prerequisite}[0]$ and increment the indegree of $\text{prerequisite}[0]$ by `1`.
...
1. We create an adjacency list `adj` in which $\text{adj}[x]$ contains all the nodes with an incoming edge from node `x`, i.e., neighbors of node `x`. We create this adjacency list by iterating over `prerequisites`. For every `prerequisite` in `prerequisites`, we add an edge from $\text{prerequisite}[1]$ to $\text{prerequisite}[0]$.
...
- After we have processed all the outgoing edges of `node`, we mark $\text{inStack}[node] = false$ to mark `node` as out of stack. We also return `false` as we didn't find any cycle.
