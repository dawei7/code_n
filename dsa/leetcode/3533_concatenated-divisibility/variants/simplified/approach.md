# Simplified Approach - Concatenated Divisibility
...
## Complexity detail
- **Time Complexity**: $O(n * k * 2^n)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(k * 2^n)$ — Utilizes standard auxiliary storage proportional to the active computation state.
