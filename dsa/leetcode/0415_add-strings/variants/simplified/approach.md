# Simplified Approach - Add Strings
...
## Complexity detail
- **Time Complexity**: $O(\max(m,n))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(\max(m,n))$ — Utilizes standard auxiliary storage proportional to the active computation state.
