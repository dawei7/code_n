...
- For each folder $\text{folder}[i]$ starting from the second folder:
  - Retrieve the last folder path added to `result` and append a `/` to it, storing it as `lastFolder`.

  - Check if $\text{folder}[i]$ starts with `lastFolder`:
- If it does, skip this folder since it is a sub-folder of `lastFolder`.
- Otherwise, add $\text{folder}[i]$ to `result` because it is not a sub-folder.
