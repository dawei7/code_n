...
The core task in **Minimum Operations to Make Array Equal** is to You have an array `arr` of length `n` where $\text{arr}[i] = (2 * i) + 1$ for all valid values of `i` (i.e., $0 \le i < n$). The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
