...
![combinations unfolding](images/377_combinations_123.png)
...
$\text{combs}(\text{target}) = \bigcup {\text{combs}(\text{target} - \text{nums}[\text{i}])} \quad \space \text{if}  \space \text{target} \ge \text{\text{nums}[i]}$
...
$\text{combs}(\text{target}) = \sum_{i=0}^{n}{\text{combs}(\text{target} - \text{nums}[\text{i}])} \quad \space \text{if}  \space \text{target} \ge \text{\text{nums}[i]}$
...
Accordingly, we can define an array named $\text{dp}[i]$ where each element corresponds to the result of function `combs(i)`, _i.e._ the number of combinations that sum up to the value of `i`.
...
In this approach though, we start from the base case (_i.e._ $\text{dp}[0]$) and then _iteratively_ calcalate all the intermediate results until we reach the target value (_i.e._ $\text{dp}[target]$).
...
- As one can see, initially all the values are set to be zero, except the first element ($\text{dp}[0]$) which is set to be one.
...
- We then move from low index to high index to work out each value in the $\text{dp}[i]$ array.
With the value of $\text{dp}[0]$ known, we can now deduct the value for $\text{dp}[1]$ which can be broken down into one single case where we place the number `1` as the last number in the combination and the remainder sum become zero.
As a result, $\text{dp}[1] = \text{dp}[0] = 1$ according to the formula.
We set the value of $\text{dp}[0]$ to one, which can be interpreted that there exists **one** combination if the target value can be reduced down to zero.

![DP array step 1](images/377_dp_step_1.png)

- We now move on the index of `2` in the $\text{dp}[i]$ array.
This time, it can be broke down into two cases, which depends on the values of $\text{dp}[1]$ and $\text{dp}[0]$ respectively.
- For case 1, we can place the number `1` as the last number in the combination. The case is now reduced down to $\text{dp}[1]$.
- For case 2, we can place the number `2` as the last number in the combination. The case is now reduced down to $\text{dp}[0]$.
- To sum up the above two cases, the value of $\text{dp}[2]$ can be expressed as $\text{dp}[2] = \text{dp}[1] + \text{dp}[0]$, as one can see in the following graph:

![DP array step 2](images/377_dp_step_2.png)

- We can repeat the above process to work out all the rest values in the $\text{dp}[i]$ array.
In the following graph, we show the final values, as well as the dependencies during the calculation.

![DP array final](images/377_dp_final.png)
...
- We allocate an array $\text{dp}[i]$ to hold all the intermediate values, which amounts to $\mathcal{O}(T)$ space.
