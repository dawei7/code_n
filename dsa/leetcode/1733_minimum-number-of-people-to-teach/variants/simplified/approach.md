# Simplified Approach - Minimum Number of People to Teach
...
## Complexity detail
- **Time Complexity**: $O(S + C)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(S + m)$ — Utilizes standard auxiliary storage proportional to the active computation state.
