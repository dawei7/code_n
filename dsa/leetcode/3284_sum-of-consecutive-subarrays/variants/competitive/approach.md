...
The core task in **Sum of Consecutive Subarrays** is to We call an array `arr` of length `n` **consecutive** if one of the following holds:. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
