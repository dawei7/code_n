...
The core task in **Slowest Key** is to A newly designed keypad was tested, where a tester pressed a sequence of `n` keys, one at a time. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
