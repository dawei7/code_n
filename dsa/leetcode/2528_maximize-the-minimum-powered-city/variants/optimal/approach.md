...
The core task in **Maximize the Minimum Powered City** is to a **0-indexed** integer array `stations` of length `n`, where $\text{stations}[i]$ represents the number of power stations in the $$i^{\text{th}}$$ city. Instead of scanning every element one by one in $O(n)$ time, this solution uses **Binary Search**. Think of looking up a word in a dictionary: you open it in the middle, see if your word comes before or after, and discard half of the remaining pages. By halving the candidate window at each step, we find the answer in fast logarithmic $O(\log n)$ time.
