...
>
>In some languages such as `Python`, there are some decorators for memoization, such as $\text{lru}_{cache}$.
>
...
1. What is $\text{dp}_{state}$?
2. What does `dp` function return?
3. What is the base case?
4. How to calculate $dp(\text{dp}_{state})$ from $dp(\text{other}_{state})$?

Back to the problem. Since we are bursting balloons in `nums` and `nums` keeps changing, it might be a good idea to use `nums` to define our $\text{dp}_{state}$. $dp(nums, \text{memo}_{dict})$ will return the maximum coins obtainable if we burst all balloons in `nums`.
...
How do we calculate `dp(nums)` from $dp(\text{other}_{state})$?
...
2. Use a customary `getOrDefault(nums, i, default)` to replace $\text{nums}[i]$.
...
* Time complexity: $O(N2^N)$
  * There are $O(2^N)$ states. For each state, determining the maximum coins requires iterating over all balloons. Thus the total time complexity is $O(2^N) \times$\mathcal{O}(N)$= O(N2^N)$.

  * From the problem description, we know that $1 \le N \le 500$. Therefore, in the worst case, time_complexity $= 2^{500} \times 500 \approx 1.6 \times 10^{154}$, which is unacceptable. Generally, a number around or less then $10^8$ is feasible.

* Space complexity: $O(N2^N)$
  * There are $O(2^N)$ states, and we need $O(N)$ to store each state. In total, this algorithm requires $O(2^N) \times$\mathcal{O}(N)$= O(N2^N)$ space.
...
$nums[i - 2] * nums[i - 1] * \text{nums}[i]$? No, $\text{nums}[i]$ has been burst, so $\text{nums}[i]$ should be replaced by **some balloon** in the right part.
...
What if...$\text{nums}[i]$ has not burst? If we keep $\text{nums}[i]$ alive **all the time**, then $nums[i - 2] * nums[i - 1] * \text{nums}[i]$ always refers to the correct balloons, and the left part and right part are independent.

How to keep $\text{nums}[i]$ alive **all the time**? Easy, just mark $\text{nums}[i]$ as the **last** burst balloon among `[left, right]`.
...
* Time complexity: $O(N^3)$. There are $O(N^2)$ states. For each state, determining the maximum coins requires iterating over all balloons in the range `[left, right]`.  Thus the total time complexity is $O(N^2) \times$\mathcal{O}(N)$= O(N^3)$.

* Space complexity: $O(N^2)$. We need $O(N^2)$ to store all states, $O(N)$ for stacks to perform recursion, and $O(N)$ to store `[1] + nums + [1]`. In total, this algorithm requires $O(N^2) +$\mathcal{O}(N)$+$\mathcal{O}(N)$= O(N^2)$ space.
...
When iterating, we need to carefully arrange the order of iteration, such that $\text{dp}[left][i - 1]$ and $dp[i + 1][right]$ are iterated **before** $\text{dp}[left][right]$, where $left \le i \le right$.

This is important because in order to calculate $\text{dp}[left][right]$, we will use the results of $\text{dp}[left][i - 1]$ and $dp[i + 1][right]$, where $left \le i \le right$.
...
We only need the top-right triangle since we only need $\text{dp}[left][right]$ where `left` will always be less than or equal to `right`.
...
Also, we cannot have $\text{dp}[0][j]$ and $\text{dp}[i][n-1]$, where $0 \le i < n$ and $0 \le j < n$, since we cannot burst the fake balloons that we added.

![DP Table Inner](images/312_dp_table_inner.drawio.svg)

Okay, now let's consider $\text{dp}[left][right]$. $\text{dp}[left][i - 1]$ and $dp[i + 1][right]$ should be iterated before $\text{dp}[left][right]$, where $left \le i \le right$. Where are they?

![DP Table Cell](images/312_dp_table_cell.drawio.svg)

Notice that $\text{dp}[left][right]$ depends on the cells directly below it and the cells to its left. If we always iterate from the lowest or the leftmost cell, then we can ensure that $\text{dp}[left][i - 1]$ and $dp[i + 1][right]$ are calculated before $\text{dp}[left][right]$.
...
3. Define an array `dp`, where $\text{dp}[left][right]$ represents the maximum coins obtainable, if we burst all balloons on the interval `[left, right]`, inclusively.

4. Iterate over the `dp` array such that $\text{dp}[left][i - 1]$ and $dp[i + 1][right]$ are visited before $\text{dp}[left][right]$ is visited.
  For $\text{dp}[left][right]$:
    ...
    Let $\text{dp}[left][right]$ be the maximum sum of these two values among all possible `i`s.

5. Finally, return $\text{dp}[1][len(nums) - 2]$.

   Note: Do not return $\text{dp}[0][len(nums) - 1]$ because the first and the last balloons were added by us and we cannot be popped.
...
* Time complexity: $O(N^3)$. There are $O(N^2)$ states. For each state, determining the maximum coins requires iterating over all balloons in the range `[left, right]`, giving $O(N^2) \times$\mathcal{O}(N)$= O(N^3)$.

* Space complexity: $O(N^2)$. We need $O(N^2)$ to store `dp`, and $O(N)$ to store `[1] + nums + [1]` (if fake balloons are added). In total, we need $O(N^2) +$\mathcal{O}(N)$+$\mathcal{O}(N)$= O(N^2)$ space.