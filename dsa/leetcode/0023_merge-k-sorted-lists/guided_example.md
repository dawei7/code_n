...
We execute the single-pass Linked List, Divide and Conquer, Heap (Priority Queue), Merge Sort pointer manipulation on a representative linked list instance.
...
The objective for **Merge k Sorted Lists** is to transform the linked structure by strictly updating `next` references in place.
