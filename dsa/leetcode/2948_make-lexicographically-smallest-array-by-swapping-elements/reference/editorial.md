...
We are given an array `nums` in which we can swap any two elements $\text{nums}[i]$ and $\text{nums}[j]$ if their absolute difference is less than or equal to `limit`. We want to find the lexicographically smallest possible array we can make by applying this swap operation an unlimited number of times on `nums`.
...
![Sorting and grouping](images/sorting_and_grouping.png)
...
![Grouping in original input](images/grouping_in_original_input.png)
