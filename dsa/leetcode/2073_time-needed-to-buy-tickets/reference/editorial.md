...
![Slide 1](images/slideshow_queue_solution_queue_slide1.png)

![Slide 2](images/slideshow_queue_solution_queue_slide2.png)

![Slide 3](images/slideshow_queue_solution_queue_slide3.png)

![Slide 4](images/slideshow_queue_solution_queue_slide4.png)

![Slide 5](images/slideshow_queue_solution_queue_slide5.png)

![Slide 6](images/slideshow_queue_solution_queue_slide6.png)

![Slide 7](images/slideshow_queue_solution_queue_slide7.png)

![Slide 8](images/slideshow_queue_solution_queue_slide8.png)

![Slide 9](images/slideshow_queue_solution_queue_slide9.png)

![Slide 10](images/slideshow_queue_solution_queue_slide10.png)

![Slide 11](images/slideshow_queue_solution_queue_slide11.png)
...
- Decrement $\text{tickets}[front]$ by 1 to buy one ticket for the person at index `front`.
- If the person at index `k` has bought all their tickets ($k = front \&\& \text{tickets}[front] = 0$):
- Return the `time`.
...
- If the person at index `k` only needs one ticket ($\text{tickets}[k] = 1$):
...
- Iterate through the `tickets` array:
- If the person at index `i` still needs to buy tickets ($\text{tickets}[i] \neq 0$), one ticket is bought for that person by decrementing $\text{tickets}[i]$ by 1 and incrementing the time by 1.
- If the person at index `k` has bought all their tickets ($\text{tickets}[k] = 0$), return the `time`.
...
- This is because we want to ensure that the `k`th person gets the tickets they need, and people before the `k`th person will only have the opportunity to buy up to $\text{tickets}[k]$ tickets.
...
- People after `k` in line will have fewer opportunities to buy tickets than person `k` does. If they need fewer than $\text{tickets}[k]$ tickets, they will be able to purchase them all. Otherwise, they will purchase $\text{tickets}[k] - 1$ tickets.
- For example: If the `k`th person needs 3 tickets ($\text{tickets}[k] = 3$), and the current person needs 1 ticket, we will buy 1 ticket for the current person. Conversely, if the current person needs 3 tickets, we will buy 2 tickets for the current person. The current person will only have the opportunity to buy 2 tickets before person `k` has purchased all of their tickets.

In simpler terms, when the current person is before or at the `k`th person, we buy the minimum number of tickets needed by both people. When we are at a person after the `k`th person, we know the current person will only have the opportunity to buy $\text{tickets}[k] - 1$ tickets, so we buy the minimum between that and the current person's needed tickets.

The following is an illustration demonstrating the one-pass approach:

![Slide 1](images/slideshow_approach_three_onepass_slide1.png)

![Slide 2](images/slideshow_approach_three_onepass_slide2.png)

![Slide 3](images/slideshow_approach_three_onepass_slide3.png)

![Slide 4](images/slideshow_approach_three_onepass_slide4_fixed.png)
...
- Iterate through the `tickets` array:
- If the current index `i` is less than or equal to `k` ($i \le k$):
- Increment `time` by the minimum of $\text{tickets}[k]$ and $\text{tickets}[i]$
- Else (if the current index `i` is greater than `k`):
- Increment `time` by the minimum of ($\text{tickets}[k] - 1$) and $\text{tickets}[i]$
