...
Let $\text{people}[i]$ to the currently lightest person, and $\text{people}[j]$ to the heaviest.

Then, as described above, if the heaviest person can share a boat with the lightest person (if $\text{people}[j] + \text{people}[i] \le limit$) then do so; otherwise, the heaviest person sits in their own boat.
