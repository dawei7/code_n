...
- $stMax[i][j]$: the maximum value in the interval starting at index $i$ with length $2^j$.
- $stMin[i][j]$: the minimum value in the interval starting at index $i$ with length $2^j$.
