# Simplified Approach - Seasonal Sales Analysis
...
## Complexity detail
- **Time Complexity**: $O(S log S + P log P + G log G)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(S + P + G)$ — Utilizes standard auxiliary storage proportional to the active computation state.
