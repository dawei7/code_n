# Simplified Approach - Ones and Zeroes
...
## Complexity detail
- **Time Complexity**: $O(k \cdot m \cdot n)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(m \cdot n)$ — Utilizes standard auxiliary storage proportional to the active computation state.
