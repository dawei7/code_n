...
Since the array is sorted, $\text{nums}[lo]$ begins as the largest negative number in the array, and $\text{nums}[hi]$  starts as the largest positive value in the array. If $-\text{nums}[lo]$ and $\text{nums}[hi]$ are equal, then we have found the largest pair of numbers with opposite signs that sum to 0, so $\text{nums}[hi]$ is the answer.
...
- While `lo` is less than `hi`:
   - If $-\text{nums}[lo]$ is equal to $\text{nums}[hi]$, return $\text{nums}[hi]$.
   - If $-\text{nums}[lo]$ is greater than $\text{nums}[hi]$, increment `lo`.
   - If $-\text{nums}[lo]$ is less than $\text{nums}[hi]$, decrement `hi`.
...
- Iterate over `nums`:
   - For each `num`, get the absolute value $\text{abs}_{num}$.
   - Check if $\text{abs}_{num}$ is greater than `ans` and if the negation of `num` is present in the `seen` set.
   - If the condition is true, update `ans` to $\text{abs}_{num}$.
...
Given the range $-1000 <= \text{nums}[i] <= 1000$, we can track all numbers encountered using a bitset. For instance, we can set a bit if the number is negative and leave it unset if the number is positive.
...
- Iterate over `nums`:
   - For each `num`, get the absolute value $\text{abs}_{num}$.
   - Check if $\text{abs}_{num}$ is greater than `ans` and if the negation of `num` (shifted by 1024) is present in the `seen` bitset.
   - If the condition is true, update $ans = \text{abs}_{num}$.
