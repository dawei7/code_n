...
-   Set $\text{result}[j][i] = \text{box}[i][j]$.
...
-   If $\text{result}[i][j]$ is an empty cell:
-   Initialize `nextRowWithStone` to `-1`.
-   Loop through all rows above `i` with `k` from `i-1` to `0`.
-   If $\text{result}[k][j]$ contains an obstacle, exit the loop.
-   If $\text{result}[k][j]$ contains a stone, set `nextRowWithStone` equal to `k` and exit the loop.
...
-   Else, let the stone in $\text{result}[nextRowWithStone][j]$ land on $\text{result}[i][j]$ by setting $\text{result}[nextRowWithStone][j] = '.'$ and $\text{result}[i][j] = '#'$.
-   Return `result`.
...
-   The second inner loop checks the rows above the current empty cell to find a stone, which in the worst case can also iterate up to $O(n)$ times.
    ...
    Therefore, for each column, the worst-case scenario for the gravity application results in $O(n) \times$\mathcal{O}(n)$= O(n^2)$
...
-   If $\text{result}[i][j]$ contains a stone, let it fall to the lowest empty cell:
-   Set $\text{result}[lowestRowWithEmptyCell][j] = '#'$.
-   Set $\text{result}[i][j] = '.'$.
-   Update `lowestRowWithEmptyCell` to `i-1`.
-   if $\text{result}[i][j]$ contains an obstacle, set $lowestRowWithEmptyCell = i-1$.
-   Return `result`.
...
-   If $\text{box}[i][j]$ contains a stone, let it fall to the lowest empty cell:
-   Set $\text{result}[lowestRowWithEmptyCell][m-i-1] = '#'$.
-   (Optionally) Set $\text{result}[j][m-i-1] = '.'$.
-   Update `lowestRowWithEmptyCell` to `i-1`.
-   If $\text{box}[i][j]$ contains an obstacle:
-   Set $\text{result}[j][m-i-1] = '*'$.
-   Update `lowestRowWithEmptyCell` to `i-1`.
-   Return `result`.
