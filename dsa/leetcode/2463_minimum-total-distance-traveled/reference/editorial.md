...
- In the `calculateMinDistance` function:
  - Check if all robots are assigned ($robotIdx = \text{robot.size}()$):
- If true, return `0` since no distance is needed.

  - Check if there are no factories left to assign ($factoryIdx = \text{factoryPositions.size}()$):
- If true, return a large value (e.g., `1e12`) to indicate an infeasible path.

  - Check the memoization table to see if the result is already computed ($\text{memo}[robotIdx][factoryIdx] \neq -1$):
- If true, return the memoized value.

  - Calculate the cost for two options:
- Option 1: Assign the current robot to the current factory:
      - Compute the distance as $abs(\text{robot}[robotIdx] - \text{factoryPositions}[factoryIdx])$ plus the result of recursively calling `calculateMinDistance` for the next robot and the next factory.

- Option 2: Skip the current factory for the current robot:
      - Call `calculateMinDistance` with the same robot but the next factory.

  - Store the minimum of the two options in $\text{memo}[robotIdx][factoryIdx]$ and return this minimum value.
...
Using a 2D DP table, let $\text{dp}[i][j]$ represent the minimum distance to assign robots starting from `i` to factories starting from `j`. We fill this table from the last robot and factory backward to ensure all future-dependent choices are precomputed.

The base case is when there are no robots left (`i` exceeds the last robot index), giving a minimum distance of `0`.

For each $\text{dp}[i][j]$, we choose the minimum of:
  - Assigning robot `i` to factory `j`, moving to $dp[i + 1][j]$,
  - Or skipping this factory, moving to $\text{dp}[i][j + 1]$.

Thus, $\text{dp}[i][j] = \min(|\text{robot}[i] - \text{factory}[j]| + dp[i + 1][j+1], \text{dp}[i][j + 1])$.

After filling the table, $\text{dp}[0][0]$ holds the minimum distance to assign all robots from the first factory onward.
...
- Create a 2D dynamic programming (DP) table `dp` of size $(robotCount + 1) x (factoryCount + 1)$ initialized to zero, where $\text{dp}[i][j]$ represents the minimum total distance for assigning robots from `i` to $robotCount - 1$ using factories from `j` to $factoryCount - 1$.

- Set base cases:
  - For each robot `i`, set $\text{dp}[i][factoryCount]$ to a large value (`1e12`) to represent that there are no factories left for assignment.
...
- Calculate the distance for the current robot to the current factory and add the result of assigning the next robot to the next factory:
      $assign = abs(\text{robot}[i] - \text{factoryPositions}[j]) + dp[i + 1][j + 1]$

- Also consider skipping the current factory for the current robot:
      $skip = \text{dp}[i][j + 1]$

- Update $\text{dp}[i][j]$ with the minimum of the two options:
      $\text{dp}[i][j] = min(assign, skip)$

- Return the value at $\text{dp}[0][0]$, which represents the minimum total distance starting from the first robot and the first factory.
...
Since calculating $\text{current}[i][j]$ only requires values from the next row ($current[i + 1][...]$), we can maintain only two rows: one for the current state we’re filling and one for the next state holding results from the previous robot in the iteration.

By iterating backwards over robots and factories, we can use a single array, `current`, of size equal to the number of factories. Starting from the last robot, we iterate over factories in reverse, updating $\text{current}[j]$ in place using $current[j + 1]$ (for skipping the factory) and $current[j + 1] + |\text{robot}[i] - \text{factoryPositions}[j]|$ (for assigning this factory).

This ensures $\text{current}[j]$ holds the minimum distance for that subproblem. After finishing the iteration, $\text{current}[0]$ will contain the minimum distance for assigning all robots to factories.
...
- Initialize $\text{current}[factoryCount]$ to a large value (1e12) for the current robot's calculations.
...
- Use $assign = abs(\text{robots}[i] - \text{factoryPositions}[j]) + next[j + 1]$.
      - Calculate the distance if the current factory is skipped for this robot:
- Use $skip = current[j + 1]$.
      - Store the minimum of `assign` and `skip` in $\text{current}[j]$.

- Move to the next robot by updating `next` to be equal to `current`.

- After processing all robots, return $\text{current}[0]$, which contains the minimum total distance for assigning all robots to factories.
