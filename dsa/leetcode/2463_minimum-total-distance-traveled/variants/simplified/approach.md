# Simplified Approach - Minimum Total Distance Traveled
...
## Complexity detail
- **Time Complexity**: $O(R\log R+F\log F+RF)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(R+F)$ — Utilizes standard auxiliary storage proportional to the active computation state.
