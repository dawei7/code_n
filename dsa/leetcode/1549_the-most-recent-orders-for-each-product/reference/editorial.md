...
For this approach, we find the most recent order by applying `max()` to the column $\text{order}_{date}$, and then merge to the other DataFrames to append the product and order information for the most recent orders.

We first find the most recent $\text{order}_{date}$ for each $\text{product}_{id}$ using `groupby()`.

```python
df = orders.groupby(['product_id'], as_index=False).order_date.max()
```

This step creates a DataFrame that contains the most recent $\text{order}_{date}$ for each $\text{product}_{id}$ as shown below.
...
Now we need to get the product and order information for these orders. Since the final output is looking for $\text{product}_{name}$ and $\text{order}_{id}$ and they are stored in two two different DataFrames `orders` and `products`, we merge the DataFrame `df` to these DataFrames.
...
In the subquery, we first identify the most recent order using `MAX()` for each $\text{product}_{id}$.
...
Next, we `JOIN ` the two tables `Products` and `Orders` on $\text{product}_{id}$ in the main query to get all required columns for the final output.
...
Lastly, we `JOIN` the main query to the subquery on $\text{product}_{id}$ and $\text{order}_{date}$ so only the wanted records (the most recent order info for each product) will be returned. We also want to make sure the final results are ordered by $\text{product}_{name}$, $\text{product}_{id}$, and $\text{order}_{id}$ in ascending order.
