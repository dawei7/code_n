...
We trace the dynamic Array, Hash Table, Binary Search, Dynamic Programming, Sliding Window sliding window on a representative input instance.
...
The objective for **Find Two Non-overlapping Sub-arrays Each With Target Sum** is to find the optimal contiguous window without evaluating all $O(N^2)$ candidate subarrays.
