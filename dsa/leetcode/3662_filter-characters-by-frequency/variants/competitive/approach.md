...
The core task in **Filter Characters by Frequency** is to a string `s` consisting of lowercase English letters and an integer `k`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
