...
2. Create a 2D-array called `memo` having `n` rows and `n` columns where $\text{memo}[i][j]$ contains the length of the longest palindromic subsequence of the substring formed from index `i` to `j` in `s`.
...
- If $\text{memo}[i][j] \neq 0$, it indicates that we have already solved this subproblem, so we return $\text{memo}[i][j]$.
- If `i > j`, the string is empty. We return `0`.
- If $i = j$, it is a substring having one character. As a result, we return `1`.
- If the first and the last characters are the same, i.e., $s[i] = s[j]$, we include these two characters in the palindromic subsequence and add it to the longest palindromic subsequence formed using the substring from index $i + 1$ to $j - 1$ (inclusive). We perform $\text{memo}[i][j] = lps(s, i + 1, j - 1, memo) + 2$.
- Otherwise, if the first and the last characters do not match, we recursively search for the longest palindromic subsequence in both the substrings formed after ignoring the first and last characters. We pick the maximum of these two. We perform $\text{memo}[i][j] = max(lps(s, i + 1, j, memo), lps(s, i, j - 1, memo))$.
- Return $\text{memo}[i][j]$.
...
Using the same method as before, we create a 2D-array `dp`, where $\text{dp}[i][j]$ contains the answer of the longest palindromic subsequence of the substring formed from index `i` to `j` in `s`. Our answer would be $\text{dp}[0][n - 1]$, where `n` is the size of `s`. The state transition would be as follows:

> 1. If $s[i] = s[j]$, perform $\text{dp}[i][j] = 2 + dp[i + 1][j - 1]$.
> 2. Otherwise, perform $\text{dp}[i][j] = max(\text{dp}[i][j - 1], dp[i + 1][j]$.
...
1. Create an integer variable `n` and initialize it to the size of `s`.
2. Create a 2D-array called `dp` having `n` rows and `n` columns where $\text{dp}[i][j]$ contains the length of the longest palindromic subsequence of the substring formed from index `i` to `j` in `s`.
3. We iterate using two loops. The outer loop iterates from $i = n - 1$ to $i = 0$ decrementing `i` by `1` after each iteration. At the end of each iteration, we will have the length of longest palindromic subsequence in all the substrings that start from index `i` in `s`. For each `i`, we first mark $\text{dp}[i][i] = 1$ because it denotes just one character and then we iterate over $j = i + 1$ to $j = n - 1$ and perform the following:
- If the first and the last characters are the same, i.e., $s[i] = s[j]$, we include these two characters in the palindromic subsequence and add it to the longest palindromic subsequence formed using the substring from index $i + 1$ to $j - 1$ (inclusive). We perform $\text{dp}[i][j] = dp[i + 1][j - 1] + 2$. We already have the answer for $dp[i + 1][j - 1]$ because we computed it for substrings starting at index $i + 1$ in the previous iteration of outer loop.
- Otherwise, if the first and the last characters do not match, we look for the longest palindromic subsequence in both the substrings formed after ignoring the first and last characters. We pick the maximum of these two. We perform $\text{dp}[i][j] = max(dp[i + 1][j], \text{dp}[i][j - 1])$.
4. Return $\text{dp}[0][n - 1]$.
...
If we look closely at this transition, to fill $\text{dp}[i][j]$ for a particular `i` and all possible values of `j`, we only need the values from the current and previous rows. To fill row $i + 1$ in the `dp` grid, we need the values from row $i + 1$ ($dp[i + 1][j - 1]$) and previously computed value in the $i^{th}$ row itself ($\text{dp}[i][j - 1]$). Values in rows $i + 2$, $i + 3$, and so on are no longer needed.
...
Now, $\text{dp}[j]$ stores the length of longest palindromic subsequence of the substring from index `i` to `j` in `s`. It is similar to what $\text{dp}[i][j]$ stored in previous approach.

The other array `dpPrev` is important to understand. It helps us by remembering the previous state that we completed previously. $\text{dpPrev}[j]$ stores the length of the longest palindromic subsequence of the substring from index $i + 1$ to `j` in `s`. It is analogous to $dp[i + 1][j]$ in the previous approach.
...
3. We iterate using two loops with outer loop running from $i = n - 1$ to $i = 0$ decrementing `i` by `1` after each iteration. For each `i`, we first mark $\text{dp}[i] = 1$ since it denotes just one character at index `i` and then we iterate over $j = i + 1$ to $j = n - 1$ and perform the following:
- If the first and the last characters are the same, i.e., $s[i] = s[j]$, we include these two characters in the palindromic subsequence and add it to the longest palindromic subsequence formed using the substring from index $i + 1$ to $j - 1$ (inclusive). We perform $\text{dp}[j] = dpPrev[j - 1] + 2$. Note that we already have computed answer for substrings starting from index $i + 1$ in the previous iteration of outer loop. We have it in `dpPrev`.
- Otherwise, if the first and the last characters do not match, we check for the longest palindromic subsequence in both the substrings formed after ignoring the first and last characters. We pick the maximum of these two. We perform $\text{dp}[j] = max(\text{dpPrev}[j], dp[j - 1])$.
