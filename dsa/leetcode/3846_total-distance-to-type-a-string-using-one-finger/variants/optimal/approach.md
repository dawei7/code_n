...
The core task in **Total Distance to Type a String Using One Finger** is to There is a special keyboard where keys are arranged in a rectangular grid as follows. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
