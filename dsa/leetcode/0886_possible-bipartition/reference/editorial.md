...
4. If a node `i` has not been colored ($\text{color}[i] = -1$), start a BFS traversal which will cover all the nodes present in the same component as node `i`.
...
4. If a node `i` has not been colored ($\text{color}[i] = -1$), start a DFS traversal using node `i` as the `source` node. A DFS traversal will cover all the nodes present in the same component as node `source`.
...
2. Create a class `UnionFind` defining standard methods `find` and $\text{union}_{set}$.
