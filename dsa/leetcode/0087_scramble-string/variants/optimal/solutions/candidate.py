...
def solve(s1: str, s2: str) -> bool:
    ...
    for i, (first, second) in enumerate(zip(s1, s2), start=1):
        for character in range(26):
            prefix1[character][i] = prefix1[character][i - 1]
            prefix2[character][i] = prefix2[character][i - 1]
        prefix1[ord(first) - ord("a")][i] += 1
        prefix2[ord(second) - ord("a")][i] += 1
