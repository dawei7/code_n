...
![split](images/diagram2.drawio.png)
...
Let $\text{dp}[length][i][j]$ be a boolean representing whether `t` is a scrambled version of `s`.

The base case, as defined by the problem is when $length = 1$. Here we do not have to split strings into smaller ones, so we can easily compare the corresponding characters: $\text{dp}[1][i][j]$ is `true` when $\text{s1}[i]$ equals $\text{s2}[j]$, and `false` otherwise.

Now we need to write down the transitions of `dp`. We will use the following image as an example during the explanations.

![split](images/diagram.drawio.png)
...
* Do not swap the blue and yellow parts. The corresponding substrings of `s2` must be scrambled versions of the substrings we just created by splitting `s1`. This means both $\text{dp}[newLength][i][j]$ (representing the blue parts) and $dp[length - newLength][i + newLength][j + newLength]$ (representing the yellow parts) must be true.
* Swap the blue and yellow parts. As you can see in the image, this misaligns the blue and yellow parts between `s1` and `s2`, but we still need the parts to match (we still need the blue part of `s1` to be a scrambled version of the blue part of `s2`, same with the yellow part). What are the new starting indices? For blue, it's `s1` starting with `i` and `s2` starting with $j + length - newLength$. For yellow, it's `s1` starting with $i + newLength$ and `s2` starting with `j`. Thus, we need both $\text{dp}[newLength][i][j+length-newLength]$ and `dp[length-newLength][i+newLength][j]` to be `true`.

Now we can formally write down the transitions. For `length > 1`, $\text{dp}[length][i][j]$ is `true` if and only if for at least one `newLength` where `0 < newLength < length`:

$(\text{dp}[newLength][i][j] \&\& dp[length-newLength][i+newLength][j+newLength]) || (\text{dp}[newLength][i][j+length-newLength] \&\& dp[length-newLength][i+newLength][j])$ is `true`.

Let `n` denote the length of the input strings. The answer to the problem is $\text{dp}[n][0][0]$, as starting at index `0` with length `n` is considering the entire input string.

#### Algorithm

1. Iterate `i` from `0` to `n-1`.
* Iterate `j` from `0` to `n-1`.
* Set $\text{dp}[1][i][j]$ to the boolean value of $\text{s1}[i] = \text{s2}[j]$. (The base case of the DP).
...
* If $\text{dp}[newLength][i][j] \&\& dp[length-newLength][i+newLength][j+newLength]) || (\text{dp}[newLength][i][j+l-newLength] \&\& dp[l-newLength][i+newLength][j]$ is `true`, set $\text{dp}[length][i][j]$ to `true`.
3. Return $\text{dp}[n][0][0]$.
