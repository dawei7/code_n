...
We trace the hierarchical Tree, Binary Search Tree, Binary Tree traversal and subtree aggregation on a representative binary tree.

- **Input:** `{"node": {"tree": [2, 1, 3], "target_index": 1}}`
- **Required output:** `2`
...
The objective for **Inorder Successor in BST II** is to evaluate tree properties by visiting nodes in topological hierarchy (post-order, pre-order, or level-order).
