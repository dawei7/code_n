...
![Slide 1](images/slideshow_680_Valid_Palindrome_II_680_1.png)

![Slide 2](images/slideshow_680_Valid_Palindrome_II_680_2.png)

![Slide 3](images/slideshow_680_Valid_Palindrome_II_680_3.png)

![Slide 4](images/slideshow_680_Valid_Palindrome_II_680_4.png)

![Slide 5](images/slideshow_680_Valid_Palindrome_II_680_5.png)

We can write a handy helper function `checkPalindrome(s, i, j)` that implements this logic, where `s` is the string we are checking, and `i` and `j` are the left and right bounds we want to consider. For example, to check if the entire string `s` is a palindrome, `i` will start at `0` and `j` will start at $\text{s.length}() - 1$.
...
2. Initialize two pointers, $i = 0$ and $j = \text{s.length}() - 1$.
