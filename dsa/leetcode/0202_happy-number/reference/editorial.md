...
![The chain of numbers starting with 116. It has the numbers 116, 38, 73, 58, and then goes in a circle to 89, 145, 42, 20, 4, 16, 37, and back to 58.](images/image2.png)
...
* Time complexity : $O(243 \cdot	3 + \log n + \log\log n + \log\log\log n)...$ = $O(\log n)$.
    ...
    For an $n$ above $243$, we need to consider the cost of each number in the chain that is above $243$. With a little math, we can show that in the worst case, these costs will be $O(\log n) +$\mathcal{O}(\\log \\log n)$+$\mathcal{O}(\\log \\log \\log n)$...$. Luckily for us, the $O(\log n)$ is the dominating part, and the others are all tiny in comparison (collectively, they add up to less than $\log n)$, so we can ignore them.
...
Recognizing that we actually have a LinkedList, it turns out that this question is almost the same as another Leetcode problem, [detecting if a linked list has a cycle](https://leetcode.com/problems/linked-list-cycle). As @Freezen [has pointed out](https://leetcode.com/problems/happy-number/discuss/56917/My-solution-in-C(-$\mathcal{O}(1)$-space-and-no-magic-math-property-involved-)), we can therefore use Floyd's Cycle-Finding Algorithm here. This algorithm is based on 2 runners running around a circular race track, a fast runner and a slow runner. In reference to a famous fable, many people call the slow runner the "tortoise" and the fast runner the "hare".
...
![Slide 1](images/slideshow_202_Happy_Number_slide1.png)

![Slide 2](images/slideshow_202_Happy_Number_slide2.png)

![Slide 3](images/slideshow_202_Happy_Number_slide3.png)

![Slide 4](images/slideshow_202_Happy_Number_slide4.png)

![Slide 5](images/slideshow_202_Happy_Number_slide5.png)

![Slide 6](images/slideshow_202_Happy_Number_slide6.png)

![Slide 7](images/slideshow_202_Happy_Number_slide7.png)

![Slide 8](images/slideshow_202_Happy_Number_slide8.png)
