...
The core task in **Binary Tree Preorder Traversal** is to the `root` of a binary tree, return *the preorder traversal of its nodes' values*. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
