...
The company would pay anyway: $\text{price}_{A}$ to send a person to the city
A, or $\text{price}_{B}$ to send a person to the city B.
By sending the person to city A, the company would lose $\text{price}_{A} - \text{price}_{B}$,
which could be negative or positive.

![bla](images/users.png)

To optimize the total costs, let's sort the persons by $\text{price}_{A} - \text{price}_{B}$
...
- Sort the persons in ascending order by $\text{price}_{A} - \text{price}_{B}$
parameter, which indicates the company's additional costs.

- To minimize the costs, send `n` persons with the smallest $\text{price}_{A} - \text{price}_{B}$
