...
Write a solution to find $\text{employee}_{id}$ of all employees that directly or indirectly report their work to the head of the company.
...
- `Employees`: a table with one row per $\text{employee}_{id}$, together with the employee's $\text{employee}_{name}$ and direct $\text{manager}_{id}$.
