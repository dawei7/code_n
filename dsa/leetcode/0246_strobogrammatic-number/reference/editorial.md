...
If no invalid pairs are found, then the number must be strobogrammatic. Note that the middle-digit-of-an-odd-number case is handled correctly; the final iteration will have $left = right$. If they are both pointing to the same `0`, `1`, or `8`, then the condition will be false, and `true` returned at the end. If they are both pointing at a `6`, then the condition will be true, and `false` will be returned, as $\text{expected}_{rotation}$ will be `9`, and $\text{num}[right]$ will be `6`.
