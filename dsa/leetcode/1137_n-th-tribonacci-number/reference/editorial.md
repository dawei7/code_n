...
In later calculations, if we find that $\text{dp}[i]$ already exists, we know that we have already computed the value of `dfs(i)`, and we can simply return the precomputed solution $\text{dp}[i]$ without further recursion. This can significantly reduce the computational time and make the algorithm more efficient.
...
1) Create a hash map `dp` to store the value of computed tribonacci numbers, initialized with the base cases $\text{dp}[0] = 0, \text{dp}[1] = 1, \text{dp}[2] = 1$.

2) Let `dfs(i)` be the value of $i^{th}$ tribonacci numbers:

- If `i` is in `dp`, return $\text{dp}[i]$.

- Otherwise, recursively solve $answer = dfs(i - 1) + dfs(i - 2) + dfs(i - 3)$ and set $\text{dp}[i] = answer$. Then return `answer`.
...
Each problem `f(i)` is related to three subproblems. Thus we can store tribonacci numbers in an array `dp` where $\text{dp}[i]$ represents the $i^{th}$ term. Then we can then iterate through the index `i` and calculate each term using the given relation, as shown in the picture below.

$\text{dp}[i] = dp[i - 1] + dp[i - 2] + dp[i - 3]$.

![img](images/bu.png)

<br>

One advantage of this solution is that we precompute each tribonacci number. If we have multiple requests on the value of the $i^{th}$ tribonacci number later, we can simply refer to $\text{dp}[i]$ in a constant time, rather than computing $\text{dp}[i]$ again. This method is called tabulation.
...
1) Initialize an array `dp` of size $n + 1$. Set the base cases $\text{dp}[0] = 0, \text{dp}[1] = 1, \text{dp}[2] = 1$.

2) Iterate over index `i` from `3` to `n`, update $\text{dp}[i]$ as $\text{dp}[i] = dp[i - 1] + dp[i - 2] + dp[i - 3]$.
