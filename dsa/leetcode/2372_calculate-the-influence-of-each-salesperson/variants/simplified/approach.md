# Simplified Approach - Calculate the Influence of Each Salesperson
...
## Complexity detail
- **Time Complexity**: $O((S+C+R) log(S+C+R))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(S+C+R)$ — Utilizes standard auxiliary storage proportional to the active computation state.
