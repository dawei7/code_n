...
Let's try to count the number of valid binary strings between $\text{groups}[i]$ and `groups[i+1]`. If we have $\text{groups}[i] = 2, groups[i+1] = 3$, then it represents either `"00111"` or `"11000"`. We clearly can make $min(\text{groups}[i], groups[i+1])$ valid binary strings within this string.  Because the binary digits to the left or right of this string must change at the boundary, our answer can never be larger.
...
Afterward, we will take the sum of $min(groups[i-1], \text{groups}[i])$.
