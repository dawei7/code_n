...
The core task in **First Day Where You Have Been in All the Rooms** is to There are `n` rooms you need to visit, labeled from `0` to $n - 1$. Each day is labeled, starting from `0`. You will go in and visit one room a day. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
