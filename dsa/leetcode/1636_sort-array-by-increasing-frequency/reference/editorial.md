...
3. Comparison logic:
   - If frequencies are equal ($\text{freq}[a] = \text{freq}[b]$), sort by value in descending order (`a > b`).
   - Otherwise, sort by frequency in ascending order ($\text{freq}[a] < \text{freq}[b]$).
...
The lambda function $lambda x: (\text{freq}[x], -x)$ is used as the `key` parameter in the `sorted` function call.
1.  `lambda x:` creates an anonymous function with `x` as its parameter.
2.  $(\text{freq}[x], -x)$ is the tuple that the lambda function returns.
3. $\text{freq}[x]$ is used to get the frequency of `x` from the `freq` dictionary as the main sorting criterion.
...
- Increase the count of `num` in the `freq` map using $\text{freq}[num]++$.
...
- If $\text{freq}[a]$ (frequency of `a`) equals $\text{freq}[b]$ (frequency of `b`), then:
- Return `a > b` to ensure that in case of tie-in frequency, larger values come first (decreasing order).
- Otherwise, return $\text{freq}[a] < \text{freq}[b]$ to sort by frequency in increasing order.
