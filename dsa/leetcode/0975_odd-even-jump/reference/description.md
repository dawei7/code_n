...
- During **odd-numbered jumps** (i.e., jumps 1, 3, 5, ...), you jump to the index `j` such that $\text{arr}[i] \le \text{arr}[j]$ and $\text{arr}[j]$ is the smallest possible value. If there are multiple such indices `j`, you can only jump to the **smallest** such index `j`.

- During **even-numbered jumps** (i.e., jumps 2, 4, 6, ...), you jump to the index `j` such that $\text{arr}[i] \ge \text{arr}[j]$ and $\text{arr}[j]$ is the largest possible value. If there are multiple such indices `j`, you can only jump to the **smallest** such index `j`.
