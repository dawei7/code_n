...
We must find the longest sequence `seq` such that the elements in `seq[i+1]` are greater than the corresponding elements in $\text{seq}[i]$ (this means that $\text{seq}[i]$ can fit into `seq[i+1]`).
...
`dp` is an array such that $\text{dp}[i]$ is the smallest element that ends an increasing subsequence of length $i + 1$. Whenever we encounter a new element `e`, we binary search inside `dp` to find the largest index `i` such that `e` can end that subsequence. We then update $\text{dp}[i]$ with `e`.
