...
The core task in **Russian Doll Envelopes** is to a 2D array of integers `envelopes` where $\text{envelopes}[i] = [w_{i}, h_{i}]$ represents the width and the height of an envelope. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
