...
![state machine](images/309_state_machine.png)
...
![mario game](images/309_game.png)
...
![graph of state transition](images/309_graph.png)
...
In order to implement the above state machine, we could define three arrays (_i.e._ $\text{held}[i]$, $\text{sold}[i]$ and $\text{reset}[i]$) which correspond to the three states that we defined before.
...
For instance, the element $\text{sold}[2]$ represents the maximal profits we gain if we sell the stock at the price point $\text{price}[2]$.
...
Therefore, we assign the initial values of `sold[-1]` and `held[-1]` to be $Integer.\text{MIN}_{VALUE}$, which are intended to _render_ the paths that start from these two states impossible.
...
![Slide 1](images/slideshow_309_LIS_slide_9.png)

![Slide 2](images/slideshow_309_LIS_slide_10.png)

![Slide 3](images/slideshow_309_LIS_slide_11.png)

![Slide 4](images/slideshow_309_LIS_slide_12.png)

![Slide 5](images/slideshow_309_LIS_slide_13.png)

![Slide 6](images/slideshow_309_LIS_slide_14.png)

![Slide 7](images/slideshow_309_LIS_slide_15.png)
...
![example of profit calculation](images/309_formula.png)
...
- We define an array $\text{MP}[i]$ to hold the values for our target function $\text{MP}(i)$. We initialize the array with zeros, which correspond to the base case where the minimal profits that we can gain is zero. Note that, here we did a trick to pad the array with two additional elements, which is intended to simplify the branching conditions, as one will see later.
<br/>

- To calculate the value for each element $\text{MP}[i]$, we need to look into two cases as we discussed in the previous section, namely:

- Case 1). we buy the stock at the price point $\text{price}[i]$, then we sell it at a later point. As one might notice, the initial padding on the $\text{MP}[i]$ array saves us from getting out of boundary in the array.

- Case 2). we do no transaction with the stock at the price point $\text{price}[i]$.

- At the end of each iteration, we then pick the largest value from the above two cases as the final value for $\text{MP}[i]$.

- At the end of the loop, the $\text{MP}[i]$ array will be populated. We then return the value of $\text{MP}[0]$, which is the desired solution for the problem.
