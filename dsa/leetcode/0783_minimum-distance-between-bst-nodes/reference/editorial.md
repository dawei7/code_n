...
1. Initialize the `minDistance` to $\text{MAX}_{VALUE}$ possible; this is the variable to store the minimum difference.
...
1. Initialize the `minDistance` to $\text{MAX}_{VALUE}$ possible; this is the variable to store the minimum difference. Initialize `prevValue` to `null`, so we can check if we have already traversed any elements before or not.
