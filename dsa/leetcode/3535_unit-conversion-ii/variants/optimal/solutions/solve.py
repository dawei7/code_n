def solve(
    ...
    graph: list[list[tuple[int, int]]] = [[] for _ in range(unit_count)]
    ...
    return [from_root[target] * pow(from_root[source], modulus - 2, modulus) % modulus for source, target in queries]
