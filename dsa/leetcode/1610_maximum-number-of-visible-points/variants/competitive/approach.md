...
The core task in **Maximum Number of Visible Points** is to an array `points`, an integer `angle`, and your `location`, where $location = [\text{pos}_{x}, \text{pos}_{y}]$ and $\text{points}[i] = [x_{i}, y_{i}]$ both denote **integral coordinates** on the X-Y plane. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
