...
- If `num < pivot`: $\text{ans}[lessI] = num$ and increment `lessI`.
- If $num = pivot$: $\text{ans}[equalI] = num$ and increment `equalI`.
- If `num > pivot`: $\text{ans}[greaterI] = num$ and increment `greaterI`.
- Return `ans`.
...
- Initialize pointer for third section $greaterI = \text{nums.length} - 1$ going right to left.
- Start a forward and backward iteration of `nums`. For forward, we initialize $i = 0$. For backward, we initialize $j = \text{nums.length} - 1$. For each iteration:
- If $\text{nums}[i] < pivot$, then write in first section: $\text{ans}[lessI] = \text{nums}[i]$ and increment `lessI`.
- If $\text{nums}[j] > pivot$, then write in third section: $\text{ans}[greaterI] = \text{nums}[j]$ and decrement `greaterI`.
- Increment `i` and decrement `j`.
- Fill in the remaining spots of `ans` with pivot:
- While $lessI \le greaterI$:
- $\text{ans}[lessI] = pivot$
- `lessI++`
- Return `ans`.
