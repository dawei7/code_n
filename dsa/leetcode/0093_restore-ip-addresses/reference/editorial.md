...
2. Set `remainingNumberOfIntegers` to $4 - \text{dots.length}$. This is how many integers we have left to form.
...
* if the last integer $\text{s.substring}(startIndex, startIndex + remainingLength)$ is valid
* Create an empty string to save this answer using the following steps.
* Set `last` to `0`.
* Iterate over all elements `dot` in the list `dots`.
* Append $\text{s.substring}(last, last + dot)$ and a '.' into the answer string.
...
* If the integer $\text{s.substring}(startIndex, startIndex + curPos)$ is valid
