...
The core task in **Number of Sub-arrays With Odd Sum** is to an array of integers `arr`, return *the number of subarrays with an **odd** sum*. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
