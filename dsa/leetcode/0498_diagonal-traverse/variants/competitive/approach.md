...
The core task in **Diagonal Traverse** is to an `m x n` matrix `mat`, return *an array of all the elements of the array in a diagonal order*. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
