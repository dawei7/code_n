...
* Now, we will consider another pointer which will act as the head of the reversed linked list. Let's call this $\text{rev}_{head}$.
* We will use a pointer, `ptr` to traverse the original list.
* For every element, we basically insert it at the beginning of the reverse list which has $\text{rev}_{head}$ as its head.
...
---
### Approach 2: Iterative $\mathcal{O}(1)$ space
