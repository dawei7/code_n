...
* Time complexity: $O(N^2 \cdot \log(N))$.
  ...
  - Thus, the overall time complexity is $O(N^2 + N^2 \cdot \log(N^2) + N^2 \cdot \alpha(N)) \approx$\mathcal{O}(N^2 \cdot \\log(N^2)$) \approx$\mathcal{O}(N^2 \cdot \\log(N)$)$.
...
- If this node is already present in our MST $( \text{inMST}[node] = true )$ we discard this edge.
...
In this approach, we use one $minDist$ array, where $\text{minDist}[i]$ stores the weight of the smallest weighted edge to reach the $i^{th}$ node from any node in the current tree.
...
Initially, we can start with any node, say node $0$. Thus we mark$\text{minDist}[0] = 0$, and for the remaining nodes, the min distance to reach them is $\infty$.
...
2. Initially, we start with node $0$, and the cost to reach this node will be $0$. To signify this, we set $\text{minDist}[0]$ equal to $0$.
