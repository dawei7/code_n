...
The core task in **Distinct Numbers in Each Subarray** is to an integer array `nums` of length `n` and an integer `k`. Your task is to find the number of **distinct** elements in **every** subarray of size `k` within `nums`. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
