...
- If $\text{stone}[i]$ shares the same row ($\text{stones}[i][0] = \text{stones}[j][0]$) or column ($\text{stones}[i][1] = \text{stones}[j][1]$) as $\text{stone}[j]$:
...
- Mark the current `stone` as visited by setting $\text{visited}[stone]$ to `true`.
...
- Time complexity: $O(n^2)$
    ...
    Thus, the overall time complexity of the algorithm is $2 \cdot$\mathcal{O}(n^2)$= O(n^2)$.
...
- Space complexity: $O(n^2)$
    ...
    Thus, the space complexity of the algorithm is $2 \cdot$\mathcal{O}(n)$+$\mathcal{O}(n^2)$= O(n^2)$.
...
- If stone `i` shares the same row ($\text{stones}[i][0] = \text{stones}[j][0]$) or column ($\text{stones}[i][1] = \text{stones}[j][1]$) as stone `j`:
      - Perform a `union` operation on `i` and `j`.
- Return $n - \text{uf.count}$.
...
- Otherwise, recursively call `find` on $\text{parent}[node]$, set its result to $\text{parent}[node]$ and return it.
...
- If `root1` and `root2` are different, merge the two components by setting $\text{parent}[root1]$ to `root2`.
- Decrement `count`.
...
- Loop through each stone `i` in `stones`:
  - Call `uf.union()` to union the x-coordinate ($\text{stones}[i][0]$) and the y-coordinate offset by `10001` ($\text{stones}[i][1] + 10001$).
- Return $n - \text{uf.componentCount}$ as our result.
...
- Otherwise, merge the two components by setting $\text{parent}[root1]$ to `root2`.
