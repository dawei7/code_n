...
**Separate the chosen start from the array's own movement**
...
**Why the initial zero is handled separately**
...
| Element processed | Prefix sum `s` | Minimum `t` |
|---:|---:|---:|
| -3 | -3 | -3 |
| 2 | -1 | -3 |
| -3 | -4 | -4 |
...
The lowest array-only total is $-4$. Choosing `startValue = 5` shifts every row of the table upward by five, so the lowest actual running total becomes one. The actual totals are 2, 4, 1, 5, and 7.

Choosing four would shift the lowest prefix only to zero, violating the “never less than 1” requirement. This demonstrates both validity of five and minimality.

**Why only the minimum prefix matters**

Every actual running total has the form $x + P_i$. If $t$ is the smallest prefix, then $P_i \ge t$ for every $i$. Once $x+t \ge 1$, it follows that:

$$
x + P_i \ge x+t \ge 1
$$

for every step. There is no need to store the full list of prefixes or re-simulate after selecting $x$.

Conversely, the step at which prefix $t$ occurs imposes the unavoidable condition $x+t \ge 1$. Any smaller integer than $1-t$ fails at that exact step. If $1-t$ is below one, the independent positivity rule makes one the smallest legal start. Thus `max(1, 1 - t)` is not merely sufficient; it is the minimum possible value.

**Why negative and positive elements need no separate branches**

Positive elements increase `s` and usually do not change `t`. Negative elements may lower the prefix and update `t`. Zero preserves both. The same two assignments correctly cover all cases, so branching on the sign of each element would only complicate the loop.

## Complexity detail

Let $n$ be the length of `nums`. The algorithm visits every element once and performs constant-time arithmetic and comparison for it, so the running time is $O(n)$.

It stores only the current prefix sum `s` and the minimum prefix `t`. Their number does not depend on $n$, giving $O(1)$ auxiliary space. The input is read without modification, and no prefix array is allocated.

The calculated sums are bounded by the total magnitude of the input values. With the given limits they are small, but the reasoning and asymptotic bounds do not depend on those particular numeric caps.
