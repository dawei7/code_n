# Simplified Approach - Closest Dessert Cost
...
## Complexity detail
- **Time Complexity**: $O(n \cdot 3^m)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(3^m)$ — Utilizes standard auxiliary storage proportional to the active computation state.
