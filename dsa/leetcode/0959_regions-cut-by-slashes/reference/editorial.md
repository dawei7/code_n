...
- Mark the starting cell as visited by setting $\text{expandedGrid}[row][col]$ to `1`.
- While the `queue` is not empty:
  - Dequeue `currentCell`.
  - For each `direction` in `DIRECTIONS`:
- Set `newRow` as $\text{currentCell}[0] + \text{direction}[0]$.
- Set `newCol` as $\text{currentCell}[1] + \text{direction}[1]$.
...
- Time complexity: $O(n^2)$
    ...
    In the worst case, the flood fill algorithm will visit every cell in the expanded grid once. The expanded grid is $3n \times 3n$, resulting in $O((3n)^2) =$\mathcal{O}(9n^2)$= O(n^2)$ operations.
...
- Space complexity: $O(n^2)$
    ...
    Thus, the total time complexity of the algorithm is $O(n^2) +$\mathcal{O}(n^2)$= O(n^2)$.
...
- If `parentX` is not equal to `parentY`:
  - Set $\text{parentArray}[parentX]$ to `parentY` and return `1`.
- Return `0`.

Helper method `findParent`:

- Define a method `findParent` with parameters: `parentArray` and the index `x`.
- If $\text{parentArray}[x]$ is equal to `-1`:
  - `x` has no parent. Return `x`.
- Set $\text{parentArray}[x]$ to the parent of $\text{parentArray}[x]$ using `findParent`. Return $\text{parentArray}[x]$.
...
* Time complexity: $O(n^2 \cdot \alpha (n))$
    ...
    Thus, the overall time complexity of the algorithm is $O(4 \cdot n^2) +$\mathcal{O}(n^2 \cdot \alpha (n)$) =$\mathcal{O}(n^2 \cdot \alpha (n)$)$
...
- Set $\text{parent}[0]$ (top-left corner) to `-1` to make it the root.
...
- If $\text{parentArray}[node]$ is equal to `-1`:
  - `node` does not have any parent. Return `node`.
- Set $\text{parentArray}[node]$ to the parent of $\text{parentArray}[node]$ using the `find` method. Return $\text{parentArray}[node]$.
...
- Set $\text{parentArray}[parent2]$ to `parent1`.
- Return `0`.
...
* Time complexity: $O(n^2 \cdot \alpha (n^2))$
    ...
    Thus, the overall time complexity is $2 \cdot$\mathcal{O}(n^2)$+$\mathcal{O}(n^2 \cdot 2 \alpha (n^2)$) =$\mathcal{O}(n^2 \cdot \alpha (n^2)$)$
