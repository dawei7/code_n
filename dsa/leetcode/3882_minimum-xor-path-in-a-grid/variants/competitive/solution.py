...
class Solution:
    def minCost(self, grid):
        ...
        l = 1<<mx.bit_length()
        dp = [[False]*l for _ in range(len(grid[0]))]
        dp[0][0] = True
        for i in range(len(grid)):
            new_dp = [[False]*l for _ in range(len(grid[0]))]
            for j in range(len(grid[0])):
                for k in range(l):
                    if dp[j][k] or (j-1 >= 0 and new_dp[j-1][k]):
                        new_dp[j][k^grid[i][j]] = True
            dp = new_dp
        return next(i for i in range(l) if dp[-1][i])
...
class Solution2(object):
    def minCost(self, grid):
        ...
        :rtype: int
        """
        dp = [set() for _ in range(len(grid[0]))]
        dp[0].add(0)
        for i in range(len(grid)):
            new_dp = [set() for _ in range(len(grid[0]))]
            for j in range(len(grid[0])):
