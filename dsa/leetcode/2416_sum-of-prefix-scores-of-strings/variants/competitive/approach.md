...
The core task in **Sum of Prefix Scores of Strings** is to an array `words` of size `n` consisting of **non-empty** strings. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
