...
![Slide 1](images/slideshow_reorganize_string_3_reorganize_string_3-1.png)

![Slide 2](images/slideshow_reorganize_string_3_reorganize_string_3-2.png)

![Slide 3](images/slideshow_reorganize_string_3_reorganize_string_3-3.png)

![Slide 4](images/slideshow_reorganize_string_3_reorganize_string_3-4.png)

![Slide 5](images/slideshow_reorganize_string_3_reorganize_string_3-5.png)

![Slide 6](images/slideshow_reorganize_string_3_reorganize_string_3-6.png)

![Slide 7](images/slideshow_reorganize_string_3_reorganize_string_3-7.png)

![Slide 8](images/slideshow_reorganize_string_3_reorganize_string_3-8.png)

![Slide 9](images/slideshow_reorganize_string_3_reorganize_string_3-9.png)

![Slide 10](images/slideshow_reorganize_string_3_reorganize_string_3-10.png)

![Slide 11](images/slideshow_reorganize_string_3_reorganize_string_3-11.png)
...
- Pop the element with the highest priority from `pq`. Assign its count and character to $\text{count}_{first}$ and $\text{char}_{first}$ respectively.
- If `ans` is empty or the current character $\text{char}_{first}$ is different from the last character in `ans`, append $\text{char}_{first}$ to `ans`. If the count of $\text{char}_{first}$ is not zero, update its count by decreasing it by one. If the updated count is larger than zero, push it back to `pq`. Continue to the next iteration.
- Otherwise, if $\text{char}_{first}$ is the same as the last character in `ans`, it means we need to choose a different character. If `pq` is empty, return an empty string as it is impossible to rearrange the characters.
- Pop the next element from `pq`, assigning its count and character to $\text{count}_{second}$ and $\text{char}_{second}$ respectively. Append $\text{char}_{second}$ to `ans`.
- If the count of $\text{char}_{second}$ is not zero, update its count by decreasing it by one. If the updated count is larger than zero, push it back to `pq`.
- Finally, push the original $\text{char}_{first}$ back to `pq`.
...
* Space complexity: $O(k)$. The counter used to count the number of occurrences will incur a space complexity of $O(k)$. Similarly, the maximum size of the priority queue will also be $O(k)$. Given that $k \le 26$ in this problem, one could argue the space complexity is $O(1)$.
...
![Slide 1](images/slideshow_reorganize_string_2_reorganize_string_2-1.png)

![Slide 2](images/slideshow_reorganize_string_2_reorganize_string_2-2.png)

![Slide 3](images/slideshow_reorganize_string_2_reorganize_string_2-3.png)

![Slide 4](images/slideshow_reorganize_string_2_reorganize_string_2-4.png)
...
![Slide 1](images/slideshow_reorganize_string_1_reorganize_string_1-1.png)

![Slide 2](images/slideshow_reorganize_string_1_reorganize_string_1-2.png)

![Slide 3](images/slideshow_reorganize_string_1_reorganize_string_1-3.png)

![Slide 4](images/slideshow_reorganize_string_1_reorganize_string_1-4.png)

![Slide 5](images/slideshow_reorganize_string_1_reorganize_string_1-5.png)

![Slide 6](images/slideshow_reorganize_string_1_reorganize_string_1-6.png)

![Slide 7](images/slideshow_reorganize_string_1_reorganize_string_1-7.png)

![Slide 8](images/slideshow_reorganize_string_1_reorganize_string_1-8.png)

![Slide 9](images/slideshow_reorganize_string_1_reorganize_string_1-9.png)

![Slide 10](images/slideshow_reorganize_string_1_reorganize_string_1-10.png)

![Slide 11](images/slideshow_reorganize_string_1_reorganize_string_1-11.png)

#### Algorithm
1. Create a counter $\text{char}_{counts}$ to store the counts of each character in the input string `s`.
2. Find the character with the maximum count ($\text{max}_{count}$) in $\text{char}_{counts}$. Set `letter` as the corresponding character.
3. Check if $\text{max}_{count}$ is greater than half of the length of the string rounded up. If so, it is not possible to rearrange the characters. Return an empty string.
4. Initialize a list `ans` of length equal to `s`.
5. Set the starting index `index` as 0.
   - Place the most frequent character `letter` in the `ans` list at every second index until its count becomes zero. Increment `index` by 2 for each placement and decrease the count of `letter` in $\text{char}_{counts}$.
6. Iterate through the remaining characters and their counts in $\text{char}_{counts}$.
...
* Space complexity: $O(k)$. The counter used to count the number of occurrences will incur a space complexity of $O(k)$. Again, one could argue that because $k \le 26$, the space complexity is constant.