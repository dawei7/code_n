# Simplified Approach - Synonymous Sentences
...
## Complexity detail
- **Time Complexity**: $O(P\alpha(V)+V\log V+KW)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(V+KW)$ — Utilizes standard auxiliary storage proportional to the active computation state.
