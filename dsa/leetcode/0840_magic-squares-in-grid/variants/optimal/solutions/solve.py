"""Optimal app-local solution for LeetCode 840."""
...
def solve(grid):
    ...
    def is_magic(top, left):
        if grid[top + 1][left + 1] != 5:
            return False

        values = {grid[top + row_offset][left + column_offset] for row_offset in range(3) for column_offset in range(3)}
