...
- $abs(id - \text{preferred}_{j})$ is **minimized**, where `abs(x)` is the absolute value of `x`.
...
- $1 \le \text{roomId}_{i}, \text{preferred}_{j} \le 10^{7}$

- $1 \le \text{size}_{i}, \text{minSize}_{j} \le 10^{7}$
