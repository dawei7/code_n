...
> These dummy nodes can have any keys and any values, it doesn't matter. We should initialize $\text{head.next} = tail$ and $\text{tail.prev} = head$.
...
2. `node` is being inserted after `previousEnd`. Therefore, set $\text{previousEnd.next} = node$.
3. Now, we set the pointers of `node`. First, $\text{node.prev} = previousEnd$.
4. Next, because the "real" tail is before `tail`, we set $\text{node.next} = tail$.
5. Finally, we update $\text{tail.prev} = node$.
...
1. Let's call $nextNode = \text{node.next}$ and $prevNode = \text{node.prev}$. Currently, $\text{nextNode.prev} = node$ and $\text{prevNode.next} = node$. To remove `node` from the linked list, we need to reassign $\text{nextNode.prev} = prevNode$ and $\text{prevNode.next} = nextNode$.
2. We can perform both these reassignments without needing to declare `prevNode` or `nextNode` using the following code:
3. $\text{node.prev}.next = \text{node.next}$
4. $\text{node.next}.prev = \text{node.prev}$
