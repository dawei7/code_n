...
![Slide 1](images/slideshow_atmostk_atmostkslide1.png)

![Slide 2](images/slideshow_atmostk_atmostkslide2.png)

![Slide 3](images/slideshow_atmostk_atmostkslide3.png)

![Slide 4](images/slideshow_atmostk_atmostkslide4.png)

![Slide 5](images/slideshow_atmostk_atmostkslide5.png)

![Slide 6](images/slideshow_atmostk_atmostkslide6.png)

![Slide 7](images/slideshow_atmostk_atmostkslide7.png)

![Slide 8](images/slideshow_atmostk_atmostkslide8.png)

Now, refer to the visual slideshow demonstrating the sliding window on $slidingWindowAtMost(nums, k - 1)$.

![Slide 1](images/slideshow_atmostk_1_atmostk_1slide1.png)

![Slide 2](images/slideshow_atmostk_1_atmostk_1slide2.png)

![Slide 3](images/slideshow_atmostk_1_atmostk_1slide3.png)

![Slide 4](images/slideshow_atmostk_1_atmostk_1slide4.png)

![Slide 5](images/slideshow_atmostk_1_atmostk_1slide5.png)

![Slide 6](images/slideshow_atmostk_1_atmostk_1slide6.png)

![Slide 7](images/slideshow_atmostk_1_atmostk_1slide7.png)

![Slide 8](images/slideshow_atmostk_1_atmostk_1slide8.png)

![Slide 9](images/slideshow_atmostk_1_atmostk_1slide9.png)
...
![img](images/atmost_venn_diagram.png)
...
- Iterate through the `nums` array using the `right` pointer:
   - Increment the frequency of $\text{nums}[right]$ in the `freqMap`.
   - While the size of `freqMap` (the number of distinct elements) is greater than `distinctK`:
      - Decrement the frequency of $\text{nums}[left]$ in the `freqMap`.
      - If the frequency of $\text{nums}[left]$ becomes 0, remove it from the `freqMap`.
...
However, if `k` becomes negative, it indicates that there are more than `k` distinct elements in the current window. We need to adjust the window from the left side by moving the left pointer and reducing the frequency of $\text{nums}[left]$ until the number of distinct elements is valid again (equal to `k`).

If there are duplicates of $\text{nums}[left]$ within the current window, we need to keep shrinking the window from the left side until the frequency of $\text{nums}[left]$ is zero. This is crucial because we need to maintain the correct count of distinct elements within the window.

Now if `k` has become 0 and if the frequency of $\text{nums}[left]$ is greater than 1, it means there are duplicates of the current left. For each duplicate, we increment `currCount`, which represents the number of subarrays that can be formed by including these duplicates along with the distinct elements in the current window.
...
![Slide 1](images/slideshow_onepass_re_onepassslide1_re.png)

![Slide 2](images/slideshow_onepass_re_onepassslide2_re.png)

![Slide 3](images/slideshow_onepass_re_onepassslide3_re.png)

![Slide 4](images/slideshow_onepass_re_onepassslide4_re.png)

![Slide 5](images/slideshow_onepass_re_onepassslide5_re.png)

![Slide 6](images/slideshow_onepass_re_onepassslide6_re.png)

![Slide 7](images/slideshow_onepass_re_onepassslide7_re.png)

![Slide 8](images/slideshow_onepass_re_onepassslide8_re.png)

#### Algorithm

- Initialize an array `distinctCount` of size $\text{nums.size}() + 1$ to store the count of distinct values encountered.
...
- Start the sliding window approach by iterating through the `nums` array using the `right` pointer:
   - Increment the count of the element at $\text{nums}[right]$ in the `distinctCount` array.
   ...
   - If `k` becomes negative, it means there are more than `k` distinct elements in the current window:
      - Move the `left` pointer until the count of distinct elements becomes valid again by decrementing the count of $\text{nums}[left]$ in the `distinctCount` array and incrementing `k`.
      - Reset `currCount` to 0.
   - If `k` becomes 0, it means there are exactly `k` distinct elements in the current window:
      - While there are duplicate elements (count > 1) in the window, move the `left` pointer, decrement the count of $\text{nums}[left]$ in the `distinctCount` array, and increment `currCount`.
