...
In performing a brute force, we repeatedly want to check whether there is some $j \ge i+2$ for which $A[i] > A[j]$.  This is the same as checking for $A[i] > min(A[i+2:])$.  If we knew these minimums `min(A[0:]), min(A[1:]), min(A[2:]), ...` we could make this check quickly.
...
A necessary and sufficient condition for these possibilities is that $\text{Math.abs}(A[i] - i) \le 1$.  So we check this for every `i`.
