# Simplified Approach - Minimum Time to Finish the Race
...
## Complexity detail
- **Time Complexity**: $O(TL + NL)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(N + L)$ — Utilizes standard auxiliary storage proportional to the active computation state.
