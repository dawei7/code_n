...
After determining the number of operations corresponding to the current $k$, we can decide whether to accumulate the answer using the `operations` array provided in the problem. If $\text{operations}[t] = 1$, we perform the accumulation; otherwise, we do not.
