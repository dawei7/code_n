...
- If the current digit $\text{numStr}[i]$ is greater than the digit at the index stored in $maxRightIndex[i + 1]$, update $\text{maxRightIndex}[i]$ to `i`.
- Otherwise, keep $\text{maxRightIndex}[i]$ as $maxRightIndex[i + 1]$.

- In a second pass, check for the first opportunity to swap for maximum value:
  - Iterate through each digit in `numStr`:
- If the current digit $\text{numStr}[i]$ is less than the digit at the index $\text{maxRightIndex}[i]$, a beneficial swap can be made.
      - Swap $\text{numStr}[i]$ with $numStr[\text{maxRightIndex}[i]]$ to maximize the number.
...
- Record the last occurrence of each digit:
  - For each index `i` in `numStr`, update $lastSeen[\text{numStr}[i] - '0']$ to `i`, which stores the last position of each digit.

- Traverse the digits in `numStr` to find the first digit that can be swapped with a larger one:
  - For each index `i`, iterate `d` from `9` down to $\text{numStr}[i] - '0'$:
- If $\text{lastSeen}[d] > i$, it means there exists a larger digit `d` that can be swapped with $\text{numStr}[i]$.
      - Perform the swap between $\text{numStr}[i]$ and $numStr[\text{lastSeen}[d]]$.
...
- Traverse the string `numStr` from right to left:
  - If `maxDigitIndex` is `-1` or the current digit $\text{numStr}[i]$ is greater than the digit at `maxDigitIndex`, update `maxDigitIndex` to `i` (indicating a new maximum digit has been found).
  - If $\text{numStr}[i]$ is less than the digit at `maxDigitIndex`, mark `swapIdx1` as `i` (the smaller digit to be swapped) and `swapIdx2` as `maxDigitIndex` (the larger digit to swap with).

- After completing the traversal, check if a valid swap has been identified:
  - If both `swapIdx1` and `swapIdx2` are not `-1`, perform the swap between $\text{numStr}[swapIdx1]$ and $\text{numStr}[swapIdx2]$.
