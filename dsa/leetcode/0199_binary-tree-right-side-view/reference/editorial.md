...
- Save the previous node $prev = curr$ and pop the current node from the queue $curr = \text{queue.poll}()$.

- While the current node is not `null`:

- Add first _left_ and then _right_ child node into the queue.

- Update both previous and current nodes: $prev = curr$, $curr = \text{queue.poll}()$.
