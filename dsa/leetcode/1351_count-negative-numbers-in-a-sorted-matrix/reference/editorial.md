...
- $n = \text{grid}[0].size()$, to store the number of elements in each row.
...
**Note:** You can use in-built STL methods for the binary search like $\text{upper}_{bound}$, etc., or implement it on your own.
