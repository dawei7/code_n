# Simplified Approach - Concatenated Words
...
## Complexity detail
- **Time Complexity**: $O(sum(|word|^2))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(sum(|word|))$ — Utilizes standard auxiliary storage proportional to the active computation state.
