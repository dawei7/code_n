...
![Components Example](images/component.png)
...
* Time complexity : $O(n^2)$
    ...
    Building the graph costs $O(V + E) =$\mathcal{O}(V)$+$\mathcal{O}(E)$=$\mathcal{O}(n)$+$\mathcal{O}(n^2)$= O(n^2)$ time, as in the worst case all intervals are mutually overlapping. Traversing the graph has the same cost (although it might appear higher at first) because our `visited` set guarantees that each node will be visited exactly once. Finally, because each node is part of exactly one component, the merge step costs $O(V) = O(n)$ time. This all adds up as follows:

    $O(n^2) +$\mathcal{O}(n^2)$+$\mathcal{O}(n)$= O(n^2)$
...
A simple proof by contradiction shows that this algorithm always produces the correct answer. First, suppose that the algorithm at some point fails to merge two intervals that should be merged. This would imply that there exists some triple of indices $i$, $j$, and $k$ in a list of intervals $\text{ints}$ such that $i < j < k$ and ($\text{\text{ints}[i]}$, $\text{\text{ints}[k]}$) can be merged, but neither ($\text{\text{ints}[i]}$, $\text{\text{ints}[j]}$) nor ($\text{\text{ints}[j]}$, $\text{\text{ints}[k]}$) can be merged. From this scenario follow several inequalities:
