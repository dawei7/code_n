## General
**Competitive Approach — Find Drivers with Improved Fuel Efficiency**

The query executes a structured relational pipeline for **Find Drivers with Improved Fuel Efficiency**. It uses Common Table Expressions (CTEs) to isolate intermediate data transformations into modular steps, Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O(T log T + D log D)$ — Operation count proportional to input scale.
