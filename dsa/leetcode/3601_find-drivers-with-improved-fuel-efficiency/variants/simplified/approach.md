# Simplified Approach - Find Drivers with Improved Fuel Efficiency
...
## Complexity detail
- **Time Complexity**: $O(T log T + D log D)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(T + D)$ — Utilizes standard auxiliary storage proportional to the active computation state.
