...
* Time complexity : $O(\sum w_i^2)$, where $w_i$ is the length of $\text{words}[i]$. Checking whether all prefixes of $\text{words}[i]$ are in the set is $O(\sum w_i^2)$.
...
* Time Complexity: $O(\sum w_i)$, where $w_i$ is the length of $\text{words}[i]$. This is the complexity to build the trie and to search it.
