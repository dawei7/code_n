...
The core task in **Loud and Rich** is to There is a group of `n` people labeled from `0` to $n - 1$ where each person has a different amount of money and a different level of quietness. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
