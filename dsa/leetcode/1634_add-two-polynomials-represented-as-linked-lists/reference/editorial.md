...
* If a pair with key `n.power` already exists in `map`, then we can append `n.coefficient` to the existing pair's value to combined terms: (`n.power`, $existingCoefficient + \text{n.coefficient}$).
