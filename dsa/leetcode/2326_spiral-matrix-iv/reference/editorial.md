...
1. Set `i` (row index) to 0, `j` (column index) to 0, and $\text{cur}_{d}$ (current direction) to 0.
...
- Assign the current node's value `head->val` to the matrix at position $\text{res}[i][j]$.
- Calculate the next position `newi` and `newj` using the current direction from the movement matrix.
- If the next position `newi, newj` is out of the matrix bounds (less than 0 or greater than/equal to m or n), or is already filled ($\text{res}[newi][newj]$ is not -1):
- Then, change the direction by incrementing $\text{cur}_{d}$ (`modulus 4` to keep within the bounds of the direction matrix).
