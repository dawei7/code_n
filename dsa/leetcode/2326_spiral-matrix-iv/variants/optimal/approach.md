...
The core task in **Spiral Matrix IV** is to two integers `m` and `n`, which represent the dimensions of a matrix. The input is a **Singly-Linked List** where nodes are linked sequentially (`val`, `next`). The algorithm iterates through the list using pointer manipulation, updating linkages or traversing step-by-step without requiring extra array allocations.
