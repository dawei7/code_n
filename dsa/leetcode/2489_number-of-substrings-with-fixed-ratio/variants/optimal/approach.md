...
The core task in **Number of Substrings With Fixed Ratio** is to a binary string `s`, and two integers `num1` and `num2`. `num1` and `num2` are coprime numbers. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
