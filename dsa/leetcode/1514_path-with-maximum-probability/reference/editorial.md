...
After $n - 1$ rounds, we have updated $\text{max}_{prob}[end]$ to be the maximum probability of reaching `end` from the staring node along every possible path.

<br>

#### Algorithm

1) Initialize an array `maxProb` as the maximum probability to reach each node from the staring node, set $\text{maxProb}[start]$ as `1`.

2) Relax all edges: for each edge `(u, v)`, if a higher probability of reaching `u` through this edge is found, update the $\text{max}_{prob}[u]$ as $\text{max}_{prob}[u] = \text{max}_{prob}[v] * \text{path}_{prob}$, if a higher probability to reach `v` through this edge is found, update the $\text{max}_{prob}[v]$.
...
2) Initialize an array $\text{max}_{prob}$ to store the maximum probability of reaching each node from the starting node. Set the probability of the starting node $\text{max}_{prob}[start]$ as 1, and the probability of all other nodes as 0.

3) Add the starting node `start` to the `queue`.

4) While `queue` is not empty, we remove the first node $\text{cur}_{node}$ from the queue.

5) For each neighbor of $\text{nxt}_{node}$, calculate the probability of traveling from the starting node to the $\text{nxt}_{node}$ through the current edge ($\text{cur}_{node} --- \text{nxt}_{node}$), and update the maximum probability for this neighbor $\text{max}_{prob}[\text{nxt}_{node}]$ if necessary.

6) If the probability to this neighbor node is increased, add $\text{nxt}_{node}$ to `queue`.

7) Repeat steps 4-6 until `queue` is empty.

8) Return $\text{max}_{prob}[end]$, the maximum probability of reaching the end node `end` from the starting node.
...
- The algorithm stores the probability array $\text{max}_{prob}$ of size $O(n)$ and a queue of vertices `queue`. In the worst-case scenario, there are $O(m)$ nodes in `queue` at the same time.
...
We start from the starting node `start`, and consider its neighbors one by one, updating the probability to each neighboring node $\text{nxt}_{node}$ if the probability of reaching $\text{nxt}_{node}$ through the current node $\text{cur}_{node}$ is higher than the previous stored probability of reaching $\text{nxt}_{node}$ (by other paths). In order to always select the node with the highest reaching probability, we use a priority queue `pq` to store the nodes to visit, where the node with the highest probability of being reached from the starting node has the highest priority.

<br>

#### Algorithm

1) Initialize a priority queue `pq` to store nodes that need to be visited, and an array $\text{max}_{prob}$ to store the maximum probability to reach each node from the starting node. Set the probability of the starting node as `1`, and the probability of all other nodes as `0`.

2) Add the starting node `start` and its probability to the priority queue.

3) While `pq` is not empty, remove $\text{cur}_{node}$, the node with the highest priority from it.

4) For each neighbor $\text{nxt}_{node}$ of the current node $\text{cur}_{node}$, calculate the probability of traveling from the starting node to the $\text{nxt}_{node}$ through the current edge $\text{cur}_{node} --- \text{nxt}_{node}$, and update the maximum probability of $\text{nxt}_{node}$ if necessary. To update the maximum probability, compare the product of the probability with the current node and the probability of the edge $\text{cur}_{node} --- \text{nxt}_{node}$, with the current maximum probability to the neighbor node. If the product is larger than the maximum probability stored in $\text{max}_{prob}[\text{nxt}_{node}]$, we update the maximum probability $\text{max}_{prob}[\text{nxt}_{node}]$ as their product.

5) If the neighbor node $\text{nxt}_{node}$ has not been visited, we add it and its probability to the `pq`.

6) Repeat steps 3-5 until the priority queue is empty or the ending node `end` has been reached.

7) Return $\text{max}_{prob}[end]$.
