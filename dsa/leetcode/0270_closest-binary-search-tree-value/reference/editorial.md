...
### Approach 1: Recursive Inorder + Linear search, $\mathcal{O}(N)$ time
...
This approach serves to identify the subproblems.
...
---
### Approach 2: Iterative Inorder, $\mathcal{O}(k)$ time
...
Second, one could stop just after identifying the closest value, there is no need to traverse the whole tree. The closest value is found if the target value is in-between between two in-order array elements $\text{nums}[i] \le target < nums[i + 1]$. Then the closest value is one of these elements.

![pic](images/iteration.png)
...
- Pop the last element from stack $root = \text{stack.pop}()$.

- If the target is in-between of `pred` and `root.val`, return the closest between these two elements.

- Set predecessor value to be equal to `root.val` and go one step right: $root = \text{root.right}$.
...
---
### Approach 3: Binary Search, $\mathcal{O}(H)$ time
