...
- If the prefix sum at index `i`, denoted as $\text{prefixSum}[i]$, is positive, we need to find the minimum prefix sum encountered so far. This is because we will maximize the sum of our subarray with a right endpoint at `i` by finding the minimum prefix sum.

- Conversely, if $\text{prefixSum}[i]$ is negative, we need to find the maximum prefix sum encountered so far. This is because when the prefix sum is negative, subtracting a positive value (the maximum prefix sum) will result in a larger negative difference, which will maximize our absolute sum.
...
- `minPrefixSum` to the maximum possible integer ($\text{INT}_{MAX}$) — tracks the smallest prefix sum encountered so far.
- `maxPrefixSum` to the minimum possible integer ($\text{INT}_{MIN}$) — tracks the largest prefix sum encountered so far.
...
2. Iterate through the array and for each element $\text{nums}[i]$ in the array:
- Add $\text{nums}[i]$ to the `prefixSum` to calculate the cumulative sum up to the current index.
...
To maximize the absolute subarray sum, we need to find two prefix sums — one that is as large as possible and another that is as small as possible. This is because the sum of any subarray between indices `i` and `j` can be expressed as $\text{prefixSum}[j] - \text{prefixSum}[i]$. The greater the difference between $\text{prefixSum}[j]$ and $\text{prefixSum}[i]$, the larger the absolute sum of the subarray. Thus, to maximize this difference, $\text{prefixSum}[j]$ should be as large as possible, while $\text{prefixSum}[i]$ should be as small as possible.
...
-  Add the current element $\text{nums}[i]$ to `prefixSum`
