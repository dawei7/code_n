...
The core task in **Maximum Absolute Sum of Any Subarray** is to an integer array `nums`. The **absolute sum** of a subarray `[nums_l, nums_l+1, ..., nums_r-1, nums_r]` is $abs(\text{nums}_{l} + \text{nums}_{l}+1 + ... + \text{nums}_{r}-1 + \text{nums}_{r})$. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
