...
The first step is to generate all possible strings of length `2n`. We can utilize a queue data structure, `queue`, for implementing a breadth-first search algorithm. For each string of length `i`, denoted as $\text{cur}_{string}$, we will enqueue two new strings of length $i + 1$ by appending either `)` or `(` to $\text{cur}_{string}$. This process continues until the string length reaches `2n`, as shown in the following figure.

![img](images/2.png)

Once we have generated a string of length `2n`, the next step is to verify its validity. As a valid combination of parentheses of length `2n` must have `n` left parentheses, with each of them matched to one of the `n` right parentheses to its right, we can keep track of a parameter called $\text{left}_{count}$. $\text{left}_{count}$ represents the number of left parentheses that have not been matched by any right parentheses, and during iteration:

- If we encounter a left parenthesis, we increment $\text{left}_{count}$ by 1.
- If we encounter a right parenthesis, we decrement $\text{left}_{count}$ by 1 as it can match and "offset" one previous left parenthesis.

However, the value of $\text{left}_{count}$ cannot be negative, which would mean that there is a right parenthesis without a corresponding left parenthesis to its left.

![img](images/3.png)

Finally, after traversing the entire string, we check if $\text{left}_{count} = 0$. If it is, every left parenthesis is matached to a corresponding right parenthesis, and the string is considered valid. Otherwise, it is not a valid string.
...
1) Initialize a queue `queue` that contains an empty string `""`, create an empty array `answer` to store all valid strings.

2) Define $isValid(p_{string})$ to check if $p_{string}$ is valid by setting $\text{left}_{count} = 0$ and iterating over $p_{string}$. For each parenthesis `p`:
- We increment $\text{left}_{count}$ by 1 if `p` equals `(`, and decrement $\text{left}_{count}$ by 1 otherwise.
- If $\text{left}_{count} < 0$, return `False`.

    Return `True` if $\text{left}_{count} = 0$ after the iteration, and return `False` otherwise.

3) Continuously dequeue strings from `queue` and process each dequeued string $\text{cur}_{string}$.

- If $len(\text{cur}_{string}) = 2n$, we will add it to `answer` if it is a valid string.
- Otherwise ($len(\text{cur}_{string}) < 2n$), we add `(` or `)` at the end of the $\text{cur}_{string}$ and enqueue the two new strings.

4) Return `answer` when the process ends.
...
- While we don't count the answer as part of the space complexity, for those interested, it is the $n^{th}$ Catalan number, which is asymptotically bounded by $\frac{4^n}{n\sqrt{n}}$. Thus `answer` takes $O(\frac{4^n}{\sqrt{n}})$ space.

    > Please find the explanation behind this intuition in approach 3!
    > You can also refer to [Catalan number on Wikipedia](https://en.wikipedia.org/wiki/Catalan_number) for more information about Catalan numbers.

- Before we dequeue the first string of length `2n` from `queue`, it has stored  $2^{2n - 1}$ strings of length $n - 1$, which takes $O(2^{2n} \cdot n)$.
- To sum up, the overall space complexity is $O(2^{2n}\cdot n)$.
...
To ensure that the current string is always valid during the backtracking process, we need two variables $\text{left}_{count}$ and $\text{right}_{count}$ that record the number of left and right parentheses in it, respectively.

Therefore, we can define our backtracking function as $backtracking(\text{cur}_{string}, \text{left}_{count}, \text{right}_{count})$ that takes the current string, the number of left parentheses, and the number of right parentheses as arguments. This function will build valid combinations of parentheses of length `2n` recursively.

The function adds more parentheses to $\text{cur}_{string}$ only when certain conditions are met:

- If $\text{left}_{count} < n$, it suggests that a left parenthesis can still be added, so we add one left parenthesis to $\text{cur}_{string}$, creating a new string $\text{new}_{string} = \text{cur}_{string} + ($, and then call $backtracking(\text{new}_{string}, \text{left}_{count} + 1, \text{right}_{count})$.

- If $\text{left}_{count} > \text{right}_{count}$, it suggests that a right parenthesis can be added to match a previous unmatched left parenthesis, so we add one right parenthesis to $\text{cur}_{string}$, creating a new string $\text{new}_{string} = \text{cur}_{string} + )$, and then call $backtracking(\text{new}_{string}, \text{left}_{count}, \text{right}_{count} + 1)$.
...
2) Define $backtracking(\text{cur}_{string}, \text{left}_{count}, \text{right}_{count})$ to generate valid strings recursively.
- If $len(\text{cur}_{string}) = 2n$, add it to `answer` and return.
- If $\text{left}_{count} < n$, add `(` to $\text{cur}_{string}$ and move on to $backtracking(\text{new}_{string}, \text{left}_{count} + 1, \text{right}_{count})$.
- If $\text{left}_{count} > \text{right}_{count}$, add `)` to $\text{cur}_{string}$ and move on to $backtracking(\text{new}_{string}, \text{left}_{count}, \text{right}_{count} + 1)$.
...
- We only track the valid prefixes during the backtracking procedure. As explained in the approach 1 time complexity analysis, the total number of valid parentheses strings is $O(\frac{4^n}{n\sqrt{n}})$.
...
>
>We observe that this general formula matches exactly with the general formula for Catalan numbers. Therefore, the $n^{th}$ Catalan numbers is precisely the number of ways to form valid combinations of $n$ pairs of parentheses.
>
...
2) Create an empty array $answer = []$. Iterate over the number of parentheses pairs with a variable $\text{left}_{count}$.

3) Iterate over each valid string $\text{left}_{string}$ from $generateParenthesis(\text{left}_{count})$.

4) Iterate over each valid string $\text{right}_{string}$ from $generateParenthesis(n - \text{left}_{count} - 1)$.

5) Construct a valid string of length `2n`: We enclose $\text{left}_{string}$ by a pair of parentheses, which is denoted as $( + \text{left}_{string} + )$, then contatenate it with $\text{right}_{string}$, and add the resulting string to `answer`.
...
- We begin by generating all valid parentheses strings of length `2`, `4`, ..., `2n`. As shown in approach 2, the time complexity for generating all valid parentheses strings of length `2n` is given by the expression $O(\frac{4^n}{\sqrt{n}})$. Therefore, the total time complexity can be expressed $T(n) = \sum\limits_{i=1}^n{\frac{4^i}{\sqrt{i}}}$ which is asymptotically bounded by $O(\frac{4^n}{\sqrt{n}})$.
