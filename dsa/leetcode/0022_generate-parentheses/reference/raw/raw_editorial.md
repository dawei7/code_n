...
2) Define `isValid(p_string)` to check if `p_string` is valid by setting `left_count = 0` and iterating over `p_string`. For each parenthesis `p`:
...
3) Continuously dequeue strings from `queue` and process each dequeued string `cur_string`.
