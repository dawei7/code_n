...
def solve(matrix: list[list[int]]) -> list[list[int]]:
    ...
    by_value: dict[int, list[tuple[int, int]]] = defaultdict(list)
    ...
    for value in sorted(by_value):
        ...
        assignments: list[tuple[list[tuple[int, int]], int]] = []
        for cells in components.values():
            rank = 1 + max(
                max(row_rank[row], column_rank[column]) for row, column in cells
            )
