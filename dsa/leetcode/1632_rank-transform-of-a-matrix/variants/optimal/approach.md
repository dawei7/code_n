...
The core task in **Rank Transform of a Matrix** is to an `m x n` `matrix`, return *a new matrix *`answer`* where *$\text{answer}[row][col]$* is the ****rank** of *$\text{matrix}[row][col]$. To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
