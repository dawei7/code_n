...
> $\text{rowMax}[i]$ means the max rank in `i` row, and $\text{colMax}[j]$ means the max rank in `j` column.
...
Now we need to store the graph. We can have a map `graphRow`, where $\text{graphRow}[i]$ represent the columns linked to `i`-th row, and a map `graphCol`, where $\text{graphCol}[j]$ represent the columns linked to `j`-th col.
...
if $i \ge 0$, $\text{graph}[i]$ represents `i`-th row's neighbors (the complement of indexes of linked columns), and if `i < 0`, `graph[~i]` gives `~i`-th column's adjacent points (the indexes of linked rows).
...
> For convenience,
> - We refer "points" to indexes in the matrix, in the form $(\text{row}_{number}, \text{column}_{number})$.
> - We refer "value" to the values in the matrix. In other words, the value of point `(i, j)` is $\text{matrix}[i][j]$.
