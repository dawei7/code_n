...
Start with the brute force approach, we just want to find all the values in the two trees and try every combination. To implement it, we can perform a traversal on each tree and add the values to the respective lists $\text{node}_{list1}$ and $\text{node}_{list2}$. We then iterate through the two lists using a nested loop and check if the sum of any two elements $value1 + value2$ equals the target. If it does, we can return true.

Suppose we perform a preorder traversal of `root1`.

![img](images/1.png)

Similarly, we can store the value of each node of `root2` in $\text{node}_{list2}$.

![img](images/2.png)

Now we need to try every pair of values from $\text{node}_{list1}$ and $\text{node}_{list2}$. It requires a nested iteration.

#### Algorithm

1) Create two empty lists $\text{node}_{list1}$ and $\text{node}_{list2}$.

2) Perform a DFS over `root1` and add the value of each node to $\text{node}_{list1}$, perform a DFS over `root2` and add the value of each node to $\text{node}_{list2}$.

3) Iterate over elements in $\text{node}_{list1}$, for each element `value1`, we iterate over $\text{node}_{list2}$ and look for $target - value2$.
- If $target - value1$ is in $\text{node}_{list2}$, return `True`.

- Otherwise, move on to the next element of $\text{node}_{list1}$.
...
- We need to visit every node in both trees to collect the value of each node into two lists $\text{node}_{list1}$ and $\text{node}_{list2}$, this takes $O(m + n)$ as each node will be visited once.
...
- We save the value of each node in two lists $\text{node}_{list1}$ and $\text{node}_{list2}$.
...
- If $\text{node.val} = target - value1$, it means that we find a valid pair. Return `true`.
- If $\text{node.val} < target - value1$, it means that $target - value1$ might be in the right subtree of `node`, so we move on to its right child.
- If $\text{node.val} > target - value1$, it means that $target - value1$ might be in the left subtree of `node`, so we move on to its left child.
...
- If $\text{node2.val} = target - value1$, it means we find a pair, return `True`.
- If $\text{node2.val} < target - value1$, we move on to the left subtree of `node2`.
- If $\text{node2.val} > target - value1$, we move on to the right subtree of `node2`.
...
Recall from approach 1 where we save the values of the two trees in two lists. If we need to find $target - value1$ in $\text{node}_{list2}$, we may traverse the entire list, which takes $O(n)$ time.

However, if we save the values of `root2` in a hash set $\text{node}_{set2}$, we can determine if $\text{node}_{set2}$ contains $target - value1$ in $O(1)$ time.

![img](images/3.png)

Therefore, we collect the value of each node in `root1` in $\text{node}_{set1}$ and collect the value of each node in `root2` in $\text{node}_{set2}$. Then we iterate over each element in $\text{node}_{set1}$ and find out if `value1` is in this hash set.

#### Algorithm

1) Create two empty hash sets $\text{node}_{set1}$ and $\text{node}_{set2}$.

2) Traverse over `root1` and add the value of each node to $\text{node}_{set1}$, traverse over `root2` and add the value of each node to $\text{node}_{set2}$.

3) Iterate over elements in $\text{node}_{set1}$, for each element `value1`, check if $target - value1$ is in $\text{node}_{set2}$.
- If $target - values$ in $\text{node}_{set2}$, return `True`.
- Otherwise, move on to the next element of $\text{node}_{set1}$.
...
- We need to visit every node in both trees to collect the value of each node into two lists $\text{node}_{set1}$ and $\text{node}_{set2}$, which takes $O(m + n)$ as each node will be visited once, and an add operation takes $O(1)$ time on a hash set.
- We iterate over elements in $\text{node}_{set1}$ which contains $O(m)$ steps, checking if $target - value1$ in $\text{node}_{set2}$ at each step takes $O(1)$ time.

* Space complexity: $O(m + n)$

- We save the value of each node in two hash sets $\text{node}_{set1}$ and $\text{node}_{set2}$.
...
It implies that we can inorder traverse over `root1` and `root2`, and collect the value of each node in $\text{node}_{list1}$ and $\text{node}_{list2}$, so both lists are already sorted.

In order to find a valid pair whose sum is `target`, we can assign two pointers `pointer1` and `pointer2`, which point to the first (the smallest) element of $\text{node}_{list1}$ and the last (the largest) element  of $\text{node}_{list2}$, respectively.

![img](images/sorted_edit.png)

Then we keep comparing the sum of $value1 = \text{node}_{list1}[pointer1]$ and $value2 = \text{node}_{list2}[pointer2]$ with `target`:

- If $value1 + value2 = target$, it means that there exists a pair whose sum is `target`.

- If $value1 + value2 < target$, we need a larger `value1` or a larger `value2`. However, we traverse $\text{node}_{list2}$ backward in descending order, which means that we have already tried all values larger than `value2`, there are no possible candidates in $\text{node}_{list2}$, so we have to try a larger `value1` by incrementing `pointer1` by 1.

- If $value1 + value2 > target$, we need a smaller `value1` or a smaller `value2`. Similarly, since we traverse $\text{node}_{list1}$ in ascending order, we have already tried all values smaller than `value1`, so we can only try a smaller `value2` by decrementing `pointer2` by 1.
...
2) Perform an inorder traversal over `root1` and add the value of each node to $\text{node}_{list1}$, and perform an inorder traversal over `root2` then add the value of each node to $\text{node}_{list2}$.

3) Initialize two pointers $pointer1 = 0$ and $pointer2 = len(\text{node}_{list2}) - 1$ that point to the first element of $\text{node}_{list1}$ and the last element of $\text{node}_{list2}$, respectively.

4) While $pointer1 < len(\text{node}_{list1})$ and $pointer2 \ge 0$, we compare $\text{node}_{list1}[pointer1] + \text{node}_{list2}[pointer2]$ with `target`:
- If $\text{node}_{list1}[pointer1] + \text{node}_{list2}[pointer2] = target$, return `True`.
- If $\text{node}_{list1}[pointer1] + \text{node}_{list2}[pointer2] < target$, increment `pointer1` by 1.
- If $\text{node}_{list1}[pointer1] + \text{node}_{list2}[pointer2] > target$, decrement `pointer2` by 1.
...
Now that we have a method to traverse a binary tree by inorder morris traversal, we can use the two-pointers method similar to the one in approach 4, where we use two-pointers on two sorted lists. Specifically, let `pointer1` and `pointer2` point to the first element of $\text{node}_{list1}$ and the last element of $\text{node}_{list2}$, respectively. Likewise, we create two iterators over `root1` and `root2` using inorder morris traversal:
