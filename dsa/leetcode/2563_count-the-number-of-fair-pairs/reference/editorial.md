...
We have an array called `nums` with `n` elements, along with two integers, `lower` and `upper`. Our task is to find out how many pairs of indices `(i, j)` exist in the array such that the sum of the elements at these indices, $\text{nums}[i] + \text{nums}[j]$, falls between `lower` and `upper`. Plus, we need to make sure that `i` is less than `j`.
...
Now, how do we find the number of pairs for the lower limit using binary search? After fixing the first element $\text{nums}[i]$, the second element must be less than $lower - \text{nums}[i]$ to keep the sum below `lower`. We can efficiently find how many elements meet this condition by performing a binary search in the array for values less than or equal to $lower - \text{nums}[i]$.

Similarly, we can find the number of elements that are less than or equal to $upper + 1 - \text{nums}[i]$. The difference between these two counts will give us the total number of valid pairs for that particular fixed element.
...
Function - $\text{lower}_{bound}(nums, low, high, element)$:
...
- If $\text{nums}[mid]$ is greater than or equal to `element`, adjust the `high` index to $mid - 1$.
...
- For each element $\text{nums}[i]$, determine the number of possible pairs with a sum less than `lower`:
- Use $\text{lower}_{bound}$ to find the index of the first element in the subarray $nums[i + 1]$ to $\text{nums}[end]$ that is greater than or equal to $lower - \text{nums}[i]$.
- Similarly, determine the number of possible pairs with a sum less than or equal to `upper`:
- Use $\text{lower}_{bound}$ to find the index of the first element in the subarray that is greater than or equal to $upper - \text{nums}[i] + 1$.
...
- Time Complexity: $O(n \log n)$
    ...
    The loop iterates through each element of the sorted array and calls the $\text{lower}_{bound}$ function twice, which itself takes $O(logn)$ time. Therefore, the overall time complexity for processing all `n` elements is $O(n \log n)$.
...
1. Sort the array `nums`.
2. Return the difference between the result of $\text{lower}_{bound}(nums, upper + 1)$ and $\text{lower}_{bound}(nums, lower)$.
