...
**Build the complete reporting grid.** Create three branches that each select the distinct spending dates and attach one literal category: `desktop`, `mobile`, or `both`. Combining those branches with `UNION ALL` produces every required `(spend_date, platform)` pair even when no classified user-date belongs to it.
...
- **Three independent aggregation branches:** Separate full queries for mobile-only, desktop-only, and both can work, but each repeats classification logic and still needs explicit zero rows for missing categories. This is distinct from the target's three lightweight branches that only construct the date-category grid.
