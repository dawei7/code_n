...
We are given that $n \le 20$. Typically, problems that ask you to find **all** of something with low bounds can be solved with backtracking.
...
At each node, we have `curr.length` elements so far. We need to reach `k` elements. Therefore, we can calculate $need = k - \text{curr.length}$ as the number of elements we still need to add.
...
- if $\text{curr.length} = k$, add a copy of `curr` to `ans` and return.
