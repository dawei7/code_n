...
The core task in **The Time When the Network Becomes Idle** is to There is a network of `n` servers, labeled from `0` to $n - 1$. You are given a 2D integer array `edges`, where $\text{edges}[i] = [u_{i}, v_{i}]$ indicates there is a message channel between servers $u_{i}$ and $v_{i}$, and they can pass **any** number of messages to **each o.... To avoid nested loops that slow down execution, this solution uses a **Hash Table (Hash Map / Hash Set)**. Think of a index index-cards file: instead of scanning through all cards to check if a number exists, the hash table allows us to instantly look up any value in constant $O(1)$ time.
