...
- If it has not, it will **resend** the message periodically. The data server `i` will resend the message every $\text{patience}[i]$ second(s), i.e., the data server `i` will resend the message if $\text{patience}[i]$ second(s) have **elapsed** since the **last** time the message was sent from this server.
...
- $1 \le \text{patience}[i] \le 10^{5}$ for $1 \le i < n$
