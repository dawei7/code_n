...
1. Keep popping elements from the stack `st` until the stack becomes empty or the element $\text{nums}[i]$ becomes greater than the element at the index at the top of the stack.
2. For each popped element, add the subarray count as $i - \text{st.top}()$.
4. Push the current index `i` into the stack.
5. Pop all the remaining elements from the stack, and for each, consider the size of `nums` as the next smaller element index. Accordingly, add $\text{nums.size}() - \text{st.top}()$ to the variable `ans`.
6. Return `ans`.
