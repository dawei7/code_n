## General
**Competitive Approach — Unpopular Books**

The query executes a structured relational pipeline for **Unpopular Books**. It uses Relational JOIN operations to correlate records across tables, `GROUP BY` aggregation with PostgreSQL standard functions (`COALESCE`, `STRING_AGG`).
...
## Complexity detail
- **Time Complexity**: $O((B + O) \log (B + O))$ — Operation count proportional to input scale.
