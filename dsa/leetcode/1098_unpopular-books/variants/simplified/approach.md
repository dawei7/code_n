# Simplified Approach - Unpopular Books
...
## Complexity detail
- **Time Complexity**: $O((B + O) \log (B + O))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(B + O)$ — Utilizes standard auxiliary storage proportional to the active computation state.
