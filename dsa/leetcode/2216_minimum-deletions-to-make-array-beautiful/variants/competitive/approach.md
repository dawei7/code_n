...
The core task in **Minimum Deletions to Make Array Beautiful** is to a **0-indexed** integer array `nums`. The array `nums` is **beautiful** if:. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
