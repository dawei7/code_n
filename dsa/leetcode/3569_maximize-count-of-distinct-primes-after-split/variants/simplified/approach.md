# Simplified Approach - Maximize Count of Distinct Primes After Split
...
## Complexity detail
- **Time Complexity**: $O(U log log U + (n + q) log(n + q))$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(U + n + q)$ — Utilizes standard auxiliary storage proportional to the active computation state.
