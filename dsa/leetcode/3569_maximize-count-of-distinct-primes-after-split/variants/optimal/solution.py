...
class Solution:
    def maximumCount(self, nums: list[int], queries: list[list[int]]) -> list[int]:
        ...
        is_prime = bytearray(b"\x01") * (value_limit + 1)
        is_prime[:2] = b"\x00\x00"
        for value in range(2, isqrt(value_limit) + 1):
            if is_prime[value]:
                start = value * value
                is_prime[start : value_limit + 1 : value] = b"\x00" * ((value_limit - start) // value + 1)
        ...
        def range_add(
            query_left: int, query_right: int, delta: int, node: int = 1, left: int = 0, right: int = split_count - 1
        ) -> None:
