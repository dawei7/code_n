...
We recursively perform DFS traversal over the `left` and `right` child to see if they create uni-value subtrees. We perform $isLeftUniValue = dfs(\text{node.left})$ and $isRightUniValue = dfs(\text{node.right})$.

If both children form uni-value subtrees, i.e., `isLeftUniValue && isRightUniValue` is `true`, we verify whether both children (if they exist) have the same value as `node`. If the `left` child exists and $\text{node.left}.val \neq \text{node.val}$, we return `false`. While the `left` child forms a uni-value subtree, its value differs from that of `node`, so the current `node`'s subtree cannot be a uni-value subtree. We apply the same logic for the `right` child.
...
- Recursively check whether the `left` child forms a uni-value subtree. We perform $isLeftUniValue = dfs(\text{node.left})$.
- Recursively check whether the `right` child forms a uni-value subtree. We perform $isRightUniValue = dfs(\text{node.right})$.
- If both the children form uni-value subtrees, i.e, `isLeftUniValue && isRightUniValue` is `true`, we compare the values of the `node`'s children with `node`' value. If the `left` child exists and $\text{node.left}.val \neq \text{node.val}$, we return `false` as the values don't match and we don't have a uni-value subtree. Likewise, if the `right` child exists and $\text{node.right}.val \neq \text{node.val}$, we return `false`. Otherwise, we increment `count` by `1` and return `true`.
...
- Recursively call `dfs` for the `left` child. We perform $left = dfs(\text{node.left})$.
- Recursively call `dfs` for the `right` child. We perform $right = dfs(\text{node.right})$.
- We create two boolean variables $isLeftUniValue = \text{left.first}$ and $isRightUniValue = \text{right.first}$, to indicate whether the `left` and `right` subtrees constitute uni-value subtrees or not. We additionally keep a `count` of the number of uni-value subtrees in the `left` and `right` subtrees.
- If both the children form uni-value subtrees, i.e, `isLeftUniValue && isRightUniValue` is `true`, we compare the values of the `node`'s children with `node`' value. If the `left` child exists and $\text{node.left}.val \neq \text{node.val}$, we return `{false, count}` as this is not a uni-value subtree and we have `count` number of uni-value subtrees in the tree rooted at `node`. Likewise, if the `right` child exists and $\text{node.right}.val \neq \text{node.val}$, we return `{false, count}`. Otherwise, the subtree rooted at `node` forms a uni-value subtree, so we return ${true, count + 1}$.
