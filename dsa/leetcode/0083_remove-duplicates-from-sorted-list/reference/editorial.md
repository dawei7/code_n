...
At the last iteration of the loop, `current` must point to the last element, because afterwards, $\text{current.next} = null$. Therefore, after the loop ends, all elements up to the last element do not contain duplicates.