...
- For each cell $\text{matrix}[i][j]$, if the value is 1, calculate the size of the square submatrix ending at that cell.
- Use the following relation to fill the `dp` matrix: $dp[i+1][j+1] = min(\text{dp}[i][j+1],dp[i+1][j],\text{dp}[i][j])+1$
...
1. If the current cell $\text{grid}[i][j]$ is outside the bounds of the grid or is 0, return 0. This means no square submatrices can be formed from this cell.
2. If a cell's result is already computed (i.e., $\text{dp}[i][j]$ != -1), return the memoized value to avoid redundant calculations.
...
$\text{dp}[j] = 1 + \min(dp[j-1], \text{dp}[j], \text{prev})$

where `prev` stores the value of $\text{dp}[j]$ from the previous row, effectively representing the top-left neighbor in the matrix.

With this dependency in mind, we can optimize the traditional 2D dynamic programming table to a 1D array. Instead of maintaining the entire DP table, we use a single array `dp`, where each element corresponds to the size of the largest square submatrix ending at a column in the current row. To handle the dependency on the top-left neighbor, we maintain an additional variable `prev` to store the value of $\text{dp}[j]$ before it is updated in the current iteration.
...
The `prev` variable is updated during each iteration to store the value of $\text{dp}[j]$ before it is modified. This ensures that the dependency on the top-left neighbor is correctly accounted for in the current calculation. After updating the value of $\text{dp}[j]$, we add it to the `result` variable, which accumulates the total number of square submatrices in the matrix.
...
- Outer loop iterates over the rows (`i` from 0 to `row-1`).
- Inner loop iterates over the columns (`j` from 0 to `col-1`):
- For each cell $\text{matrix}[i][j]$, if the value is 1:
- Temporarily store the current value of $\text{dp}[j]$ in a variable `temp`.
- Update $\text{dp}[j]$ using the formula $\text{dp}[j] = 1 + min(prev, min(dp[j-1], \text{dp}[j]))$.
- Update `prev `to the value stored in `temp`.
- Add $\text{dp}[j]$ to `result` to increment the count of square submatrices.
- Otherwise, set $\text{dp}[j]$ to 0 as no square submatrix ends at this cell.
