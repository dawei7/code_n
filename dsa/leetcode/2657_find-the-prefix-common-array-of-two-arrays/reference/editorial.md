...
- For each `bIndex` from `0` to `currentIndex`:
  ...
  - Store `commonCount` in $\text{prefixCommonArray}[currentIndex]$ to record the number of common elements for the current prefix.
...
- If the `element` exists in `elementsInB`, increment `commonCount`.
  ...
  - Set $\text{prefixCommonArray}[currentIndex]$ to `commonCount`.
...
- Time complexity: $O(n^2)$
    ...
    Overall, the time complexity is $O(n) \times$\mathcal{O}(n)$= O(n^2)$.
...
- Iterate through each index `currentIndex` from `0` to $n - 1$:
  ...
  - Assign the value of `commonCount` to $\text{prefixCommonArray}[currentIndex]$.
