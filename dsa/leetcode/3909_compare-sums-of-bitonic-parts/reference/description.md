...
Let $p$ be the unique peak index. Strict increase means $\text{nums}[i] < nums[i + 1]$ for every $0\le i<p$, while strict decrease means $\text{nums}[i] > nums[i + 1]$ for every $p\le i<n-1$. Define the two inclusive sums as
...
The peak value $\text{nums}[p]$ appears once in each sum.
