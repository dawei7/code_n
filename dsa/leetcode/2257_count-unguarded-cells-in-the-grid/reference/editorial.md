...
- Time Complexity: $O(m \cdot n)$
    ...
    Combining all steps, the overall time complexity is: $O(m \cdot n) +$\mathcal{O}(m \cdot n)$+$\mathcal{O}(m \cdot n)$= O(m \cdot n)$.
...
- Mark the positions of guards in the grid:
  - For each guard in `guards`, set $grid[\text{guard}[0]][\text{guard}[1]] = GUARD$.

- Mark the positions of walls in the grid:
  - For each wall in `walls`, set $grid[\text{wall}[0]][\text{wall}[1]] = WALL$.
