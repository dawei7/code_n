...
The primary goal is to find the highest grade for each student from the `enrollments` table, and in the case of ties, select the record with the smallest $\text{course}_{id}$.
...
To determine each student's highest grade, we first prioritize based on the grade. In case of tied grades, we further prioritize based on the $\text{course}_{id}$. This is achieved by first sorting the data based on our prioritization, then grouping by the $\text{student}_{id}$ and taking the top record for each.
...
**Step 2** - Rank the records within each student's data based on `grade` (in descending order) and $\text{course}_{id}$ (in ascending order).
```python
...
**Step 3** - Join this information with the original table on $\text{student}_{id}$ and `grade`.
```python
...
**Step 4** - In case of a tie on the grade, select the smallest $\text{course}_{id}$.
```python
result = (
...
**Step 4** - For ties, choose records with the smallest $\text{course}_{id}$.
```python
result = (
...
Use a window function to rank the grades of each student. The window function allows us to perform calculations across a set of rows related to the current row. This approach leverages the capability of the window function to rank each student's grades, and if there's a tie, prioritize the row with the smallest $\text{course}_{id}$.
...
For this, we can use the $\text{DENSE}_{RANK}()$ function in SQL, where we rank data within each student's partition based on grade and course ID.
...
**Step 3** - Join this information with the original table on $\text{student}_{id}$ and `grade` using a SQL `JOIN`.
...
Find the highest grade for each student using a subquery. Then, filter the main table using this information. If there are ties in grades, pick the one with the smallest $\text{course}_{id}$.
