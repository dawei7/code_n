...
The core task in **Cycle Length Queries in a Tree** is to an integer `n`. There is a **complete binary tree** with $2^n - 1$ nodes. The root of that tree is the node with the value `1`, and every node with a value `val` in the range $[1, 2^n - 1 - 1]$ has two children where:. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
