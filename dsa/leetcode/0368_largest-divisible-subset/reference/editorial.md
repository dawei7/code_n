...
The main algorithm remains almost the same as Approach #1, which includes calculating **the size** of the largest divisible subset that ends with each element $X_i$. We denote this resulting vector as $\text{dp}[i]$.

The difference is that we need some additional logic to extract the resulting subset from $\text{dp}[i]$. Here we elaborate the procedure with a concrete example shown in the graph below.

![pic](images/368_dp_less_space.png)
...
- The reconstruction of the resulting subset begins with finding the largest size (_i.e._ $4$) and its index in $\text{dp}[i]$. The resulting largest divisible subset would end with the element that has the max value of $\text{dp}[i]$.
...
- We have two criteria to determine this next element: (1). the element should be able to divide the previous tail element in the resulting subset, _e.g._ $16 \bmod 8 = 0$. (2). The value of $\text{dp}[i]$ should correspond to the current size of the divisible subset, _e.g._ the element 7 would NOT be the next neighbor element after the element 8, since their $\text{dp}[i]$ values do not match up.
...
* Space complexity : $\mathcal{O}(N)$. The vector $\text{dp}[i]$ that keeps track of the size of the largest divisible subset which ends with each of the elements, would take $\mathcal{O}(N)$ space in all cases.
<br />
<br />
...
![pic](images/368_recursion_memoization.png)
