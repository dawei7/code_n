# Simplified Approach - Design Excel Sum Formula
...
## Complexity detail
- **Time Complexity**: $O(N + F)$ — Each input element is processed in a bounded number of direct operations.
- **Space Complexity**: $O(N + F)$ — Utilizes standard auxiliary storage proportional to the active computation state.
