...
### Approach 2: $\mathcal{O}(1)$ Space InPlace Modification Solution
...
2. For each element $\text{nums}[i]$, mark the element at the corresponding location negative if it's not already marked so i.e. $nums[\; \text{nums}[i] \;- 1\;] \times -1$ .
3. Now, loop over numbers from $1 \cdots N$ and for each number check if $\text{nums}[j]$ is negative. If it is negative, that means we've seen this number somewhere in the array.
