...
The core task in **Find All Numbers Disappeared in an Array** is to an array `nums` of `n` integers where $\text{nums}[i]$ is in the range `[1, n]`, return *an array of all the integers in the range* `[1, n]` *that do not appear in* `nums`. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
