...
- Push the character $\text{word}[index]$ onto the `stack`.
...
Once the prefix has been added to the `result`, we can then proceed with the `for` loop, appending $\text{word}[i]$ to the result.
...
- Otherwise, the character at this index of `result` should contain a character in the original order. Append $\text{word}[i]$ to `result`.
...
- If $\text{result}[right]$ is equal to `ch`:
