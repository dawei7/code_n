...
The core task in **Traffic Signal Color** is to an integer `timer` representing the remaining time (in seconds) on a traffic signal. The algorithm processes the input using a single-pass linear iteration, maintaining state variables that update as each element is inspected to produce the result cleanly.
