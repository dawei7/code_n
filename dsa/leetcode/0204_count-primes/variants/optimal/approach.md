...
Process candidates `p` from `2` through $\lfloor\sqrt{n - 1}\rfloor$. The source computes that inclusive limit as `int((n - 1) ** 0.5)`; the public bound $n \le 5 \times 10^6$ keeps this floating calculation exact for the needed integer boundary. If `p` is still marked, it is prime: any composite `p` would have a smaller factor that had already cleared it. Mark the multiples
...
Using a byte array rather than a general-purpose object list reduces memory constants. The Python source clears each arithmetic-progression slice in one bytearray assignment, but the logical sieve remains the same.
...
## Complexity detail
For each prime $p$, the sieve marks about $n/p$ multiples. The sum of reciprocals over primes yields total $O(n \log \log n)$ marking work. The marker array uses $O(n)$ space.
...
- Testing $p \cdot p < n$ is an integer-only alternative to the bounded-safe square-root limit used here; fixed-width languages should guard multiplication overflow for extreme bounds.
