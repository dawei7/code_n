...
**Resolve the all-equal boundary immediately:** Since `nums` is non-decreasing, equal first and last values mean that every value is equal. Each strictly increasing subsequence can then contain only one element, so this case succeeds exactly when `k = 1`. Returning immediately avoids an unnecessary full scan on this important boundary.
...
- **All values equal:** Equal endpoints detect this case in constant time. Then $f = n$ and every sequence can contain only one element, so success requires `k = 1`.
