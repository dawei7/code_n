...
description, LeetCode source URL), its contract with the player's
...
``SAMPLES``). The spec framework keeps templates, hints, samples,
and navigation metadata derived from each canonical package.
"""
...
from engine.challenge import Challenge, ChallengeInfo
...
class AlgorithmSpec:
    ...
    # Source-native difficulty plus an optional external contest Elo. Not all
    ...
    # ``source`` remains the executable cOde(n) harness implementation.
