...
# === segtree_04: Range Minimum Query ===
...
def _setup_segtree_04(challenge, n, seed):
    import random
    rng = random.Random(seed)
    n = max(1, min(n, 12))
    arr = [rng.randint(0, 99) for _ in range(n)]
...
def _verify_segtree_04(challenge, result):
    arr = challenge._arr
    if not all(isinstance(v, int) for v in result):
        return False
...
def _setup_segtree_05(challenge, n, seed):
    import random
    rng = random.Random(seed)
    n = max(1, min(n, 10))
...
def _verify_segtree_05(challenge, result):
...
# === segtree_06: Range Min with Lazy Updates ===
...
def _setup_segtree_06(challenge, n, seed):
    import random
    rng = random.Random(seed)
    n = max(1, min(n, 10))
    arr = [rng.randint(0, 20) for _ in range(n)]
...
def _verify_segtree_06(challenge, result):
    work = list(challenge._arr)
    for l, r, val in challenge._range_updates:
        for i in range(l, r + 1):
            work[i] = val
...
SPECS.extend([
    AlgorithmSpec(
        ...
        source=SEGTREE_04_SOURCE,
        setup_fn=_setup_segtree_04,
        verify_fn=_verify_segtree_04,
        samples=[
            Sample("arr = [1, 3, 5, 7, 9, 11], n = 6, queries = [(1, 3), (0, 5)], q = 2", "[3, 1]"),
            Sample("arr = [5, 2, 8, 1, 9, 3], n = 6, queries = [(0, 2), (3, 5)], q = 2", "[2, 1]"),
        ],
    ...
    AlgorithmSpec(
        ...
        source=SEGTREE_05_SOURCE,
        setup_fn=_setup_segtree_05,
        verify_fn=_verify_segtree_05,
        samples=[
            Sample("arr = [1, 3, 5, 7, 9, 11], n = 6, range_updates = [(1, 3, 5)], queries = [(0, 5)], q = 1", "[46] (= 1+8+10+12+9+11)"),
            Sample("arr = [1, 2, 3], n = 3, range_updates = [(0, 2, 10)], queries = [(0, 0), (1, 2), (0, 2)], q = 3", "[11, 25, 36]"),
        ],
    ...
    AlgorithmSpec(
        ...
        source=SEGTREE_06_SOURCE,
        setup_fn=_setup_segtree_06,
        verify_fn=_verify_segtree_06,
        samples=[
            Sample("arr = [1, 3, 5, 7, 9, 11], n = 6, range_updates = [(1, 3, 5)], queries = [(0, 5)], q = 1", "[1] (after assignment [1,5,5,5,9,11], min is 1)"),
            Sample("arr = [5, 5, 5, 5, 5], n = 5, range_updates = [(1, 3, 0)], queries = [(0, 4)], q = 1", "[0] (after [5,0,0,0,5], min is 0)"),
        ],
